...
'G':TrainImage8->Image8
	"Training image (fimage)",
'i':InitRandCB->InitRandCB
        "Initiate algorithm with a randomly drawn codebook of size InitRandCB",
'r':[RepeatRand=1]->RepeatRand
        "Run the algorithm RepeatRand times (irrelevant if InitRandCB is smaller than 1)",
'f':NResCB->NResCB
        "Index of first residual codebook (ResCodeBook)",
'g':NResResCB->NResResCB
        "Index of second residual codebook (ResResCodeBook)",
...
void mk_codebook(int *, double *, double *, int *, int *, Fimage);
