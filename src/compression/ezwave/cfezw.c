...
static unsigned char *ptrc;       /* Pointer to compress->gray for next 
				   * codewords to encode */
/* DRCurve         drc; */             /* Distorsion rate curve */
...

{
  ...
  if (Rate) {
    ...
    if (PSNR) {
      COMPUTE_CHANNEL_PSNR(Conv, PSNR, GRate, BRate, &redpsnr, &greenpsnr, &bluepsnr);
      RedP = &redpsnr;
      GreenP = &greenpsnr;
      BlueP = &bluepsnr;
    }
  ...
  fezw(NumRec, Edge_Ri, Ri2, FilterNorm, WeightFac, DistRate, RedR, RedP, SelectedArea, bufcomp, ChImage, Ri, QChImage, (char *) &drcchannel);
  if (QImage)
  ...
  fezw(NumRec, Edge_Ri, Ri2, FilterNorm, WeightFac, DistRate, GreenR, GreenP, SelectedArea, bufcomp, ChImage, Ri, QChImage, (char *) &drcchannel);
  if (QImage)
  ...
  fezw(NumRec, Edge_Ri, Ri2, FilterNorm, WeightFac, DistRate, BlueR, BlueP, SelectedArea, bufcomp, ChImage, Ri, QChImage, (char *) &drcchannel);
  if (QImage) 
  ...
  if (DistRate) 
    for (count_dr = 0; count_dr < drcchannel.npoints; count_dr++) {
      ...
      printf("%d\t%.2f\n", (int) drctotal.cr[count_dr], 10.0 * log10((double) MAX_GREYVAL * MAX_GREYVAL / drctotal.mse[count_dr])); 
    }
  if (Output)
