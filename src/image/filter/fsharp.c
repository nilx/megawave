...
{ 
  ...
  fOptimalCriterium = (float)mw_curve_length(pChild->boundary); 
  ...
  while(pCurrentShape != pParent) 
    { 
      ...
      fCriterium = (float)mw_curve_length(pCurrentShape->boundary); 
...
/* This removes the gradations from an image. In the tree of shapes representing the input 
   ...
   already found */ 
void fsharp(pPercentIncreaseArea, pFloatImageInput, pFloatImageOutput) 
