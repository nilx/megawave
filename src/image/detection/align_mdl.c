...
/* mwcommand
  ...
  usage = {  
 'd':[d=8]->d        "minimal precision in gradient orientation (default: 8 orientations)",
 'l':[nd=3]->nd      "number of dyadic gradient precision levels, default 3",
 'n':[nl=256]->nl    "number of line directions to scan, default 256",
 'g':[g=1.0]->g      "noise level used to threshold gradient, default 1.",
 ...
 'c':crv<-crv        "store segments as curves (output, Flists)",
  u->u               "input Fimage",
  out<-align_mdl     "result 6xN Fimage (x1 y1 x2 y2 -log10(nfa) precision_level)"
          };
*/
/*
