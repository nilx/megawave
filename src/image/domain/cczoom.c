     ...
     int *o;
{
  ...
  memcpy(out->red,aux_out->gray,out->nrow*out->ncol*sizeof(unsigned char));

  /* green component */
  aux_in->gray = in->green;
  aux_out = czoom(aux_in,aux_out,x_flg,y_flg,zoom,o,i_flg);
  memcpy(out->green,aux_out->gray,out->nrow*out->ncol*sizeof(unsigned char));

  /* blue component */
  aux_in->gray = in->blue;
  aux_out = czoom(aux_in,NULL,x_flg,y_flg,zoom,o,i_flg);
  memcpy(out->blue,aux_out->gray,out->nrow*out->ncol*sizeof(unsigned char));
