...
/* Is it a bug of the Silicon Graphics compiler ? */
/* but standard declarations cause errors */
#ifdef IRIX
extern extract_connex();
extern sr_normalize();
extern Fimage  sr_signature(Fcurves, int *, Fimage);
#else
extern Fcurves extract_connex(Cimage,Fcurves,int *);
extern Fcurves sr_normalize(Fcurves);
extern Fimage  sr_signature(Fcurves, int *, Fimage);
#endif
