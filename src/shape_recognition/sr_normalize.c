...
#ifdef __STDC__
extern Cimage fkplot(Fcurves,Cimage,char *);
extern Fcurves fkzrt(Fcurves,float,float,float,float);
extern void fkcenter(Fcurves, float *, float *);
extern Cimage opening(Cimage,Cimage,float*,Curve,int*,char *);
#else
...
float    close;
{
  ...
  if (close!=0.0) {    
    ...
    /* closing with a disk of radius=close */
    n = 1;
...
Fcurves in;
{
    ...
    fkcenter(in,&xg,&yg);
    fkzrt((Fcurves) in,1.0,0.0,(float) -xg, (float) -yg);
