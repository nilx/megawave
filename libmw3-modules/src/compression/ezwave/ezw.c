...
'r':NLevel->NumRec
        "Quantize wavelet transform up to level NLevel
        (Default : number of level in WavTrans)",
...
's':SelectArea->SelectedArea
        "Polygonal regions to be encoded with a different rate
        or PSNR (polygons)",
'o':Compress<-Compress
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fillpoly() */
...
/* Maximum value for the dimensions of image */
...
#define  POS_SYMB 3             /* Symbol for a significant
                                 * positive coefficient */
#define  NEG_SYMB 4             /* Symbol for a significant
                                 * negative coefficient */
#define  SIG_SYMB 5             /* Symbol for an already significant
                                 * coefficient */
...
#define  ENDED 0                /* Flag for areas whose encoding
                                 * has been stopped */
#define  PROGRESS 1             /* Flag for areas whose encoding
                                 * is in progress */
...
static long targnbit_sa[MAX_NPOLYG];    /* Target number of bits for each
                                         * selcected area */
static long effnbit_sa[MAX_NPOLYG];     /* Effective current number
                                         * of bits used
...
static void SCALE_WAVT(Wtrans2d wtrans, int nrec, float weightfac)
{
    long x, size;
    float scalefac;
    int i, j;
...
static void
COMPUTE_EFF_TARGET_RATE_MSE(Wtrans2d wtrans, Wtrans2d output, int nrec,
                            unsigned char ***areamap, Polygons selectarea,
                            float *psnr, float *rate)
                                 ...
                                 /* Target psnr for background */
{
  ...
  /*--- Compute target MSE for all selected areas ---*/
    ...
    {
        targmse =
            (double) size *MAX_GREYVAL * MAX_GREYVAL / pow((double) 10.0,
                                                           (double) *psnr /
                                                           10.0);
        targmse_sa[0] =
            (double) region_area[0] * MAX_GREYVAL * MAX_GREYVAL /
        ...
        if (selectarea)
        {
            ...
            {
                targmse_sa[p] =
                    (double) region_area[p] * MAX_GREYVAL * MAX_GREYVAL /
  ...
  /*--- Compute initial effective MSE and init output ---*/
    ...
    output->images[nrec][0] =
        mw_change_fimage(output->images[nrec][0],
                         wtrans->images[nrec][0]->nrow,
                         wtrans->images[nrec][0]->ncol);
    if (!output->images[nrec][0])
        mwerror(FATAL, 1,
                "Reallocation for output wavelet transform refused!\n");
    size = wtrans->images[nrec][0]->nrow * wtrans->images[nrec][0]->ncol;
    ...
    for (j = nrec; j >= 1; j--)
        ...
        {
            output->images[j][i] =
                mw_change_fimage(output->images[j][i],
                                 wtrans->images[j][i]->nrow,
...
static void COMPUTE_TOTAL_MSE(void)
{
    int p;                      /* Index for selected area */
...
static void INIT_SIGMAP(Wtrans2d wtrans, int nrec, unsigned char ***sigmap)
                                  ...
                                  * and mask for selected areas */
{
    ...
    {
        sigmap[j] =
            (unsigned char **) malloc((int) 4 * sizeof(unsigned char *));
        if (sigmap[j] == NULL)
    ...
    for (j = 1; j <= nrec; j++)
    {
        for (i = 1; i <= 3; i++)
        {
            size = wtrans->images[j][i]->nrow * wtrans->images[j][i]->ncol;
            sigmap[j][i] =
                (unsigned char *) malloc((int) size * sizeof(unsigned char));
            if (sigmap[j][i] == NULL)
                mwerror(FATAL, 1,
                        "Allocation for significance map refused!\n");
...
static void
UPDATE_AREAMAP(Wtrans2d wtrans, int nrec, unsigned char ***areamap,
               Cimage bitmap, unsigned char p)
  ...
  /*--- Memory allocation ---*/
    ...
    for (j = 1; j <= nrec; j++)
    {
        for (i = 1; i <= 3; i++)
        {
            ...
            if (areamap[j][i] == NULL)
                mwerror(FATAL, 1,
                        "Allocation for selected area mask refused!\n");
...
static float COMPUTES_THRES_ADAP(Wtrans2d wtrans, int nrec)
                                 /* Input wavelet transforms */
                                 /* Number of level to consider in wavelet
                                  * transform */
{
    ...
    for (j = nrec; j >= 1; j--)
        ...
        {
            size = wtrans->images[j][i]->nrow * wtrans->images[j][i]->ncol;
            for (x = 0, ptrd = wtrans->images[j][i]->gray; x < size;
                 x++, ptrd++)
            {
                avcoeff = fabs((double) *ptrd);
                if (avcoeff > max)
...
static void TEST_END_ENCODING(void)
{
    int p;
...
static void
REDRAW_AREAMAP(Wtrans2d wtrans, Wtrans2d output, int nrec,
               unsigned char ***areamap, unsigned char p)
    ...
    for (i = 1; i <= 3; i++)
    {
        ...
        /*--- Check parents for area index ---*/
          ...
          /*--- Compute column and row index of parent ---*/
                    ...
                    {
                        if ((wtrans->images[jp - 1][i]->nrow % 2 == 0)
                            || (rp != wtrans->images[jp][i]->nrow))
                            mwerror(WARNING, 0,
                                    "rp too large in REDRAW_AREAMAP\n"
                                    "jp = %d, i = %d, rp = %d, "
                                    "nrow = %d, xp = %d\n",
                                    jp, i, rp, wtrans->images[jp][i]->nrow,
                                    xp);
                        rp--;
                    }
                    cp = (xp % ncol) / 2;
                    ...
                    {
                        if ((wtrans->images[jp - 1][i]->ncol % 2 == 0)
                            || (cp != wtrans->images[jp][i]->ncol))
                            mwerror(WARNING, 0,
                                    "cp too large in REDRAW_AREAMAP\n"
                                    "jp = %d, i = %d, cp = %d, "
                                    "ncol = %d, xp = %d\n",
                                    jp, i, cp, wtrans->images[jp][i]->ncol,
                                    xp);
                        cp--;
                    }
                    ...
                    if (xp >=
                        wtrans->images[jp][i]->ncol *
                        wtrans->images[jp][i]->nrow)
                        mwerror(WARNING, 0,
                                "xp too large in REDRAW_AREAMAP\n"
                                "jp = %d, i = %d, rp = %d, "
                                "cp = %d, xp = %d, size = %d\n",
                                jp, i, rp, cp, xp, size);
...
static void RESIZE_COMPRESS(Cimage compress)
{
    int i;
    int ncolo, nrowo;
...
static void REALLOCATE_COMPRESS(Cimage compress)
{
    int i;
    Cimage bufcomp;
...
static void ADD_BIT_TO_OUTPUT(int bit, Cimage compress)
{
    buffer >>= 1;
    if (bit)
        buffer += 128;
...
static void ENCODE_INT(int symb, int max, Cimage compress)
{
...
static void
INIT_ENCODING(int nrec, int nrow, int ncol, int thressymb,
              Polygons selectarea, Cimage compress)
      ...
      /*--- Count number of vertices in polygon ---*/
        ...
        while (ptr_polyg)
        {
            ...
            while (ptr_point)
            {
                ...
                if ((ptr_point->x < 0) || (ptr_point->x >= ncol)
                    || (ptr_point->y < 0) || (ptr_point->y >= nrow))
                    mwerror(FATAL, 2,
                            "Vertex of polygon outside of image!\n"
        ...
        /*--- Encode one vertex of polygon ---*/
                ...
                if (ptrc)
                {
                    ENCODE_INT(ptr_point->x, 1 << (NBIT_SIZEIM - 1),
                               compress);
                    ENCODE_INT(ptr_point->y, 1 << (NBIT_SIZEIM - 1),
                               compress);
                }
...
static void OUTPUT_BITS(int bit, Cimage compress)
{
...
static void
ENCODE_SYMBOL(unsigned char symbol, long int *cum_freq, long int *effnbitar,
              Cimage compress)
{
    long range;
...
static void
ENCODE_SIGNIF_MAP(Wtrans2d wtrans, Wtrans2d output, Cimage compress,
                  unsigned char ***sigmap, unsigned char ***areamap,
                  long int *nsignif, int nrec, float thres, int *printfull,
                  int *distrate)
                                     /* Map of significance information */
                                     /* Mask for selected areas */
{
  ...
  /*--- Initialize encoding ---*/
    ...
    for (x = 0; (x < size) && (stop_test == TRUE); x++)
        ...
        /*--- Coefficient is significant positive ---*/
                ...
                *ptro = quant_step;
                effmse_sa[*ptra] +=
                    (*ptrw - quant_step) * (*ptrw - quant_step) -
                    *ptrw * *ptrw;
                (*nsignif)++;
                count_sigpos++;
            }
            else if (*ptrs == NEG_SYMB)
            {
        ...
        /*--- Test if target mse or rate is reached ---*/
                ...
                if (distrate)
                {
                    ...
                    if (effnbit + bits_to_follow + 5 >= targnbit_dr[count_dr])
                    {
                        ...
                        if (!drc)
                            printf("%.4f  %.2f\n",
                                   ((double) effnbit + EOF_nbit +
                                    bits_to_follow + 1.0) / sizei,
                                   10.0 * log10((double) sizei * MAX_GREYVAL *
                                                MAX_GREYVAL / effmse));
                        if (drc)
                ...
                else
                {
                    ...
                    if ((effmse_sa[*ptra] <= targmse_sa[*ptra])
                        || (effnbit_sa[*ptra] + bits_to_follow >=
                            targnbit_sa[*ptra] - 5))
        ...
        else if ((stop_test_area[*ptra] == END_NEXT) && (*ptrs <= NEG_SYMB)
                 && (*ptrs >= ZTR_SYMB))
        {
    ...
    for (j = nrec; j >= 1; j--)
        for (i = 1; i <= 3; i++)
        {
            ...
            /*--- Test if target mse or rate is reached ---*/
                        ...
                        {
                            effnbit += effnbitar;
                            if (effnbit + bits_to_follow + 5 >=
                                targnbit_dr[count_dr])
                            {
                                EOF_nbit = 0;
                                ENCODE_SYMBOL(EOP_symb, cum_freq, &EOF_nbit,
                                              NULL);
                                COMPUTE_TOTAL_MSE();
                                if (!drc)
                                    printf("%.4f  %.2f\n",
                ...
                else if ((stop_test_area[*ptra] == END_NEXT)
                         && (*ptrs <= NEG_SYMB) && (*ptrs >= ZTR_SYMB))
                {
...
static void
ENCODE_QUANT_STEP(Wtrans2d wtrans, Wtrans2d output, Cimage compress,
                  unsigned char ***sigmap, unsigned char ***areamap, int nrec,
                  float thres, int *printfull, int *distrate)
                                     /* Map of significance information */
                                     /* Mask for selected areas */
{
    int i;                      /* Index for orientation in wav. trans. */
  ...
  /*--- Initialize encoding ---*/
    ...
    for (x = 0; (x < size) && (stop_test == TRUE); x++)
        ...
        else if ((stop_test_area[*ptra] == END_NEXT) && (*ptrs == SIG_SYMB))
        {
...
static void
ZERO_TREE_ENCODE(Wtrans2d wtrans, Wtrans2d output, Cimage compress, int nrec,
                 float thres, float *rate, float *psnr, Polygons selectarea,
                 int *printfull, int *distrate)
                                 /* Input and quantized wavelet transforms */
                                 /* Compressed file */
                                 /* Number of level to consider in wavelet
                                  * transform */
                                 ...
                                 /* Flag for computation of dist. rate curve */
{
  ...
  /*--- Computes exact threshold ---*/
    ...
    if (thressymb > max_thressymb)
    {
        ...
        mwerror(WARNING, 0,
                "Threshold is smaller than half "
                "of the max. coeff. amplitude :\n %.2f / %.2f\n",
  ...
  /*--- Init encoding ---*/
    ...
    INIT_ENCODING(nrec, wtrans->nrow, wtrans->ncol, thressymb, selectarea,
                  compress);

    /*--- Memory allocation and initialisation for significance map ---*/

    sigmap =
        (unsigned char ***) malloc((int) (nrec + 1) *
  ...
  /*--- of bits and initialize quantized wavelet transform ---*/
    ...
    COMPUTE_EFF_TARGET_RATE_MSE(wtrans, output, nrec, areamap, selectarea,
                                psnr, rate);
  ...
  /*--- Encoding ---*/
    ...
    /*--- Dominant pass ---*/
        ...
        for (j = nrec; j >= 1; j--)
            ...
            {
                size =
                    wtrans->images[j][i]->nrow * wtrans->images[j][i]->ncol;
                ptrw = wtrans->images[j][i]->gray;
                ptrs = sigmap[j][i];
                ptra = areamap[j][i];
    ...
    /*--- Search new zerotree root ---*/
                        ...
                        if (j < nrec)
                        {
                            ...
                            if (rp >= wtrans->images[jp][i]->nrow)
                            {
                                if ((wtrans->images[jp - 1][i]->nrow % 2 == 0)
                                    || (rp != wtrans->images[jp][i]->nrow))
                                    mwerror(WARNING, 0,
                                            "rp too large in ZERO_TREE_ENCODE\n"
                                            "j = %d, jp = %d, i = %d, "
                                            "rp = %d, nrow = %d, xp = %d\n",
                                            j, jp, i, rp,
                                            wtrans->images[jp][i]->nrow, xp);
                                rp--;
                            }
                            cp = (x % ncol) / 2;
                            if (cp >= wtrans->images[jp][i]->ncol)
                            {
                                if ((wtrans->images[jp - 1][i]->ncol % 2 == 0)
                                    || (cp != wtrans->images[jp][i]->ncol))
                                    mwerror(WARNING, 0,
                                            "cp too large in ZERO_TREE_ENCODE\n"
                                            "j = %d, jp = %d, i = %d, "
                                            "cp = %d, ncol = %d, xp = %d\n",
                                            j, jp, i, cp,
                                            wtrans->images[jp][i]->ncol, xp);
                                cp--;
                            }
                            xp = rp * wtrans->images[jp][i]->ncol + cp;
                            if (xp >=
                                ...
                                wtrans->images[jp][i]->nrow)
                                mwerror(WARNING, 0,
                                        "xp too large in ZERO_TREE_ENCODE\n"
                                        "j = %d, jp = %d, i = %d,"
                                        " rp = %d, cp = %d, xp = %d, "
                                        "size = %d\n",
                                        j, jp, i, rp, cp, xp,
        ...
        ENCODE_SIGNIF_MAP(wtrans, output, compress, sigmap, areamap, &nsignif,
                          nrec, thres, printfull, distrate);

        if (stop_test == TRUE)
            ENCODE_QUANT_STEP(wtrans, output, compress, sigmap, areamap, nrec,
                              thres, printfull, distrate);
        ...
        if (!distrate && printfull)
        {
            ...
            printf
                ("Rate = %.4f, Rate AR = %.4f, PSNR = %.2f,  nsignif = %ld\n",
                 effrate, (double) effnbit / size,
                 10.0 * log10((double) size * MAX_GREYVAL * MAX_GREYVAL /
                              effmse), nsignif);
        }
    }
...
void
ezw(int *PrintFull, int *NumRec, float *WeightFac, float *Thres,
    int *Max_Count_AC, int *DistRate, float *Rate, float *PSNR,
    Polygons SelectedArea, Cimage Compress, Wtrans2d Wtrans, Wtrans2d Output,
    char *PtrDRC)
                                 /* Print full set of information */
                                 ...
                                 /* Distorsion rate curve */
{
  ...
  /*--- Memory allocation for quantized wavelet transform ---*/
    ...
    if (mw_alloc_ortho_wtrans2d
        (Output, Wtrans->nlevel, Wtrans->nrow, Wtrans->ncol) == NULL)
        mwerror(FATAL, 1,
  ...
  /*--- Memory allocation for compressed image buffer ---*/
    ...
    if (Compress)
    {
        ...
        if (Compress == NULL)
            mwerror(FATAL, 1,
                    "Memory allocation refused for `RecompImage`!\n");
        sizec = Compress->ncol * Compress->nrow;
    }
  ...
  /*--- Quantize and encode wavelet transform Wtrans ---*/
    ...
    ZERO_TREE_ENCODE(Wtrans, Output, Compress, NRec, Threshold, Rate, PSNR,
                     SelectedArea, PrintFull, DistRate);
