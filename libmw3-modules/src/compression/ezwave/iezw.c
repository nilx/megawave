...
static void INIT_AREAMAP(Wtrans2d wtrans, int nrec, unsigned char ***areamap)
                                 /* Input wavelet transforms */
                                 /* Number of level to consider in wavelet
                                  * transform */
                                 /* Mask for selected areas */
...
static void REALLOCATE_SUBIMAGES(Wtrans2d output)
{
    int j, nrec;
    int nrow, ncol;
    int nrow2, ncol2;
    ...
    for (j = 1; j <= nrec; j++)
    {
        ...
        output->images[j][0] =
            mw_change_fimage(output->images[j][0], nrow, ncol);
        if (!output->images[j][0])
            mwerror(FATAL, 1,
                    "Reallocation for output wavelet transform refused!\n");
        output->images[j][1] =
            mw_change_fimage(output->images[j][1], nrow2, ncol);
        if (!output->images[j][1])
            mwerror(FATAL, 1,
                    "Reallocation for output wavelet transform refused!\n");
        output->images[j][2] =
            mw_change_fimage(output->images[j][2], nrow, ncol2);
        if (!output->images[j][2])
            mwerror(FATAL, 1,
                    "Reallocation for output wavelet transform refused!\n");
        output->images[j][3] =
            mw_change_fimage(output->images[j][3], nrow2, ncol2);
...
static void TEST_END_DECODING(void)
{
    int p;
...
static void
REDRAW_AREAMAP(Wtrans2d wtrans, int nrec, unsigned char ***areamap,
               unsigned char p)
                                     /* Mask for selected areas */
...
static void DECODE_INT(int *symb, int max)
{
    int bit;
...
READ_HEADER(Cimage compress, int *nrec, int *nrow, int *ncol, float *thres)
{
  ...
  /*--- Decode number of polygons ---*/
    ...
    /*--- Memory allocation for vertices ---*/
        ...
        alloc_point =
            (Point_curve) malloc(npoints_total * sizeof(struct point_curve));
        if (alloc_point == NULL)
...
static int DECODE_SYMBOL(long int *cum_freq)
{
    int symbol;
    int cum;
    long range;
...
static void
MODIFY_CHILDREN(int j, int i, long int x, unsigned char ***sigmap,
                Wtrans2d output)
                                  ...
                                  /* Buffer index for current coefficient */
{
    ...
    if (rc == nrow - 1)
    {
        ...
        {
            sigmap[j - 1][i][xc + output->images[j - 1][i]->ncol] =
                sigmap[j - 1][i][xc + 2 * output->images[j - 1][i]->ncol] =
                ZTR_SYMB;
            if (cc == ncol - 1)
            {
                if (ncol << 1 < output->images[j - 1][i]->ncol)
                {
                    sigmap[j - 1][i][xc + 1] =
                        sigmap[j - 1][i]
                        [xc + output->images[j - 1][i]->ncol + 1] =
                        sigmap[j - 1][i]
                        [xc + 2 * output->images[j - 1][i]->ncol + 1] =
                        ZTR_SYMB;
                    sigmap[j - 1][i][xc + 2] =
                        sigmap[j - 1][i]
                        [xc + output->images[j - 1][i]->ncol + 2] =
                        sigmap[j - 1][i]
                        [xc + 2 * output->images[j - 1][i]->ncol + 2] =
                        ZTR_SYMB;
                }
                else if (cc << 1 < output->images[j - 1][i]->ncol - 1)
                    sigmap[j - 1][i][xc + 1] =
                        sigmap[j - 1][i]
                        [xc + output->images[j - 1][i]->ncol + 1] =
                        sigmap[j - 1][i]
                ...
                sigmap[j - 1][i][xc + 1] =
                    ...
                    [xc + output->images[j - 1][i]->ncol + 1] =
                    sigmap[j - 1][i]
                    [xc + 2 * output->images[j - 1][i]->ncol + 1] = ZTR_SYMB;
        }
        else if (rc << 1 < output->images[j - 1][i]->nrow - 1)
        {
            ...
            if (cc == ncol - 1)
            {
                if (ncol << 1 < output->images[j - 1][i]->ncol)
                {
                    sigmap[j - 1][i][xc + 1] =
                        sigmap[j - 1][i]
                        [xc + output->images[j - 1][i]->ncol + 1] = ZTR_SYMB;
                    sigmap[j - 1][i][xc + 2] =
                        sigmap[j - 1][i]
                        [xc + output->images[j - 1][i]->ncol + 2] = ZTR_SYMB;
                }
                else if (cc << 1 < output->images[j - 1][i]->ncol - 1)
                    sigmap[j - 1][i][xc + 1] =
                        sigmap[j - 1][i]
...
static void
DECODE_SIGNIF_MAP(Wtrans2d output, unsigned char ***sigmap,
                  unsigned char ***areamap, long int *nsignif, float thres,
                  int *printfull)
...
static void
DECODE_QUANT_STEP(Wtrans2d output, unsigned char ***sigmap,
                  unsigned char ***areamap, float thres, int *printfull)
                                    /* Reconstructed wavelet transform */
                                    /* Map of significance information */
                                    /* Mask for selected areas */
                                    /* Current threshold */
                                    /* Flag for information printing */
{
  ...
  /*--- Initialize decoding ---*/
    ...
    if (symbol != EOP_symb)
        mwerror(WARNING, 1,
                "Something wrong with decoding of quantization steps :\n"
                "expecting end of pass symbol (%d), getting %d.\n",
                EOP_symb, symbol);
    if (garbage_bits == 0)
        if (bits_to_go >= 2)
...
static void
ZERO_TREE_DECODE(Cimage compress, Wtrans2d output, float thres,
                 int *printfull)
                                  /* Compressed buffer */
                                  /* Reconstructed wavelet transform */
                                  /* Initial threshold */
                                  /* Flag for information printing */
{
  ...
  /*--- Decoding ---*/
    ...
    while (stop_test == TRUE)
    {
        ...
        DECODE_SIGNIF_MAP(output, sigmap, areamap, &nsignif, thres,
                          printfull);
