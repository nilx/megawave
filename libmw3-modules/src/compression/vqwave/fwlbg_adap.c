...
function = {"Generates training set(s) (starting from image(s))
            and constructs a codebook sequence for wavelet transform
...
usage = {
 ...
 'O':OldCodeBook->OldCodeBook            "Modify the first class
                                         codebook sequence (fimage)",
 'X':OldAdapCodeBook1->OldAdapCodeBook2  "Modify the second class
                                         codebook sequence (fimage)",
 'Y':OldAdapCodeBook2->OldAdapCodeBook3  "Modify the third class
                                         codebook sequence (fimage)",
 'x':AdapCodeBook2<-Output2              "Sequence of second class
                                         codebooks (fimage)",
 'y':AdapCodeBook3<-Output3              "Sequence of third class
                                         codebooks (fimage)",
 ...
 'Q':ResCodeBook->ResCodeBook            "Generate codebook(s)
                                         for residu of quantization
                                         with ResCodeBook (fimage)",
 'R':ResResCodeBook->ResResCodeBook      "Generate codebook(s)
                                         for residu of quantization
                                         with ResCodeBook
                                         and ResResCodeBook (fimage)",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for wlbg_adap(), dybiowave2(), dyowave2() */
...
static void WCB2FCB(Wtrans2d wcb, Fimage fcb)
                           ...
                           /* Codebook in the fimage format */
{
  ...
  /*--- Memory allocation and initialisation of fcb ---*/
    ...
    if (mw_change_fimage(fcb, nrow, ncol) == NULL)
        mwerror(FATAL, 1,
                "Not enough memory for fimage buffer for codebook.\n");
    ...
    for (j = 1; j <= wcb->nlevel; j++)
        for (i = 0; i <= 3; i++)
        {
            ...
            if (((float) nrowcb != fcb->gray[ncol + (j - 1) * 4 + i])
                || (nrowcb < 0))
                mwerror(FATAL, 2,
                        "Bad value for size of codebook for sub-image %d/%d.\n",
                        j, i);
...
static void
fwlbg_adap_wcb(int *NumRecMax, int *Level, int *Orient, Fimage Edge_Ri,
               Fsignal Ri2, int *FilterNorm, int *StopDecim, int *Width,
               int *Height, int *MultiCB, int *Lap, int *Sizec1, int *Sizec2,
               int *Sizec3, float *ThresVal1, float *ThresVal2,
               float *ThresVal3, Wtrans2d OldCodeBook,
               Wtrans2d OldAdapCodeBook2, Wtrans2d OldAdapCodeBook3,
               Wtrans2d * Output2, Wtrans2d * Output3, Fimage Image2,
               Fimage Image3, Fimage Image4, Wtrans2d ResCodeBook,
               Wtrans2d ResResCodeBook, Fimage Image1, Fsignal Ri,
               Wtrans2d * Output1)
...
/* Sequence of generated codebooks */
{
  ...
  /*--- Wavelet decompositions of training images ---*/
    ...
    if (Ri2)
    {
        ...
        dybiowave2(&NumRec, StopDecim, &Ortho, &Edge, &FiltNorm, Image1,
                   TrainWtrans1, Ri, Ri2);
        ...
        if (Image2)
        {
            ...
            dybiowave2(&NumRec, StopDecim, &Ortho, &Edge, &FiltNorm, Image2,
                       TrainWtrans2, Ri, Ri2);
        }
        ...
        if (Image3)
        {
            ...
            dybiowave2(&NumRec, StopDecim, &Ortho, &Edge, &FiltNorm, Image3,
                       TrainWtrans3, Ri, Ri2);
        }
        ...
        if (Image4)
        {
            ...
            dybiowave2(&NumRec, StopDecim, &Ortho, &Edge, &FiltNorm, Image4,
                       TrainWtrans4, Ri, Ri2);
        }
    }
    else
    {
        ...
        TrainWtrans1 = mw_new_wtrans2d();
        dyowave2(&NumRec, StopDecim, &Ortho, &Edge, &Precond, &FiltNorm,
                 Image1, TrainWtrans1, Ri, Edge_Ri);

        if (Image2)
        {
            TrainWtrans2 = mw_new_wtrans2d();
            dyowave2(&NumRec, StopDecim, &Ortho, &Edge, &Precond, &FiltNorm,
                     Image2, TrainWtrans2, Ri, Edge_Ri);
        }
        ...
        if (Image3)
        {
            TrainWtrans3 = mw_new_wtrans2d();
            dyowave2(&NumRec, StopDecim, &Ortho, &Edge, &Precond, &FiltNorm,
                     Image3, TrainWtrans3, Ri, Edge_Ri);
        }
        ...
        if (Image4)
        {
            TrainWtrans4 = mw_new_wtrans2d();
            dyowave2(&NumRec, StopDecim, &Ortho, &Edge, &Precond, &FiltNorm,
                     Image4, TrainWtrans4, Ri, Edge_Ri);
        }
    }
  ...
  /*--- Construction of codebook(s) ---*/
    ...
    Edge = (Ri->size + 1) / 2;
    wlbg_adap(NumRecMax, Level, Orient, StopDecim, Lap, &Edge, Width, Height,
              MultiCB, Sizec1, Sizec2, Sizec3, ThresVal1, ThresVal2,
              ThresVal3, OldCodeBook, OldAdapCodeBook2, OldAdapCodeBook3,
              Output2, Output3, TrainWtrans2, TrainWtrans3, TrainWtrans4,
              ResCodeBook, ResResCodeBook, TrainWtrans1, Output1);

}

void
fwlbg_adap(int *NumRecMax, int *Level, int *Orient, Fimage Edge_Ri,
           Fsignal Ri2, int *FilterNorm, int *StopDecim, int *Width,
           int *Height, int *MultiCB, int *Lap, int *Sizec1, int *Sizec2,
           int *Sizec3, float *ThresVal1, float *ThresVal2, float *ThresVal3,
           Fimage OldCodeBook, Fimage OldAdapCodeBook2,
           Fimage OldAdapCodeBook3, Fimage Output2, Fimage Output3,
           Fimage Image2, Fimage Image3, Fimage Image4, Fimage ResCodeBook,
           Fimage ResResCodeBook, Fimage Image1, Fsignal Ri, Fimage Output1)
/* Number of level in Output1 */
/* Generate codebook only at one scale */
/* Generate codebook only in one orientation */
/* Impulse responses of filters for special
...
/* Sequence of generated codebooks */
{
    Wtrans2d WOldCodeBook;
    /* Modified sequence of first class codebook */
    Wtrans2d WOldAdapCodeBook2, WOldAdapCodeBook3;
    /* Modified sequence
     * of second and third class codebooks */
    Wtrans2d WOutput2, WOutput3;
    /* Generated sequences of second and third
     * class codebook */
    Wtrans2d WResCodeBook, WResResCodeBook;
    /* Generate residu codebook after
     * quantization with ResCodeBook
     * and ResResCodeBook */
    Wtrans2d WOutput1;
    /* Sequence of generated codebooks */
  ...
  /*--- Construction of new codebooks ---*/
    ...
    fwlbg_adap_wcb(NumRecMax, Level, Orient, Edge_Ri, Ri2, FilterNorm,
                   StopDecim, Width, Height, MultiCB, Lap, Sizec1, Sizec2,
                   Sizec3, ThresVal1, ThresVal2, ThresVal3, WOldCodeBook,
                   WOldAdapCodeBook2, WOldAdapCodeBook3, &WOutput2, &WOutput3,
                   Image2, Image3, Image4, WResCodeBook, WResResCodeBook,
                   Image1, Ri, &WOutput1);
