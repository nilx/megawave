...
usage = {
 ...
 'e':EdgeIR->Edge_Ri          "Impulse reponses of edge
                              and preconditionning filters
                              for orthogonal transform (fimage)",
 'b':ImpulseResponse2->Ri2    "Impulse response of filter 2
                              for biorthogonal transform (fsignal)",
 ...
 's':ScalQuant->NumRecScal [0,15]  "Use uniform scalar quantization
                                   for the resume at level NLevelWav
                                   if ScalQuant = NLevelWav
                                   or all subimages
                                   at levels larger than SalQuant otherwise",
 'u':[UnifQuantStep=0]->NStep      "Use UnifQuantStep levels
                                   to uniformly scalar quantize
                                   the resume or subimages",
 'm':[MultiCB=2]->MultiCB [1,2]    "1: approximative memory
                                   allocation procedure,
                                   2: exhaustive memory allocation procedure",
 'x':CodeBook2->CodeBook2          "Sequence of codebooks
                                   for second class vectors (fimage)",
 'y':CodeBook3->CodeBook3          "Sequence of codebooks
                                   for third class vectors (fimage)",
 'A':ResCodeBook1->ResCodeBook1    "Sequence of codebooks
                                   for residu quantization
                                   after quantization with CodeBook1 (fimage)",
 'B':ResCodeBook2->ResCodeBook2    "Sequence of codebooks
                                   for residu quantization
                                   after quantization with CodeBook2 (fimage)",
 'C':ResCodeBook3->ResCodeBook3    "Sequence of codebooks
                                   for residu quantization
                                   after quantization with CodeBook3 (fimage)",
 'D':ResResCodeBook1->ResResCodeBook1   "Sequence of codebooks
                                        for residu quantization
                                        after quantization
                                        with CodeBook1
                                        and ResCodeBook1 (fimage)",
 'E':ResResCodeBook2->ResResCodeBook2   "Sequence of codebooks
                                        for residu quantization
                                        after quantization
                                        with CodeBook2
                                        and ResCodeBook2 (fimage)",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fvq(), fscalq(), owave2(), iowave2(),
...
static void FCB2WCB(Fimage fcb, Wtrans2d wcb)
...
/* Codebook in the wtrans2d format */
{
  ...
  /*--- Memory allocation and initialisation of wcb ---*/
    ...
    if (mw_alloc_ortho_wtrans2d(wcb, nlevel, nrow, ncol) == NULL)
        mwerror(FATAL, 1,
                "Allocation of wavelet transform for codebook refused!\n");
  ...
  /*--- Copy fcb to wcb ---*/
    ...
    for (j = 1; j <= wcb->nlevel; j++)
        for (i = 0; i <= 3; i++)
        {
            ...
            if (((float) nrowcb != fcb->gray[ncol + (j - 1) * 4 + i])
                || (nrowcb < 0))
                mwerror(FATAL, 2,
                        "Bad value for size of codebook "
                        "for sub-image %d/%d.\n", j, i);
            ncolcb = (int) fcb->gray[2 * ncol + (j - 1) * 4 + i];
            if (((float) ncolcb != fcb->gray[2 * ncol + (j - 1) * 4 + i])
                || (ncolcb < 0))
                mwerror(FATAL, 2,
                        "Bad value for dimension of vector "
                        "for sub-image %d/%d.\n", j, i);

            if ((nrowcb > 4) && (ncolcb > 0))
            {
                height = (int) fcb->gray[(nrowcb + 1) * ncol + incrc];
                if (((float) height != fcb->gray[(nrowcb + 1) * ncol + incrc])
                    || (height <= 0))
                    mwerror(FATAL, 2,
                            "Bad value for height of vector "
                            "for sub-image %d/%d.\n", j, i);
                if (ncolcb % height != 0)
                    mwerror(FATAL, 2,
                            "Dimension and height of vector "
                            "for sub-image %d/%d are incompatible!\n", j, i);
                width = ncolcb / height;

                if (mw_change_fimage(wcb->images[j][i], nrowcb, ncolcb) ==
                    NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for wavelet "
                            "transform buffer for codebook.\n");

                wcb->images[j][i]->firstcol = width;
                wcb->images[j][i]->firstrow = height;
                for (r = 0; r < nrowcb; r++)
                    for (c = 0; c < ncolcb; c++)
                        wcb->images[j][i]->gray[r * ncolcb + c] =
                            fcb->gray[(r + 3) * ncol + c + incrc];
                incrc += ncolcb;
            }
            else
            {
...
static void INIT_TARGNBIT_DR(void)
{
...
static void INIT_RI(Fsignal ri1, Fsignal ri2, Fimage edge_ri)
{
    int i, j;
    ...
    if (edge_ri)
    {
        ...
        for (j = 0; j < edge_ri->nrow; j++)
            for (i = 0; i < edge_ri->ncol; i++)
                OEDGE_RI->gray[j * edge_ri->ncol + i] =
                    edge_ri->gray[j * edge_ri->ncol + i];
    }
}

static void REFRESH_FILTERS(Fsignal ri1, Fsignal ri2, Fimage edge_ri)
{
    int i, j;
...
static float SPHERE_CONST(int dim)
{
    float r;
...
static void COMPUTE_CONST(Wtrans2d codebook, int nlevel, int nlevelscal)
{
    int j, i, nadap;
        ...
        for (i = 0; i <= 3; i++)
            if (codebook->images[j][i])
                if (codebook->images[j][i]->nrow > 6)
                    for (nadap = 0; nadap < nadapcb[j][i]; nadap++)
                        bfacmse[nadap][j][i] =
                            12.0 * SPHERE_CONST(codebook->images[j][i]->ncol);
...
static void
fvariance(Wtrans2d wtrans, int j, int i, Wtrans2d cb1, Wtrans2d cb2,
          Wtrans2d cb3, double *var1, double *var2, double *var3,
          double *varthres, long int *num1, long int *num2, long int *num3,
          long int *numthres)
{
    int sizeb;
    int height, width;
    ...
    if (cb1->nlevel >= j)
        if (cb1->images[j][i]->nrow > 4)
        {
            ...
            height =
                cb1->images[j][i]->gray[(cb1->images[j][i]->nrow - 2) *
                                        sizeb];
            width = sizeb / height;
            thres1 =
                cb1->images[j][i]->gray[(cb1->images[j][i]->nrow - 3) *
                                        sizeb];
            if (cb2)
            {
                thres2 =
                    cb2->images[j][i]->gray[(cb2->images[j][i]->nrow - 3) *
                                            sizeb];
                if (cb3)
                    thres3 =
                        cb3->images[j][i]->gray[(cb3->images[j][i]->nrow -
                                                 3) * sizeb];
            }
        }
    ...
    for (r = 0; r < nrowb; r++)
        for (c = 0; c < ncolb; c++)
        {
            ...
            for (rb = 0; rb < height; rb++)
                ...
                {
                    energy +=
                        wtrans->images[j][i]->gray[(r * height + rb) * ncol +
                                                   c * width +
                                                   cb] *
                        wtrans->images[j][i]->gray[(r * height + rb) * ncol +
                                                   c * width + cb];
...
static double det_energy(Fimage image)
{
    long x, size;
    double e;
...
static void RESIZE_COMPRESS_WTRANS(Cimage compress)
{
    int i;
    int ncolo, nrowo;
...
static void REALLOCATE_COMPRESS_WTRANS(Cimage compress)
{
    int i;
    Cimage bufcomp;
    long size;
    ...
    printf
        ("Reallocation of Compress for wavelet coefficients "
         "vector quantization.\n");
...
static void ADD_BIT_TO_COMPRESS_WTRANS(int bit, Cimage compress)
{
    buffer >>= 1;
    if (bit)
        buffer += 128;
...
static void ENCODE_INT_WTRANS(int symb, int max, Cimage compress)
...
static void
INIT_ENCODING_WTRANS(int nrow, int ncol, int nlevel, Wtrans2d codebook1,
                     int nlevelscal, Cimage compress)
  ...
  /*--- Memory allocation for compressed image buffer ---*/
    ...
    /*--- Encode info for scalar / vector quantization ---*/
        ...
        for (j = 1; j <= codebook1->nlevel; j++)
            if (j <= nlevel)
                for (i = 1; i <= 3; i++)
                    if ((count_cb(codebook1->images[j][i]) <= 0)
                        || (j > nlevelscal))
        ...
        for (j = codebook1->nlevel + 1; j <= nlevel; j++)
            for (i = 1; i <= 3; i++)
                ENCODE_INT_WTRANS(0, 1, compress);
        if ((nlevel >= nlevelscal)
            || (count_cb(codebook1->images[nlevel][0]) <= 0))
            ENCODE_INT_WTRANS(0, 1, compress);
        else
            ENCODE_INT_WTRANS(1, 1, compress);
        ...
        if (codebook->nrow > 4)
        {
            ...
            if (codebook->nrow > 6)
            {
                ...
                sizef = floor(codebook->gray[(codebook->nrow - 6) *
                                             codebook->ncol] + .5);
                if (((float) sizei ==
                     codebook->gray[(codebook->nrow - 5) * codebook->ncol])
                    && ((float) sizef ==
                        codebook->gray[(codebook->nrow - 6) * codebook->ncol])
                    && (sizei > 0) && (sizef >= sizef))
                {
                    n = 2;
                    size = 1;
...
static void
compute_indexcb(int (*indcb)[6][4], float targrate, float rateheader,
                int numrec)
                                /* Index of codebooks to be used */
                                /* Bit rate to encode header */
{
    int q, qmax = 0;
    int j, i, nadap;
    ...
    while (effrate < targrate)
    {
        ...
        for (j = 1; j <= numrec; j++)
            for (i = 1; i <= 3; i++)
                for (nadap = 0; nadap < nadapcb[j][i]; nadap++)
                {
                    ...
                    for (q = indcb[nadap][j][i] + 1; q < numcb[nadap][j][i];
                         q++)
                    {
                        diffrate =
                            diffrate1 + brate[nadap][j][i][q] -
                            brate[nadap][j][i][indcb[nadap][j][i]];
                        if (diffrate != 0.0)
                        {
                            diff =
                                (bmse[nadap][j][i][indcb[nadap][j][i]] -
                                 bmse[nadap][j][i][q]) / diffrate;
                            if ((diff > maxdiff)
                                && (effrate + diffrate <= targrate))
                            {
                                jmax = j;
                                imax = i;
                                nadapmax = nadap;
        ...
        for (q = indcb[0][numrec][0] + 1; q < numcb[0][numrec][0]; q++)
        {
            diffrate =
                brate[0][numrec][0][q] -
                brate[0][numrec][0][indcb[0][numrec][0]];
            if (diffrate != 0.0)
            {
                diff =
                    (bmse[0][numrec][0][indcb[0][numrec][0]] -
                     bmse[0][numrec][0][q]) / diffrate;
...
static void
wvq_loc(int *NumRec, float *ScaleWeight, int *NumRecScal, int *NStep,
        int *MultiCB, float *TargRate, Wtrans2d CodeBook2, Wtrans2d CodeBook3,
        Wtrans2d ResCodeBook1, Wtrans2d ResCodeBook2, Wtrans2d ResCodeBook3,
        Wtrans2d ResResCodeBook1, Wtrans2d ResResCodeBook2, Cimage Compress,
        Wtrans2d Wtrans, Wtrans2d CodeBook1, Wtrans2d Output)
/* Quantize wavelet transform up
 * to level NumRec */
/* Apply a weighting of coefficients
 * before quantization */
/* Scalar quantize wavelet subimages
 * at level NumRecScal and higher */
/* Number of steps in case of scalar
 * quantization of resume */
/* Not selected : one codebook per file
...
/* Compressed representation of Wtrans */
/* Input wavelet transform */
/* First sequence of codebooks for
 * for quantization */
/* Quantized wavelet transform (can be
 * reconstructed from compress */
{
    Fimage AdapCB2, AdapCB3;
    /* Codebooks for adaptive quantization
     * of details */
    Fimage ResCB1, ResCB2, ResCB3;
    /* Codebooks for residual
     * quantization  of detail */
    Fimage ResResCB1, ResResCB2;
    /* Codebooks for residual quantization
     * of details (second level) */
    int NLevelScal;
    /* Scalar quantize wavelet subimages
     * at level NLevelScal and higher */
    int J;
    /* Scale index for subimages */
    int i;
    /* Orientation index for subimages */
    int nadap;
    /* Adaptive class index for codebooks */
    int q, q1, q2, q3;
    /* Indices of codebooks used for
     * computation of distortion-rate
     * curves */
    int ncb1, ncb2, ncb3;
    /* Indices of codebooks
     * for quantization */
    int nrescb1, nrescb2, nrescb3;
    /* Indices of residual codebooks
     * for quantization */
    int nresrescb1, nresrescb2;
    /* Indices of residual codebooks
     * for quantization */
    int *ptrncb1 = NULL, *ptrncb2, *ptrncb3;
    /* Pointers to indices
     * of codebooks for quantization */
    int *ptrnrescb1 = NULL, *ptrnrescb2, *ptrnrescb3;
    /* Pointers to indices
     * of codebooks for quantization */
    int *ptrnresrescb1 = NULL, *ptrnresrescb2;
    /* Pointers to indices
     * of codebooks for quantization */
    int indext;
    int test_dr;
    double var1, var2, var3, var4;
    /* Varainces of different classes
     * in a subimage */
    double variance;
    /* Total variance of a subimage */
    long num1, num2, num3, num4, num;
    ...
    float allocweight;
    float frac;
    /* Inverse of subsampling factor
     * in different subimages */
    double Ent, RateAr;
    /* Entropy and rate with arithmetic
     * coding for subimages */
    double SNR;
    double msetot, MSE;
    /* m.s.e. for global image
     * and subimages */
    int bitmapcode;
    /* Encode bitmap even if all codebooks
     * have size 1 */
    bufind indcb;
    /* Buffer for indices of codebooks */
    bufind numcb1, numcb2;
    /* Buffer for number of codebooks
     * for each class and each subimage */
    int oldnumcb, resnumcb;
    int sizeb;
    /* Dimaension of vectors */
    int *Print, PPrint;
    /* Control of info print by fvq */
    int Print_Scal;
    /* control info print by fscalq */
    int testprint;
    /* test value for info print */
    int smallheader;
    /* Flag to specify small header
     * for fvq and fscalq */
    float rateheader;
    /* Bit rate to encode header */
    int center;
    /* Flag to specify centering of
     * quantization grid for fscalq */
    int nstep_dr;
    /* Number of step for scalar
     * quantization */
    ...
    rateheader =
        ((double) 2.0 * NBIT_SIZEIM + NBIT_NLEVEL + 1.0 +
         3.0 * (double) *NumRec) / (double) size;

  /*--- Memory allocation for wavelet transform Output ---*/

    if (mw_alloc_ortho_wtrans2d(Output, *NumRec, Wtrans->nrow, Wtrans->ncol)
        == NULL)
        mwerror(FATAL, 1,
                "Allocation of buffer for wavelet transform refused!");
  ...
  /*--- Set indices for codebooks to 0 ---*/
    ...
    /*--- Search best memory allocation between subimages ---*/
      ...
      /*--- Set number of codebooks to 0 for each subimage ---*/
            ...
            numcb[0][*NumRec][0] = numcb1[0][*NumRec][0] =
                numcb2[0][*NumRec][0] = 0;
            for (J = 1; J <= *NumRec; J++)
                for (i = 1; i <= 3; i++)
                    for (nadap = 0; nadap < nadapcb[J][i]; nadap++)
                        numcb[nadap][J][i] = numcb1[nadap][J][i] =
                            numcb2[nadap][J][i] = 0;
        }
      ...
      /*--- Compute approximated distorsion rate curves for details ---*/
          ...
          /*--- Compute variance of coefficients in each adaptive class ---*/
                    ...
                    fvariance(Wtrans, J, i, CodeBook1, CodeBook2, CodeBook3,
                              &var1, &var2, &var3, &var4, &num1, &num2, &num3,
                              &num4);
          ...
          /*--- Compute number of codebooks in each adaptive class ---*/
                    ...
                    {
                        sizeb = 1;
                        numcb1[0][J][i] = numcb2[0][J][i] = numcb[0][J][i] =
                            NUMSTEP_SCALQ;
                        nadapcb[J][i] = 1;
                        brl[0][J][i] = 0.0;
                        variance = 0.0;
                        var1 =
                            num1 * var1 + num2 * var2 + num3 * var3 +
                            num4 * var4;
                        num1 += num2 + num3 + num4;
                        var1 /= (double) num1;
                    }
                    else
                    {
                        ...
                        {
                            numcb[0][J][i] +=
                                count_cb(ResCodeBook1->images[J][i]);
                            numcb2[0][J][i] = numcb[0][J][i];
                            if (ResResCodeBook1)
                                numcb[0][J][i] +=
                                    count_cb(ResResCodeBook1->images[J][i]);
                        }
                        else
                            numcb2[0][J][i] = numcb[0][J][i];
                        ...
                        {
                            numcb[1][J][i] =
                                count_cb(CodeBook2->images[J][i]);
                            numcb1[1][J][i] = numcb[1][J][i];
                            if (ResCodeBook2)
                            {
                                numcb[1][J][i] +=
                                    count_cb(ResCodeBook2->images[J][i]);
                                numcb2[1][J][i] = numcb[1][J][i];
                                if (ResResCodeBook2)
                                    numcb[1][J][i] +=
                                        count_cb(ResResCodeBook2->images[J]
                                                 [i]);
                            }
                            else
                                numcb2[1][J][i] = numcb[1][J][i];
                        ...
                        {
                            numcb[2][J][i] =
                                count_cb(CodeBook3->images[J][i]);
                            numcb1[2][J][i] = numcb[2][J][i];
                            if (ResCodeBook2)
                                numcb[2][J][i] +=
                                    count_cb(ResCodeBook3->images[J][i]);
              ...
              /*--- Compute approximate rate for coding classes map ---*/
                        ...
                        if (num2 > 0)
                        {
                            brl[0][J][i] =
                                (num1 + num2 + num3 +
                                 num4) * log10((double) (num1 + num2 + num3 +
                                                         num4));
                            if (num1 > 0)
                                brl[0][J][i] -= num1 * log10((double) num1);
                            brl[0][J][i] -=
                                (num2 + num3 +
                                 num4) * log10((double) (num2 + num3 + num4));
                            brl[0][J][i] /= ((double) num1 + num2 + num3 +
                                             num4) * log10((double) 2.0) *
                                sizeb;
                            brl[0][J][i] *= frac;
                        }
                        else
          ...
          /*--- Compute approximate m.s.e. and rate for coding ---*/
                    ...
                    {
                        brate[0][J][i][q] =
                            num1 * q / (double) size / (double) sizeb +
                            brl[0][J][i];
                        bmse[0][J][i][q] =
                            num1 * var1 * bfacmse[0][J][i] * pow((double) 2.0,
                                                                 -(double) 2.0
                                                                 * q /
                                                                 (double)
                                                                 sizeb) /
                            (double) size + variance;
                    }
                    for (q = numcb1[0][J][i]; q < numcb2[0][J][i]; q++)
                    {
                        brate[0][J][i][q] =
                            num1 * (q -
                                    1.0) / (double) size / (double) sizeb +
                            brl[0][J][i];
                        bmse[0][J][i][q] =
                            num1 * var1 * bfacmse[0][J][i] * pow((double) 2.0,
                                                                 -(double) 2.0
          ...
          /*--- Compute approximate rate for coding classes map ---*/
                        ...
                        if (nadap == 1)
                        {
                            ...
                            if (num3 > 0)
                            {
                                ...
                                if (num2 > 0)
                                    brl[1][J][i] -=
                                        num2 * log10((double) num2);
                                brl[1][J][i] -=
                                    (num3 +
                                     num4) * log10((double) (num3 + num4));
                                brl[1][J][i] /= ((double) num1 + num2 + num3 +
                                                 num4) * log10((double) 2.0) *
                                    sizeb;
                                brl[1][J][i] *= frac;
                                brl[1][J][i] -= brl[0][J][i];
                        ...
                        else
                        {
                            ...
                            /* brl[3][J][i] = 0.0; */
                            fprintf(stderr,
                                    "abort, there is a bug in the code.");
                            exit(EXIT_FAILURE);
                        }
          ...
          /*--- Compute approximate m.s.e. and rate for coding ---*/
                     /*--- of quantization symbols ---*/
                        ...
                        {
                            brate[nadap][J][i][q] =
                                num * q / (double) size / (double) sizeb +
                                brl[nadap][J][i];
                            bmse[nadap][J][i][q] =
                                variance * bfacmse[nadap][J][i] *
                                (pow
                                 ((double) 2.0,
                                  -(double) 2.0 * q / (double) sizeb) - 1.0);
                        }
                        for (q = numcb1[nadap][J][i]; q < numcb2[nadap][J][i];
                             q++)
                        {
                            brate[nadap][J][i][q] =
                                num * (q -
                        ...
                        for (q = numcb2[nadap][J][i]; q < numcb[nadap][J][i];
                             q++)
                        {
                            brate[nadap][J][i][q] =
                                num * (q -
      ...
      /*--- Compute variance of coefficients ---*/
            ...
            fvariance(Wtrans, *NumRec, 0, CodeBook1, CodeBook2, CodeBook3,
                      &var1, &var2, &var3, &var4, &num1, &num2, &num3, &num4);
      ...
      /*--- Compute number of codebooks and prepare codebooks ---*/
            ...
            else
            {
                ...
                {
                    numcb[0][*NumRec][0] +=
                        count_cb(ResCodeBook1->images[*NumRec][0]);
                    numcb2[0][*NumRec][0] = numcb[0][*NumRec][0];
                    if (ResResCodeBook1)
                        numcb[0][*NumRec][0] +=
                            count_cb(ResResCodeBook1->images[*NumRec][0]);
                }
                else
                    numcb2[0][*NumRec][0] = numcb[0][*NumRec][0];
            }
      ...
      /*--- Compute approximate m.s.e. and rate for coding ---*/
            ...
            for (q = 0; q < numcb1[0][*NumRec][0]; q++)
            {
                brate[0][*NumRec][0][q] =
                    num1 * q / (double) size / (double) sizeb;
                bmse[0][*NumRec][0][q] =
                    num1 * var1 * bfacmse[0][*NumRec][0] * pow((double) 2.0,
                                                               -(double) 2.0 *
                                                               q /
                                                               (double) sizeb)
                    / (double) size;
            ...
            {
                brate[0][*NumRec][0][q] =
                    num1 * (q - 1.0) / (double) size / (double) sizeb;
                bmse[0][*NumRec][0][q] =
                    num1 * var1 * bfacmse[0][*NumRec][0] * pow((double) 2.0,
                                                               -(double) 2.0 *
                                                               (q -
            ...
            {
                brate[0][*NumRec][0][q] =
                    num1 * (q - 2.0) / (double) size / (double) sizeb;
                bmse[0][*NumRec][0][q] =
                    num1 * var1 * bfacmse[0][*NumRec][0] * pow((double) 2.0,
                                                               -(double) 2.0 *
                                                               (q -
            ...
            for (J = 1; J <= *NumRec; J++)
            {
                ...
                for (i = 1; i <= 3; i++)
                {
                    ...
                    if ((numcb[0][J][i] <= 0) || (J > NLevelScal))
                    {
                        ...
                        for (q = 0; q < numcb[0][J][i]; q++)
                        {
                            if (q > 0)
                                ...
                                nstep_dr = 1;
                            fscalq(Print, &smallheader, &nstep_dr, NULL,
                                   &center, NULL, Wtrans->images[J][i],
                                   Output->images[J][i], &MSE, &SNR, &Ent,
                                   &RateAr);
                    ...
                    else
                    {
                        ...
                        for (q = 0; q < numcb[0][J][i]; q++)
                        {
                            fvq(Print, NULL, &bitmapcode, NULL, &q, NULL,
                                NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                                NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                                NULL, NULL, NULL, NULL, Wtrans->images[J][i],
                                CodeBook1->images[J][i], Output->images[J][i],
                                &MSE, &SNR, &Ent, &RateAr);
                            if ((q == 0)
                                && (CodeBook1->images[J][i]->
                                    gray[(CodeBook1->images[J][i]->nrow -
                                          3) * sizeb] != 0.0))
                            {
                                brate[0][J][i][q] = 0.0;
                                bmse[0][J][i][q] =
                                    frac * det_energy(Wtrans->images[J][i]);
                        ...
                        if (ResCodeBook1)
                            if ((ResCodeBook1->images[J][i]->nrow > 6)
                                && (ResCodeBook1->images[J][i]->ncol ==
                                    sizeb))
                            {
                                q1 = numcb[0][J][i] - 1;
                                ...
                                {
                                    fvq(Print, NULL, &bitmapcode, NULL, &q1,
                                        NULL, NULL, NULL, NULL, NULL, NULL,
                                        &q, NULL, NULL, NULL,
                                        ResCodeBook1->images[J][i], NULL,
                                        NULL, NULL, NULL, NULL, NULL, NULL,
                                        NULL, Wtrans->images[J][i],
                                        CodeBook1->images[J][i],
                                        Output->images[J][i], &MSE, &SNR,
                                        &Ent, &RateAr);
                                    bmse[0][J][i][oldnumcb + q] = frac * MSE;
                                    brate[0][J][i][oldnumcb + q] =
                                        frac * RateAr;
                                }

                                if (ResResCodeBook1)
                                    if ((ResResCodeBook1->images[J][i]->nrow >
                                         6)
                                        && (ResResCodeBook1->images[J][i]->
                                            ncol == sizeb))
                                    {
                                        q2 = resnumcb - 1;
                                        oldnumcb = numcb[0][J][i];
                                        resnumcb =
                                            count_cb(ResResCodeBook1->images
                                                     [J][i]);
                                        numcb[0][J][i] += resnumcb;
                                        for (q = 0; q < resnumcb; q++)
                                        {
                                            fvq(Print, NULL, &bitmapcode,
                                                NULL, &q1, NULL, NULL, NULL,
                                                NULL, NULL, NULL, &q2, NULL,
                                                NULL, NULL,
                                                ResCodeBook1->images[J][i],
                                                NULL, NULL, NULL, &q, NULL,
                                                ResResCodeBook1->images[J][i],
                                                NULL, NULL,
                                                Wtrans->images[J][i],
                                                CodeBook1->images[J][i],
                                                Output->images[J][i], &MSE,
                                                &SNR, &Ent, &RateAr);
                                            bmse[0][J][i][oldnumcb + q] =
                                                frac * MSE;
                                            brate[0][J][i][oldnumcb + q] =
                                                frac * RateAr;
                                        }
                        ...
                        if (CodeBook2 && (nadapcb[J][i] >= 2))
                            if ((CodeBook2->images[J][i]->nrow > 6)
                                && (CodeBook2->images[J][i]->ncol == sizeb))
                            {
                                q1 = 1;
                                numcb[1][J][i] =
                                    count_cb(CodeBook2->images[J][i]);
                                for (q = 0; q < numcb[1][J][i]; q++)
                                {
                                    fvq(Print, NULL, &bitmapcode, NULL, &q1,
                                        &q, NULL, NULL,
                                        CodeBook2->images[J][i], NULL, NULL,
                                        NULL, NULL, NULL, NULL, NULL, NULL,
                                        NULL, NULL, NULL, NULL, NULL, NULL,
                                        NULL, Wtrans->images[J][i],
                                        CodeBook1->images[J][i],
                                        ...
                                        &Ent, &RateAr);
                                    if ((q == 0)
                                        && (CodeBook2->images[J][i]->
                                            gray[(CodeBook2->images[J][i]->
                                                  nrow - 3) * sizeb] != 0.0))
                                    {
                                        brate[1][J][i][q] = 0.0;
                                        bmse[1][J][i][q] = 0.0;
                                ...
                                if (ResCodeBook2)
                                    if ((ResCodeBook2->images[J][i]->nrow > 6)
                                        && (ResCodeBook2->images[J][i]->
                                            ncol == sizeb))
                                    {
                                        q2 = numcb[1][J][i] - 1;
                                        oldnumcb = numcb[1][J][i];
                                        resnumcb =
                                            count_cb(ResCodeBook2->images[J]
                                                     [i]);
                                        numcb[1][J][i] += resnumcb;
                                        for (q = 0; q < resnumcb; q++)
                                        {
                                            fvq(Print, NULL, &bitmapcode,
                                                NULL, &q1, &q2, NULL, NULL,
                                                CodeBook2->images[J][i], NULL,
                                                ...
                                                Output->images[J][i], &MSE,
                                                &SNR, &Ent, &RateAr);
                                            bmse[1][J][i][oldnumcb + q] =
                                                frac * MSE -
                                                bmse[0][J][i][q1];
                                            brate[1][J][i][oldnumcb + q] =
                                                frac * RateAr -
                                                brate[0][J][i][q1];
                                        }

                                        if (ResResCodeBook2)
                                            if ((ResResCodeBook2->images[J]
                                                 [i]->nrow > 6)
                                                &&
                                                (ResResCodeBook2->images[J]
                                                 [i]->ncol == sizeb))
                                            {
                                                q3 = resnumcb - 1;
                                                oldnumcb = numcb[1][J][i];
                                ...
                                if (CodeBook3 && (nadapcb[J][i] >= 3))
                                            ...
                                            sizeb))
                                    {
                                        ...
                                        if (ResCodeBook3)
                                            if ((ResCodeBook3->images[J][i]->
                                                 nrow > 6)
                                                &&
                                                (ResCodeBook3->images[J][i]->
                                                 ncol == sizeb))
                                            {
                                                q3 = numcb[2][J][i] - 1;
        ...
        /*--- Compute distorsion rate curves for average ---*/
            ...
            if ((numcb[0][*NumRec][0] <= 0) || (*NumRec >= NLevelScal))
            {
                ...
                {
                    fscalq(Print, &smallheader, &nstep_dr, NULL, NULL, NULL,
                           Wtrans->images[*NumRec][0],
                           Output->images[*NumRec][0], &MSE, &SNR, &Ent,
                           &RateAr);
            ...
            else
            {
                ...
                {
                    fvq(Print, NULL, NULL, NULL, &q, NULL, NULL, NULL, NULL,
                        NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                        NULL, NULL, NULL, NULL, NULL, NULL,
                        Wtrans->images[*NumRec][0],
                        CodeBook1->images[*NumRec][0],
                        Output->images[*NumRec][0], &MSE, &SNR, &Ent,
                        &RateAr);
                    bmse[0][*NumRec][0][q] = frac * MSE;
                    brate[0][*NumRec][0][q] = frac * RateAr;
                ...
                if (ResCodeBook1->images[*NumRec][0])
                    if ((ResCodeBook1->images[*NumRec][0]->nrow > 6)
                        && (ResCodeBook1->images[*NumRec][0]->ncol == sizeb))
                    {
                        ...
                        for (q = 0; q < resnumcb; q++)
                        {
                            fvq(Print, NULL, NULL, NULL, &q1, NULL, NULL,
                                NULL, NULL, NULL, NULL, &q, NULL, NULL, NULL,
                                ResCodeBook1->images[*NumRec][0], NULL, NULL,
                                NULL, NULL, NULL, NULL, NULL, NULL,
                                Wtrans->images[*NumRec][0],
                                CodeBook1->images[*NumRec][0],
                                Output->images[*NumRec][0], &MSE, &SNR, &Ent,
                            ...
                            brate[0][*NumRec][0][oldnumcb + q] =
                                frac * RateAr;
                        }

                        if (ResResCodeBook1->images[*NumRec][0])
                            if ((ResResCodeBook1->images[*NumRec][0]->nrow >
                                 6)
                                && (ResResCodeBook1->images[*NumRec][0]->
                                    ncol == sizeb))
                            {
                                q2 = resnumcb - 1;
                                oldnumcb = numcb[0][*NumRec][0];
                                resnumcb =
                                    count_cb(ResResCodeBook1->images[*NumRec]
                                             [0]);
                                numcb[0][*NumRec][0] += resnumcb;
                                for (q = 0; q < resnumcb; q++)
                                {
                                    fvq(Print, NULL, NULL, NULL, &q1, NULL,
                                        NULL, NULL, NULL, NULL, NULL, &q2,
                                        NULL, NULL, NULL,
                                        ResCodeBook1->images[*NumRec][0],
                                        NULL, NULL, NULL, &q, NULL,
                                        ResResCodeBook1->images[*NumRec][0],
                                        NULL, NULL,
                                        Wtrans->images[*NumRec][0],
                                        CodeBook1->images[*NumRec][0],
                                        Output->images[*NumRec][0], &MSE,
                                        &SNR, &Ent, &RateAr);
                                    bmse[0][*NumRec][0][oldnumcb + q] =
                                        frac * MSE;
                                    brate[0][*NumRec][0][oldnumcb + q] =
                                        frac * RateAr;
                                }
    ...
    /*--- Multiply m.s.e. errors by a weight in order to get better ---*/
        ...
        /*--- Compute distorsion-rate curve for the wavelet transform ---*/
          ...
          /*--- Compute indices of codebooks for optimal allocation ---*/
                    ...
                    compute_indexcb(indcb, targrate_dr[q], rateheader,
                                    *NumRec);
          ...
          /*--- Compute resulting m.s.e. and rate ---*/
                    ...
                    printf("%.4f  %.2f\n", RateAr,
                           10.0 * log10((double) 255.0 * 255.0 / MSE));
                }
            }
    ...
    /*--- Perform vector quantization using indices of codebooks ---*/
        ...
        INIT_ENCODING_WTRANS(Wtrans->nrow, Wtrans->ncol, *NumRec, CodeBook1,
                             NLevelScal, Compress);
    ...
    /*--- Quantization of details ----*/
        ...
        for (J = 1; J <= *NumRec; J++)
        {
            ...
            if (CodeBook1->images[J][1])
                ...
                {
                    indext =
                        (CodeBook1->images[J][1]->nrow -
                         3) * CodeBook1->images[J][1]->ncol;
                    if ((CodeBook1->images[J][1]->gray[indext] > 0.0)
                        && (testprint == 1))
                        printf("\nLevel %d :\n", J);
                }
          ...
          /*--- Scalar quantization ---*/
                    ...
                    fscalq(&Print_Scal, &smallheader, &nstep_dr, NULL,
                           &center, Compress, Wtrans->images[J][i],
                           Output->images[J][i], &MSE, &SNR, &Ent, &RateAr);
            ...
            /*--- Vector quantization ---*/
                    ...
                    indext =
                        (CodeBook1->images[J][i]->nrow -
                         3) * CodeBook1->images[J][i]->ncol;
                    if ((CodeBook1->images[J][i]->gray[indext] > 0.0)
                        && (testprint == 1))
                        printf("Detail %d :\n", i);
              ...
              /*--- Prepare indices for codebooks for details ---*/
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (resnumcb < oldnumcb)
                                    ...
                                    mwerror(FATAL, 2,
                                            "Something wrong "
                                            "with value of codebook "
                                            "index for detail %d/%d.\n", J,
                                            i);
                            }
                        }
                    ...
                    fvq(Print, &smallheader, NULL, NULL, ptrncb1, ptrncb2,
                        ptrncb3, NULL, AdapCB2, AdapCB3, NULL, ptrnrescb1,
                        ptrnrescb2, ptrnrescb3, NULL, ResCB1, ResCB2, ResCB3,
                        NULL, ptrnresrescb1, ptrnresrescb2, ResResCB1,
                        ResResCB2, Compress, Wtrans->images[J][i],
                        CodeBook1->images[J][i], Output->images[J][i], &MSE,
                        &SNR, &Ent, &RateAr);
                }
    ...
    /*--- Quantization of resume ---*/
        ...
        if ((count_cb(CodeBook1->images[*NumRec][0]) <= 0)
            || (*NumRec >= NLevelScal))
        {
      ...
      /*--- Scalar quantization ---*/
            ...
            fscalq(&Print_Scal, &smallheader, &nstep_dr, NULL, NULL, Compress,
                   Wtrans->images[*NumRec][0], Output->images[*NumRec][0],
                   &MSE, &SNR, &Ent, &RateAr);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (resnumcb < oldnumcb)
                            ...
                            mwerror(FATAL, 2,
                                    "Something wrong with value of "
                                    "codebook index for average.\n");
                    }
                }

            }

            fvq(Print, &smallheader, NULL, NULL, ptrncb1, NULL, NULL, NULL,
                NULL, NULL, NULL, ptrnrescb1, NULL, NULL, NULL,
                ResCodeBook1->images[*NumRec][0], NULL, NULL, NULL,
                ptrnresrescb1, NULL, ResResCodeBook1->images[*NumRec][0],
                NULL, Compress, Wtrans->images[*NumRec][0],
                CodeBook1->images[*NumRec][0], Output->images[*NumRec][0],
                &MSE, &SNR, &Ent, &RateAr);
    ...
    /*--- print info on quantization of each sub-image ---*/
        ...
        msetot =
            bmse[0][*NumRec][0][0] *
            (float) Output->images[*NumRec][0]->nrow *
            Output->images[*NumRec][0]->nrow;
        ...
        for (J = 1; J <= *NumRec; J++)
        {
            ...
            {
                rateartot +=
                    brate[0][J][i][0] * (float) Wtrans->images[J][i]->nrow *
                    Wtrans->images[J][i]->ncol;
                msetot +=
                    bmse[0][J][i][0] * (float) Wtrans->images[J][i]->nrow *
                    Wtrans->images[J][i]->ncol;
                if (testprint == 1)
                {
                    if (CodeBook3)
                        printf("  %2d/%2d/%d\t     %d", indcb[0][J][i],
                               indcb[1][J][i], indcb[2][J][i],
                               CodeBook1->images[J][i]->ncol);
                    else if (CodeBook2)
                        printf("   %2d/%d \t     %d", indcb[0][J][i],
                               indcb[1][J][i], CodeBook1->images[J][i]->ncol);
                    else
                        printf("     %2d\t\t     %d", indcb[0][J][i],
                               CodeBook1->images[J][i]->ncol);
        ...
        if (testprint == 1)
        {
            printf("\nAver. Entropy  = %f (%d),  aver. m.s.e. = %f\n",
                   brate[0][*NumRec][0][0], indcb[0][*NumRec][0],
                   bmse[0][*NumRec][0][0]);
            printf
                ("\nTotal Entropy  = %f, total m.s.e. = %f, "
                 "total PSNR = %f\n",
...
static void
fwvq_wcb(int *NumRec, Fimage Edge_Ri, Fsignal Ri2, int *FilterNorm,
         float *WeightFac, int *NumRecScal, int *NStep, int *MultiCB,
         Wtrans2d CodeBook2, Wtrans2d CodeBook3, Wtrans2d ResCodeBook1,
         Wtrans2d ResCodeBook2, Wtrans2d ResCodeBook3,
         Wtrans2d ResResCodeBook1, Wtrans2d ResResCodeBook2, int *DistRate,
         float *TargRate, Cimage Output, Fimage Image, Wtrans2d CodeBook1,
         Fsignal Ri, Fimage QImage)
...
/* Output quantized image */
{
    ...
    /* Size of input image */
    bufmse loc_bmse;
    /* Local buffer for m.s.e. for subimages
     * and different codebooks */
    bufmse loc_brate;
    /* Local buffer for rate for subimages
     * and different codebooks */
    bufrl loc_brl;
    /* Local buffer for classes map rates for
     * subimages and different codebooks */
    float targrate;
    /* Target rate for vector quantization */
    Wtrans2d Wtrans = NULL, QWtrans;
    /* Wavelet transform of Image, quantized
     * wavelet transform */
    Fimage QImage_dr;
    /* Reconstructed image (for biorthogonal
     * wavelet transform and dist.rate curve) */
    int multicb_dr;
    /* flag for memory allocation in wvq */
    int *multicb_null;
    /* flag for multi codebooks in wvq */
    int Haar;
    ...
    int Precond;
    /* Preconditionning mode for orthogonal
     * transform */
    double mse;
    /* Mean square error between Image and Qimage_dr */
    double mrd;
    /* Maximal relative difference */
    double snr;
    /* Signal to noise ratio / `Image` */
    double psnr;
    /* Peak signal to noise ratio / `Image` */
    char PsnrFlg = '1';
    /* To compute 255-PSNR */
  ...
  /*--- Wavelet decomposition ---*/
    ...
    else
    {
        ...
        owave2(&NumLev, &Haar, &Edge, &Precond, NULL, &FiltNorm, Image,
               Wtrans, Ri, Edge_Ri);
    }
  ...
  /*--- Set target bit rates for R-D curve ---*/
    ...
    if (DistRate
        && (Ri2 || (multicb_dr == 4) || WeightFac || (FiltNorm == 1)))
    {
    ...
    /*--- Compute R-D curve in case of biorthogonal transform ---*/
        ...
        wvq_loc(&NumLev, WeightFac, NumRecScal, NStep, &multicb_dr, NULL,
                CodeBook2, CodeBook3, ResCodeBook1, ResCodeBook2,
                ResCodeBook3, ResResCodeBook1, ResResCodeBook2, NULL, Wtrans,
                CodeBook1, QWtrans);
        ...
        for (count_dr = 0; count_dr <= max_count_dr; count_dr++)
        {
            ...
            wvq_loc(&NumLev, WeightFac, NumRecScal, NStep, &multicb_dr,
                    &targrate, CodeBook2, CodeBook3, ResCodeBook1,
                    ResCodeBook2, ResCodeBook3, ResResCodeBook1,
                    ResResCodeBook2, NULL, Wtrans, CodeBook1, QWtrans);
            ...
            if (Ri2)
            {
                ...
                ibiowave2(&NumLev, &Haar, &Edge, &FiltNorm, QWtrans,
                          QImage_dr, Ri, Ri2);
            }
            else
            {
                REFRESH_FILTERS(Ri, NULL, Edge_Ri);
                iowave2(&NumLev, &Haar, &Edge, &Precond, NULL, &FiltNorm,
                        QWtrans, QImage_dr, Ri, Edge_Ri);
            }
    ...
    else
    {
      ...
      /*--- Compute R-D curve in case of orthogonal transform ---*/
        ...
        if (TargRate || DistRate)
            ...
            multicb_null = NULL;
        wvq_loc(&NumLev, WeightFac, NumRecScal, NStep, multicb_null, TargRate,
                CodeBook2, CodeBook3, ResCodeBook1, ResCodeBook2,
                ResCodeBook3, ResResCodeBook1, ResResCodeBook2, Output,
                Wtrans, CodeBook1, QWtrans);
    }
  ...
  /*--- Wavelet reconstruction ---*/
    ...
    if (QImage && !DistRate)
        ...
        {
            REFRESH_FILTERS(Ri, Ri2, NULL);
            ibiowave2(&NumLev, &Haar, &Edge, &FiltNorm, QWtrans, QImage, Ri,
                      Ri2);
        ...
        else
        {
            if (FiltNorm == 1)
                REFRESH_FILTERS(Ri, NULL, Edge_Ri);
            iowave2(&NumLev, &Haar, &Edge, &Precond, NULL, &FiltNorm, QWtrans,
                    QImage, Ri, Edge_Ri);
...
void
fwvq(int *NumRec, Fimage Edge_Ri, Fsignal Ri2, int *FilterNorm,
     float *WeightFac, int *NumRecScal, int *NStep, int *MultiCB,
     Fimage CodeBook2, Fimage CodeBook3, Fimage ResCodeBook1,
     Fimage ResCodeBook2, Fimage ResCodeBook3, Fimage ResResCodeBook1,
     Fimage ResResCodeBook2, int *DistRate, float *TargRate, Cimage Output,
     Fimage Image, Fimage CodeBook1, Fsignal Ri, Fimage QImage)
/*--- Computes the orthogonal wavelet transform of image `Image` ---*/
/* Number of recursion (-j) */
/* Impulse responses of filters for special
 * edge processing (including preconditionning
...
/* Output quantized image */
{
    Wtrans2d WCodeBook1, WCodeBook2, WCodeBook3;
    /* Sequence of codebooks
     * extracted form CodeBook1, CodeBook2
     * and CodeBook3 */
    Wtrans2d WResCodeBook1, WResCodeBook2, WResCodeBook3;
    /* Sequence of
     * codebooks extracted form ResCodeBook1,
     * ResCodeBook2 and ResCodeBook3 */
    Wtrans2d WResResCodeBook1, WResResCodeBook2;
    /* Sequence of
  ...
  /*--- Wavelet transform of the input image and vector quantization ---*/
    ...
    fwvq_wcb(NumRec, Edge_Ri, Ri2, FilterNorm, WeightFac, NumRecScal, NStep,
             MultiCB, WCodeBook2, WCodeBook3, WResCodeBook1, WResCodeBook2,
             WResCodeBook3, WResResCodeBook1, WResResCodeBook2, DistRate,
             TargRate, Output, Image, WCodeBook1, Ri, QImage);
