...
function = {"Generates training set(s)
            (starting from wavelet transform(s))
            and constructs a codebook sequence
            for wavelet transform vector quantization using LBG algorithm"};
usage = {
 'r':[MaxLevel=1]->NumRecMax [1,16]  "Quantization is performed
                                     on MaxLevel scales",
 ...
 'M'->MultiCB                    "Generate codebooks of size
 ...
 'Q':ResCodeBook->ResCodeBook     "Generate codebook(s)
                                  ...
                                  with ResCodeBook (wtrans2d)",
 'R':ResResCodeBook->ResResCodeBook "Generate codebook(s)
                                    for residu of quantization
                                    with ResCodeBook
                                    and ResResCodeBook (wtrans2d)",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for flbg(), flbg_adap() */

/*--- Constants ---*/

static void WSUBIMCHANGE(Wtrans2d codebook, int nlevel)
{
    int i, j;
    ...
    for (j = nlevel + 1; j <= codebook->nlevel; j++)
        for (i = 0; i <= 3; i++)
            if (mw_change_fimage(codebook->images[j][i], 1, 1) == NULL)
                mwerror(FATAL, 1,
                        "Not enough memory "
                        "for wavelet transform buffer for codebook.\n");
...
void
wlbg_adap(int *NumRecMax, int *Level, int *Orient, int *Dyadic, int *Lap,
          int *Edge, int *Width, int *Height, int *MultiCB, int *Sizec1,
          int *Sizec2, int *Sizec3, float *ThresVal1, float *ThresVal2,
          float *ThresVal3, Wtrans2d OldCodeBook, Wtrans2d OldAdapCodeBook2,
          Wtrans2d OldAdapCodeBook3, Wtrans2d * Output2, Wtrans2d * Output3,
          Wtrans2d TrainWtrans2, Wtrans2d TrainWtrans3, Wtrans2d TrainWtrans4,
...
/* Sequence of generated codebooks */
{
    ...
    Fimage ResCB, ResResCB;
    /* Residual codebooks for subimages */
    Fimage AdapCB2, AdapCB3;
    /* Codebooks for adaptive quantization
     * of details */
    Fimage TrainIm1, TrainIm2, TrainIm3, TrainIm4;
    /* Training
        ...
        if (numrec > OldCodeBook->nlevel)
        {
            *Output1 = mw_new_wtrans2d();
            if (mw_alloc_ortho_wtrans2d(*Output1, numrec, nrow, ncol) == NULL)
                mwerror(FATAL, 1,
                        "Allocation of wavelet transform "
                        "for codebook refused!\n");
        ...
        if (numrec > OldAdapCodeBook2->nlevel)
        {
            *Output2 = mw_new_wtrans2d();
            if (mw_alloc_ortho_wtrans2d(*Output2, numrec, nrow, ncol) == NULL)
                mwerror(FATAL, 1,
                        "Allocation of wavelet transform "
                        "for adaptive codebook 2 refused!\n");
        ...
        if (numrec > OldAdapCodeBook3->nlevel)
        {
            *Output3 = mw_new_wtrans2d();
            if (mw_alloc_ortho_wtrans2d(*Output3, numrec, nrow, ncol) == NULL)
                mwerror(FATAL, 1,
                        "Allocation of wavelet transform "
                        "for adaptive codebook 3 refused!\n");
    ...
    if (!Orient)
    {
        ...
        /*--- Genarate codebook for average sub-image at level numrec ---*/
          ...
          /*--- If not, do not generate codebook for average ---*/
                ...
                {
                    testsize = (float)
                        floor(ResCodeBook->images[J2][0]->
                              gray[(sizec - 6) * sizeb] + .5);
                    if ((ResCodeBook->images[J2][0]->
                         gray[(sizec - 6) * sizeb] != testsize)
                        || (testsize < 1.0))
                        testres = 0;
                    testsize = (float)
                        floor(ResCodeBook->images[J2][0]->
                              gray[(sizec - 5) * sizeb] + .5);
                    if ((ResCodeBook->images[J2][0]->
                         gray[(sizec - 5) * sizeb] != testsize)
                        || (testsize < 1.0))
                        testres = 0;
                }
            }
          ...
          /*--- Check that ResResCodeBook content is consistent ---*/
            /*--- If not, do not generate codebook for average ---*/
                ...
                {
                    testsize = (float)
                        floor(ResResCodeBook->images[J2][0]->
                              gray[(sizec - 6) * sizeb] + .5);
                    if ((ResResCodeBook->images[J2][0]->
                         gray[(sizec - 6) * sizeb] != testsize)
                        || (testsize < 1.0))
                        testres = 0;
                    testsize = (float)
            ...
            if (testres == 1)
            {
                ...
                if (MultiCB)
                    if (ResCodeBook)
                    {
                        ...
                        {
                            nresres =
                                count_cb(ResResCodeBook->images[J2][0]) - 1;
                            pnresres = &nresres;
                        }
                    }
                ...
                flbg(Sizec1, Width, Height, Lap, &decim, Edge, NULL, MultiCB,
                     &Print, TrainIm2, TrainIm3, TrainIm4, NULL, NULL, NULL,
                     NULL, NULL, NULL, pnres, pnresres, ResCB, ResResCB,
                     TrainIm1, (*Output1)->images[J2][0], &mse);
                printf("m.s.e. 0 = %f\n", mse);
            }
        }
    }
    ...
    for (J = J2; J >= J1; J--)
    {
      ...
      /*--- Genarate codebook for sub-image at level J and orientation i ---*/
        ...
        /*--- Generate adaptive codebooks for classified quantization ---*/
                ...
                flbg_adap(Sizec1, Width, Height, Lap, &decim, Edge, ThresVal1,
                          ThresVal2, ThresVal3, NULL, MultiCB, &Print, Sizec2,
                          Sizec3, NULL, TrainIm2, TrainIm3, TrainIm4, NULL,
                          NULL, NULL, NULL, AdapCB2, AdapCB3, NULL, TrainIm1,
                          (*Output1)->images[J][i]);
            }
            else
            {
          ...
          /*--- Generate codebooks for plain vector quantization ---*/
           ...
           /*--- If not, do not generate codebook for sub-image ---*/
                    ...
                    {
                        testsize = (float)
                            floor(ResCodeBook->images[J][i]->
                                  gray[(sizec - 6) * sizeb] + .5);
                        if ((ResCodeBook->images[J][i]->
                             gray[(sizec - 6) * sizeb] != testsize)
                            || (testsize < 1.0))
                            testres = 0;
                        testsize = (float)
            ...
            /*--- Check that ResResCodeBook content is consistent ---*/
             /*--- If not, do not generate codebook for sub-image ---*/
                    ...
                    {
                        testsize = (float)
                            floor(ResResCodeBook->images[J][i]->
                                  gray[(sizec - 6) * sizeb] + .5);
                        if ((ResResCodeBook->images[J][i]->
                             gray[(sizec - 6) * sizeb] != testsize)
                            || (testsize < 1.0))
                            testres = 0;
                        testsize = (float)
                ...
                if (testres == 1)
                {
                    ...
                    if (MultiCB)
                        if (ResCodeBook)
                        {
                            ...
                            {
                                nresres =
                                    count_cb(ResResCodeBook->images[J][i]) -
                                    1;
                                pnresres = &nresres;
                            }
                        }
                    ...
                    flbg(Sizec1, Width, Height, Lap, &decim, Edge, NULL,
                         MultiCB, &Print, TrainIm2, TrainIm3, TrainIm4, NULL,
                         NULL, NULL, NULL, NULL, NULL, pnres, pnresres, ResCB,
                         ResResCB, TrainIm1, (*Output1)->images[J][i], &mse);
                    printf("m.s.e. %d = %f\n", i, mse);
                }
            }
