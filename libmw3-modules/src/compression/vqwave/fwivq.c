...
function = {"Decompress an image compressed by vector quantization
...
usage = {
 ...
 'A':ResCodeBook1->ResCodeBook1
        "Sequence of codebooks for residu quantization
 ...
 'D':ResResCodeBook1->ResResCodeBook1
        "Sequence of codebooks for residu quantization
        after quantization with CodeBook1 and ResCodeBook1 (fimage)",
 'E':ResResCodeBook2->ResResCodeBook2
        "Sequence of codebooks for residu quantization
        after quantization with CodeBook2 and ResCodeBook2 (fimage)",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for iowave2(), ibiowave2(),
                                 * fivq(), fiscalq() */
...
static int READ_BIT_WTRANS(void)
{
    ...
    if (bits_to_go == 0)
    {
        if (ncwreadow == sizecomp)
            mwerror(FATAL, 1,
                    "Buffer ended to soon while reading"
                    " header for wavelet transform!\n");
        else
        {
            ptrc++;
            buffer_wtrans = *ptrc;
...
static void DECODE_INT_WTRANS(int *symb, int max)
...
static void
READ_HEADER_WTRANS(int *nrow, int *ncol, int *nlevel, int (*scalvec)[4],
                   Cimage compress)
                                 /* Size of image */
                                 ...
                                 /* Compressed wavelet transform */
{
  ...
  /*--- Read number of levels in wavelet transform ---*/
    ...
    if ((*nlevel == 0) || (*nrow == 0) || (*ncol == 0))
        mwerror(FATAL, 4,
                "Bad value for size of image or number of levels!\n");
  ...
  /*--- Reconstruction of details ---*/
    ...
    for (J = 1; J <= NLevel; J++)
    {
        ...
        for (i = 1; i <= 3; i++)
        {
            ...
            if (ScalVec[J][i] == 0)
                fiscalq(&Print, &NRow, &NCol, Compress, Output->images[J][i],
                        &RateAr);
            else
            {
                ...
                fivq(&Print, &NRow, &NCol, AdapCB2, AdapCB3, NULL, ResCB1,
                     ResCB2, ResCB3, NULL, ResResCB1, ResResCB2, Compress,
                     CodeBook1->images[J][i], Output->images[J][i], &RateAr);
            }
        }
    }
    ...
    if (ScalVec[NLevel][0] == 0)
        fiscalq(&Print, &NRow, &NCol, Compress, Output->images[NLevel][0],
                &RateAr);
    else
    {
        ...
        fivq(&Print, &NRow, &NCol, NULL, NULL, NULL, ResCB, NULL, NULL, NULL,
             ResResCB, NULL, Compress, CodeBook1->images[NLevel][0],
             Output->images[NLevel][0], &RateAr);
    }
}

static void
fwivq_wcb(Wtrans2d CodeBook2, Wtrans2d CodeBook3, Wtrans2d ResCodeBook1,
          Wtrans2d ResCodeBook2, Wtrans2d ResCodeBook3,
          Wtrans2d ResResCodeBook1, Wtrans2d ResResCodeBook2, Fimage Edge_Ri,
          Fsignal Ri2, int *FilterNorm, Cimage Compress, Wtrans2d CodeBook1,
          Fimage Output, Fsignal Ri)
...
/* Impulse response of the low pass filter */
{
  ...
  /*--- Reconstruct wavelet transform ---*/
    ...
    wivq_loc(CodeBook2, CodeBook3, ResCodeBook1, ResCodeBook2, ResCodeBook3,
             ResResCodeBook1, ResResCodeBook2, Compress, CodeBook1, QWtrans);
  ...
  /*--- Inverse wavelet transform ---*/
    ...
    else
    {
        ...
        if (FilterNorm)
            ...
            FiltNorm = 2;
        Precond = 0;
        iowave2(&J, &Haar, &Edge, &Precond, NULL, &FiltNorm, QWtrans, Output,
                Ri, Edge_Ri);
    }
...
void
fwivq(Fimage Edge_Ri, Fsignal Ri2, int *FilterNorm, float *WeightFac,
      Fimage CodeBook2, Fimage CodeBook3, Fimage ResCodeBook1,
      Fimage ResCodeBook2, Fimage ResCodeBook3, Fimage ResResCodeBook1,
      Fimage ResResCodeBook2, Cimage Compress, Fimage CodeBook1, Fsignal Ri,
      Fimage Output)
/*--- Computes the orthogonal wavelet transform of image `Image` ---*/
/* Impulse responses of filters for special
 * edge processing (including preconditionning
...
/* Impulse response of the low pass filter */
{
  ...
  /*--- Reconstruction of the image from the Compress file ---*/
    ...
    fwivq_wcb(WCodeBook2, WCodeBook3, WResCodeBook1, WResCodeBook2,
              WResCodeBook3, WResResCodeBook1, WResResCodeBook2, Edge_Ri, Ri2,
              FilterNorm, Compress, WCodeBook1, Output, Ri);
