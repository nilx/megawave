...
usage = {
 ...
 'd':[Decim=1]->Decim          "Decimation factor
                               in training images (for wavelet transforms)",
 'e':[Edge=0]->Edge         "Do not take overlapping vectors
                            if the distance to an edge is smaller than Edge",
...
static void
THRES_ADAP(int width, int height, float *thresval1, float *thresval2,
           float *thresval3, Fimage image, Fimage image2, Fimage image3,
           Fimage image4, Fimage image5, Fimage image6, Fimage image7,
           Fimage image8)
        ...
        if (edge > 1)
        {
            if (2 * edge >= imgsize / decim)
                ...
                size =
                    (imgsize / decim -
                     (vecsize - 1 +
                      2 * (edge - (edge + vecsize - 1) / vecsize))) * decim;
        }
        else
...
static unsigned char SWITCH_TS(float energy, float *tv1, float *tv2,
                               float *tv3)
...
static double
ENERGY_BLOCK(Fimage image, int decim, int width, int height, long int ldxi,
             long int imgj)
...
/* Width and height of block */
{
    ...
    for (r = 0; r < height; r++)
    {
        for (c = 0; c < width; c++)
            energy +=
                image->gray[ldxi + imgj + decim * c] * image->gray[ldxi +
                                                                   imgj +
                                                                   decim * c];
        ldxi += incldxi;
    }
...
static void
SEARCH_LINE_TS(Fimage image, long int ldxi, Cimage indts, long int *t,
               int width, int height, int *lap, int decim, int edge,
    ...
    for (d = 0; d < decim; d++)
    {
        if (lap)
        {
            ...
            for (j = 0; j < (edge + width - 1) / width; j++)
            {
                ...
                indts->gray[*t] =
                    SWITCH_TS(energy, thresval1, thresval2, thresval3);
                imgj += decim * width;
                (*t)++;
            }
        }
...
static void
SEARCH_TS(Fimage image, Cimage indts, int width, int height, int *lap,
          int decim, int edge, float *thresval1, float *thresval2,
          float *thresval3)
 ...
 * between codebooks */
{
    ...
    int inci;
    long incldxi;
    long ldxi;                  /* index of the first pixel
                                 * of the line in image */
    int d;
    ...
    for (d = 0; d < decim; d++)
    {
        if (lap)
        {
            ...
            {
                SEARCH_LINE_TS(image, ldxi, indts, &t, width, height, lap,
                               decim, edge, thresval1, thresval2, thresval3);
        ...
        {
            SEARCH_LINE_TS(image, ldxi, indts, &t, width, height, lap, decim,
                           edge, thresval1, thresval2, thresval3);
            ldxi += incldxi;
            i += inci;
        }
...
static void
COPY_BLOCK(Fimage image, Cimage indts, long int t, int decim, Fimage result,
           Fimage result2, Fimage result3, Fimage result4, int width,
           int height, long int ldxi, long int imgj, long int *ldxr,
           long int *ldxr2, long int *ldxr3, long int *ldxr4)
 ...
 * and first pixel of the block in image */
{
    ...
    if (indts)
        ...
        case 1:
            ...
            {
                for (c = 0; c < width; c++)
                    result->gray[*ldxr + r * width + c] =
                        image->gray[ldxi + imgj + decim * c];
...
static void
COPY_LINE_OF_BLOCK(Fimage image, Cimage indts, int *lap, int decim, int edge,
                   Fimage result, Fimage result2, Fimage result3,
                   Fimage result4, int width, int height, long int *t,
                   long int ldxi, long int *ldxr, long int *ldxr2,
                   long int *ldxr3, long int *ldxr4)
 ...
 * currently added */
{
    ...
    for (d = 0; d < decim; d++)
    {
        if (lap)
        {
            ...
            for (j = 0; j < (edge + width - 1) / width; j++)
            {
                COPY_BLOCK(image, indts, *t, decim, result, result2, result3,
                           result4, width, height, ldxi, imgj, ldxr, ldxr2,
                           ldxr3, ldxr4);
                imgj += decim * width;
                (*t)++;
            }
        ...
        {
            COPY_BLOCK(image, indts, *t, decim, result, result2, result3,
                       result4, width, height, ldxi, imgj, ldxr, ldxr2, ldxr3,
                       ldxr4);
            imgj += incj;
            (*t)++;
        }
...
static void
ADD_IMAGE_ADAP(int width, int height, long int *t, long int *ldxr,
               long int *ldxr2, long int *ldxr3, long int *ldxr4, int *lap,
               int decim, int edge, Fimage image, Cimage indts, Fimage result,
...
/* Index of training set for an image block */
{
    ...
    for (d = 0; d < decim; d++)
    {
        if (lap)
        {
            ...
            for (i = 0; i < (edge + height - 1) / height; i++)
            {
                COPY_LINE_OF_BLOCK(image, indts, lap, decim, edge, result,
                                   result2, result3, result4, width, height,
                                   t, ldxi, ldxr, ldxr2, ldxr3, ldxr4);
                ldxi += decim * height * image->ncol;
            }
        }
        ...
        {
            COPY_LINE_OF_BLOCK(image, indts, lap, decim, edge, result,
                               result2, result3, result4, width, height, t,
                               ldxi, ldxr, ldxr2, ldxr3, ldxr4);
            ldxi += incldxi;
            i += inci;
        }
...
static void
MK_TRAINSET_ADAP(int width, int height, int *lap, int decim, int edge,
                 float *thresval1, float *thresval2, float *thresval3,
                 int *sizecb, Fimage image, Fimage image2, Fimage image3,
                 Fimage image4, Fimage image5, Fimage image6, Fimage image7,
                 Fimage image8, Fimage result, Fimage result2, Fimage result3,
                 Fimage result4)
...
/* Resulting codebook */
{
    ...
    THRES_ADAP(width, height, thresval1, thresval2, thresval3, image, image2,
    ...
    if (mw_alloc_cimage(indts, sizet, 1) == NULL)
        mwerror(FATAL, 1,
                "Not enough memory for training set index buffer\n");
    ...
    while ((sizet == 0) && (niter < MAX_NITER))
    {
        ...
        indts->firstrow = 0;
        SEARCH_TS(image, indts, width, height, lap, decim, edge, thresval1,
                  thresval2, thresval3);

        if (image2 != NULL)
            SEARCH_TS(image2, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);

        if (image3 != NULL)
            SEARCH_TS(image3, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);

        if (image4 != NULL)
            SEARCH_TS(image4, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);

        if (image5 != NULL)
            SEARCH_TS(image5, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);

        if (image6 != NULL)
            SEARCH_TS(image6, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);

        if (image7 != NULL)
            SEARCH_TS(image7, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);

        if (image8 != NULL)
            SEARCH_TS(image8, indts, width, height, lap, decim, edge,
                      thresval1, thresval2, thresval3);
        ...
        else if (thresval2 && (sizet2 < min_trainsize))
        {
            if (niter == MAX_NITER)
        ...
        else if (thresval3 && (sizet3 < min_trainsize))
        {
            if (niter == MAX_NITER)
    ...
    ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                   decim, edge, image, indts, result, result2, result3,
                   result4);

    if (image2 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image2, indts, result, result2, result3,
                       result4);

    if (image3 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image3, indts, result, result2, result3,
                       result4);

    if (image4 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image4, indts, result, result2, result3,
                       result4);

    if (image5 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image5, indts, result, result2, result3,
                       result4);

    if (image6 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image6, indts, result, result2, result3,
                       result4);

    if (image7 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image7, indts, result, result2, result3,
                       result4);

    if (image8 != NULL)
        ADD_IMAGE_ADAP(width, height, &t, &ldxr, &ldxr2, &ldxr3, &ldxr4, lap,
                       decim, edge, image8, indts, result, result2, result3,
                       result4);
...
static void
MK_TRAINSET(int width, int height, int *lap, int decim, int edge,
            Fimage image, Fimage image2, Fimage image3, Fimage image4,
...
/* No overlapping on edges */
{
    ...
    dum = ldxr = 0;
    ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap, decim,
                   edge, image, NULL, result, NULL, NULL, NULL);

    if (image2 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image2, NULL, result, NULL, NULL, NULL);

    if (image3 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image3, NULL, result, NULL, NULL, NULL);

    if (image4 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image4, NULL, result, NULL, NULL, NULL);

    if (image5 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image5, NULL, result, NULL, NULL, NULL);

    if (image6 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image6, NULL, result, NULL, NULL, NULL);

    if (image7 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image7, NULL, result, NULL, NULL, NULL);

    if (image8 != NULL)
        ADD_IMAGE_ADAP(width, height, &dum, &ldxr, NULL, NULL, NULL, lap,
                       decim, edge, image8, NULL, result, NULL, NULL, NULL);

}

void
mk_trainset(int *Width, int *Height, int *Lap, int *Decim, int *Edge,
            float *ThresVal1, float *ThresVal2, float *ThresVal3, int *SizeCB,
            Fimage Image2, Fimage Image3, Fimage Image4, Fimage Image5,
            Fimage Image6, Fimage Image7, Fimage Image8, Fimage Result2,
            Fimage Result3, Fimage Result4, Fimage Image, Fimage Result)
...
/* Resulting adapted codeook */
{
    ...
    if (ThresVal1)
        MK_TRAINSET_ADAP(*Width, *Height, Lap, *Decim, *Edge, ThresVal1,
                         ThresVal2, ThresVal3, SizeCB, Image, Image2, Image3,
                         Image4, Image5, Image6, Image7, Image8, Result,
                         Result2, Result3, Result4);
    else
        MK_TRAINSET(*Width, *Height, Lap, *Decim, *Edge, Image, Image2,
                    Image3, Image4, Image5, Image6, Image7, Image8, Result);
