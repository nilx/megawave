...
usage = {
 ...
 'a':ResCodeBook1->ResCodeBook1  "Codebook for residu quantization
                                 after quantization with CodeBook1 (fimage)",
 'b':ResCodeBook2->ResCodeBook2  "Codebook for residu quantization
                                 after quantization with CodeBook2 (fimage)",
 'c':ResCodeBook3->ResCodeBook3  "Codebook for residu quantization
                                 after quantization with CodeBook3 (fimage)",
 'd':ResCodeBook4->ResCodeBook4  "Codebook for residu quantization
                                 after quantization with CodeBook4 (fimage)",
 'E':NResResCB1->NResResCB1      "Index of codebook in ResResCodeBook1",
 'F':NResResCB2->NResResCB2      "Index of codebook in ResResCodeBook2",
 'e':ResResCodeBook1->ResResCodeBook1  "Codebook for residu quantization
                                       after quantization with CodeBook1
                                       and ResCodeBook1 (fimage)",
 'f':ResResCodeBook2->ResResCodeBook2  "Codebook for residu quantization
                                       after quantization with CodeBook2
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* arencode2(), entropy() */
...
static void
CHECK_INPUT(int *ratedist, int *ncb1, Fimage codebook1, Fimage codebook2,
            Fimage codebook3, Fimage codebook4, int *nrescb1,
            Fimage rescodebook1, Fimage rescodebook2, Fimage rescodebook3,
            Fimage rescodebook4, int *nresrescb1, Fimage resrescodebook1,
            Fimage resrescodebook2, Fimage image)
/* Codebooks for
 * different classes */
...
static void INIT_TARGRATE_DR(void)
{
...
static void clear_histo(Fsignal histo)
{
    int i;
...
static void Variance(Fimage image)
{
    int r, c;
    long rdx;
...
static void ENCODE_INT_FIMAGE(int symb, int max, Cimage compress)
...
static void
INIT_ENCODING_FIMAGE(int *smallheader, int nrow, int ncol, int testmulticb,
                     int *indexcb, Cimage compress)
    ...
    /*--- Init encoding ---*/
        ...
        if (compress->gray)
        {
            ...
            if ((bits_to_go < 0) || (bits_to_go >= 8))
                mwerror(FATAL, 4,
                        "Wrong value for compress->firstrow! (%d)\n",
                        bits_to_go);
            if (bits_to_go == 0)
            {
        ...
        /*--- Memory allocation for compressed image buffer ---*/
            ...
            if (compress == NULL)
                mwerror(FATAL, 1,
                        "Memory allocation refused for `Compress`!\n");
...
static void
compute_rates(Fimage symbol, int sizec, Fsignal histo, double *rate,
              double *ratearc, double *ent, Cimage compress)
                                /* Compressed image */
{
    ...
    *ent += e * size;
    ...
    if (sizec > 1)
        arencode2(&sizec, NULL, &nsymb, NULL, predic, NULL, NULL, symbol, &e,
                  bufcomp);
    else
        e = 0.0;
...
static void
extract_symbol(Fimage symbol, Fimage symbex, Fimage indcb, float index,
               long int size, int *recfl, int recfac)
{
...
static void
indexcb_adap(int *indexcb, int nadap, int qprec, int *numcb,
             float (*bmse)[50], float effmse, float *minmse,
             float (*brate)[50], float *brl, float effrate, float targrate,
             int *testopt, int opt)
                                 /* Indices of selected codebooks
                                  * for quantization */
{
    ...
    else
    {
        for (q = numcb[nadap] - 1; q >= 0; q--)
        {
            ...
            if ((neweffrate <= targrate) && (neweffmse < *minmse))
            {
                indexcb_adap(indexcb, nadap + 1, q, numcb, bmse, neweffmse,
                             minmse, brate, brl, neweffrate, targrate,
                             testopt, opt + 1);
                if (*testopt == opt)
                {
                    (*testopt)--;
...
static void
compute_indexcb(int *indexcb, int *numcb, float (*bmse)[50],
                float (*brate)[50], float *brl, float targrate, int test_dr)
{
    int q;
    int nadap;
    float effrate;
    ...
    for (q = numcb[0] - 1; q >= 0; q--)
    {
        ...
        {
            indexcb_adap(indexcb, nadap + 1, q, numcb, bmse, effmse, &minmse,
                         brate, brl, effrate, targrate, &testopt, opt + 1);
            if (testopt == opt)
            {
                indexcb[nadap] = q;
                testopt--;
...
static void THRESBLOCK(long int rcol, Fimage image)
                                  /* Input image */
{
    long r, c, i;
...
static void
block_vq_adap(Fimage image, long int i, long int j, Fimage codebook,
              Fimage rescodebook, Fimage resrescodebook, Fimage result,
              Fimage symbol, Fimage symbres, Fimage symbresres,
              long int recfac, Fsignal histo, Fsignal reshisto,
              Fsignal resreshisto, float *bmse)
                                /* Input image */
                                ...
                                /* Quantization mean square error */
{
    ...
    if (rescodebook)
    {
        ...
        if (symbres)
            block_vq_adap(image, i, j, rescodebook, resrescodebook, NULL,
                          result, symbres, symbresres, NULL, 0L, reshisto,
                          resreshisto, NULL, bmse);
        else
            block_vq_adap(image, i, j, rescodebook, resrescodebook, NULL,
                          result, symbol, NULL, NULL, recfac * sizec,
                          reshisto, resreshisto, NULL, bmse);
...
static void
FULLSEARCH(Fimage image, Fimage codebook, Fimage rescodebook,
           Fimage resrescodebook, Cimage compress, Fimage result, double *mse,
           double *rate, double *ratearc, double *ent)
                                /* Input image */
                                /* Codebook */
                                             /* Codebooks for residus */
                                /* Compressed image */
                                /* Quantized image */
{
    ...
    *mse = 0.0;
    ...
    if (rescodebook)
    {
        ...
        if (resrescodebook)
        {
            ...
            if (mw_alloc_fsignal(resreshisto, sizecresres) == NULL)
                mwerror(FATAL, 1,
                        "Not enough memory for histogram of index\n");
    ...
    for (r = 0; r < nrowb; r++)
        for (c = 0; c < ncolb; c++)
        {
            block_vq_adap(image, r, c, codebook, rescodebook, resrescodebook,
                          result, symbol, symbres, symbresres, 0L, histo,
                          reshisto, resreshisto, &bmse);
    ...
    *mse /= (float) nrow *ncol;
    ...
    if (rescodebook)
    {
        compute_rates(symbres, sizecres, reshisto, rate, ratearc, ent,
                      compress);
        if (resrescodebook)
        {
            compute_rates(symbresres, sizecresres, resreshisto, rate, ratearc,
                          ent, compress);
...
static void
FULLSEARCH_ADAP(int *printsnr, int *bitmapcode, Fimage image,
                Fimage codebook1, Fimage codebook2, Fimage codebook3,
                ...
                Cimage compress, Fimage result, double *mse, double *rate,
                double *ratearc, double *ent)
...
/* Quantized image */
{
    ...
    *rate = *ratearc = *ent = 0.0;
    ...
    if ((sizec1 > 1) || (sizec2 > 1) || (sizec3 > 1) || (sizec4 > 1)
        || bitmapcode)
    {
    ...
    /*--- Memory allocation for histogram of residu quantized levels ---*/
        ...
        if (histo1 == NULL)
            mwerror(FATAL, 1,
                    "Not enough memory for histogram of symbols 1.\n");
        clear_histo(histo1);
        ...
        if (rescodebook1)
        {
            ...
            if (mw_alloc_fsignal(reshisto1, rescodebook1->nrow - 4) == NULL)
                mwerror(FATAL, 1,
                        "Not enough memory for histogram "
                        "of symbols 1 (residu)\n");
            ...
            if (resrescodebook1)
            {
                ...
                if (mw_alloc_fsignal(resreshisto1, resrescodebook1->nrow - 4)
                    == NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for histogram "
                            "of symbols 1 (second level residu)\n");
                clear_histo(resreshisto1);
                symbresres = mw_change_fimage(symbresres, nrowb, ncolb);
                if (symbresres == NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for symbol buffer 1\n");
                for (x = 0; x < sizeb; x++)
    ...
    /*--- Memory allocation for histograms of adaptative quantized levels ---*/
        ...
        if (codebook2)
        {
            ...
            if (mw_alloc_fsignal(histo2, sizec2) == NULL)
                mwerror(FATAL, 1,
                        "Not enough memory for histogram of symbols 2\n");
            clear_histo(histo2);

            if (rescodebook2)
            {
                reshisto2 = mw_new_fsignal();
                if (mw_alloc_fsignal(reshisto2, rescodebook2->nrow - 4) ==
                    NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for histogram "
                            "of symbols 2 (residu)\n");
                ...
                if (symbres->nrow == 0)
                {
                    ...
                    if (symbres == NULL)
                        mwerror(FATAL, 1,
                                "Not enough memory for symbol buffer 2\n");
                    for (x = 0; x < sizeb; x++)
                        symbres->gray[x] = 0.0;
                }
                ...
                if (resrescodebook2)
                {
                    ...
                    if (mw_alloc_fsignal
                        (resreshisto2, resrescodebook2->nrow - 4) == NULL)
                        mwerror(FATAL, 1,
                                "Not enough memory for histogram "
                                "of symbols 2 (second level residu)\n");
                    clear_histo(resreshisto2);
                    if (symbresres->nrow == 0)
                    {
                        symbresres =
                            mw_change_fimage(symbresres, nrowb, ncolb);
                        if (symbresres == NULL)
                            mwerror(FATAL, 1,
                                    "Not enough memory for symbol buffer 2\n");
                        for (x = 0; x < sizeb; x++)
        ...
        if (codebook3)
        {
            ...
            if (mw_alloc_fsignal(histo3, sizec3) == NULL)
                mwerror(FATAL, 1,
                        "Not enough memory for histogram of symbols 3\n");
            clear_histo(histo3);

            if (rescodebook3)
            {
                reshisto3 = mw_new_fsignal();
                if (mw_alloc_fsignal(reshisto3, rescodebook3->nrow - 4) ==
                    NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for histogram "
                            "of symbols 3 (residu)\n");
                ...
                if (symbres->nrow == 0)
                {
                    ...
                    if (symbres == NULL)
                        mwerror(FATAL, 1,
                                "Not enough memory for symbol buffer 3\n");
                    for (x = 0; x < sizeb; x++)
                        symbres->gray[x] = 0.0;
                }
            }
        ...
        if (codebook4)
        {
            ...
            if (mw_alloc_fsignal(histo4, sizec4) == NULL)
                clear_histo(histo4);
            mwerror(FATAL, 1,
                    "Not enough memory for histogram of symbols 4\n");

            if (rescodebook4)
            {
                reshisto4 = mw_new_fsignal();
                if (mw_alloc_fsignal(reshisto4, rescodebook4->nrow - 4) ==
                    NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for histogram "
                            "of symbols 4 (residu)\n");
                ...
                if (symbres->nrow == 0)
                {
                    ...
                    if (symbres == NULL)
                        mwerror(FATAL, 1,
                                "Not enough memory for symbol buffer 4\n");
                    for (x = 0; x < sizeb; x++)
                        symbres->gray[x] = 0.0;
                }
            }
    ...
    /*--- Memory allocation for adapted codebook indices ---*/
        ...
        if (indexcb == NULL)
            mwerror(FATAL, 1,
                    "Not enough memory for codebook index buffer!\n");
    ...
    /*--- Quantization of image ---*/
        ...
        /*--- Computation of block benergy ---*/
                ...
                for (r = 0; r < height; r++)
                {
                    ...
                    {
                        benergy +=
                            image->gray[rcol + c] * image->gray[rcol + c];
                    }
                    rcol += ncol;
                }
                rcol -= height * ncol;
          ...
          /*--- Quantization with first codebook ---*/
                    ...
                    block_vq_adap(image, rb, cb, codebook1, rescodebook1,
                                  resrescodebook1, result, symbol, symbres,
                                  symbresres, 0L, histo1, reshisto1,
                                  resreshisto1, &bmse);
              ...
              /*--- Quantization with second codebook ---*/
                        ...
                        block_vq_adap(image, rb, cb, codebook2, rescodebook2,
                                      resrescodebook2, result, symbol,
                                      symbres, symbresres, 0L, histo2,
                                      reshisto2, resreshisto2, &bmse);
                  ...
                  /*--- Quantization with third codebook ---*/
                            ...
                            block_vq_adap(image, rb, cb, codebook3,
                                          rescodebook3, NULL, result, symbol,
                                          symbres, NULL, 0L, histo3,
                                          reshisto3, NULL, &bmse);
                      ...
                      /*--- Quantization with fourth codebook ---*/
                                ...
                                block_vq_adap(image, rb, cb, codebook4,
                                              rescodebook4, NULL, result,
                                              symbol, symbres, NULL, 0L,
                                              histo4, reshisto4, NULL, &bmse);
                                mse4 += bmse;
                                num4++;
                            }
                            else
    ...
    /*--- Computation of entropic rates of codebook index buffer ---*/
        ...
        *ent = (double) sizeb *log((double) sizeb);
        ...
        if (compress)
            ...
            bufcomp = NULL;
        arencode2(&nsymb, NULL, &nsymb, NULL, predic, NULL, NULL, indexcb, &e,
                  bufcomp);
        if (compress)
            ADD_BUF_TO_COMPRESS_FIMAGE(compress, bufcomp);
    ...
    /*--- Computation of entropy of quantized levels ---*/
        ...
        if (num1 > 0)
        {
            ...
            if (resrescodebook1)
            {
                extract_symbol(symbresres, symbex, indexcb, 1.0, num1, NULL,
                               recfac);
                compute_rates(symbex, resrescodebook1->nrow - 4, resreshisto1,
                              rate, ratearc, ent, compress);
                mw_delete_fsignal(resreshisto1);
            }
            if (rescodebook1)
            {
                extract_symbol(symbres, symbex, indexcb, 1.0, num1, NULL,
                               recfac);
                compute_rates(symbex, rescodebook1->nrow - 4, reshisto1, rate,
                              ratearc, ent, compress);
                mw_delete_fsignal(reshisto1);
            }
            extract_symbol(symbol, symbex, indexcb, 1.0, num1, NULL, recfac);
            compute_rates(symbex, sizec1, histo1, rate, ratearc, ent,
                          compress);
        ...
        if (printsnr)
            if (*printsnr == 1)
            {
                e = *ratearc - e;
                printf
                    ("cb1: block number = %ld,  entropy = %.4f,  mse = %.2f\n",
        ...
        if (codebook2)
        {
            if (num2 > 0)
            {
                ...
                if (resrescodebook2)
                {
                    extract_symbol(symbresres, symbex, indexcb, indexcbval[2],
                                   num2, NULL, recfac);
                    compute_rates(symbex, resrescodebook2->nrow - 4,
                                  resreshisto2, rate, ratearc, ent, compress);
                    mw_delete_fsignal(resreshisto2);
                }
                if (rescodebook2)
                {
                    extract_symbol(symbres, symbex, indexcb, indexcbval[2],
                                   num2, NULL, recfac);
                    compute_rates(symbex, rescodebook2->nrow - 4, reshisto2,
                                  rate, ratearc, ent, compress);
                    mw_delete_fsignal(reshisto2);
                }
                extract_symbol(symbol, symbex, indexcb, indexcbval[2], num2,
                               NULL, recfac);
                compute_rates(symbex, sizec2, histo2, rate, ratearc, ent,
                              compress);
        ...
        if (codebook3)
        {
            if (num3 > 0)
            {
                ...
                if (rescodebook3)
                {
                    extract_symbol(symbres, symbex, indexcb, indexcbval[3],
                                   num3, NULL, recfac);
                    compute_rates(symbex, rescodebook3->nrow - 4, reshisto3,
                                  rate, ratearc, ent, compress);
                    mw_delete_fsignal(reshisto3);
                }
                extract_symbol(symbol, symbex, indexcb, indexcbval[3], num3,
                               NULL, recfac);
                compute_rates(symbex, sizec3, histo3, rate, ratearc, ent,
                              compress);
        ...
        if (codebook4 && (thres3 > 0.0))
        {
            if (num4 > 0)
            {
                ...
                if (rescodebook4)
                {
                    extract_symbol(symbres, symbex, indexcb, 0.0, num4, NULL,
                                   recfac);
                    compute_rates(symbex, rescodebook4->nrow - 4, reshisto4,
                                  rate, ratearc, ent, compress);
                    mw_delete_fsignal(reshisto4);
                }
                extract_symbol(symbol, symbex, indexcb, 0.0, num4, NULL,
                               recfac);
                compute_rates(symbex, sizec4, histo4, rate, ratearc, ent,
                              compress);
            ...
            if (printsnr)
                if (*printsnr == 1)
                {
                    e = *ratearc - e;
                    printf
                        ("cb4: block number = %ld, entropy = %.2f, "
...
static void
VQ(int *printsnr, int *bitmapcode, Fimage image, int testmulticb, int ncb1,
   int ncb2, int ncb3, int ncb4, Fimage codebook1, Fimage codebook2,
   Fimage codebook3, Fimage codebook4, int nrescb1, int nrescb2, int nrescb3,
   int nrescb4, Fimage rescodebook1, Fimage rescodebook2, Fimage rescodebook3,
   Fimage rescodebook4, int nresrescb1, int nresrescb2,
   Fimage resrescodebook1, Fimage resrescodebook2, Cimage compress,
   Fimage result, double *mse, double *snr, double *rate, double *ratearc,
   double *ent)
/* Control info print on SNR */
/* Encode bitmap even if all codebooks
 * have size 1 */
...
/* Signal to noise ratio */
{
    ...
    if (testmulticb >= 1)
    {
        ...
        if (cb1->gray[(cb1->nrow - 3) * cb1->ncol] == 0.0)
            FULLSEARCH(image, cb1, rescb1, resrescb1, compress, result, mse,
                       rate, ratearc, ent);
        else
            FULLSEARCH_ADAP(printsnr, bitmapcode, image, cb1, cb2, cb3, cb4,
                            rescb1, rescb2, rescb3, rescb4, resrescb1,
                            resrescb2, compress, result, mse, rate, ratearc,
                            ent);
    ...
    else if (codebook1->gray[(codebook1->nrow - 3) * codebook1->ncol] == 0.0)
        FULLSEARCH(image, codebook1, rescodebook1, resrescodebook1, compress,
                   result, mse, rate, ratearc, ent);
    else
        FULLSEARCH_ADAP(printsnr, bitmapcode, image, codebook1, codebook2,
                        codebook3, codebook4, rescodebook1, rescodebook2,
                        rescodebook3, rescodebook4, resrescodebook1,
                        resrescodebook2, compress, result, mse, rate, ratearc,
                        ent);
...
void
fvq(int *PrintSNR, int *SmallHeader, int *BitMapCode, int *RateDist,
    int *NCB1, int *NCB2, int *NCB3, int *NCB4, Fimage CodeBook2,
    Fimage CodeBook3, Fimage CodeBook4, int *NResCB1, int *NResCB2,
    int *NResCB3, int *NResCB4, Fimage ResCodeBook1, Fimage ResCodeBook2,
    Fimage ResCodeBook3, Fimage ResCodeBook4, int *NResResCB1,
    int *NResResCB2, Fimage ResResCodeBook1, Fimage ResResCodeBook2,
    Cimage Compress, Fimage Image, Fimage CodeBook1, Fimage Result,
    double *MSE, double *SNR, double *Entropy, double *RateAr)
/* Control info print on SNR */
...
/* Arithmetic coding rate */
{
    int n1, n2, n3, n4;         /* Indices of codebooks for different
                                 * classes */
    int ninit1 = 0, ninit2 = 0, ninit3 = 0, ninit4 = 0; /* Initial values
                                                         * for n1,
                                                         * n2, n3 and n4 */
    int nfin1 = 0, nfin2 = 0, nfin3 = 0, nfin4 = 0;     /* Final values for n1,
                                                         * n2, n3 and n4 */
    int nres1 = 0, nres2 = 0, nres3 = 0, nres4 = 0;     /* Indices of residus
                                                         * codebooks
                                                         * for different
                                                         * classes */
  ...
  /*--- Check input data ---*/
    ...
    CHECK_INPUT(RateDist, NCB1, CodeBook1, CodeBook2, CodeBook3, CodeBook4,
                NResCB1, ResCodeBook1, ResCodeBook2, ResCodeBook3,
                ResCodeBook4, NResResCB1, ResResCodeBook1, ResResCodeBook2,
                Image);
  ...
  /*--- Compute maximum number of codebooks ---*/
    ...
    /*--- Search index of codebook to be used in codebook files ---*/
        ...
        {
            ninit1 =
                count_cb(CodeBook1) + count_cb(ResCodeBook1) + *NResResCB1;
            nfin1 =
                count_cb(CodeBook1) + count_cb(ResCodeBook1) + *NResResCB1 +
                1;
        }
        ...
        {
            ninit2 =
                count_cb(CodeBook2) + count_cb(ResCodeBook2) + *NResResCB2;
            nfin2 =
                count_cb(CodeBook2) + count_cb(ResCodeBook2) + *NResResCB2 +
                1;
        }
    ...
    /*--- Codebook buffers contain only one codebook, or indices ---*/
        ...
        indcb[2] = ninit3;
        indcb[3] = ninit4;
        INIT_ENCODING_FIMAGE(SmallHeader, Image->nrow, Image->ncol,
                             testmulticb, indcb, Compress);

        if (testmulticb == 0)
        {
            VQ(PrintSNR, NULL, Image, testmulticb, 0, 0, 0, 0, CodeBook1,
               CodeBook2, CodeBook3, CodeBook4, 0, 0, 0, 0, ResCodeBook1,
               ResCodeBook2, ResCodeBook3, ResCodeBook4, 0, 0,
               ResResCodeBook1, ResResCodeBook2, Compress, Result, &mse, &snr,
               &Rate, &Ratearc, &ent);
        ...
        else
        {
            ...
            VQ(PrintSNR, BitMapCode, Image, testmulticb, ninit1, ninit2,
               ninit3, ninit4, CodeBook1, CodeBook2, CodeBook3, CodeBook4,
               nres1, nres2, nres3, nres4, ResCodeBook1, ResCodeBook2,
               ResCodeBook3, ResCodeBook4, nresres1, nresres2,
               ResResCodeBook1, ResResCodeBook2, Compress, Result, &mse, &snr,
               &Rate, &Ratearc, &ent);
        }
    ...
    else
    {
      ...
      /*--- Compute rate and distortion associated to each codebook ---*/
        ...
        {
            VQ(PrintSNR, &n1, Image, testmulticb, n1, ninit2, ninit3, ninit4,
               CodeBook1, NULL, NULL, NULL, nres1, nres2, nres3, nres4,
               ResCodeBook1, NULL, NULL, NULL, nresres1, nresres2,
               ResResCodeBook1, ResResCodeBook2, NULL, Result, &mse, &snr,
               &Rate, &Ratearc, &ent);
            if ((n1 == 0)
                && (CodeBook1->gray[(CodeBook1->nrow - 3) * CodeBook1->ncol] >
                    0.0))
            {
                bmse[0][n1] = energy;
                brate[0][n1] = 0.0;
        ...
        {
            VQ(PrintSNR, NULL, Image, testmulticb, ninit1, n2, ninit3, ninit4,
               CodeBook1, CodeBook2, NULL, NULL, nres1, nres2, nres3, nres4,
               ResCodeBook1, ResCodeBook2, NULL, NULL, nresres1, nresres2,
               ResResCodeBook1, ResResCodeBook2, NULL, Result, &mse, &snr,
               &Rate, &Ratearc, &ent);
            if (PrintSNR)
                if (*PrintSNR != 1)
                    printf("%.4f  %.4f  %.4f  %.2f\n", Rate, Ratearc, ent,
                           snr);
            if ((n2 == 0)
                && (CodeBook2->gray[(CodeBook2->nrow - 3) * CodeBook2->ncol] >
                    0.0))
            {
                bmse[1][n2] = 0.0;
                brate[1][n2] = 0.0;
        ...
        {
            VQ(PrintSNR, BitMapCode, Image, testmulticb, ninit1, ninit2, n3,
               ninit4, CodeBook1, CodeBook2, CodeBook3, NULL, nres1, nres2,
               nres3, nres4, ResCodeBook1, ResCodeBook2, ResCodeBook3, NULL,
               nresres1, nresres2, ResResCodeBook1, ResResCodeBook2, NULL,
               Result, &mse, &snr, &Rate, &Ratearc, &ent);
            if (PrintSNR)
                if (*PrintSNR != 1)
                    printf("%.4f  %.4f  %.4f  %.2f\n", Rate, Ratearc, ent,
                           snr);
            if ((n3 == 0)
                && (CodeBook3->gray[(CodeBook3->nrow - 3) * CodeBook3->ncol] >
                    0.0))
            {
                bmse[2][n3] = 0.0;
                brate[2][n3] = 0.0;
            ...
            else
            {
                ...
                bratearc[2][n3] =
                    Ratearc - bratearc[0][ninit1] - bratearc[1][ninit2];
        ...
        {
            VQ(PrintSNR, NULL, Image, testmulticb, ninit1, ninit2, ninit3, n4,
               CodeBook1, CodeBook2, CodeBook3, CodeBook4, nres1, nres2,
               nres3, nres4, ResCodeBook1, ResCodeBook2, ResCodeBook3,
               ResCodeBook4, nresres1, nresres2, ResResCodeBook1,
               ResResCodeBook2, NULL, Result, &mse, &snr, &Rate, &Ratearc,
               &ent);
            if (PrintSNR)
                if (*PrintSNR != 1)
                    printf("%.4f  %.4f  %.4f  %.2f\n", Rate, Ratearc, ent,
                           snr);
            bmse[3][n4] =
                mse - bmse[0][ninit1] - bmse[1][ninit2] - bmse[2][ninit3];
            brate[3][n4] =
                Rate - brate[0][ninit1] - brate[1][ninit2] - brate[2][ninit3];
            bratearc[3][n4] =
                Ratearc - bratearc[0][ninit1] - bratearc[1][ninit2] -
                bratearc[2][ninit3];
            bent[3][n4] =
                ent - bent[0][ninit1] - bent[1][ninit2] - bent[2][ninit3];
        }
      ...
      /*--- Compute rate distortion curve ---*/
        ...
        {
            compute_indexcb(indcb, numcb, bmse, bratearc, brl, targrate_dr[q],
                            test_dr);
            mse = 0.0;
            Ratearc = 0.0;
            for (nadap = 0; nadap < nadapcb; nadap++)
