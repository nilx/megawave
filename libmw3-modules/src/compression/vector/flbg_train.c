...
function = {"Generates a sequence of codebooks from a training set
            of vectors using LBG (generalized Lloyd) algorithm"};
usage = {
 's':[CodeBookSize=1]->Size          "Size of output codebook",
 'W':Weight->Weight                  "Weighting factors for the components
                                     of vector (fsignal)",
 'M'->MultiCB                        "Generate a sequence of codebooks
                                     of size equal to a power of two
                                     and <= Size",
 'i':InitCodeBook->InitCodeBook      "Initial codebook (fimage)",
 'p'->PrintSNR                       "Print number of iterations
                                     for each loop instead of
                                     distortion rate results",
 'f':NResCB->NResCB                  "Index of first residual codebook
                                     (in ResCodeBook)",
 'g':NResResCB->NResResCB            "Index of second residual codebook
                                     (in ResResCodeBook)",
...
static void ENERGY_TS(Fimage trainset)
                                /* Input training set */
{
                                 ...
                                 /* Coordinates weights in block */
{
    ...
    for (t = 0; t < sizet; t++)
    {
        if (bweight)
            for (c = 0; c < sizeb; c++)
            {
                e = codebook->gray[sizeb * cell[t] + c] - trainset->gray[ts +
                                                                         c];
...
static void copy_multicb(Fimage codebook, int size)
{
    long sizeb, sizec;
...
static void extract_cb(Fimage codebook, int n)
          ...
          /*--- in 'codebook', put the result in 'codebook' ---*/
{
            ...
            codebook->gray[(codebook->nrow - 6) * codebook->ncol]))
    {
        ...
        codebook =
            mw_change_fimage(codebook, xf / codebook->ncol, codebook->ncol);
        if (codebook == NULL)
            mwerror(FATAL, 1, "Not enough memory for extracted codebook.\n");
    }
    else
        mwerror(WARNING, 0,
                "ResCodeBook or ResResCodeBook is not "
                "a multiple codebook file!\n");
}
                                 ...
                                 /* Coordinates weights in block */
{
  ...
  /*--- Discard blocks of training set not in threshold range ---*/
    ...
    if ((codebook->gray[sizec * sizeb] != 0.0)
        || (codebook->gray[(sizec + 1) * sizeb] != 0.0))
    {
        ...
        for (t = 0; t < sizet; t++)
        {
            ...
            if (bweight)
                for (c = 0; c < sizeb; c++)
                    energy +=
                        bweight->values[c] * trainset->gray[tsizeb +
                                                            c] *
                        trainset->gray[tsizeb + c];
            else
                for (c = 0; c < sizeb; c++)
                    energy +=
                        trainset->gray[tsizeb + c] * trainset->gray[tsizeb +
                                                                    c];
            if ((energy >= thresmin)
                && ((energy < thressup) || (thressup == 0.0)))
            {
                for (c = 0; c < sizeb; c++)
    ...
    /*--- Search for nearest vector in codebook ---*/
        ...
        for (n = 0; n < sizec; n++)
        {
            ...
            if (bweight)
                ...
                {
                    diff =
                        trainset->gray[tsizeb + c] - codebook->gray[nsizeb +
                                                                    c];
  ...
  /*--- Computes rate ---*/
    ...
    if (printsnr)
    {
        ...
        if (totaltest != trainset->nrow)
        {
            ...
            printf
                ("Number of vector in tr.set : %d,  sum of weightcell : %ld\n",
                 sizet, totaltest);
        }
...
static void
split(Fimage trainset, Fimage codebook, Fsignal bweight, short int *cell,
      int *sizec, int sizecf, int *printsnr)
    ...
    if ((maxweight + sizet) * sizev <= MAXSIZE_SPLITBUF)
    {
        ...
        for (l = 0; l <= 1; l++)
        {
            ...
            for (t = 0; t < maxweight; t++)
            {
                ...
                if (mds[l][t] == NULL)
                    mwerror(FATAL, 1,
                            "Not enough memory for m.s.d. buffer.\n");
            }
        }
    }
    else
...
static void
opticell(Fimage trainset, Fimage codebook, Fsignal bweight, short int *cell,
         short int sizec)
...
static void
optimize(Fimage trainset, Fimage codebook, Fsignal bweight, short int *cell,
         int *sizec, int sizecf, int *printsnr, int *multicb)
                                 /* Training set of vectors */
                                 /* Generated (sequence of) codebook */
                                 /* Coordinates weights in block */
                                 /* Cell indices of tr. set vectors */
                                 ...
                                 /* Generate a sequence of codebooks */
{
    ...
    if (printsnr)
    {
        ...
        if (!multicb || (*sizec == sizecf))
            printf("%.3f  %.3f  %.3f\n",
                   raterec +
                   log((double) *sizec) / ((double) trainset->ncol *
                                           log((double) 2.0)), rate,
                   10.0 * log10(varts / dist));
    }
    else
...
void
flbg_train(int *Size, Fsignal Weight, int *MultiCB, Fimage InitCodeBook,
           int *NResCB, int *NResResCB, Fimage ResCodeBook,
           Fimage ResResCodeBook, int *PrintSNR, Fimage TrainSet, float *MSE,
           Fimage CodeBook)
                            /* Size of codebook */
                            /* Coordinates weights in block */
                             ...
                             * one vector per row */
{
  ...
  /*--- Decide if block coordinates are weighted ---*/
    ...
    else if (ResCodeBook)
    {
        ...
        if (TestWeight != 0)
        {
            ...
            {
                BWeight->values[c] =
                    ResCodeBook->gray[(ResCodeBook->nrow - 1) * sizev + c];
                if (BWeight->values[c] <= 0.0)
                    mwerror(FATAL, 2,
                            "Bad value in rescodebook for weighting!\n");
            }
        }
        else
            BWeight = NULL;
  ...
  /*--- Quantization of codebook if residual quantization ---*/
    ...
    if (sizec > sizet)
        mwerror(FATAL, 1,
                "Size of codebook is bigger than size of training set!\n");
  ...
  /*--- Memory allocation for CodeBook codebook buffer ---*/
    ...
    if (MultiCB)
    {
        ...
        if (InitCodeBook)
            CodeBook->gray[(CodeBook->nrow - 5) * sizev] =
                InitCodeBook->nrow - 4;
        else
    ...
    for (c = 0; c < sizev; c++)
        CodeBook->gray[(CodeBook->nrow - 2) * sizev + c] =
            CodeBook->gray[(CodeBook->nrow - 3) * sizev + c] =
            CodeBook->gray[(CodeBook->nrow - 4) * sizev + c] = 0.0;

    if (BWeight)
        for (c = 0; c < sizev; c++)
            CodeBook->gray[(CodeBook->nrow - 1) * sizev + c] =
                BWeight->values[c];
    else if (sizev >= 2)
        for (c = 0; c < sizev; c++)
    ...
    if (InitCodeBook)
    {
        *Size = InitCodeBook->nrow - 4;
        for (r = 0; r < *Size; r++)
            for (c = 0; c < sizev; c++)
                CodeBook->gray[r * sizev + c] =
                    InitCodeBook->gray[r * sizev + c];
        optimize(TrainSet, CodeBook, BWeight, cell, Size, *Size, PrintSNR,
                 MultiCB);
    }
    else
    {
            ...
            *Size = InitCodeBook->nrow - 4;
            for (r = 0; r < *Size; r++)
                for (c = 0; c < sizev; c++)
                    CodeBook->gray[r * sizev + c] =
                        InitCodeBook->gray[r * sizev + c];
            optimize(TrainSet, CodeBook, BWeight, cell, Size, sizemcb,
                     PrintSNR, MultiCB);
        }
        else
        {
