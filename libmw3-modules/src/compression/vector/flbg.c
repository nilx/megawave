...
function = {"Generates a (sequence of) codebook(s) from
            a training set of images using LBG (generalized Lloyd) algorithm"};
version = {"2.3"};
usage = {
 ...
 'i':InitRandCB->InitRandCB    "Initiate algorithm with a randomly
                               drawn codebook of size InitRandCB",
 'r':[RepeatRand=1]->RepeatRand  "Run the algorithm RepeatRand times
                                 (irrelevant if InitRandCB is smaller than 1)",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for mk_trainset(),
                                 * mk_codebook(), flbg_train() */

/*--- Constants ---*/

static void VARMEAN_TS(Fimage trainset, double *mean, double *var)
...
void
flbg(int *Size, int *Width, int *Height, int *Lap, int *Decim, int *Edge,
     Fsignal Weight, int *MultiCB, int *PrintSNR, Fimage Image2,
     Fimage Image3, Fimage Image4, Fimage Image5, Fimage Image6,
     Fimage Image7, Fimage Image8, int *InitRandCB, int *RepeatRand,
     int *NResCB, int *NResResCB, Fimage ResCodeBook, Fimage ResResCodeBook,
     Fimage Image1, Fimage CodeBook, float *MSE)
                            /* Size of codebook */
                            ...
                            /* Quantization m.s.e. for training set */
{
    ...
    mk_trainset(Width, Height, Lap, Decim, Edge, NULL, NULL, NULL, NULL,
                Image2, Image3, Image4, Image5, Image6, Image7, Image8, NULL,
                NULL, NULL, Image1, TrainSet);
    ...
    if (InitRandCB)
    {
        ...
        {
            mk_codebook(&sizev, &Mean, &Variance, InitRandCB, &sizev,
                        InitCodeBook);
            flbg_train(Size, Weight, MultiCB, InitCodeBook, NResCB, NResResCB,
                       ResCodeBook, ResResCodeBook, PrintSNR, TrainSet,
                       &msetr, CodeBook);
        }
    }
    else
    {
        ...
        flbg_train(Size, Weight, MultiCB, InitCodeBook, NResCB, NResResCB,
                   ResCodeBook, ResResCodeBook, PrintSNR, TrainSet, &msetr,
                   CodeBook);
    }
