...
/* mwcommand
 ...
 function = {"Generates a (sequence of) codebook(s)
             adapted for classified V.Q. from a training set of images
             using LBG algorithm"};
 usage = {
   ...
   'd':[Decim=1]->Decim          "Decimation factor in training images
                                 (for wavelet transform)",
   'e':[Edge=0]->Edge            "Do not take overlapping vectors
                                 if the distance to an edge is smaller
                                 than Edge",
   ...
   'W':Weight->Weight       "Weighting factors for the components
                            of vector (fsignal)",
   'M'->MultiCB             "Generate codebooks of size equal to
                            a power of two and smaller than Size",
...
#include <stdio.h>
#include "mw3.h"
#include "mw3-modules.h"         /* mk_trainset(), flbg_train() */

/*--- Constants ---*/

static void orderf(float *a, float *b)
...
static void ORDER_THRES(float **thres1, float **thres2, float **thres3)
...
void
flbg_adap(int *Size, int *Width, int *Height, int *Lap, int *Decim, int *Edge,
          float *ThresVal1, float *ThresVal2, float *ThresVal3,
          Fsignal Weight, int *MultiCB, int *PrintSNR, int *Size2, int *Size3,
          int *Size4, Fimage Image2, Fimage Image3, Fimage Image4,
          Fimage Image5, Fimage Image6, Fimage Image7, Fimage Image8,
          Fimage Output2, Fimage Output3, Fimage Output4, Fimage Image1,
          Fimage Output)
                            /* Size of codebook */
                            ...
                            /* Generated codebook */
{
    /* Training sets of blocks extracted form training images
     * for the different adaptive classes of vectors */
    Fimage TrainSet1, TrainSet2, TrainSet3, TrainSet4;
    float MSE;

    if (Weight)
        if (Weight->size != *Width * *Height)
            mwerror(FATAL, 1,
                    "Number of weights is different from size of block!\n");
  ...
  /*--- Compute training sets ---*/
    ...
    mk_trainset(Width, Height, Lap, Decim, Edge, ThresVal1, ThresVal2,
                ThresVal3, Size, Image2, Image3, Image4, Image5, Image6,
                Image7, Image8, TrainSet2, TrainSet3, TrainSet4, Image1,
                TrainSet1);
    ...
    /*--- (blocks whose energy is less than *ThresVal3) ---*/
        ...
        flbg_train(Size4, Weight, MultiCB, NULL, NULL, NULL, NULL, NULL,
                   PrintSNR, TrainSet4, &MSE, Output4);
  ...
  /*--- Compute codebook for third adaptive class (blocks whose energy ---*/
       /*--- is less than *ThresVal2 and greater than *ThresVal3) ---*/
        ...
        flbg_train(Size3, Weight, MultiCB, NULL, NULL, NULL, NULL, NULL,
                   PrintSNR, TrainSet3, &MSE, Output3);
  ...
  /*--- Compute codebook for second adaptive class (blocks whose energy ---*/
       /*--- is less than *ThresVal1 and greater than *ThresVal2) ---*/
        ...
        flbg_train(Size2, Weight, MultiCB, NULL, NULL, NULL, NULL, NULL,
                   PrintSNR, TrainSet2, &MSE, Output2);
  ...
  /*--- Compute codebook for first adaptive class (blocks whose energy ---*/
    ...
    flbg_train(Size, Weight, MultiCB, NULL, NULL, NULL, NULL, NULL, PrintSNR,
               TrainSet1, &MSE, Output);

    mw_delete_fimage(TrainSet1);

    Output->gray[(Output->nrow - 4) * Output->ncol] =
        Output->gray[(Output->nrow - 2) * Output->ncol + 1] = 0.0;
    if (ThresVal1)
