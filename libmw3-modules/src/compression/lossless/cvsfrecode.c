...
function = {"Encode the change of direction of points in the curves :
            return bit rate for lossless compression"};
usage = {
 C->C  "input curves",
...
#include <stdio.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for arencode2(), fentropy() */
...
static unsigned int Freeman(Curves C, Fimage dirchg)
{
    ...
    for (cv = C->first; cv; cv = cv->next)
    {
        ...
        if (secondpoint == NULL)
            mwerror(FATAL, 1,
                    "Cannot apply Freeman algorithm : "
                    "a curve contains only one point !\n");
        else
            for (point = secondpoint; point; point = point->next)
            {
                ...
                if (abs(dx) + abs(dy) != 1)
                    mwerror(FATAL, 1,
                            "Cannot apply Freeman algorithm : "
                            "two successive points are not "
                            "neighbours for the 4-connexity !\n");
                if (dx == 1)    /* right */
                    dir1 = 1;
                else if (dx == -1)      /* left */
                    dir1 = 3;
                ...
                if (point == secondpoint)       /* Code the direction itself */
                    *dptr = (float) dir1;
                else            /* Code the change of direction :
                                 * straight, right or left */
                {
                    ...
                    if (ddir == 0)      /* straight ahead */
                        ...
                        *dptr = 2.0;
                        ...
                        mwerror(FATAL, 1,
                                "Cannot apply Freeman algorithm : "
                                "a curve comes backs on its path !\n");
                }
                dptr++;
                size++;
                dir0 = dir1;
...
double cvsfrecode(Curves C, unsigned int *N, double *B)
{
    ...
    unsigned int npts;          /* Total number of points
                                 * in the curves of C (=*N) */
    ...
    /* Initialisations */
    /* The maximal size of dirchg = nb of points
     * but origins + nb of end_of_curves */
    ...
    mwdebug
        ("Rates in bits per points to encode the curves (but the origins) :\n");
