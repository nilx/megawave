...
function = {"Encode the origins of curves : return bit rate
...
static void Get_Size_Org(Curves C, int *ncol, int *nrow, int *mincol,
                         int *minrow)
{
    Curve cv;
...
static int Fill_Bitmap_With_Origins(Fimage bitmap, Curves C, int mincol,
                                    int minrow)
{
    int ncol, nsymb;
    Curve cv;
    ...
    for (cv = C->first; cv; cv = cv->next)
    {
        ptr =
            &(bitmap->gray
              [(cv->first->y - minrow) * ncol + (cv->first->x - mincol)]);
        (*ptr)++;
        if (*ptr > nsymb - 1)
            nsymb = (*ptr) + 1;
    }
...
double cvsorgcode(Curves C, unsigned int *N, double *B)
{
  ...
  /*-- Method #2 : code the distance between 2 successive points --*/
    ...
    mwdebug
        ("\n--- Method #2 : code the distance between "
         "two successive origin points :\n");
