...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for arencode2(), entropy(), fmse() */
...
static void ENCODE_INT_FIMAGE(int symb, int max_symb, Cimage compress)
...
static int FLOAT2INT(float *f, int nbitlog, int nbitstep, int *testoverflow)
{
    int n;
    int logf;
    ...
    *testoverflow = 0;
    ...
    else
    {
        ...
        n = (1 << nbitstep) * logf + ((double) fabsf -
                                      ((int) 1 <<
                                       ((int) logstep * (logf - 1)))) / fstep;
        if (n >= 1 << (nbitstep + nbitlog))
            n = (1 << (nbitstep + nbitlog)) - 1;
        fabsf =
            ((int) 1 << ((int) logstep * (logf - 1))) + (n -
                                                         ((int) 1 << nbitstep)
                                                         * logf +
                                                         1.0) * fstep;
    }
...
static void
INIT_ENCODING_FIMAGE(int *smallheader, int nrow, int ncol, int c_stepsize,
                     int nstep, int *center, int minstep, float ashift,
                     int c_ashift, Cimage compress)
                                 ...
                                 /* Compressed file */
{
    ...
    /*--- Check if there are different codebooks for each class ---*/
        ...
        if (center)
        {
            ...
            if (nstep > 1)
            {
                if (minstep >= 0)
                {
                    ...
                    ENCODE_INT_FIMAGE(minstep, 1 << (NBIT_MINSTEP - 1),
                                      compress);
                }
                else
                {
        ...
        else
        {
            ...
            if (ashift >= 0)
                ...
                ENCODE_INT_FIMAGE(1, 1, compress);
            ENCODE_INT_FIMAGE(c_ashift, 1 << (NBIT_LOG + NBIT_STEP - 1),
                              compress);
        }
    ...
    /*--- Encode quantization step width ---*/
        ...
        if (nstep > 1)
            ENCODE_INT_FIMAGE(c_stepsize, 1 << (NBIT_LOG + NBIT_STEP - 1),
                              compress);
...
static void min_max(Fimage image)
{
    long i;
...
static void
unif_quant(int *printsnr, int *smallheader, int *nstep, float *sstep,
           int *center, Fimage image, Fimage result, Cimage compress,
           double *ent, double *ratear)
                                ...
                                /* Arithmetic coding rate */
{
    ...
    if (!ent)
        mwerror(FATAL, 1, "NULL <ent> in unif_quant()\n");
  ...
  /*--- Computes size of histogram and step size for quantization ---*/
    ...
    if (sstep)
    {
        ...
        if (hsize > MAX_NSTEP)
            mwerror(FATAL, 2,
                    "StepSize is to small => nstep is too large!\n");
  ...
  /*--- Quantization of image ---*/
    ...
    INIT_ENCODING_FIMAGE(smallheader, image->nrow, image->ncol, c_stepsize,
                         hsize, center, minstep, ashift, c_ashift, compress);
    ...
    if (hsize > 1)
        arencode2(&hsize, NULL, &hsize, NULL, predic, NULL, NULL, symbol, &e,
                  bufcomp);
    else
        e = 0.0;
...
void
fscalq(int *PrintSNR, int *SmallHeader, int *NStep, float *SStep, int *Center,
       Cimage Compress, Fimage Image, Fimage Result, double *MSE, double *SNR,
       double *Ent, double *RateAr)
                                /* Control info print on SNR */
                                ...
                                /* Arithmetic coding rate */
{
     ...
     */
    if (!Ent)
        Ent = (double *) malloc(sizeof(double));
    if (!RateAr)
        RateAr = (double *) malloc(sizeof(double));
    ...
    unif_quant(PrintSNR, SmallHeader, NStep, SStep, Center, Image, Result,
               Compress, Ent, RateAr);

    if (MSE)
        fmse(Image, Result, NULL, &PsnrFlg, SNR, &psnr, MSE, &mrd);
