...
#include <stdio.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for readpoly(), fillpolys(), emptypoly()
                                 * fmask(), lsnakes() */

void lsnakes_demo(Cimage u, Cmovie out, int *Niter, int *Nframes, float *thre,
                  float *force)
                 ...
                 /* Output movie */
{
    ...
    printf
        ("Please select the polygons (initial snakes) "
         "on the image using the mouse.\n");
    printf
        ("Click on the mouse left button to create "
         "a new point in the current coordinates(x,y).\n");
    printf
        ("To remove the last recorded point, click on "
         "the mousse middle button.\n");
    printf
        ("To terminate your polygon and to go ready "
         "for the next one, click on the mouse right button");
    printf
        ("(it links the last recorded point "
         "to the first recorded point).\n");
    ...
    if ((fimage_polys1 =
         (Fimage) mw_conv_internal_type(cimage_polys1, "cimage",
                                        "fimage")) == NULL)
        mwerror(FATAL, 1, "Not enough memory.\n");

    if ((fimage_org =
         (Fimage) mw_conv_internal_type(u, "cimage", "fimage")) == NULL)
        mwerror(FATAL, 1, "Not enough memory.\n");

    mwdebug("Calling fmask...\n");
    fmask(fimage_polys1, fimage_org, NULL, fimage, NULL, &fmaskopt1,
          &fmaskopt2);

    if ((cimage0 =
         (Cimage) mw_conv_internal_type(fimage, "fimage", "cimage")) == NULL)
        mwerror(FATAL, 1, "Not enough memory.\n");
    ...
    for (k = 2; k <= *Nframes; k++)
    {
        ...
        if ((fimage_polys0 =
             (Fimage) mw_conv_internal_type(cimage_polys0, "cimage",
                                            "fimage")) == NULL)
            mwerror(FATAL, 1, "Not enough memory.\n");
        ...
        if ((cimage_polys0 =
             (Cimage) mw_conv_internal_type(fimage_polys0, "fimage",
                                            "cimage")) == NULL)
            mwerror(FATAL, 1, "Not enough memory.\n");
        ...
        if ((cimage1 =
             (Cimage) mw_conv_internal_type(fimage, "fimage",
                                            "cimage")) == NULL)
            mwerror(FATAL, 1, "Not enough memory.\n");
