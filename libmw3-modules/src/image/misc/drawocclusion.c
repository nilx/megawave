...
/* We assume than there are no more than LABELS_NUMBER labels obtained
 * at first pass (before updating) */
                       ...
                       char *connectivity)
{
    ...
    /* The frame used for processing is 2 pixels wider and higher than
     * the input to avoid problems with borders */
            ...
            if (*ptrout)
            {
                if (label > LABELS_NUMBER)
                    mwerror(FATAL, 1, "There are more than %d labels !\n",
                            LABELS_NUMBER);
                /* 'kernel' contains the four upper nearest neighbours
                 * of current point
                 *    * * *
                 *    * + x
                 *    x x x
                 * if 4-connectivity is used then only 2 neighbors are
                 * taken into account */
                kernel[0] = *(ptrout - 1);
                if (connectivity)
                    kernel[1] = (-1);
...
static void Help(void)
{
    ...
    printf("\nMouse:\n");
    printf
        ("\tLeft button: Create a new point "
         "in the current coordinates (x,y).\n");
    printf("\tMiddle button: Remove the last recorded point.\n");
    printf
        ("\tRight button: Terminate the polygon. "
         "Go ready for the next one.\n");
    ...
    for (p = poly->first; p; p = p->next)
        for (c = p->first; c && (c->x != -1); c = c->next)
        {
            ...
            else
            {
                ...
                b0 = b1;
                if (!c->next)
                    DrawLinePoly(ImageWindow, (p->first)->x, (p->first)->y,
                                 a0, b0);
            }
        }
...
static int readpoly_notify(Wframe * ImageWindow, void *foo)
{
    ...
    case W_MS_MIDDLE:
        ...
        if (npc != NULL)
        {
            ...
            mwerror(WARNING, 0, "Polygon completed: cannot delete\n");
        break;
    ...
    case W_MS_RIGHT:
        ...
        if (pc->x != -1)
        {
            WDrawLine(ImageWindow, (pl->first)->x, (pl->first)->y, pc->x,
                      pc->y);
                   ...
                   char *window, int *zoom, int *gray)
{
    ...
    if (((interlabelimage = mw_new_fimage()) == NULL) ||
        (mw_alloc_fimage(interlabelimage, image->nrow + 2, image->ncol + 2) ==
         NULL))
        mwerror(FATAL, 1, "Not enough memory !\n");
    /* "interlabelimage" is 2 pixels higher and larger than
     * "labelimage" in order to deal properly with borders */
    if (hole_image)
    {
    ...
    if (zoom)
    {
        if ((*zoom <= 0) || ((double) (*zoom) / 2 != (double) ((*zoom) / 2)))
            mwerror(USAGE, 1,
                    "-z option parameter must be "
                    "a stricly positive even integer !\n");
        if ((image_zoom = mw_new_cimage()) == NULL)
            mwerror(FATAL, 0, "Not enough memory\n");
        flt_zoom = (float) *zoom;
        czoom(image, image_zoom, (char *) NULL, (char *) NULL, &flt_zoom,
              &interp, (char *) NULL);
    }
    ...
    if (zoom)
        ImageWindow = (Wframe *)
            mw_get_window((Wframe *) window, image_zoom->ncol,
                          image_zoom->nrow, 100, 100, image->name);
    else
        ImageWindow = (Wframe *)
            mw_get_window((Wframe *) window, image->ncol, image->nrow, 100,
                          100, image->name);
    ...
    if (zoom)
    {
        WLoadBitMapImage(ImageWindow, image_zoom->gray, image_zoom->ncol,
                         image_zoom->nrow);
        WRestoreImageWindow(ImageWindow, 0, 0, image_zoom->ncol,
                            image_zoom->nrow);
    }
    else
    {
    ...
    for (p = poly->first; p; p = p->next)
    {
        if (p->first->next)
        {
            for (c = p->first; c->next; c = c->next)
                mw_draw_fimage(interlabelimage, c->x + 1, c->y + 1,
                               c->next->x + 1, c->next->y + 1, 1.0);
            mw_draw_fimage(interlabelimage, c->x + 1, c->y + 1,
                           p->first->x + 1, p->first->y + 1, 1.0);
        }
        else
            /* In this case, the polygon reduces to a single point */
            mw_plot_fimage(interlabelimage, p->first->x + 1, p->first->y + 1,
                           1.0);
    }

    /* Step 2 : occlusions are defined as those sets enclosed by the
     * polygons */
    foreground = 0.0;
    fconnected(interlabelimage->gray, interlabelimage->nrow,
               interlabelimage->ncol, foreground, &Number, (char *) NULL,
               (char *) NULL, (char *) 1);

    /* Remark that now only the background has value 1. The polygon
     * borders have value 0 while the regions they enclose have value
     * > 1. Thus it remains to compute the connected components of the
    ...
    for (i = 0, ptrlabel = labelimage->gray; i < labelimage->nrow;
         i++, ptrinterlabel += 2)
    ...
    if (hole_image)
    {
        for (i = 0, ptrlabel = labelimage->gray, ptrhole =
             (*hole_image)->gray; i < labelimage->nrow; i++)
            for (j = 0; j < labelimage->ncol; j++, ptrlabel++, ptrhole++)
                if (*ptrlabel)
                    *ptrhole = (unsigned char) (*gray);
    }
