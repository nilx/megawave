...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for czoom(), fderiv() */
...
static void init_visu(Fimage u, Dlists in, float zoom)
{
    ...
    win =
        (Wframe *) mw_get_window(NULL, bg->ncol, bg->nrow + 12, 10, 10,
                                 bg->name);
    if (!win)
...
static int visu(Dlists in, float zoom, int niter, double energy, int refresh)
{
    ...
    if (refresh)
    {
        WLoadBitMapColorImage(win, bg->gray, bg->gray, bg->gray, bg->ncol,
                              bg->nrow);
                  ...
                  double *power, char *v, float *V)
{
    ...
    sizemax = stop = 0;
    for (j = 0; j < in->size; j++)
        if (in->list[j]->size > sizemax)
            sizemax = in->list[j]->size;
    if (V)
        init_visu(u, in, *V);
    ...
    mingrad = 0.;
    nsize = 8;
    fderiv(u, NULL, NULL, NULL, NULL, imux, imuy, NULL, NULL, &mingrad,
           &nsize);
    fderiv(imux, NULL, NULL, NULL, NULL, imuxx, imuxy, NULL, NULL, &mingrad,
           &nsize);
    fderiv(imuy, NULL, NULL, NULL, NULL, NULL, imuyy, NULL, NULL, &mingrad,
           &nsize);
    ...
    /********** ONE ITERATION **********/
      ...
      /********** ONE CURVE **********/
            ...
            size = in->list[j]->size;
            closed =
                ((in->list[j]->values[0] == in->list[j]->values[2 * size - 2]
                  && in->list[j]->values[1] ==
                  in->list[j]->values[2 * size - 1]) ? 1 : 0);
            ...
            for (k = 0; k < size - 1; k++)
            {
                ...
                mx = 0.5 * (in->list[j]->values[2 * k] +
                            in->list[j]->values[2 * k + 2]);
                my = 0.5 * (in->list[j]->values[2 * k + 1] +
                            in->list[j]->values[2 * k + 3]);
                ...
                dx[k] =
                    in->list[j]->values[2 * k + 2] -
                    in->list[j]->values[2 * k];
                dy[k] =
                    in->list[j]->values[2 * k + 3] -
                    in->list[j]->values[2 * k + 1];
            ...
            if (i < *niter)
                for (k = (closed ? 0 : 1); k < size - 1; k++)
                {
                    ...
                    tx = (-dg[kp] * uy[kp] + dg[k] * uy[k]
                          ...
                          + 0.5 * dg[kp] * (-uxy[kp] * dx[kp] +
                                            uxx[kp] * dy[kp])) / length -
                        (dx[kp] / dn[kp] -
                         dx[k] / dn[k]) * e / (length * length);
                    ty = (dg[kp] * ux[kp] - dg[k] * ux[k] + h[kp] * dy[kp] -
                          h[k] * dy[k] + 0.5 * dg[k] * (-uyy[k] * dx[k] +
                                                        uxy[k] * dy[k]) +
                          0.5 * dg[kp] * (-uyy[kp] * dx[kp] +
                                          uxy[kp] * dy[kp])) / length -
                        (dy[kp] / dn[kp] -
                         dy[k] / dn[k]) * e / (length * length);
        ...
        if (V)
        {
            stop = (WUserEvent(win) == W_KEYPRESS
                    && WGetKeyboard() == (int) 'q');
            if (stop)
                *niter = i;
            if (i % 100 == 0 || i == *niter)
                visu(in, *V, i, energy, 1);
            else if (i % 10 == 0)
                visu(in, *V, i, energy, 0);
        }
        if (i == 0 || i == *niter || v)
            printf("energy: %g\n", energy);
    }
    ...
    mw_delete_fimage(imuyy);
    mw_delete_fimage(imuxy);
    mw_delete_fimage(imuxx);
    mw_delete_fimage(imuy);
    mw_delete_fimage(imux);
    free(uyy);
    free(uxy);
    free(uxx);
    free(uy);
