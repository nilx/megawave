  ...
  /** Premier passage derivee en Y **/
    ...
    for (j = 0; j < nx; ++j)
    {
        ...
        for (i = 2; i < ny; ++i)
            a3[i * nx + j]
                = an11 * a2[(i - 1) * nx + j]
                - ad1 * a3[(i - 1) * nx + j] - ad2 * a3[(i - 2) * nx + j];
    }
    ...
    for (j = 0; j < nx; ++j)
    {
        ...
        for (i = lig_3; i >= 0; --i)
            a4[i * nx + j]
                = -an11 * a2[(i + 1) * nx + j]
                - ad1 * a4[(i + 1) * nx + j] - ad2 * a4[(i + 2) * nx + j];
    }
    ...
    for (i = 1; i <= lig_2; ++i)
    {
        ...
        for (j = 1; j <= col_2; ++j)
        {
            ...
            if (a3[icoll + j] > 0.)
            {
                ...
                {
                    gun =
                        a2[icoll + jp1] + (a2[ip1 * nx + jp1] -
                                           a2[icoll + jp1]) / wd;
                    if (a2[icoll + j] <= gun)
                        continue;
                    gzr =
                        a2[icoll + jm1] + (a2[im1 * nx + jm1] -
                                           a2[icoll + jm1]) / wd;
                    if (a2[icoll + j] < gzr)
                        continue;
                ...
                {
                    gun =
                        a2[ip1 * nx + j] + (a2[ip1 * nx + jp1] -
                                            a2[ip1 * nx + j]) * wd;
                    if (a2[icoll + j] <= gun)
                        continue;
                    gzr =
                        a2[im1 * nx + j] + (a2[im1 * nx + jm1] -
                                            a2[im1 * nx + j]) * wd;
                    if (a2[icoll + j] < gzr)
                        continue;
                    a3[icoll + j] = a2[icoll + j];
                ...
                if (wd >= -1)
                {
                    icolonnes = ip1 * nx;
                    gun =
                        a2[icolonnes + j] - (a2[icolonnes + jm1] -
                                             a2[icolonnes + j]) * wd;
                    if (a2[icoll + j] <= gun)
                        continue;
                    icolonnes = im1 * nx;
                    gzr =
                        a2[icolonnes + j] - (a2[icolonnes + jp1] -
                                             a2[icolonnes + j]) * wd;
                    if (a2[icoll + j] < gzr)
                        continue;
                    a3[icoll + j] = a2[icoll + j];
                ...
                gun =
                    a2[icoll + jm1] - (a2[ip1 * nx + jm1] -
                                       a2[icoll + jm1]) / wd;
                if (a2[icoll + j] <= gun)
                    continue;
                gzr =
                    a2[icoll + jp1] - (a2[im1 * nx + jp1] -
                                       a2[icoll + jp1]) / wd;
                if (a2[icoll + j] < gzr)
                    continue;
                a3[icoll + j] = a2[icoll + j];
            ...
            gzr =
                a2[icoll + jm1] - (a2[ip1 * nx + jm1] - a2[icoll + jm1]) / wd;
            if (a2[icoll + j] < gzr)
                continue;
            gun =
                a2[icoll + jp1] - (a2[im1 * nx + jp1] - a2[icoll + jp1]) / wd;
            if (a2[icoll + j] <= gun)
                continue;
            a3[icoll + j] = a2[icoll + j];
        }
