...
/* mwcommand
 ...
 'd':[d=16]->d      "number of allowed gradient directions",
 'n':[nl=96]->nl    "number of line directions to scan",
 'g':[g=2.0]->g     "minimum gradient norm to define a direction",
 'e':[eps=0.]->eps  "-log10(max. number of false alarms)",
 'c':crv<-crv       "store segments as curves (output, Flists)",
  u->u              "input Fimage",
  out<-falign       "result: 5-Flist (x1 y1 x2 y2 -log10(nfa))"
          };
*/

/*----------------------------------------------------------------------
 v1.2: align becomes falign to avoid collision with Linux command (LM)
...
struct one_segment {
    short start;
    /* starting position (distance from border) */
    short end;
    /* ending position (hence, length is end-start+1) */
    double nfa;
             ...
             Flists crv)
{
    ...
    n = ceil(sqrt((double) nx * (double) nx + (double) ny * (double) ny)) + 1;
  ...
  /*** compute P(k,l) ***/
    test =
        tab(n, 1.0 / (double) (*d), (double) (nx * ny) * (double) (nx * ny));
  ...
  /******************** memory allocation ********************/
    ...
    seg =
        (struct one_segment *) malloc(size_seg * sizeof(struct one_segment));
      ...
      /*** third loop : start positions ***/
        ...
        /*** fourth loop : phase for two-spaced pixels ***/
          ...
          /*** detect aligned points by blocs ***/
                    ...
                    xx = ox + pos * mx +
                        (int) (dx * (float) (l * 2 + lphase));
                    yy = oy + pos * my +
                        (int) (dy * (float) (l * 2 + lphase));
                    ...
                    for (i = 0; i < iseg; i++)
                        for (j = 0; j < iseg; j++)
                            if (i != j)
                                ...
                                if (seg[i].start >= seg[j].start
                                    && seg[i].end <= seg[j].end)
                                {

                                    /* remove the less meaningful
                                     * of seg[i] and seg[j] */
                                    if (seg[i].nfa < seg[j].nfa)
                                        seg[j].ok = 0;
                                    else
                                        seg[i].ok = 0;
        ...
        /*** store detected segments ***/
                for (i = 0; i < iseg; i++)
                    if (seg[i].ok)
                    {
                        seglist[iseglist * 5] =
                            (float) (ox + pos * mx) +
                            dx * (float) (seg[i].start);
                        seglist[iseglist * 5 + 1] =
                            (float) (oy + pos * my) +
                            dy * (float) (seg[i].start);
                        seglist[iseglist * 5 + 2] =
                            (float) (ox + pos * mx) +
                            dx * (float) (seg[i].end);
                        seglist[iseglist * 5 + 3] =
                            (float) (oy + pos * my) +
                            dy * (float) (seg[i].end);
                        seglist[iseglist * 5 + 4] =
                            -(float) log10(seg[i].nfa);
                        iseglist++;
