...
/* mwcommand
 ...
 'c':crv<-crv        "store segments as curves (output, Flists)",
  u->u               "input Fimage",
  out<-falign_mdl    "result 6xN Fimage
                     (x1 y1 x2 y2 -log10(nfa) precision_level)"
          };
*/
...
struct one_segment {
    ...
    short length;               /* number of points
                                 * (-> ending position is start+2*length) */
    short k;                    /* number of aligned points */
    double nfa;                 /* number of false alarms */
    int ngrad;                  /* orientation precision level,
                                 * i.e. # gradient directions */
    int precision;              /* orientation precision level,
                                 * i.e. ngrad = ngrad_min * 2^precision */
};
                  ...
                  double *eps, float *g, Flists crv)
{
        ...
        /*** third loop : start positions ***/
          ...
          /*** detect aligned points by blocs ***/
                    ...
                    for (; xx >= 0 && xx < nx && yy >= 0 && yy < ny;)
                    {
                        ...
                        if (error > -1000.0)
                        {       /* orientation defined at this point */
                            error -= theta;
                            while (error <= -M_PI)
                                error += 2.0 * M_PI;
                            while (error > M_PI)
                    ...
                    for (i = 0; i < nblocs; i++)
                        for (j = i; j < nblocs; j++)
                        {
                            ...
                            if ((length > 0) && (nfa < max_nfa))
                            {
                                ...
                                /* update owner for each point
                                 * of a dilated segment */
                                for (l = 0; l < length; l++)
                                {
                                    xx = ox + pos * mx +
                                        (int) (dx *
                                               (float) (l + seg[iseg].start));
                                    yy = oy + pos * my +
                                        (int) (dy *
                                               (float) (l + seg[iseg].start));
                                    error = angle->gray[yy * nx + xx];
  ...
  /*** free memory (1/2) ***/
    ...
    if (crv)
        crv = mw_change_flists(crv, iseg, 0);
    j = 0;
    for (i = 0; i < iseg; i++)
        if (seg[i].k > 0)
        {
            ...
            oy = ((side == 2) ? ny - 1 : 0);
            theta =
                0.5 * M_PI * (double) side + (float) (seg[i].itheta) * dtheta;
            dx = (float) cos((double) theta);
            dy = (float) sin((double) theta);
            ...
            {
                xx = ox + seg[i].pos * mx +
                    (int) (dx * (float) (seg[i].start));
                yy = oy + seg[i].pos * my +
                    (int) (dy * (float) (seg[i].start));

                mw_plot_fimage(res, 0, j, (float) xx);
                mw_plot_fimage(res, 1, j, (float) yy);

                xx = ox + seg[i].pos * mx +
                    (int) (dx * (float) (seg[i].start + seg[i].length));
                yy = oy + seg[i].pos * my +
                    (int) (dy * (float) (seg[i].start + seg[i].length));
