...
#include <time.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fderiv(), fsepconvol() */
...
Flist harris(Fimage in, float *k, float *g, int *size, double *t)
{
    ...
    fderiv(in, NULL, NULL, NULL, NULL, gradx, grady, NULL, NULL, &fzero,
           &nsize);
    for (i = 0; i < nrow; i++)
        for (j = 0; j < ncol; j++)
        {
    ...
    for (i = 0; i < nrow; i++)
        for (j = 0; j < ncol; j++)
        {
            ...
            if (_(corner, i, j) > (float) pow(10., *t)
                && _(corner, i, j) > max)
            {
