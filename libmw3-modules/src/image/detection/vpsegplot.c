...
/* mwcommand
  ...
  function = {"Display vanishing points and associated segments
             (output of vpoint)"};
  usage = {
  'l'->lines        "show supporting lines instead of segments",
  image->image      "input image (only used to obtain dimensions)",
  allsegs->allsegs  "input 5-Flist of all detected segments
                    (x1 y1 x2 y2 -log10(nfa))",
  vpoints->vpoints  "input 9-Flist of vanishing regions
                    detected by vpoint (x1 y1 x2 y2 x3 y3 x4 y4 -log10(nfa))",
  vsegs->vsegs      "input segments indexes associated to
                    each detected vanishing region (Flists)",
  n->n              "input integer: index of vanishing region
                    to display (for n out of bounds,
                    segments not contributing to any vp will be displayed)",
  crv<-crv          "store n-th vp and contributing segments
                    as curves (output, Flists)"
          };
*/
               ...
               int n, Flists crv, char *lines)
{
    ...
    /*
     * if (n>=vpoints->size)
     * mwerror(ERROR, 1, "The number n should be smaller "
     *         "than the total number of detected vanishing points!");
     */
    vdim = vpoints->dim;
    if (vdim < 8)
    ...
    if (sdim < 4)
        mwerror(ERROR, 1,
                "allsegs should be a list (Fimage or Flist) "
                "of dimension >= 4 !");
    if ((n >= vpoints->size) || (n < 0))
    {
        /* For n out of bounds display segments
         * not contributing to any vpoint */
