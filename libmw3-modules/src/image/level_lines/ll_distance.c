                 ...
                 Fimage nearest)
{
    ...
    if (nearest)
    {
        ...
        if (nearest == NULL)
            mwerror(FATAL, 1, "ll_distance: Not enough memory.\n");
        ...
        for (i = 0; i < dx; i++)
        {
            ...
            if (cur <= *level)
            {
                ...
                if (i > 0)
                    ...
                    max = cur;
                if (i < dx - 1)
                    if (in->gray[pos + 1] > max)
                        max = in->gray[pos + 1];
                if (j > 0)
                    if (in->gray[pos - dx] > max)
                        max = in->gray[pos - dx];
                if (j < dy - 1)
                    if (in->gray[pos + dx] > max)
                        max = in->gray[pos + dx];
                ...
                if (!ok && cur == *level)
                {
                    ...
                    if (i > 0)
                        ...
                        min = cur;
                    if (i < dx - 1)
                        if (in->gray[pos + 1] < min)
                            min = in->gray[pos + 1];
                    if (j > 0)
                        if (in->gray[pos - dx] < min)
                            min = in->gray[pos - dx];
                    if (j < dy - 1)
                        if (in->gray[pos + dx] < min)
                            min = in->gray[pos + dx];
                ...
                if (ok)
                {
                    /* that is, if (i,j) belongs to the level line */
