...
static void draw_mlines(Morpho_line mline, unsigned char **im, int NL, int NC,
                        char *border)
{
...
/* Set the original bitmap image bimage in the background
 * of the morpho-line bitmap
 * mlimage
 */
...
static void setimage(char *border, Cimage bimage, Cimage mlimage)
{
    ...
    if (((border) && ((NLml != 2 * NLb + 1) || (NCml != 2 * NCb + 1)))
        ...
        mwerror(FATAL, 1,
                "Bad size for input bitmap image... "
                "Are you sure these is the original one ?\n");
    ...
    for (xml = x0, xb = 0; xml < x1; xml += 2, xb++)
        for (yml = y0, yb = 0; yml < y1; yml += 2, yb++)
        {
            ...
            if (xml + 1 < x1)
                mw_plot_cimage(mlimage, xml + 1, yml, c);
            if (yml + 1 < y1)
                mw_plot_cimage(mlimage, xml, yml + 1, c);
            if ((xml + 1 < x1) && (yml + 1 < y1))
                mw_plot_cimage(mlimage, xml + 1, yml + 1, c);
        }
}
...
Cimage ml_draw(Mimage m_image, Cimage bimage, char *border, Cmovie movie)
{
    ...
    {                           /* draw morpho lines level per level */
        ...
        for (mline = mline_list; mline != NULL;)
        {
            ...
            if (bimage != NULL)
                setimage(border, bimage, newcb);
            im[0] = newcb->gray;
            for (l = 1; l < newcb->nrow; l++)
                im[l] = im[l - 1] + newcb->ncol;
            ...
            while ((mline != NULL)
                   && (mline->previous->minvalue == mline->minvalue)
