...
Cmimage cll_remove(Cmimage cmimage, int *L)
{
    ...
    if ((ll->minvalue.red == -infty) &&
        (ll->minvalue.green == -infty) && (ll->minvalue.blue == -infty))
        ll_type = 1;
    else if ((ll->maxvalue.red == infty) &&
             (ll->maxvalue.green == infty) && (ll->maxvalue.blue == infty))
        ll_type = 0;
    else
        mwerror(FATAL, 1,
                "Input cmimage does not contain level lines "
                "but morpho lines !\n");

    N = Nrm = 0;
    for (ll = cmimage->first_ml; ll; ll = ll_next, N++)
    {
        if (((ll_type == 0) && ((ll->maxvalue.red != infty) ||
                                (ll->maxvalue.green != infty) ||
                                (ll->maxvalue.blue != infty))) ||
            ((ll_type == 1) && ((ll->minvalue.red != -infty) ||
                                (ll->minvalue.green != -infty) ||
                                (ll->minvalue.blue != -infty))))
            mwerror(FATAL, 1,
                    "Input cmimage contains mixed types of level lines !\n");
        ll_next = ll->next;
        /*  check_ll(ll); */
    ...
    if (ll_type == 0)
        mwdebug
            ("Removed %d level lines of length "
             "< %d (local maxima) over %d (%3.1f %%).\n"
             "\tRemain %d level lines of type {(x,y) / f(x,y) >= v}\n",
             Nrm, *L, N, (100.0 * Nrm) / N, N - Nrm);
    else
        mwdebug
            ("Removed %d level lines of length "
             "< %d (local minima) over %d (%3.1f %%).\n"
             "\tRemain %d level lines of type {(x,y) / f(x,y) <= v}\n",
             Nrm, *L, N, (100.0 * Nrm) / N, N - Nrm);
