...
#include <assert.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fsaddles() */
...
typedef struct {
    short int x, y;             /* For saddle point, north-east corner
                                 * of dual pixel */
...
typedef struct {
    ...
    PIXEL_T otherBound;         /* Min gray level if max-oriented,
                                 * max if min-oriented */
} Neighborhood;
...
static void init_neighborhood(Neighborhood * pNeighborhood,
                              int init_neigh_iMaxArea)
{
    ...
    pNeighborhood->tabPoints =
        (Neighbor *) malloc((init_neigh_iMaxArea + 1) * sizeof(Neighbor));
...
static void add_neighbor(Neighborhood * pNeighborhood, short int x,
                         short int y, float value, char bSaddle)
{
                                         ...
                                         int ***ptabtabVisitedSaddles)
{
    ...
    ARRAY_2D_ALLOC(tabtabVisitedNeighborSaddles, iWidth - 1, iHeight - 1,
                   int);
}
...
{
    tabPointsInShape =
        (PixelOrSaddle *) malloc(init_region_iMaxArea *
                                 sizeof(PixelOrSaddle));
...
static void levelize(float **tabtabPixelsOutput, float **tabtabSaddleValues,
                     PixelOrSaddle * tabPoints, int iNbPoints, float newGray)
{
    int i;
...
static unsigned char configuration(int **tabtabVisitedPixels,
                                   int **tabtabVisitedSaddles, short int x,
                                   short int y)
{
...
#define DIAGONAL_IN_REGION(pixel, x, y, diag) ( \
  ...
  ((cPattern&(diag))==(diag) && tabtabVisitedSaddles[y][x] == iExploration) \
    ...
    if (x != 0 && y != 0 &&
        DIAGONAL_IN_REGION(tabtabVisitedPixels[y - 1][x - 1], x - 1, y - 1,
                           NORTH | WEST))
        cPattern |= NORTH_WEST;
    if (x != 0 && y != iMaxY &&
        DIAGONAL_IN_REGION(tabtabVisitedPixels[y + 1][x - 1], x - 1, y,
                           SOUTH | WEST))
        cPattern |= SOUTH_WEST;
    if (x != iMaxX && y != 0 &&
        DIAGONAL_IN_REGION(tabtabVisitedPixels[y - 1][x + 1], x, y - 1,
                           NORTH | EAST))
        cPattern |= NORTH_EAST;
    if (x != iMaxX && y != iMaxY &&
        DIAGONAL_IN_REGION(tabtabVisitedPixels[y + 1][x + 1], x, y,
                           SOUTH | EAST))
        cPattern |= SOUTH_EAST;
...
static void connect(PixelOrSaddle * tabPoints, int iNbPoints,
...
static void new_connection(PixelOrSaddle * pPoint, float level,
...
static void store_neighbors_to_saddle(float **ou, short int x, short int y,
                                      Neighborhood * pNeighborhood)
{
...
static void store_saddle_neighbors(float **ou, short int x, short int y,
                                   Neighborhood * pNeighborhood)
{
    if (x > 0)
    {
...
static char saddle_change(int **tabtabVisitedPixels, short int x, short int y)
{
    ...
    if (cDiag2)
    {
        /* Pixels of 1st diag inside (cDiag1) or outside (!cDiag1)? */
...
of success. */
static char add_iso_level(PixelOrSaddle * tabIsoPointsInShape,
                          int *pCurrentArea, int *pNbPixels,
                          ...
                          float **tabtabSaddleValues)
{
    ...
    {                           /* 1) Neighbor is added to the region */
        ...
        tabIsoPointsInShape[iCurrentArea++].bSaddle =
            pNeighbor->point.bSaddle;
        if (!pNeighbor->point.bSaddle)
        {
            ++*pNbPixels;
            cPattern =
                configuration(tabtabVisitedPixels, tabtabVisitedSaddles, x,
                              y);
    ...
    if (*pNbPixels <= iMaxArea &&
        ...
        pNeighborhood->type != INVALID && iNbHoles == 0)
    {
...
static void find_terminal_branch(float **ou, int **tabtabVisitedPixels,
                                 int **tabtabVisitedSaddles,
                                 float **tabtabSaddleValues, short int x,
                                 short int y, Neighborhood * pNeighborhood,
                                 enum TypeOfTree type,
                                 Connection * tabConnections)
{
    PIXEL_T level;
                         ...
                         tabtabVisitedSaddles, tabtabSaddleValues) != 0)
    {
        if ((type == MAX && level < pNext->value) ||
            (type == MIN && level > pNext->value))
        {                       /* Type change */
            type = (level < pNext->value) ? MIN : MAX;
            if (pLastShape != NULL)
                connect(tabPointsInShape, iLastArea, tabConnections,
                        pSmallestShape);
            pSmallestShape = NULL;
        }
        if (iMinArea <= iNbPixels)
        {                       /* Store new shape? */
            pLastShape =
                new_shape(iNbPixels, level, (char) (type == MIN), pLastShape);
            if (pSmallestShape == NULL)
                pSmallestShape = pLastShape;
            update_smallest_shapes(tabPointsInShape + iLastArea,
                                   iArea - iLastArea);
            iLastArea = iArea;
        }
...
static void scan(float **tabtabPixelsOutput, int **tabtabVisitedPixels,
                 int **tabtabVisitedSaddles, float **tabtabSaddleValues,
        ...
        for (j = 0; j < iWidth; j++)
            if (tabtabVisitedPixels[i][j] <= iExplorationInit &&
                (is_local_min(tabtabPixelsOutput, j, i) ||
                 (is_local_max(tabtabPixelsOutput, j, i)
                  && (type = MAX) == MAX)))
            {
                find_terminal_branch(tabtabPixelsOutput,
                                     tabtabVisitedPixels,
                                     tabtabVisitedSaddles, tabtabSaddleValues,
                                     j, i, pNeighborhood, type,
                                     tabConnections);
                type = MIN;
            }
}
...
{
    PIXEL_T **tabtabPixelsOutput;       /* Array accessing pixels
                                         * of output image */
    ...
    tabConnections =
        (Connection *) malloc(2 * iAreaImage * sizeof(Connection));
    ...
    pTree->the_shapes = (Shape)
        realloc(pTree->the_shapes,
                (iNbSaddles + iAreaImage + 1) * sizeof(struct shape));
