...
#include <assert.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fsaddles(), flst_reconstruct(),
...
static float curve_perimeter(Flist pCurve, int iHeight, int iWidth)
{
    ...
    pointNext = *pPoint;
    while (pointNext.x != pCurve->values[0]
...
static float curve_area(Flist pCurve, int iHeight, int iWidth)
{
    ...
    for (i = 0; i + 1 < pCurve->size; i++, pPoint++)
        dArea +=
            (pPoint->x + pPoint[1].x) * (pPoint->y - (double) pPoint[1].y);
           ...
           || pointNext.y != pCurve->values[1])
    {
        ...
        else /* if(point.y == 0) */ if (pCurve->values[1] == 0
                                        && pCurve->values[0] < point.x)
            pointNext.x = pCurve->values[0];
        else
            pointNext.x = 0;
    }
...
static void decrease_tv(float *tabScales, float fQuantizationLevel,
                        int iQuantizationCurve, Fimage pImage, Shapes pTree,
                        float **tabtabSaddleValues)
{
    ...
    pListBoundaries =
        mw_change_flists(NULL, pTree->nb_shapes, pTree->nb_shapes);
    ...
    do
    {
        ...
        for (i = pTree->nb_shapes - 1; i > 0; i--, pShape--)
        {
            ...
            while (tabScales[i] < tabScales[0])
            {
                ...
                /* FIXME: cast */
                pBoundary =
                    flstb_boundary(&iQuantizationCurve, pImage, pTree, pShape,
                                   pListBoundaries->list[i], NULL,
                                   (char *) tabtabSaddleValues);
                fPerimeter =
                    curve_perimeter(pBoundary, pTree->nrow, pTree->ncol);
                fArea = curve_area(pBoundary, pTree->nrow, pTree->ncol);
                assert((fArea > 0 && fPerimeter > 0)
                       || (fArea == 0 && fPerimeter >= 0));
                ...
                if (pShape->removed)
                {
                    ...
                    pShape->value +=
                        pShape->inferior_type ? fDeltaLevel : -fDeltaLevel;
            }
            if (pShape->removed)
                bDecrease = (char) 1;
        }
