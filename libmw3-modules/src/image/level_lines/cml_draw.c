...
static void draw_cmlines(Cmorpho_line mline, Ccimage image, int NL, int NC,
                         char *border)
{
 ...
 * of the cmorpho-line bitmap mlimage
 */
...
static void setimage(char *border, Ccimage bimage, Ccimage mlimage)
{
    ...
    for (xml = x0, xb = 0; xml < x1; xml += 2, xb++)
        for (yml = y0, yb = 0; yml < y1; yml += 2, yb++)
        {
            ...
            if (xml + 1 < x1)
                mw_plot_ccimage(mlimage, xml + 1, yml, r, g, b);
            if (yml + 1 < y1)
                mw_plot_ccimage(mlimage, xml, yml + 1, r, g, b);
            if ((xml + 1 < x1) && (yml + 1 < y1))
                mw_plot_ccimage(mlimage, xml + 1, yml + 1, r, g, b);
        }
}
