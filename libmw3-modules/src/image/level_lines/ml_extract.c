...
#include <assert.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for ml_draw() */

#define INSIDE(A)   ((minvalue<=(A))&&((A)<=maxvalue))
#define OUTSIDE(A)  (!INSIDE(A))

static void
produce_HV(float **im, unsigned int NL, unsigned int NC, float minvalue,
           float maxvalue, unsigned char **H, unsigned char **V)
{
    unsigned int l, c;
...
count_X(unsigned char **H, unsigned char **V, unsigned int NL,
        unsigned int NC)
{
...
static Morpho_line
produce_lline(Morpho_line old_lline, float minvalue, float maxvalue,
              Point_curve current_point, Point_type current_type,
              unsigned char open)
{
...
static void
follow_open_line(int NL, int NC, unsigned char **H, unsigned char **V, int ll,
                 int cc, int sum, Point_curve * p, Point_type * t)
{
...
static void
get_open_lines(float **im, int NL, int NC, float minvalue, float maxvalue,
               unsigned char **H, unsigned char **V, Morpho_line * lline,
               Point_curve * p, Point_type * t)
{
    ...
    for (c = 0; c < NC - 1; c++)
        if ((V[0][c] == 1) && OUTSIDE(im[0][c]))
        {                       /* such that levelset at left */
            old_lline = *lline;
            *lline =
                produce_lline(old_lline, minvalue, maxvalue, current_point,
                              current_type, 1);
            if (old_lline != NULL)
                old_lline->previous = *lline;
            current_point->x = c + 1;
            current_point->y = 0;
            ...
            assert(sum != 0);
            follow_open_line(NL, NC, H, V, 0, c, sum, &current_point,
                             &current_type);
        }

    /* open lines starting in the last row */
    for (c = 0; c < NC - 1; c++)
        if ((V[NL - 1][c] == 1) && INSIDE(im[NL - 1][c]))
        {                       /* ...levelset at left */
            old_lline = *lline;
            *lline =
                produce_lline(old_lline, minvalue, maxvalue, current_point,
            ...
            assert(sum != 0);
            follow_open_line(NL, NC, H, V, (int) NL - 2, c, sum,
                             &current_point, &current_type);
        }

    /* open lines starting in the first column */
    for (l = 0; l < NL - 1; l++)
        if ((H[l][0] == 1) && INSIDE(im[l][0]))
        {                       /* ...levelset at left       */
            old_lline = *lline;
            *lline =
                produce_lline(old_lline, minvalue, maxvalue, current_point,
            ...
            assert(sum != 0);
            follow_open_line(NL, NC, H, V, l, 0, sum, &current_point,
                             &current_type);
        }

    /* open lines starting in the last column */
    for (l = 0; l < NL - 1; l++)
        if ((H[l][NC - 1] == 1) && OUTSIDE(im[l][NC - 1]))
        {                       /* ...levelset at left  */
            old_lline = *lline;
            *lline =
                produce_lline(old_lline, minvalue, maxvalue, current_point,
            ...
            assert(sum != 0);
            follow_open_line(NL, NC, H, V, l, (int) NC - 2, sum,
                             &current_point, &current_type);
        }
    *p = current_point;
    *t = current_type;
...
static void
follow_closed_line(unsigned char **H, unsigned char **V, int ll, int cc,
                   int sum, Point_curve * p, Point_type * t)
{
    Point_curve current_point = *p;
    Point_type current_type = *t;
...
static void
get_closed_lines(float **im, int NL, int NC, float minvalue, float maxvalue,
                 unsigned char **H, unsigned char **V, Morpho_line * lline,
                 Point_curve * p, Point_type * t)
{
    Morpho_line old_lline;
    ...
    for (l = 0; l < NL - 1; l++)
        for (c = 0; c < NC - 1; c++)
            ...
            {                   /* start a closed line */
                ...
                old_lline = *lline;
                *lline =
                    produce_lline(old_lline, minvalue, maxvalue,
                                  current_point, current_type, 0);
                if (old_lline != NULL)
                    old_lline->previous = *lline;
                current_point->x = c + 1;
                current_point->y = l + 1;
                ...
                {               /* such that the levelset is at left */
                    ...
                    assert(sum != 0);
                    follow_closed_line(H, V, l + 1, c, sum, &current_point,
                                       &current_type);
                }
                else
                {
                    ...
                    assert(sum != 0);
                    follow_closed_line(H, V, l, c + 1, sum, &current_point,
                                       &current_type);
                }
            }
...
void
ml_extract(float *level, Fsignal levels, int *opt, Cimage c_out, char *m_flag,
           Fimage image_org, Mimage m_image)
{
    ...
    if (levels)
    {
        ...
        if (((ml_opt == 3) || (ml_opt == 4)) && (levels->size < 2))
            mwerror(USAGE, 1,
                    "At least two values needed for ml_opt 3 and 4");
    }
    ...
    do
    {
        switch (ml_opt)
        {
            /*    case 0 : {minvalue=levels->values[i];
             * maxvalue=MORPHO_INFTY;  break;} */
        case 0:
            {
                minvalue = levels->values[i];
                maxvalue = MORPHO_INFTY;
                break;
            }
        case 1:
            {
                minvalue = -MORPHO_INFTY;
        ...
        default:
            mwerror(FATAL, 1, "Bad option number");
        }
        ...
        if (nb_points != 0)
        {
            if (m_flag)
            {
                 ...
                 * allows no free on points structure */
                current_point =
                    (Point_curve) malloc(nb_points *
                                         sizeof(struct point_curve));
                current_type =
                    (Point_type) malloc(nb_points *
                                        sizeof(struct point_type));
                ...
                current_point[nb_points - 1].previous =
                    current_point + nb_points - 2;
                current_point[nb_points - 1].next = NULL;
                current_type[0].previous = NULL;
                current_type[0].next = current_type + 1;
                current_type[nb_points - 1].previous =
                    current_type + nb_points - 2;
                current_type[nb_points - 1].next = NULL;
            }
            else
            {
                /* dont optimize...
                 * but possibility to free(points)                 */
                next_point = NULL;
                next_type = NULL;
                while (nb_points-- > 0)
                {
        ...
        else
        {
            ...
            case 0:
                {
                    mwerror(WARNING, 1,
                            "Value %.5f is smaller or bigger "
                            "than values of image_in.\n", levels->values[i]);
                    break;
                }
            case 1:
                {
            ...
            case 2:
                {
                    mwerror(WARNING, 1,
                            "Value %.5f doesn't appear image_in.\n",
                            levels->values[i]);
                    break;
                }
            case 3:
                {
                    mwerror(WARNING, 1,
                            "No pixels with values "
                            "between %.5f and %.5f in image_in.\n",
    ...
    if (c_out)
        *c_out =
            *ml_draw(m_image, (Cimage) NULL, (char *) NULL, (Cmovie) NULL);
