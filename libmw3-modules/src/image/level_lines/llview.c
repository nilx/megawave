...
static void my_llmap(Fimage in, Ccimage out, float ofs, float step)
{
    ...
    if (b2.button < 2)
        faded_fimage_to_ccimage(interpolate, out,
                                (b2.button == 0 ? 1. : 0.75));
    else
        mw_clear_ccimage(out, 255, 255, 255);
...
static int redisplay(Wp_toggle wt, int n)
{
    ...
    WLoadBitMapColorImage(win2, image->red, image->green, image->blue, nx,
                          ny);
...
static int win2_notify(Wframe * window, void *param)
{
    ...
    float nc, nd;
    int c;                      /* Key code must be int
                                 * and not char to handle non-printable keys */
...
static void init_buttons(void)
{
    ...
    b2.x = 10;
    b2.y = 5 + linestep * line++;
    b2.proc = redisplay;
    Wp_SetButton(WP_TOGGLE, wp, (void *) (&b2));

    /* button 3 */
    b3.text = "Interpolation ";
    b3.nbuttons = 6;
    b3.color = WP_RED;
            ...
            Ccimage * out, char *n)
{
    ...
    {                           /* interactive mode */
        ...
        if (!
            (win1 =
             (Wframe *) mw_get_window(NULL, 460, 115, 230, 5,
                                      "llview (control)")))
            mwerror(INTERNAL, 1, "NULL window returned by mw_get_window\n");
        if (!
            (win2 =
             (Wframe *) mw_get_window(NULL, nx, ny, 50, 145,
                                      "llview (display)")))
            mwerror(INTERNAL, 1, "NULL window returned by mw_get_window\n");
