...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for finvspline() */
           ...
           float X3, float Y3, float *x4, float *y4)
{
    ...
    if (y4)
    {
        xx = ((*x4 - X1) * (Y3 - Y1) - (*y4 - Y1) * (X3 - X1))
            / ((X2 - X1) * (Y3 - Y1) - (Y2 - Y1) * (X3 - X1));
        yy = ((*x4 - X1) * (Y2 - Y1) - (*y4 - Y1) * (X2 - X1))
  ...
  /********** MAIN LOOP **********/
    ...
    for (x = 0; x < *sx; x++)
        for (y = 0; y < *sy; y++)
        {
            ...
            if (i)
            {
                xx = 0.5 + (((float) x - X1) * y13 -
                            ((float) y - Y1) * x13) / (x12 * y13 - y12 * x13);
                yy = 0.5 - (((float) x - X1) * y12 -
                            ((float) y - Y1) * x12) / (x12 * y13 - y12 * x13);
                d = 1.0 - (a / (a + 1.0)) * xx / (float) nx -
                    (b / (b + 1.0)) * yy / (float) ny;
                xp = xx / ((a + 1.0) * d);
                yp = yy / ((b + 1.0) * d);
            }
                        ...
                        for (dy = n1; dy <= n2; dy++)
                            for (dx = n1; dx <= n2; dx++)
                                res += cy[n2 - dy] * cx[n2 - dx]
                                    * ref->gray[adr + nx * dy + dx];
                    }
                    else
                        for (dy = n1; dy <= n2; dy++)
                            for (dx = n1; dx <= n2; dx++)
                                res += cy[n2 - dy] * cx[n2 - dx]
                                    * v(ref, xi + dx, yi + dy, *bg);
                }
            }
            out->gray[y * (*sx) + x] = res;
        }
