...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for finvspline(), fdirspline() */
...
static double binom(int k, int n)
{
    ...
    for (i = 0, value = 1.; i < k; i++)
        value *= (double) (n - i) / (double) (k - i);
    return value;
}
...
static void ppaddint(Ppfunction f, Ppfunction g, int i, int sign, double w)
{
    ...
    for (j = 0; j < f->k; j++)
    {
        ...
        for (k = 0; k <= j + 1; k++)
            g->a[n][k] +=
                value * binom(k, j + 1) * ipow(w * (double) sign, j + 1 - k);
...
Fimage funzoom(Fimage in, Fimage out, float *z, int *o, float *tx, float *ty)
{
    ...
    if (*z < 1.)
        mwerror(FATAL, 1,
                "illegal unzoom factor (%f) - should be at least 1.\n", *z);
  ...
  /********** CONVOLUTION AND SAMPLING **********/
    ...
    if (*o == 0)
        ...
        out0 = mw_change_fimage(NULL, nsy, nsx);
