...
static void linear(unsigned char *dst, unsigned char *a, unsigned char *b,
                   int n)
{
    for (; n--; dst++, a++, b++)
...
Cmovie cmparitysep(Cmovie u, char *e, char *l)
{
    ...
    while (src)
    {
        ...
        for (y = 0; y < ny; y++)
        {
            ...
            memcpy(dst->gray + adr + nx * field, src->gray + adr + nx * field,
                   nx);
            /* duplication or interpolation for the other field */
            if (l && (y != 0 || field != 1) && (y != ny - 1 || field != 0))
                linear(dst->gray + adr + nx * (1 - field),
                       src->gray + adr + nx * (2 - 3 * field),
                       src->gray + adr + nx * field, nx);
            else
                memcpy(dst->gray + adr + nx * (1 - field),
                       src->gray + adr + nx * field, nx);
        }
        /* link fields */
        *next = dst;
