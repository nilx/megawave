...
#include "mw3.h"
#include "mw3-modules.h"         /* for fop() */

               /* GLOBAL VARIABLES */

/****global variables: not very beautiful, but avoid functions with 50
 * arguments ;-)******/
...
/* derivative of function \psi(s^2)=\epsilon
 * s^2+(1-\epsilon)\lambda^2\sqrt{1+\frac{s^2}{\lamda^2}}
        ...
        for (j = 0; j < ny; j++)
            for (i = 0; i < nx; i++)
            {
                ...
                else
                {
                    ...
                    Ex[adr] =
                        -0.1464466095 * a_NO + 0.1464466095 * a_NE -
                        0.2071067810 * a_O + 0.2071067810 * a_E -
                        0.1464466095 * a_SO + 0.1464466095 * a_SE;
                    Ey[adr] =
                        -0.1464466095 * a_SE + 0.1464466095 * a_NE -
                        0.2071067810 * a_S + 0.2071067810 * a_N -
                        0.1464466095 * a_SO + 0.1464466095 * a_NO;
                    Et[adr] = at_post - a_or;
                }
...
static void schema_ws(float *threshold, float eps, float tau, float alpha,
                      float lambda, int niter)
{
    int i, j, l, p;
    long adr;
    ...
    /*
     * one pixel has six neighbours in 3D (North, South, Est, West
     * [Ouest in french], before [South in z] and after
     * [North in z]) and four in 2D (North, South, Est,
     * West)
     */
    long taille;
            ...
            for (j = 1; j < ny - 1; j++)
                for (i = 1; i < nx - 1; i++)
                {
                    ...
                    if ((l > 0) && (l < (nzo - 1)))
                        grad_2u =
                            (float)
                            pow((double) ((U[adr + 1] - U[adr - 1]) / 2),
                                2.0) +
                            (float)
                            pow((double) ((U[adr + nx] - U[adr - nx]) / 2),
                                2.0) +
                            (float)
                            pow((double)
                                ((U[adr + nx * ny] - U[adr - nx * ny]) / 2),
                                2.0);
                    if (l == 0)
                        grad_2u =
                            (float)
                            ...
                            pow((double) ((U[adr + nx] - U[adr - nx]) / 2),
                                2.0) +
                            (float) pow((double) (U[adr + nx * ny] - U[adr]),
                                        2.0);
                    if (l == (nzo - 1))
                        grad_2u =
                            (float)
                    ...
                    if ((l > 0) && (l < (nzo - 1)))
                        grad_2v =
                            (float)
                            pow((double) ((V[adr + 1] - V[adr - 1]) / 2),
                                2.0) +
                            (float)
                            pow((double) ((V[adr + nx] - V[adr - nx]) / 2),
                                2.0) +
                            (float)
                            pow((double)
                                ((V[adr + nx * ny] - V[adr - nx * ny]) / 2),
                                2.0);
                    if (l == 0)
                        grad_2v =
                            (float)
                            ...
                            pow((double) ((V[adr + nx] - V[adr - nx]) / 2),
                                2.0) +
                            (float) pow((double) (V[adr + nx * ny] - V[adr]),
                                        2.0);
                    if (l == (nzo - 1))
                        grad_2v =
                            (float)
                            ...
                            (float)
                            pow((double) ((V[adr + nx] - V[adr - nx]) / 2),
                                2.0) + (float) pow((double) (V[adr] -
                                                             V[adr -
                                                               nx * ny]),
                                                   2.0);

                    diffus[adr] =
                        psip((float) sqrt((double) (grad_2u + grad_2v)), eps,
                             lambda);
                    tampU[adr] = U[adr];
                    tampV[adr] = V[adr];
                }
        ...
        for (l = 0; l < nzo; l++)
        {
            for (j = 1; j < ny - 1; j++)
                for (i = 1; i < nx - 1; i++)
                {
                    ...
                    /*
                     * semi-implicit scheme (diffusion term
                     * discretized at order n)
                     * (u^{n+1}-u^{n})/delta=tau*sum_{x\in 6-neighbourhood}
                     * W(u^{n})(x)u^{n}(x)
                    ...
                    /*if(p) */
                    residue[l] +=
                        ((U[adr] - tampU[adr]) * (U[adr] - tampU[adr]) +
                         (V[adr] - tampV[adr]) * (V[adr] - tampV[adr]));
                }
            /*if(p){ */
             ...
             float *alpha, Fmovie norm, Fmovie movie, Fmovie wsU, Fmovie wsV)
{
    ...
    for (l = 0; l < nzo; l++)
    {
        ...
        if (norm)
        {
            fop(uV, nd, uU, NULL, NULL, NULL, NULL, NULL, NULL, &normflag,
