...
float prolate(int s, float d, int *m, Fimage ker)
{
    ...
    do
    {
        ...
        for (x = s; x--;)
            for (y = s; y--;)
                tmp->gray[((y - s / 2 + n) % n) * n + (x - s / 2 + n) % n] =
                    ker->gray[y * s + x];
        ...
        for (x = n; x--;)
            for (y = n; y--;)
            {
                xx = (x > n / 2 ? n - x : x);
                yy = (y > n / 2 ? n - y : y);
                if ((float) (xx * xx + yy * yy) > rad2)
                    re->gray[y * n + x] = im->gray[y * n + x] = 0.;
                else
                    norm +=
                        (double) re->gray[y * n +
                                          x] * (double) re->gray[y * n + x] +
                        (double) im->gray[y * n +
                                          x] * (double) im->gray[y * n + x];
            }
