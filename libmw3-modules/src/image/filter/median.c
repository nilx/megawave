...
#include <stdio.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for disc() */

/*--------------- Median of a cimage : one iteration  -------------*/

static int comp(const void *i, const void *j)
{
    return ((int) *(const unsigned char *) i -
            (int) *(const unsigned char *) j);
}
...
Cimage median(Cimage u, Cimage v, float *r, Curve s, int *n)
{
    ...
    if (*n <= 0)
        mwerror(FATAL, 1, "The -n option parameter must be positive.");

    if (*r <= 0)
        mwerror(FATAL, 1, "Radius parameter must be positive.");
    if (s)
        shape = s;
    else
        shape = disc(*r, NULL);
