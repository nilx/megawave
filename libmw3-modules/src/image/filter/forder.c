...
/* mwcommand
   ...
   usage = {
     'e':[e=5]->e[1,9]  "order of the pixel in a ascending list
                        which sorts the values of the pixels in the window",
...
{
    if (*(const float *) k > *(const float *) l)
        return (1);
    if (*(const float *) k < *(const float *) l)
        return (-1);
    return (0);
