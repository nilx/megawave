    ...
    for (x = 0; x < nx - 1; x++)
        for (y = 0; y < ny - 1; y++)
        {
            ...
            if (norm != 0.)
            {
                ...
                grad->gray[adr + nx + 1] +=
                    (float) ((p * (c - d) + q * f) / norm);
                grad->gray[adr + nx] +=
                    (float) ((p * (d - a) - q * e) / norm);
            }
        }
                 ...
                 double *p)
{
    ...
    if (*W != 0.)
        energy += mytvgrad(cur, dE, *eps, *p, *W);
    energy /= (double) (nx * ny);
    if (v)
        printf("init:  E = %f\n", energy);
    ...
    do
    {
        ...
        if (energy > old_energy)
        {
            ...
            else
            {
                ...
                {
                    if (cont)
                        printf("reduction to step=%f\n", step);
                    else
                        printf("Step reduction limit reached, stop.\n");
                }
            }
        }
        else
        {
            ...
            if (v)
            {
                ...
                printf("n=%3d: E = %f, dt = %f, |u-u0| = %f", i, energy, step,
                       norm1);
                if (prevok)
                    printf(", |du| = %f\n", norm2);
                else
