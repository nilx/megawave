...
#include "mw3.h"
#include "mw3-modules.h"         /* for flst(), flst_reconstruct() */

/* An elementary shape is a shape having a child of a different type.
 * To keep consistency between the tree and the associated image,
 * it is forbidden to remove such a shape. This function detects if
 * pShape is an elementary shape, and if the answer is positive, mark
...
static void keep_elementary_shape(Shape pShape)
{
    ...
    for (pChild = mw_get_first_child_shape(pShape); pChild != NULL;
         pChild = mw_get_next_sibling_shape(pChild))
...
/* Find the gradation containg pShape,
   remove it but keep only one representative shape of
   the gradation.
   ...
   - the areas of C and S do not differ by a factor greater
   than fFactorIncreaseArea;
   - C is not an elementary shape.
   It is not hard to see that this relation induces a partition
   of the family of shapes, each
   class of the partition is then called a gradation.
   The representative shape of the gradation is the shape of
   the gradation with smallest L^2/A,
...
static void remove_gradation(Shape pShape, float fFactorIncreaseArea)
{
    ...
    fOptimalCriterium =
        (float) (pChild->boundary ? pChild->boundary->size : 0);
    ...
    {
        fMeanGrayLevel +=
            (pCurrentShape->area - pChild->area) * pCurrentShape->value;
        fCriterium =
            (float) (pCurrentShape->boundary ? pCurrentShape->
                     boundary->size : 0);
...
/* This removes the gradations from an image.
   In the tree of shapes representing the input
   image, the field data of each shape is used
   to mark the shapes whose gradation was
   already found */
              ...
              Fimage pFloatImageOutput)
{
    ...
    if (mw_change_fimage
        (pFloatImageOutput, pFloatImageInput->nrow,
         pFloatImageInput->ncol) == NULL)
        mwerror(FATAL, 1,
                "fkill_grains --> Not enough memory "
                "to allocate the output image");
    if ((pTree = mw_new_shapes()) == NULL)
        mwerror(FATAL, 1,
                "fkill_grains --> Not enough memory "
                "to allocate the tree of shapes");
    /* Compute the Level Sets Transform of the input image,
     * and store the associated level lines */
    flst(NULL, pFloatImageInput, pTree);

    /* First find the shapes whose containing gradation
     * is reduced to the shape itself */
    for (i = pTree->nb_shapes - 1; i > 0; i--)
        keep_elementary_shape(&pTree->the_shapes[i]);

    /* Kill gradations.
     * We use the field data of a shape to track the shapes
     which were already dealt with */
    for (i = pTree->nb_shapes - 1; i > 0; i--)
