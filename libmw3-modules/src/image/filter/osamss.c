...
#include <stdio.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for amss(), fvalues() */
                        ...
                        /* Result                                           */
{
    ...
    if (levels->size >= 50)
        mwerror(WARNING, 1,
                "%d different grey levels : osamss may take some time !\n",
                levels->size);
    ...
    for (i = 0; i < levels->size; i++)
    {
        ...
        amss(isotrop, power, Step, MinGrad, &zero, firstScale, lastScale,
             binary, &evol, &null_image, &null_image, NULL, NULL, NULL, NULL);

        /* reconstruct output */
        for (adr = img_size; adr--;)
            if ((v = evol->gray[adr]) >= mid)
                output->gray[adr] = l;
    }
}
