...
static int nx, ny;
/* image size (global variable, never changes) */
...
static double energy_evol(double *px, double *py, float *ref, double *u,
                  ...
                  double *W, int *V)
{
    ...
    if (V)
        image = (unsigned char *) malloc(nx * ny * sizeof(unsigned char));

    /* initialize display */
    if (V)
        init_display(in->name);
