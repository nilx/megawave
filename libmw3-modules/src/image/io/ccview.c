...
static void ccview_notify_help(void)
{
    ...
    printf("\nMouse:\n");
    printf
        ("\tLeft button: toggle to print the RGB value "
...
static int ccview_notify(Wframe * ImageWindow, void *param)
...
#define ZFMAX 4                 /* Zoom Factor Max */
    ...
    if (event != W_DESTROY)
    {
        ...
        if (GLprint == 1)
        {
            ...
            sprintf(mess, " (%3d,%3d): (%3d,%3d,%3d) ", x1, y1, nred, ngreen,
                    nblue);
    ...
    case W_MS_RIGHT:
        ...
        else
        {
            if (zfactor > 2)
            {
                ...
                memcpy(image->green, green_save, imsize);
                memcpy(image->blue, blue_save, imsize);
                if (WLoadBitMapColorImage
                    (ImageWindow, image->red, image->green, image->blue,
                     image->ncol, image->nrow) != 0)
                    return (-1);
            ...
            int *no_refresh, char *window)
{
    ...
    if (*zoom != 1)
    {
        ...
        {
            inverse_zoom = 1. / (*zoom);
            cczoom(image, zimage, NULL, NULL, &inverse_zoom, order,
                   (char *) 1);
        }
