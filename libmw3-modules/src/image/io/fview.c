...
#include <string.h>
#include "mw3.h"
#include "mw3-x11.h"             /* for fthre() */
#include "mw3-modules.h"         /* for fthre() */
...
static void fview_notify_help(void)
{
    ...
    printf("\nMouse:\n");
    printf
        ("\tLeft button: toggle to print the gray level "
         "at the current coordinates (x,y).\n");
...
#define ZFMAX 4                 /* Zoom Factor Max */
    ...
    case W_MS_MIDDLE:
        ...
        if (WLoadBitMapImage
            (ImageWindow, image->gray, image->ncol, image->nrow) != 0)
            return (-1);
        WRestoreImageWindow(ImageWindow, 0, 0, image->ncol, image->nrow);
        WFlushWindow(ImageWindow);
        zfactor = 2;
        ...
        else
        {
            if (zfactor > 2)
            {
                ...
                if (WLoadBitMapImage
                    (ImageWindow, image->gray, image->ncol, image->nrow) != 0)
                    return (-1);
                WRestoreImageWindow(ImageWindow, 0, 0, image->ncol,
                                    image->nrow);
                WFlushWindow(ImageWindow);
                zfactor = 2;
                oldevent = event;
    ...
    case W_KEYPRESS:
        ...
        default:
            if (c >> 8 == 0)
                mwerror(WARNING, 1,
                        "Unrecognized Key '%c'. Type H for Help.\n", c);
        }
        oldevent = event;
        break;
    }
...
void fview(Fimage input, int *x0, int *y0, float *zoom, int *order,
           int *no_refresh, Wframe * window, char *linear, float *m, float *M,
           float *d)
{
    Wframe *ImageWindow;
    Fimage fimage, tmp;
    ...
    if (*zoom != 1.0)
    {
        ...
        {
            inverse_zoom = 1. / (*zoom);
            fzoom(input, fimage, NULL, NULL, &inverse_zoom, order, NULL,
                  (char *) 1);
        }
    ...
    if (gray_save == NULL)
        mwerror(FATAL, 1, "Not enough memory\n");
    fgray_save =
        (float *) malloc(cimage->ncol * cimage->nrow * sizeof(float));
    if (fgray_save == NULL)
        mwerror(FATAL, 1, "Not enough memory\n");
    ...
    if (fimage == input)
        param->has_to_delete_image = 0;
    else
        param->has_to_delete_image = 1;
