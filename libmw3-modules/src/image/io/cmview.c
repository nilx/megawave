...
/* mwcommand
 ...
 usage = {
  ...
  'p'->pause            "pause the movie on start",
   cmovie->input        "Input movie (should be a cmovie)",
...
#include <string.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for cmzoom() */
...
static void cmview_notify_help(void)
{
    printf("\n\t\tHelp on line\n");

    printf("\nMouse:\n");
    printf
        ("\tLeft button: Toggle to print information on the current frame.\n");

    printf("\nKeyboard:\n");
    printf
        ("\tF: Play forward or show next frame in the step by step mode.\n");
    printf
        ("\tB: Play backward or show previous frame "
...
static int cmview_notify(Wframe * ImageWindow, void *param)
                      /* Users's parameters: don't forget the cast ! */
{
    ...
    if (ret != -1)
    {
        event = WUserEvent(ImageWindow);
        /* User's event on ImageWindow */
        if (event < 0)
            ret = 1;
        else
            ret = event;
    ...
    if (ret != -1)
    {
        ...
        if (InfoPrint == 1)
        {
            sprintf(mess, "#%d  %s", CurrentFrameNumber, image->cmt);
            ...
            int *loop, int *pause, char *window)
{
    ...
    if (*zoom != 1.0)
    {
        ...
        {
            inverse_zoom = 1. / (*zoom);
            cmzoom(input, movie, NULL, NULL, &inverse_zoom, order,
                   (char *) 1);
        }
