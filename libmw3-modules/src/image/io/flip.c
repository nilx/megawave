...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fcrop() */
...
static void max_contrast(void)
{
    ...
    fny = (float) ny;
    if (num == 1)
        fcrop(u1, u, &fnx, &fny, NULL, &bg, &ordervalue[order], &a, X1, Y1,
              X2, Y2);
    else
        fcrop(u2, u, &fnx, &fny, NULL, &bg, &ordervalue[order], &a, X1, Y1,
              X2, Y2);
    min = max = u->gray[0];
    for (adr = nx * ny; adr--;)
    {
...
static void recompute(void)
{
    ...
    /* zoom image 1 */
    fcrop(u1, u, &fnx, &fny, NULL, &bg, &ordervalue[order], &a, X1, Y1, X2,
          Y2);
    for (adr = nx * ny; adr--;)
    {
        ...
        if (c > 255.)
            ...
            c = 0.;
        image1->red[adr] = image1->green[adr] = image1->blue[adr] =
            (unsigned char) c;
    }

    /* zoom image 2 */
    fcrop(u2, u, &fnx, &fny, NULL, &bg, &ordervalue[order], &a, X1, Y1, X2,
          Y2);
    for (adr = nx * ny; adr--;)
    {
        c = 256. * (u->gray[adr] - min) / (max - min);
        if (c > 255.)
            ...
            c = 0.;
        image2->red[adr] = image2->green[adr] = image2->blue[adr] =
            (unsigned char) c;
    }
}
...
static void redisplay(void)
{
    ...
    if (num == 1)
        WLoadBitMapColorImage(win, image1->red, image1->green, image1->blue,
                              nx, ny);
    else
        WLoadBitMapColorImage(win, image2->red, image2->green, image2->blue,
                              nx, ny);
