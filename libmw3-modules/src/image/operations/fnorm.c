...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* fop(), fderiv() */
            ...
            char *n, float *t)
{
    ...
    if (v)
    {
        ...
        if (ref)
        {
            ...
            /* gradn = || D(in-ref) || */
            fop(ref, diff, in,
                NULL, NULL, (char *) 1, NULL, NULL, NULL, NULL, NULL, NULL,
                NULL, NULL, NULL);
            fderiv(diff, NULL, NULL, NULL, NULL, NULL, NULL, gradn, NULL,
                   &zero, &four);
        }
        else
            /* gradn = || D(in) || */
            fderiv(in, NULL, NULL, NULL, NULL, NULL, NULL, gradn, NULL, &zero,
                   &four);
    }
    ...
    for (x = *b; x < in->ncol - (*b); x++)
        for (y = *b; y < in->nrow - (*b); y++)
        {
            if (p || s)
            {
                ...
                if (ref)
                    val -= (double) ref->gray[y * in->ncol + x];
                if (val < 0)
                    val = -val;
                if (p)
    ...
    if (n)
        sum /= (double) fnorm(ref, NULL, p, s, v, b, NULL, NULL);
    if (t)
        if (sum <= *t)
            sum = 0.0;
    if (p)
