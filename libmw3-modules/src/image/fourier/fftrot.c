...
static void yr_shear(double dAmountOfShear, Fimage pFloatImageInput,
                     Fimage pFloatImageOutput, int iAxis, float fDelta,
                     char *PleaseDoNotCenter)
{
    int i, j, iSize, iHalfSize, iOtherSize, iHalfOtherSize;
    Fsignal pRealSignal, pImaginarySignal, pRealSignalTransformed,
        pImaginarySignalTransformed;
    double dTranslation;
    ...
    {
        if (iAxis == HORIZONTAL)
            memcpy(pRealSignal->values, pFloatImageInput->gray + i * iSize,
                   iSize * sizeof(float));
        else
            for (j = 0; j < iSize; j++)
                pRealSignal->values[j] =
                    pFloatImageOutput->gray[j * iOtherSize + i];
        ...
        if (NULL != PleaseDoNotCenter)
            ...
            dTranslation =
                -((i - iHalfOtherSize) * dAmountOfShear + fDelta) * 2. * M_PI;
        else
            dTranslation =
                -((i - iHalfOtherSize + .5) * dAmountOfShear +
                  fDelta) * 2. * M_PI;
        ...
        for (j = 1; j < iHalfSize + odd; j++)
        {
            ...
            pRealSignalTransformed->values[iSize - j] =
                pRealSignalTransformed->values[j];
            pImaginarySignalTransformed->values[iSize - j] =
                -pImaginarySignalTransformed->values[j];
        }

        if (odd == 0)
        {
            pRealSignalTransformed->values[iHalfSize] =
                cos(dTranslation * .5) *
                pRealSignalTransformed->values[iHalfSize] -
                sin(dTranslation * .5) *
                pImaginarySignalTransformed->values[iHalfSize];
        ...
        if (iAxis == HORIZONTAL)
            memcpy(pFloatImageOutput->gray + i * iSize, pRealSignal->values,
                   iSize * sizeof(float));
        else
            for (j = 0; j < iSize; j++)
                pFloatImageOutput->gray[j * iOtherSize + i] =
