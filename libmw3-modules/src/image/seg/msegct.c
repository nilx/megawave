...
/* mwcommand
 ...
 function = {"Region-Growing method using the energy model
             of Mumford and Shah with piecewise constant approximation
             function, any number of channels"};
 usage = {
   'w':w_of_channels->weight
       "weights of channels, MW2-fsignal formated,
       (default weights 1/number_of_channels)",
...
/* #define NC_REG
 * Nombre de canaux pour une region */
...
typedef struct energie {
    ...
    /* Longeur des bords                                  */
    float e;
    /* g etant l'image, l'energie elastique = S( (u-g)^2 ) */
} ENERGIE;
...
static float SomGris(Fmovie orig_data, short unsigned int i,
                     short unsigned int j, float *channels),
eval_lambda(BORDPTR bord);
...
RegCanalInit(Fmovie orig_data, short unsigned int i, short unsigned int j,
             float *canal), BorCanalInit(float *canal);
static void Repointer(SOMMETPTR som, BORDPTR newbord, BORDPTR oldbord),
ElimBordeReg(REGIONPTR reg, BORDPTR bord), RegMerge(REGIONPTR reg,
                                                    REGIONPTR regvois,
                                                    BORDPTR bordcom);
...
static void RegCanalInit(Fmovie orig_data, short unsigned int i,
                         short unsigned int j, float *canal)
...
static float SomGris(Fmovie orig_data, short unsigned int i,
                     short unsigned int j, float *channels)
                               /* Calcule la somme de gris dans le rectangle */
                               ...
                               /* Determine l'energie elastique initale !!   */
{
    ...
    for (h = 1, imptr = orig_data->first; h < image.NC_REG;
         h++, imptr = imptr->next)
    {
        assert(imptr != NULL);
        somme = 0.0;
        for (k = 0; k < image.PAS; k++)
            ...
            {
                somme +=
                    (float) imptr->gray[(j * image.PAS + l) * imptr->ncol +
                                        i * image.PAS + k];
            }
        channels[h] = somme;
        sommeSpas2 = somme / (float) image.PAS * image.PAS;
        image.chmean[h] += sommeSpas2;  /* sum of mean on grid     */
        image.chenergy[h] += sommeSpas2 * sommeSpas2;
        /*  id. (total mean later) */
    }
    return somme;
}

static void BorCanalInit(float *canal)
/* Initialise les canaux associes aux bords */
/* de la region [i,j]                       */
...
static void InitPixel(LI_PIXELSPTR pixelptr, char dir)
/* Met un segment de longeur 'PAS' et de */
...
#define h_bord(A)        (image.heap[(u_long)(A)].bord)
/* type BORDPTR */
#define h_lambda(A)      (image.heap[(u_long)(A)].lambda)
/* type float   */
...
static void init_heap(BORDPTR bord, long unsigned int n)
{
    u_long m;
    float lam;
...
static void update_heap(BORDPTR bord)
{
    u_long n, m0, m1;
    float lam;
    ...
    if (n > 0 && (lam < h_lambda(m0 = h_up(n))))
        do
        {
            /* bord is too light... it will bubble 'up' to the root */
    ...
    {
        /* bord is too heavy... it will sink to the ground */
        while ((m0 = h_left(n)) < image.nbbords)
        {
            if (((m1 = h_right(n)) < image.nbbords)
                && (h_lambda(m1) < h_lambda(m0)))
                m0 = m1;
            if (h_lambda(m0) > lam)
                break;
            bord_heap(h_bord(m0)) = n;
...
static void subtract_heap(BORDPTR bord)
{
    BORDPTR lbord;
    u_long n;
...
static void Estimation(void)    /* Estimations occupation memoire */
{
    unsigned long simage, stotal, total,
        ...
        sheap, sdata, nbregions, nbbords, nbsommets;
    ...
    stotal =
        image.NC_REG * image.gx * image.PAS * image.gy * image.PAS *
        sizeof(float);
    total = simage + stotal;
...
static void Initialisation(Fmovie orig_data)
/* On cree sept blocs pour reserver de la memoire    */
...
/* des places memoire on peut facilement initialiser   */
{
    ...
    unsigned long l, n, dbreg, dbbor, dbsom, dblibor, dblipix,
        /* Dimensions des blocs */
     dbheap, lbreg, lbbor, lbsom;
    /* Largeur des blocs    */
    ...
    block_data = (float *) calloc((long) dbreg, image.NC_REG * sizeof(float));
    ...
    image.energie.e = 0.0;
    image.energie.l = (long) image.PAS * image.nbbords;
    /*                                     */
    ...
    for (j = 0; j <= image.gy; j++)
        for (i = 0; i <= image.gx; i++)
        {
            ...
            {
                block_som[l].x = i * image.PAS;
                /* Frontiere dans la region droite */
                block_som[l].y = j * image.PAS;
...
/* memoire de reg                          */
{
    ...
    for (i = 0; i < image.NC_REG; i++)
        reg->canal[i] += regvois->canal[i];
...
/* evaluate for which value of lambda this border will */
...
static void make_curves(Curves curves)
{
    ...
    for (regptr = image.tete; regptr != NULL; regptr = regptr->Rsuiv)
        for (libptr = regptr->Lbords; libptr != NULL; libptr = libptr->suiv)
            for (cnxeptr = libptr->bord->cnxe; cnxeptr != NULL;
...
static void Dess_u(Cimage boundary, Fmovie u)
...
/*on dessine en utilisant la fonction correspondant */
{
    ...
    for (i = 0; i < ncols; i++)
    {
        ...
        for (im = u->first, channel = &regact->canal[1]; im != NULL;
             im = im->next, channel++)
              ...
              Fmovie orig_data)
{
    ...
    /*
     * if ((orig_data->ncol % image.PAS != 0)||(orig_data->nrow %
     * image.PAS != 0))
     */
    ...
    if (weight)
    {
        ...
        for (l = 1; l < image.NC_REG; l++)
            image.weight[l] = weight->values[l - 1];
    }
    else
        for (l = 1; l < image.NC_REG; l++)
            image.weight[l] = 1.0 / (image.NC_REG - 1);
    for (l = 0; l < image.NC_REG; l++)
        image.chmean[l] = image.chenergy[l] = 0.0;
    ...
    printf("\nElastic energie = %.4g , boundary length= %ld.",
           image.energie.e, image.energie.l);
    printf("\nNumber of regions: %lu. \n", image.nbregions);
    if (lambda == NULL)
        while (*nb_of_regions < (int) image.nbregions)
            segment();
    else
        while (*lambda >= h_lambda(h_root))
            segment();          /* h_lambda(h_root) contains   */
