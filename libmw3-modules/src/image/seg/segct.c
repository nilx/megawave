...
typedef struct energie {
    ...
    float e;                    /* g etant l'image, l'energie
                                 * elastique = S( (u-g)^2 ) */
} ENERGIE;
...
static float SomGris(Fimage image_org, short unsigned int i,
                     short unsigned int j), eval_lambda(BORDPTR bord);
...
RegCanalInit(Fimage image_org, short unsigned int i, short unsigned int j,
...
static void RegCanalInit(Fimage image_org, short unsigned int i,
...
static float SomGris(Fimage image_org, short unsigned int i, short
                     unsigned int j)
...
/* Determine l'energie elastique initale !!   */
{
    ...
    for (k = 0; k < image.PAS; k++)
        for (l = 0; l < image.PAS; l++)
            somme +=
                (float) image_org->gray[(j * image.PAS + l) *
                                        image_org->ncol + i * image.PAS + k];
    return (somme);
}
...
static void Estimation(void)    /* Estimations occupation memoire */
{
    unsigned long simage, stotal, total,
        ...
        sheap, sdata, nbregions, nbbords, nbsommets, systeme;
    ...
    stotal =
        systeme + image.gx * image.PAS * image.gy * image.PAS * sizeof(float);
    total = simage + stotal;
    ...
    for (j = 0; j <= image.gy; j++)
        for (i = 0; i <= image.gx; i++)
        {
                ...
                && ((i != image.gx) || (j != 0 && j != image.gy)))
            {
                ...
                /*           respectivement la region dessous */
...
/* evaluate for which vakue of lambda this border will */
...
/* a la region actuelle.   */
{
    ...
    for (i = 0; i < ncols; i++)
    {
        ...
        for (j = 1; j < nrows; j++)
        {
            ...
            u->gray[ncols * j + i] =
                (float) regact->canal[1] / regact->canal[0];
        }
    }
}
             ...
             Fimage image_org)
{
    ...
    image.NC_REG = 2;
    /*
     * if((image_org->ncol % image.PAS != 0) || (image_org->nrow %
     * image.PAS != 0))
     */
    ...
    *f_nb_of_regions = image.nbregions;
    ...
    if ((boundary =
         mw_change_cimage(NULL, image_org->nrow, image_org->ncol)) == NULL)
    {
        Image_free(&image);
        mwerror(FATAL, 1, "Not enough memory.");
    }
    ...
    {
        if ((u =
             mw_change_fimage(u, image_org->nrow, image_org->ncol)) == NULL)
        {
            Image_free(&image);
            mwerror(FATAL, 1, "Not enough memory !\n");
        }
