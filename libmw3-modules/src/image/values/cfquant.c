...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fquant() */
...
void cfquant(Cfimage A, Cfimage Q, int Mr, int Mg, int Mb, char *left)
{
    ...
    for (i = 0, ptrA = A->red, ptr = channel->gray; i < A->nrow * A->ncol;
         i++, ptrA++, ptr++)
        *ptr = *ptrA;
    ...
    for (i = 0, ptrA = A->green, ptr = channel->gray; i < A->nrow * A->ncol;
         i++, ptrA++, ptr++)
        *ptr = *ptrA;
    ...
    for (i = 0, ptrA = A->blue, ptr = channel->gray; i < A->nrow * A->ncol;
         i++, ptrA++, ptr++)
        *ptr = *ptrA;
