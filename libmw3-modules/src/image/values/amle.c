...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fmse() */
...
static void neighbor(register int x, register int y, register int xmax,
                     register int ymax, register float *p, float **left,
                     float **right, float **up, float **down,
                     float **right_down, float **left_up, float **right_up,
                     float **left_down)
{
    if (x > 0)
    {
...
/* Compute the new gray value of the current point (x,y) by solving an
 * implicit Euler scheme */
/* of the PDE du/dt = D2u (Du/|Du|, Du/|Du|)
 * */

static void compute_point(register int x, register int y, register int xmax,
                          register int ymax, register float *p,
                          float prev_value, float ht, float omega)
{
    float *left, *right, *up, *down, *right_down, *left_up, *right_up,
        *left_down;
    float ux, uy, uxx, uyy, uxy;
    double A, B, NG2;

    neighbor(x, y, xmax, ymax, p, &left, &right, &up, &down, &right_down,
    ...
    if (fabs(NG2) > EPS)
    {
        A = (2 + 4 * ht) * NG2 * (*p) - 2 * prev_value * NG2 -
            2 * ht * uxx * ux * ux - 2 * ht * uyy * uy * uy -
            ht * ux * uy * uxy;
        B = 2 * NG2 * (1 + 2 * ht) + EPS;
    }
    else
  ...
  /*--- Copy the initial condition Cimage into the output Fimage ---*/
    ...
    if (Init)
    {
        if ((Init->ncol == Input->ncol) && (Init->nrow == Input->nrow))
            for (x = 0, ptrIn = Init->gray, ptrOut = Output->gray;
                 x < Input->ncol * Input->nrow; x++)
                *(ptrOut++) = *(ptrIn++);
        else
            mwerror(WARNING, 0,
                    "Bad size for initial condition image "
                    "image_init!\n -> -i option ignored.\n");
    }

  /*--- Copy the input Cimage into the output Fimage ---*/

    for (x = 0, ptrIn = Input->gray, ptrOut = Output->gray;
         x < Input->ncol * Input->nrow; x++, ptrOut++, ptrIn++)
        if (*ptrIn != 0)
            *ptrOut = *ptrIn;
    ...
    /*--- First pass : from left to right and up to down ---*/
        ...
        for (y = 0; y < Input->nrow; y++)
            for (x = 0; x < Input->ncol; x++, ptrOut++, ptrPrev++)
                if (*(ptrIn++) == 0)
                    compute_point(x, y, xmax, ymax, ptrOut, *ptrPrev, *ht,
                                  *omega);
