...
/* mwcommand
 ...
 version = {"1.1"};
 author = {"Frederic Cao"};
 function = {"Compute the solution of the AMLE Model using the inf sup
             scheme in the 3d case"};
 usage = {
   'n':[num=100]->num     "Number of iterations",
   'i':movie_init->init   "Initial condition",
   in->in                 "Input quantized movie obtained from the
                          amle3d_init procedure",
    ...
    while (iminput != NULL)
    {
        ...
        for (y = 0; y < nl; y++)
            for (x = 0; x < nc; x++, O++, I++, ptrmov++)
            {
                if (*I == 0)
                {
                    neighbor_4(x, y, nc - 1, nl - 1, O, &left, &right, &up,
                               &down);
                    if (imoutput->previous != NULL)
                        previ = mw_getdot_fimage(imoutput->previous, x, y);
                    else
                        previ = mw_getdot_fimage(imoutput->next, x, y);
                    if (imoutput->next != NULL)
                        nex = mw_getdot_fimage(imoutput->next, x, y);
                    else
...
void amle3d(int *num, Fmovie init, Fmovie in, Fmovie out)
{
    ...
    if (in == NULL)
        mwerror(FATAL, 1, "Null input fmovie.\n");
    NL = in->first->nrow;
    NC = in->first->ncol;
    ...
    if (out == NULL)
        mwerror(FATAL, 1, "Not enough memory.\n");
    if (init)
        copymovie(init, out, NL, NC);   /* copy initial condition in output */
    else
        createmovie(in, out, NL, NC);   /* create a movie with value
                                         * 0.0 with the same number of
                                         * images as the input */
    for (i = 0; i < *num; i++)
