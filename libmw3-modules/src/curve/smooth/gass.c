  ...
  /*** MAIN LOOP : compute the middle points of significative chords ***/
    ...
    /*** more precise computation of cur_area if needed ***/
        ...
        if (is_closed)
            ...
            stop = ((q0 + 2 == pmax) && (cur_area <= area_sz));
                      ...
                      /* output: number of connected components after evol. */
{
  ...
  /*** apply aceros to convex components and link result ***/
    ...
    for (i = 0; i < nc; i++)
        l->size +=
            aceros(pts2[i], (pts2[i + 1] - pts2[i]) / 2,
                   l->values + l->size * 2, narea);
}
            ...
            double *step, int *n, double *r, char *v)
{
  ...
  /*----- FIRST LOOP (all curves) -----*/
    ...
    /*** compute scale (area) step ***/
        if (step)
            step_a = (*step) * (*step) * pow((0.75 / omega), 1.5);
        else
            step_a = pow(remaining_h / (double) (*n), 1.5);
    ...
    /*----- SECOND LOOP (iterations) -----*/
      ...
      /*** one step ***/
            ...
            {
                t = *last -
                    pow(omega * (remaining_h >= 0.0 ? remaining_h : 0.0) /
                        0.75, 0.75);
                printf("Iter %3d : %6d cc, effective area = %f, "
                       "current scale = %f\n", i, ncc, a, t);
                if (!out->list[j]->size)
                    printf("The curve has collapsed.\n");
            }
        }
