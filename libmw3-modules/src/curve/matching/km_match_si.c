...
function = {"Compute similitude-invariant matches
            between pieces of meaningful boundaries of two images"};
usage = {
   maxError1->maxError1
        "maximum Hausdorff distance between normalized codes allowed",
   maxError2->maxError2
        "maximum Hausdorff distance (in pixels)
        between de-normalized codes (to the frame of image 2) allowed",
   minLength->minLength
...
#include <float.h>
#include "mw3.h"
#include "mw3-modules.h"         /* km_prematchings() */
...
static void getMatrixSI(float xini1, float yini1, float xfin1, float yfin1,
                        float xini2, float yini2, float xfin2, float yfin2,
                        float (*A)[3])
{
...
static void get_last_index_matching(float errorMax,
                                    struct NormDataSIconcat *info1,
                                    struct NormDataSIconcat *info2,
                                    Flist fcrv1, Flist fcrv2, int *iL1,
                                    int *iL2, int iLast1, int iLast2,
                                    unsigned char type)
{
    ...
    while (ee < errorMax * errorMax)
    {
        ...
        i1 = get_next_point_length(fcrv1, &x1, &y1, i1, iLast1, info1->disc,
                                   type);
        N_Points = fcrv2->size;
        Closed = ((_(fcrv2, 0, 0) == _(fcrv2, fcrv2->size - 1, 0))
                  && (_(fcrv2, 0, 1) == _(fcrv2, fcrv2->size - 1, 1)));
        i2 = get_next_point_length(fcrv2, &x2, &y2, i2, iLast2, info2->disc,
                                   type);
        x1p = x1;
        y1p = y1;
        map1to2_SI(&x1p, &y1p);
...
static unsigned char extend_matchingSI(float errorMax, float minLength,
                                       struct NormDataSIconcat *info1,
                                       struct NormDataSIconcat *info2,
                                       Flist fcrv1, Flist fcrv2, int *ini1,
                                       int *fin1, int *ini2, int *fin2,
...
static unsigned char check_matchingSI(Flist fcrv1, Flist fcrv2,
                                      struct NormDataSIconcat *info1,
                                      struct NormDataSIconcat *info2,
                                      float minComplex, float errorMax,
                                      float minLength, float (*A)[3],
                                      struct matchdata *matchdataaux)
{
    int i1, f1, i2, f2;

    getMatrixSI(info1->xR1, info1->yR1, info1->xR2, info1->yR2, info2->xR1,
                info2->yR1, info2->xR2, info2->yR2, A);

    if (!extend_matchingSI
                 ...
                 Flist matching_pieces)
{
    ...
    if (dict1->list[0]->size != dict2->list[0]->size)
        mwerror(FATAL, 1,
                "error: codes in dict1 and dict2 don't have "
                "the same number of points\n");
    if (dict1->list[0]->data_size != sizeof(struct NormDataSIconcat))
        mwerror(FATAL, 1,
                "error: dict1 is not a similitude-invariant dictionary code\n");
    if (dict2->list[0]->data_size != sizeof(struct NormDataSIconcat))
        mwerror(FATAL, 1,
                "error: dict2 is not a similitude-invariant dictionary code\n");
    ...
    for (m = 0; m < prematchings->size; m++)
    {
        ...
        for (n = 0; n < prematchings->list[m]->size; n++)
        {
            ...
            if (check_matchingSI
                (levlines1->list
                 [((struct NormDataSIconcat *) (dict1->list[m]->
                                                data))->Numcurve],
                 levlines2->list[((struct NormDataSIconcat
                                   *) (dict2->list[(int)
                  ...
                  *) (dict2->list[(int) _(prematchings->list[m], n, 0)]->
                      data), minComplex, maxError2, minLength, M_SI,
                 matchdataaux))
            {
