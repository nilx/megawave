...
static float hmax;              /* maximal distance between
                                 * bitangent and curve */

/* check the position of a point w.r.t. a line */
static float halfplane(float x1, float y1, float x2, float y2, float x,
                       float y)
{
    float u, v;
    double dx, dy;
...
/* compute the first triplet of three consecutive points
 * following i in the direction type */
static unsigned char get_first_triplet(int iFirst, int *i, int *iP, int *iN,
                                       int iLast, unsigned char type)
{
    *iP = iFirst;
    if (type == 1)
    {
...
static unsigned char get_next_triplet(int *i, int *iP, int *iN, int last,
                                      unsigned char type)
{
    *iP = *i;
    *i = *iN;
...
static unsigned char get_bitangent(Flist curve_IP, Flist curve_BP, Flist fcrv,
                                   int IP1, int IP2, int *nBts, int nBts_max,
                                   int nIPs)
{
    ...
    {
        if (Closed)
            iLast = _(curve_IP, nIPs - 1, 0);
        else
            iLast = 0;
    }
    else
        iLast = _(curve_IP, IP1 - 1, 0);
    jFirst = _(curve_IP, IP2, 0);
    if (IP2 == nIPs - 1)
    {
        if (Closed)
            jLast = _(curve_IP, 0, 0);
        else
            jLast = N_Points - 1;
    }
    else
        jLast = _(curve_IP, IP2 + 1, 0);
    ...
    if (!get_first_triplet(iFirst, &i, &iP, &iN, iLast, 0))
        return 1;
    if (!get_first_triplet(jFirst, &j, &jP, &jN, jLast, 1))
        return 1;
    ...
    do
    {
        ...
        do
        {
            ...
            if (!ok)
            {
                if (same_convex == 1)
                    ...
                    last = iLast;
                if (!get_next_triplet(&i, &iP, &iN, last, 0))
                    return 1;
                change_i = 1;
            }
        }
        ...
        do
        {
            ...
            if (!ok)
            {
                if (same_convex == 1)
                    ...
                    last = jLast;
                if (!get_next_triplet(&j, &jP, &jN, last, 1))
                    return 1;
                change_j = 1;
            }
        }
        ...
        {
            if ((i == j) || (i == (j + 1) % N_Points)
                || (i == (j - 1) % N_Points))
                return 1;
            if (check_crossing_curve(fcrv, i, j))
                return 1;
            if (*nBts >= nBts_max)
                return 0;
    ...
    do
    {
        ...
        for (i = 0; (i < nIPs) && (!out_of_bounds); i++)
        {
            ...
            if (Closed)
            {
                ...
                if ((i_next_next >= nIPs) && (Closed))
                    i_next_next = 0;
            }
            while (iterate)
            {
                if (get_bitangent
                    (curve_IP, curve_BP, curve, i, i_next, &nBts, nBts_max,
                     nIPs))
                {
                    i_next++;
                    if ((i_next >= nIPs) && (Closed))
        ...
        if (out_of_bounds)
        {
            nBts_max *= 2;
            if ((curve_BP =
                 mw_change_flist(curve_BP, nBts_max, 0, 2)) == NULL)
                mwerror(FATAL, 1, "error, not enough memory\n");
            nBts = 0;
        }
