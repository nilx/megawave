...
usage = {
   ...
   matching_lines1<-aux1
      "output Flists of curves from image 1 that contain
      at least one matching piece of curve",
   matching_lines2<-aux2
      "output Flists of curves from image 2 that contain
      at least one matching piece of curve",
                      ...
                      Flists pieceaux1, Flists pieceaux2)
{
    ...
    if ((aux1 =
         mw_change_flists(aux1, matching_pieces->size,
                          matching_pieces->size)) == NULL)
        mwerror(FATAL, 1, "error, not enough memory\n");

    if ((aux2 =
         mw_change_flists(aux2, matching_pieces->size,
                          matching_pieces->size)) == NULL)
        mwerror(FATAL, 1, "error, not enough memory\n");
    if ((pieceaux1 =
         mw_change_flists(pieceaux1, matching_pieces->size,
                          matching_pieces->size)) == NULL)
        mwerror(FATAL, 1, "error, not enough memory\n");
    if ((pieceaux2 =
         mw_change_flists(pieceaux2, matching_pieces->size,
                          matching_pieces->size)) == NULL)
        mwerror(FATAL, 1, "error, not enough memory\n");
    ...
    for (m = 0; m < matching_pieces->size; m++)
    {
        ...
        aux1->list[m] =
            mw_copy_flist(levlines1->list[(int) _(matching_pieces, m, 0)],
                          NULL);
        aux2->list[m] =
            mw_copy_flist(levlines2->list[(int) _(matching_pieces, m, 1)],
                          NULL);

        Closed = ((_(levlines1->list[(int) _(matching_pieces, m, 0)], 0, 0) ==
                   _(levlines1->list[(int) _(matching_pieces, m, 0)],
                     levlines1->list[(int) _(matching_pieces, m, 0)]->size -
                     1, 0))
                  && (_(levlines1->list[(int) _(matching_pieces, m, 0)], 0, 1)
                      == _(levlines1->list[(int) _(matching_pieces, m, 0)],
                           levlines1->
                           list[(int) _(matching_pieces, m, 0)]->size - 1,
                           1)));

        N_Points = levlines1->list[(int) _(matching_pieces, m, 0)]->size;

        piecesize =
            (int) (_(matching_pieces, m, 3) - _(matching_pieces, m, 2));
        if (piecesize < 0)
            piecesize = piecesize + N_Points;
        ...
        {
            pieceaux1->list[m] =
                mw_change_flist(NULL, piecesize, piecesize, 2);
            inext = (int) _(matching_pieces, m, 2);
            ilast = (int) _(matching_pieces, m, 3) + 1;

            for (n = 0; n < piecesize; n++)
            {
                _(pieceaux1->list[m], n, 0) =
                    _(levlines1->list[(int) _(matching_pieces, m, 0)], inext,
                      0);
                _(pieceaux1->list[m], n, 1) =
                    _(levlines1->list[(int) _(matching_pieces, m, 0)], inext,
                      1);
        ...
        Closed = ((_(levlines2->list[(int) _(matching_pieces, m, 1)], 0, 0) ==
                   _(levlines2->list[(int) _(matching_pieces, m, 1)],
                     levlines2->list[(int) _(matching_pieces, m, 1)]->size -
                     1, 0))
                  && (_(levlines2->list[(int) _(matching_pieces, m, 1)], 0, 1)
                      == _(levlines2->list[(int) _(matching_pieces, m, 1)],
                           levlines2->
                           list[(int) _(matching_pieces, m, 1)]->size - 1,
                           1)));

        N_Points = levlines2->list[(int) _(matching_pieces, m, 1)]->size;

        piecesize =
            (int) (_(matching_pieces, m, 5) - _(matching_pieces, m, 4));
        if (piecesize < 0)
            piecesize = piecesize + N_Points;
        ...
        else
        {
            ...
            for (n = 0; n < piecesize; n++)
            {
                _(pieceaux2->list[m], n, 0) =
                    _(levlines2->list[(int) _(matching_pieces, m, 1)], inext,
                      0);
                _(pieceaux2->list[m], n, 1) =
                    _(levlines2->list[(int) _(matching_pieces, m, 1)], inext,
                      1);
                inext = get_next_index(inext, ilast, 1);
            }
