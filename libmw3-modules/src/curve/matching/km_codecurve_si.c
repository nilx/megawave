...
static int get_next_point_angle(Flist fcrv, int i1, int i2, int i_max,
                                float angle_max, unsigned char type)
{
    int i, i_next;
...
static void find_intersection_point(Flist curve, int i1, int i2, int i,
                                    float *xR, float *yR)
{
...
static void find_central_point(Flist curve, float xR1, float yR1, float xR2,
                               float yR2, int iFirst, int iLast, float *xC,
                               float *yC, int *ileft, int *iright)
{
    int i_last, i_next, i;
...
static void add_codeSI(Flist arc_code_SI, float x, float y, int m, float x0,
                       float y0, float vux, float vuy, float L0)
{
    float xx, yy;
...
static int get_code_SI(Flist arc_code_SI, Flist curve, float xC, float yC,
                       ...
                       float xR2, float yR2)
{
    ...
    my_data =
        (struct NormDataSIconcat *) malloc(sizeof(struct NormDataSIconcat));
    x0 = (xR1 + xR2) / 2;
    y0 = (yR1 + yR2) / 2;
    dx = xR2 - xR1;
    dy = yR2 - yR1;
    ...
    y = yC;
    iNtotal =
        get_next_point_length(curve, &x, &y, i_right, i_left,
                              L0 * FNorm / 2.0f, 1);
    if (iNtotal < 0)
        return -1;
    ...
    y = yC;
    iPtotal =
        get_next_point_length(curve, &x, &y, i_left, iNtotal,
                              L0 * FNorm / 2.0f, 0);
    if (iPtotal < 0)
        return -1;

    disc_sz = (L0 * FNorm) / ((float) (NNorm - 1));
    /* Nnorm must be an odd number */
...
static int codeSI(Flist arc_code_SI, Flist curve, int i1, int i2)
{
    ...
    find_central_point(curve, xR1, yR1, xR2, yR2, iP, iN, &xC, &yC, &ileft,
                       &iright);
    mwdebug("xC: %f  yC: %f  i1: %d  i2: %d  ileft: %d  iright: %d\n", xC, yC,
            i1, i2, ileft, iright);
    return get_code_SI(arc_code_SI, curve, xC, yC, ileft, iright, xR1, yR1,
                       xR2, yR2);
}
                       ...
                       Flist curve_BP, Flists dict, int NC, int NN, float FN)
{
    ...
    if ((arc_code_SI = mw_change_flist(NULL, NNorm, NNorm, 2)) == NULL)
    ...
    for (k = 0; k < curve_IP->size; k++)
    {
        ...
        {
            dict->list[dict->size] =
                mw_copy_flist(arc_code_SI, dict->list[dict->size]);
            dict->size++;
        }
    }
