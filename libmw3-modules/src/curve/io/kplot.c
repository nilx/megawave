...
static void bound_curves(Curves cs, int *xmin, int *ymin, int *xmax,
                         int *ymax)
{
    Curve c;
    Point_curve p;
    ...
    if (!cs->first)
        mwerror(INTERNAL, 1, "[bound_curves] NULL curves\n");
    if (!cs->first->first)
        mwerror(INTERNAL, 1, "[bound_curves] NULL curve\n");
...
static void bitmap_curve_with_lines(Curve curve, Cimage A, Cimage B, int xmin,
                                    int ymin, int mode)
{
    ...
    if (p->next == NULL)        /* Only one point in the curve */
    {
        ...
        if (A && mode == 1)
        {
            if (y0 - ymin < B->nrow / 2)
                ...
                mw_plot_cimage(B, x0 - xmin, y0 - ymin - 1, 255);
            mwdebug
                ("[bitmap_curve_with_lines] Only one point (%d,%d) "
                 "in the input curve !\n", x0, y0);
        }
    }
    else
        for (p = p->next; p; p = p->next)
        {
            ...
            mw_draw_cimage(B, x0 - xmin, y0 - ymin, p->x - xmin, p->y - ymin,
                           0);
            if (A && mode == 1)
            {
                if (abs(x0 - p->x - xmin) >= abs(y0 - p->y - ymin))
                    mw_draw_cimage(B, x0 - xmin, y0 + 1 - ymin, p->x - xmin,
                                   p->y - ymin + 1, 255);
                else
                    mw_draw_cimage(B, x0 - xmin + 1, y0 - ymin,
                                   p->x - xmin + 1, p->y - ymin, 255);
            }
            x0 = p->x;
            y0 = p->y;
        }
...
void kplot(Cimage A, char *line, Curves curves, Cimage B, int *d)
{
    ...
    if (curves->first == NULL)
        mwerror(FATAL, 1, "No curve found !\n");
    ...
    if (line)
        ...
        for (curve = curves->first, i = 1; curve; curve = curve->next, i++)
        {
            mwdebug("Curve #%d\n", i);
            if (curve->first != NULL)
                bitmap_curve(curve, B, xmin, ymin);
            else
                mwdebug("Empty curve found !\n");
        }
}
