...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for ccputstring(), disc(), fcrop() */
...
static void getrule(double a, double b, double *ofs, double *step, int *nsub)
{
    ...
    r = 1.0;
    if (x <= 0.)
        mwerror(FATAL, 1, "(getrule): empty interval [%f,%f]\n", a, b);
    while (x <= 3.)
    {
        x *= 10.;
        r *= 10.;
...
static void draw_framed(int xa, int ya, int xb, int yb, unsigned char r,
                        unsigned char g, unsigned char b, int xmin, int xmax,
                        int ymin, int ymax, int i)
{
    ...
    else
    {
        ...
        if (dx)
        {
            txa =
                (double) ((xa <
                           xmin ? xmin : (xa > xmax ? xmax : xa)) - xa) / dx;
            txb =
                (double) ((xb <
                           xmin ? xmin : (xb > xmax ? xmax : xb)) - xa) / dx;
        }
        if (dy)
        {
            tya =
                (double) ((ya <
                           ymin ? ymin : (ya > ymax ? ymax : ya)) - ya) / dy;
            tyb =
                (double) ((yb <
                           ymin ? ymin : (yb > ymax ? ymax : yb)) - ya) / dy;
        }
        if (tya > txa)
            txa = tya;
        if (tyb < txb)
...
static void draw_curve_framed(int x, int y, Curve c, unsigned char r,
                              unsigned char g, unsigned char b, int xmin,
                              int xmax, int ymin, int ymax, int i)
{
    Point_curve p;
    int xx, yy, adr;
...
static void plot_one_curve(int i, int color, int pcolor, int mode)
{
    ...
    for (j = 0; j < c->size; j++)
    {
        ...
        if (s_flag)
            y = Y1 + (int) ((double) (Y2 - Y1) * (v - sy1) / (sy2 - sy1));
        else
            y = Y2 + (int) ((double) (Y1 - Y2) * (v - sy1) / (sy2 - sy1));

        if (mode & 2)           /* lines */
            if (line)
                draw_framed(ox, oy, x, y, C_RED(color), C_GREEN(color),
                            C_BLUE(color), X1 + 1, X2 - 1, Y1 + 1, Y2 - 1, i);

        if (e_flag && (j == 0 || j == c->size - 1))     /* extremal points */
            draw_curve_framed(x, y, disc2, 0, 0, 0, X1 + 1, X2 - 1, Y1 + 1,
                              Y2 - 1, i);

        if (mode & 1)           /* points */
            draw_curve_framed(x, y, disc1, C_RED(pcolor), C_GREEN(pcolor),
                              C_BLUE(pcolor), X1 + 1, X2 - 1, Y1 + 1, Y2 - 1,
                              i);
...
static void plot_curves(void)
{
    ...
    /*---------- DRAW AXES ----------*/
        ...
        do
        {
            ...
            for (k = 0;
                 (v = yofs + (double) k * ystep * (double) ysub) <= sy2; k++)
            {
                if (s_flag)
                    y = Y1 +
                        (int) ((double) (Y2 - Y1) * (v - sy1) / (sy2 - sy1));
                else
                    y = Y2 +
                        (int) ((double) (Y1 - Y2) * (v - sy1) / (sy2 - sy1));
                if (y >= Y1 && y <= Y2)
                {
        ...
        for (k = 0; (v = yofs + (double) k * ystep) <= sy2; k++)
        {
            ...
            if (y >= Y1 && y <= Y2)
            {
                if (k % ysub)
                {
                    ...
                    {
                    case 4:
                        mw_draw_ccimage(image, X1, y, X2, y, 200, 255, 200);
                    case 3:
                    case 2:
                        mw_draw_ccimage(image, X2 + 1, y, X2 + 4, y, 255, 0,
                                        0);
                    case 1:
                        mw_draw_ccimage(image, X1 - 4, y, X1 - 1, y, 255, 0,
                                        0);
                    }
                }
                else
                {
                    ...
                    case 4:
                    case 3:
                        mw_draw_ccimage(image, X1, y, X2, y, 255, 200, 200);
                    case 2:
                        mw_draw_ccimage(image, X2 + 1, y, X2 + 7, y, 255, 0,
                                        0);
                    case 1:
                        mw_draw_ccimage(image, X1 - 7, y, X1 - 1, y, 255, 0,
                                        0);
                    }
                    sprintf(str, "%g", local_trunc(v, truncref));
                    ccputstring(image, X1 - 7 - FONTWIDTH * strlen(str),
                                y - FONTHEIGHT / 2, &fgcolor, &bgcolor, NULL,
                                str);
                }
            }
        }
        ...
        for (k = 0; (v = xofs + (double) k * xstep) <= sx2; k++)
        {
            ...
            if (x >= X1 && x <= X2)
            {
                if (k % xsub)
                {
                    ...
                    {
                    case 4:
                        mw_draw_ccimage(image, x, Y1, x, Y2, 200, 255, 200);
                    case 3:
                    case 2:
                        mw_draw_ccimage(image, x, Y1 - 4, x, Y1 - 1, 255, 0,
                                        0);
                    case 1:
                        mw_draw_ccimage(image, x, Y2 + 1, x, Y2 + 4, 255, 0,
                                        0);
                    }
                }
                else
                {
                    ...
                    case 4:
                    case 3:
                        mw_draw_ccimage(image, x, Y1, x, Y2, 255, 180, 180);
                    case 2:
                        mw_draw_ccimage(image, x, Y1 - 7, x, Y1 - 1, 255, 0,
                                        0);
                    case 1:
                        mw_draw_ccimage(image, x, Y2 + 1, x, Y2 + 7, 255, 0,
                                        0);
                    }
                    sprintf(str, "%g", local_trunc(v, truncref));
                    ccputstring(image, x - strlen(str) * FONTWIDTH / 2,
                                Y2 + 7, &fgcolor, &bgcolor, NULL, str);
                }
            }
        }
            ...
            for (j = 0; j < c->size; j++)
            {
                ...
                if (s_flag)
                    ...
                    y = Y2 +
                        (int) ((double) (Y1 - Y2) * (v - sy1) / (sy2 - sy1));
                if (line)
                    draw_framed(ox, oy, x, y, C_RED(ref_color),
                                C_GREEN(ref_color), C_BLUE(ref_color), X1 + 1,
                                X2 - 1, Y1 + 1, Y2 - 1, -1);
                line = 1;
                ox = x;
                oy = y;
  ...
  /*---------- PLOT CURVES ----------*/
    ...
    if (show_all)
        for (i = 0; i < curves->size; i++)
            plot_one_curve(i, in_color, pcolor,
                           (curves->list[i]->size > 1 ? draw_mode : 1));
    mode = (curves->list[cur_index]->size > 1 ? draw_mode : 1);
    if (cur_display)
        plot_one_curve(cur_index, 900, 700, mode);
    else if (!show_all)
        plot_one_curve(cur_index, in_color, pcolor, mode);
...
static void zoom_sxy(int x, int y)
{
    ...
    d = .5 * (sy2 - sy1);
    if (s_flag)
        sy1 += .5 * (double) (y - Y1) * (sy2 - sy1) / (double) (Y2 - Y1);
    else
        sy1 += .5 * (double) (y - Y2) * (sy2 - sy1) / (double) (Y1 - Y2);
    sy2 = sy1 + d;
}
...
static int win_notify(Wframe * window, void *param)
{
    ...
    int c;
/* Key code must be int and not char to handle non-printable keys */
    char mess[80];
...
void fkview(Flists in, Ccimage * out, int *sx, int *sy, Flists ref, Fimage bg,
            int *i, char *a, char *s, char *e, int *d, int *g, int *c, int *C,
            char *n, char *window, int *x_0, int *y_0, Flist * curve)
{
    ...
    ref_curves = ref;
    bg_image = bg;
    cur_index = cur_display = motion_flag = 0;
    show_all = 1;
    nx = *sx;
    ny = *sy;
    bg_order = *i;
  ...
  /*** plot curves ***/
    ...
    if (!n)
    {
        ...
        win =
            (Wframe *) mw_get_window((Wframe *) window, *sx, *sy, *x_0, *y_0,
                                     in->name);
        if (!win)
