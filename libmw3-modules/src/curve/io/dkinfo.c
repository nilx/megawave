...
#include <stdio.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for area(), perimeter(), dsplit_convex() */
...
static int is_closed(Dlist c)
{
    if (c->size < 2)
        ...
        return ((c->values[0] == c->values[c->dim * (c->size - 1)]) &&
...
void dkinfo(Dlists in)
{
    ...
    for (i = 0; i < in->size; i++)
    {
        ...
        if (!c || c->size == 0)
            ...
            printf("This is not a curve (dim=%d)\n", c->dim);
        else
        {
            printf("This is a %s curve with %d point%s",
                   is_closed(c) ? "closed" : "non-closed", c->size,
                   (c->size < 2 ? "" : "s"));
            if (c->dim > 2)
                printf(" (dimension %d)\n", c->dim);
            else
            ...
            if (c->size >= 2)
            {
                ...
                printf("Perimeter: %g\n", per);
                if (c->size >= 3)
                    printf("Algebraic Area: %g\n", area(c));
                eps = 1e-15;
                res = dsplit_convex(c, NULL, &ncc, &eps);
                printf
                    ("Number of convex components (double resolution): %d\n",
                     ncc);
                eps = 1e-7;
                res = dsplit_convex(c, res, &ncc, &eps);
                mw_delete_dlists(res);
                printf
                    ("Number of convex components (float resolution) : %d\n",
                     ncc);
            }
        }
    }
