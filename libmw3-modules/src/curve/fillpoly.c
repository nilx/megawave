  ...
  /*--- First pass ---*/
    ...
    if (current_point != NULL)
    {
        if (current_point->x - c == 1)
            ...
            mwerror(FATAL, 2,
                    "Something wrong in filling morpho set (erase borders) :\n"
                    "successive points in morpho lines "
                    "are not neighbours!\n"
                    " r1 = %d, c1 = %d, r2 = %d, c2 = %d, "
                    "line = %d, point = %d\n",
      ...
      /*--- Contour goes right ---*/
        ...
        else if (current_point->y - r == 1)
        {
          ...
          /*--- Contour goes down ---*/
            ...
            mwerror(FATAL, 2,
                    "Something wrong in filling morpho set :\n"
                    "successive points in morpho lines "
                    "are not neighbours!\n"
...
static void CHANGE_ORIENTATION_CONTOUR(Polygon contour)
                                     /* Contour of polygon */
{
...
static char GET_ORIENTATION_CONTOUR(Cimage bitmap, Polygon contour)
                                     ...
                                     /* Contour of polygon */
{
    ...
    while ((current_point) && (test_found == FALSE))
    {
        ...
        {
            r = 0;
            while ((r < current_point->y)
                   && (bitmap->gray[r * ncol + c] == BG_SYMB))
                r++;
            if (r == current_point->y)
            {
                if (current_point->next)
                {
                    if (((current_point->previous->x >= c)
                         && (current_point->next->x < c))
                        || ((current_point->previous->x > c)
                            && (current_point->next->x <= c)))
                    {
                        dir = 0;
                        test_found = TRUE;
                    }
                    else if (((current_point->previous->x <= c)
                              && (current_point->next->x > c))
                             || ((current_point->previous->x < c)
                                 && (current_point->next->x >= c)))
                    {
            ...
            if (test_found == FALSE)
            {
                ...
                if (c == current_point->x)
                {
                    if (current_point->next)
                    {
                        if (((current_point->previous->y >= r)
                             && (current_point->next->y < r))
                            || ((current_point->previous->y > r)
                                && (current_point->next->y <= r)))
                        {
                            dir = 1;
                            test_found = TRUE;
                        }
                        else if (((current_point->previous->y <= r)
                                  && (current_point->next->y > r))
                                 || ((current_point->previous->y < r)
                                     && (current_point->next->y >= r)))
                        {
...
static int GET_NPOINTS_CONTOUR(Polygon poly)
                                 /* Input polygon */
{
    int n;
    int x, y;
...
static void DRAW_CONTOUR(Cimage bitmap, Polygon contour)
                                 /* bitmap image */
                                 /* Contour of polygon */
{
    Point_curve ptr_point;      /* Pointer to the current point
    ...
    while (ptr_point)
    {
        ...
        if ((ptr_point->x < 0) || (ptr_point->x >= bitmap->ncol)
            || (ptr_point->y < 0) || (ptr_point->y >= bitmap->nrow))
            mwerror(FATAL, 2,
      ...
      /*--- Check if point is the extremity of a segment ---*/
            ...
            if (ptr_point->next && ptr_point->previous)
                if ((ptr_point->next->x == ptr_point->previous->x)
                    && (ptr_point->next->y == ptr_point->previous->y))
                    *ptrb = 1;
        }
        else
      /*--- Contour has already passed through this point ---*/
        ...
        if (ptr_point->next)
            (*ptrb)++;
        else if ((contour->first->next->x == ptr_point->previous->x)
                 && (contour->first->next->y == ptr_point->previous->y))
            (*ptrb)++;
                                  ...
                                  * allocation */
{
    ...
    if ((poly_point->x < 0) || (poly_point->x >= bitmap->ncol)
        || (poly_point->y < 0) || (poly_point->y >= bitmap->nrow))
        mwerror(FATAL, 2,
                "Vertex of polygon outside of image!\n"
                "r = %d, c = %d, nrow = %d, ncol = %d.\n",
                                     ...
                                     /* Output bitmap */
{
  ...
  /*--- Memory allocation for contour points ---*/
    ...
    alloc_point =
        (Point_curve) malloc(npoints_cont * sizeof(struct point_curve));
    if (alloc_point == NULL)
        mwerror(FATAL, 1,
                "Not enough memory for polygon's contour points!\n");
