...
function = {"Computes the biorthogonal wavelet coefficients of an
            image without decimation"};
usage = {
...
static void
COMLINE_ERR(Fsignal ri1, Fsignal ri2, short int numrec, long int dy,
            long int dx)
...
/* Size of the image */
{
    ...
    if (((dx % (1 << numrec)) != 0) || ((dy % (1 << numrec)) != 0))
        mwerror(FATAL, 1,
                ...
                "of image are not a multiple of 2^J!\n");

}

static void ALLOC_WAV(Wtrans2d wtrans, Fimage image, int numrec,
                      int stopdecim)
{
    int i, J;
    int nrow, ncol;
    ...
    for (J = 1; J <= numrec; J++)
    {
        ...
        for (i = 0; i <= 3; i++)
        {
            if (mw_change_fimage(wtrans->images[J][i], nrow, ncol) == NULL)
                mwerror(FATAL, 1,
                        "Not enough memory for wavelet transform buffer.\n");
        }
    }
...
static void NORM_FIL(Fsignal ri1, Fsignal ri2, int *filternorm)
...
static void
COLUMN_WAVEL(Fimage Im, Fimage Im1, Fimage Im2, int decim, int nsubim,
             int *edge, Fsignal ri1, Fsignal ri2)
...
/* Impulse responses of the low-pass filters */
{
    long c, l;                  /* Variables for line and column in
                                 * resume or detail */
...
static void
WAVEL(Wtrans2d wtrans, int J, int stopdecim, int *ortho, Fsignal ri1,
      Fsignal ri2, int *edge)
 ...
 * in biowave2) */
{
  ...
  /*--- Convolution of rows with low-pass filter ---*/
    ...
    for (s = 0; s < nsubim; s++)
    {
        ...
        for (l = 0; l < dy; l++)
        {
            for (c = 0; c < dxs; c++)
                Tabin->values[c] =
                    wtrans->images[J - 1][0]->gray[ldx + nsubim * c];
  ...
  /*--- Convolution of column with low and high-pass filters ---*/
    ...
    COLUMN_WAVEL(Tab, wtrans->images[J][0], wtrans->images[J][1], decim,
                 nsubim, edge, ri1, ri2);
  ...
  /*--- Convolution of columns with low and high-pass filters ---*/
    ...
    COLUMN_WAVEL(Tab, wtrans->images[J][2], wtrans->images[J][3], decim,
                 nsubim, edge, ri1, ri2);
...
void
dybiowave2(int *NumRec, int *StopDecim, int *Ortho, int *Edge,
