...
usage = {
 ...
 WavTrans<-Output       "Wavelet transform of Signal (wtrans1d)",
...
static void
COMLINE_ERR(Fsignal ri1, Fsignal ri2, int edge, short int numrec,
            long int size)
                                ...
                                /* Size of the signal */
{
  ...
  /*--- Size of signal ---*/
    ...
    if ((size >> (numrec - 1) < ri1->size)
        || (size >> (numrec - 1) < ri2->size))
        mwerror(FATAL, 1,
                "The size of signal must be greater than N "
                "where N is the size of the i.r.!\n");

    if ((edge == 1) && ((size % (1 << numrec)) != 0))
        mwerror(FATAL, 1,
                "The size of the signal is not a multiple of 2^J!\n");

}

static void
COMMENT(Wtrans1d result, Fsignal signal, int edge, int *filternorm,
...
static void WAVEL(Wtrans1d wtrans, int J, Fsignal ri1, Fsignal ri2, int *edge)
 ...
 * in biowave1) */
{
  ...
  /*----- Computation of average at level J -----*/
    ...
    sconvolve(wtrans->A[J - 1][0], wtrans->A[J][0], &DECIM, &INTERPOL,
              &REFLIR, NULL, edge, &PROLONG, ri2, NULL);

  /*----- Computation of detail at level J -----*/

    sconvolve(wtrans->A[J - 1][0], wtrans->D[J][0], &DECIM, &INTERPOL,
              &REFLIR, &HIGH, edge, &PROLONG, ri1, NULL);

}

void
biowave1(int *NumRec, int *Edge, int *FilterNorm, Fsignal Signal,
