...
usage = {
 ...
 'h':HaarNLevel->Haar           "Start reconstruction with Haar filter
                                from level HaarNLevel down to level
                                NLevel + 1",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* sconvolve() */
...
static void
COMLINE_ERR(Wtrans2d wtrans, Fsignal ri1, Fsignal ri2, int *numrec, int *haar,
            int *haar_test)
...
/* Test value for use of Haar filter */
{
  ...
  /*--- Size of detail and average sub-images ---*/
    ...
    for (j = 1; j <= *numrec; j++)
    {
        if ((wtrans->images[j][1]->ncol != (dx + 1) / 2)
            || (wtrans->images[j][1]->nrow != dy / 2))
            mwerror(FATAL, 1, "The size of detail are incompatible!\n");
        if ((wtrans->images[j][2]->ncol != dx / 2)
            || (wtrans->images[j][2]->nrow != (dy + 1) / 2))
            mwerror(FATAL, 1, "The size of detail are incompatible!\n");
        if ((wtrans->images[j][3]->ncol != dx / 2)
            || (wtrans->images[j][3]->nrow != dy / 2))
            mwerror(FATAL, 1, "The size of detail are incompatible!\n");
        dx = (dx + 1) / 2;
        dy = (dy + 1) / 2;
    }
    if ((wtrans->images[*numrec][0]->ncol != dx)
        || (wtrans->images[*numrec][0]->nrow != dy))
        mwerror(FATAL, 1, "The size of average are incompatible!\n");

}

static void COMMENT(Fimage result, Wtrans2d wtrans, int edge, int *filternorm)
...
static void NORM_FIL(Fsignal ri1, Fsignal ri2, int filternorm, int haar_test)
...
static void HAAR_INV_WAVEL2(Wtrans2d wtrans, int numrec, int haar)
                                /* Wavelet transform */
...
static void
RECOMP_LINES(Fimage Tab, Fimage A, int haarx, int *band, Fsignal ri,
             int *edge)
...
static void
RECOMP_COLUMNS(Fimage Tab, Fimage AD, int haary, int *band, Fsignal ri,
               int *edge)
...
static void
INV_WAVEL(int J, Wtrans2d wtrans, Fimage A, Fsignal ri1, Fsignal ri2,
          int *edge)
 ...
 * in ibiowave2) */
{
  ...
  /*--- Initialization of A and Tab ---*/
    ...
    if ((A->nrow != L2) || (A->ncol != K2))
    {
        ...
        if (A == NULL)
            mwerror(FATAL, 1,
                    "Allocation of buffer for average at level %d refused!\n",
                    J);
    }
...
void
ibiowave2(int *NumRec, int *Haar, int *Edge, int *FilterNorm, Wtrans2d Wtrans,
          Fimage Output, Fsignal Ri1, Fsignal Ri2)
