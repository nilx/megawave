...
static int EDGE_HAAR = 0;       /* Index for edge processing mode with
                                 * Haar filter */
...
static void
COMLINE_ERR(Wtrans1d wtrans, Fsignal ri, Fimage edge_ri, int edge,
  ...
  /*--- Number of levels ---*/
    ...
    if (!haar)
    {
        if (wtrans->A[*numrec][0]->size < ri->size)
            mwerror(FATAL, 1,
                    "The size of average must be greater "
                    "than the size of the i.r.!\n");
    }
...
static void
COMMENT(Fsignal result, Wtrans1d wtrans, int edge, int precond,
        int *filternorm)
...
static void INIT_SHRINK(Wtrans1d wtrans, Fsignal ri, short int numrec,
                        int edge)
 ...
 * (see `Edge` in iowave2) */
{
    ...
    if ((edge == 2) || (edge == 0))
    {
        ...
        for (j = 1; j <= numrec; j++)
        {
            ...
            sizeav[j] =
                firstphigh + (sizeav[j - 1] - 3 + ri->size +
                              (long) ri->shift) / 2 + 1;
            ...
            sizeav[j] =
                firstp + (sizeav[j - 1] - 1 - (long) ri->shift) / 2 + 1;
...
static void HAAR_INV_WAVEL1(Wtrans1d wtrans, int haar, int filternorm)
/* Wavelet transform */
...
/* Type of normalisation : see adapowave2 */
{
    ...
    for (j = haar; j > numrec; j--)
    {
        ...
        {
            Tab->values[2 * c] =
                wtrans->A[numrec][0]->values[c] +
                wtrans->A[numrec][0]->values[size + c];
            Tab->values[2 * c + 1] =
                wtrans->A[numrec][0]->values[c] -
                wtrans->A[numrec][0]->values[size + c];
        }
...
static void
INV_WAVEL(int J, Wtrans1d wtrans, Fsignal S, Fsignal int_ri, int *inverse,
          int edge, Fimage edge_ri, Fsignal haar_ri)
...
/* Impulse response for Haar filter */
{
    ...
    if (!inverse || (edge == 1) || (edge == 3) || (haars == 1))
    {
        if (wtrans->A[J][0]->size != wtrans->D[J][0]->size)
            mwerror(FATAL, 1,
                    "Average and Detail are not of the same size!\n");
        prolong = 0;
    }
    else
    {
    ...
    if (haars == 0)
        sconvolve(wtrans->A[J][0], Tabout, &DECIM, &INTERPOL, NULL, NULL,
                  &edge, &prolong, int_ri, edge_ri);
    else
        sconvolve(wtrans->A[J][0], Tabout, &DECIM, &INTERPOL, NULL, NULL,
    ...
    if (haars == 0)
        sconvolve(wtrans->D[J][0], Tabout, &DECIM, &INTERPOL, NULL, &HIGH,
                  &edge, &prolong, int_ri, edge_ri);
    else
        sconvolve(wtrans->D[J][0], Tabout, &DECIM, &INTERPOL, NULL, &HIGH,
                  &EDGE_HAAR, &prolong, haar_ri, NULL);
...
void
iowave1(int *NumRec, int *Haar, int *Edge, int *Precond, int *Inverse,
 ...
 * matrices */
{
    ...
    if (*Precond >= 2)
        precond1d(NULL, Wtrans->A[*NumRec][0], Wtrans->A[*NumRec][0],
                  Edge_Ri);
    ...
    for (J = *NumRec; J > 1; J--)
        INV_WAVEL(J, Wtrans, Wtrans->A[J - 1][0], Ri, Inverse, *Edge, Edge_Ri,
                  Haar_Ri);
