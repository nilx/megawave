...
usage = {
 ...
 'h':HaarNLevel->Haar "Continue decomposition with Haar filter until
                      level HaarNLevel",
 ...
 'n':[FilterNorm=2]->FilterNorm [0,2]  "Filter taps normalization. 0:
                                       no normalization, 1: sum equal
                                       to 1.0, 2: squares sum equal to
                                       1.0",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for sconvolve(), precond2d() */
...
static void
COMMENT(Wtrans2d result, Fimage image, int edge, int precond, int filternorm,
        Fsignal ri)
...
static void
COMLINE_ERR(Fsignal ri, Fimage edge_ri, int edge, int *inverse, int precond,
            int *numrec, int *haar, long int dy, long int dx)
...
/* Size of the image */
{
    ...
    if ((edge < 3) && (precond >= 1))
        mwerror(USAGE, 1,
                "Preconditionning only with special edge processing\n");
    ...
    if (!haar)
        if (edge >= 1)
            if ((dx >> (*numrec) < ri->size) || (dy >> (*numrec) < ri->size))
                mwerror(FATAL, 1,
                        "The size of image must be greater "
                        "than N where N is the size of the i.r.!\n");

        /*--- Inner and edge filters compatibility ---*/

    if ((edge == 3)
        && ((edge_ri->nrow != 4 * ri->size)
            || (edge_ri->ncol != 3 * ri->size / 2 - 1)))
...
static void HAAR_WAVEL2(Wtrans2d wtrans, int numrec, int haar, int filternorm)
/* Wavelet transform */
/* Level of average */
/* Continue decomposition with Haar wavelet
 * until ultimate level */
/* Type of normalisation : see 'owave2' */
{
    ...
    if (mw_alloc_fimage(Tab, dy, dx) == NULL)
        mwerror(FATAL, 1,
                "Allocation for sum's separation buffer refused!\n");
    ...
    while (((dx >= 2) || (dy >= 2)) && (j < haar))
    {
        ...
        if (dx >= 2)
        {
            ...
            for (r = 0; r < dy; r++)
                ...
                {
                    Tab->gray[r * dxtab + c] =
                        wtrans->images[numrec][0]->gray[r * dxtab + 2 * c] +
                        wtrans->images[numrec][0]->gray[r * dxtab + 2 * c +
                                                        1];
                    Tab->gray[r * dxtab + dxres + c] =
                        wtrans->images[numrec][0]->gray[r * dxtab + 2 * c] -
                        wtrans->images[numrec][0]->gray[r * dxtab + 2 * c +
                                                        1];
                }
            if (dx % 2 == 1)
                for (r = 0; r < dy; r++)
                    Tab->gray[r * dxtab + dx2] =
                        wtrans->images[numrec][0]->gray[r * dxtab + dx - 1];

            for (r = 0; r < dy; r++)
                for (c = 0; c < dx2; c++)
                {
                    wtrans->images[numrec][0]->gray[r * dxtab + c] =
                        cnorm * Tab->gray[r * dxtab + c];
                    wtrans->images[numrec][0]->gray[r * dxtab + dxres + c] =
                        cnorm * Tab->gray[r * dxtab + dxres + c];
                }
            if (dx % 2 == 1)
                for (r = 0; r < dy; r++)
                    wtrans->images[numrec][0]->gray[r * dxtab + dx2] =
                        shaar * Tab->gray[r * dxtab + dx2];
        ...
        if (dy >= 2)
        {
            ...
            for (r = 0; r < dy2; r++)
                ...
                {
                    Tab->gray[r * dxtab + c] =
                        wtrans->images[numrec][0]->gray[2 * r * dxtab + c] +
                        wtrans->images[numrec][0]->gray[(2 * r + 1) * dxtab +
                                                        c];
                    Tab->gray[(r + dyres) * dxtab + c] =
                        wtrans->images[numrec][0]->gray[2 * r * dxtab + c] -
                        wtrans->images[numrec][0]->gray[(2 * r + 1) * dxtab +
                                                        c];
                }
            if (dy % 2 == 1)
                for (c = 0; c < dx; c++)
                    Tab->gray[dy2 * dxtab + c] =
                        wtrans->images[numrec][0]->gray[(dy - 1) * dxtab + c];
            ...
            for (r = 0; r < dy2; r++)
                for (c = 0; c < dx; c++)
                {
                    wtrans->images[numrec][0]->gray[r * dxtab + c] =
                        cnorm * Tab->gray[r * dxtab + c];
                    wtrans->images[numrec][0]->gray[(r + dyres) * dxtab + c] =
                        cnorm * Tab->gray[(r + dyres) * dxtab + c];
                }
            if (dy % 2 == 1)
                for (c = 0; c < dx; c++)
                    wtrans->images[numrec][0]->gray[dy2 * dxtab + c] =
                        shaar * Tab->gray[dy2 * dxtab + c];
        }
...
static void
COLUMN_WAVEL(Fimage Im, Fimage Im1, Fimage Im2, int *haar, int *edge,
             int prolong, Fsignal int_ri, Fimage edge_ri)
 ...
 * matrices */
{
    ...
    /*--- Initialization of Im1 and Im2 ---*/
        ...
        if ((L2 != Im1->nrow) || (dx != Im1->ncol))
        {
            ...
            if (Im1 == NULL)
                mwerror(FATAL, 1,
                        "Reallocation for average buffer refused!\n");
        }
        ...
        if ((dy2 != Im2->nrow) || (dx != Im2->ncol))
        {
            ...
            if (Im2 == NULL)
                mwerror(FATAL, 1,
                        "Reallocation for detail buffer refused!\n");
        }
    ...
    else
    {
        ...
        L2 = (dy + risize) / 2 - (1 -
                                  (dy +
                                   risize) / 2) *
            (long) fabs((double) ((risize + (long) int_ri->shift - 1) % 2));
        Im1 = mw_change_fimage(Im1, L2, K2);

        L2 = (dy + risize) / 2 - (1 -
                                  (dy +
                                   risize) / 2) *
            (long) fabs((double) (((long) int_ri->shift - 1) % 2));
  ...
  /*--- Initialization of Tabin and Tabout ---*/
    ...
    /*--- Convolution with low-pass filter ---*/
        ...
        if (haary == 0)
            sconvolve(Tabin, Tabout, &DECIM, &INTERPOL, &REFLIR, NULL, edge,
                      &prolong, int_ri, edge_ri);
        else
            sconvolve(Tabin, Tabout, &DECIM, &INTERPOL, &REFLIR, NULL,
                      &EDGE_HAAR, &prolong, haar_ri, NULL);
    ...
    /*--- Convolution with high-pass filter ---*/
        ...
        if (haary == 0)
            sconvolve(Tabin, Tabout, &DECIM, &INTERPOL, &REFLIR, &HIGH, edge,
                      &prolong, int_ri, edge_ri);
        else
            sconvolve(Tabin, Tabout, &DECIM, &INTERPOL, &REFLIR, &HIGH,
                      &EDGE_HAAR, &prolong, haar_ri, NULL);
...
static void
WAVEL(Wtrans2d wtrans, int J, Fsignal int_ri, Fimage edge_ri, int *edge,
      int *inverse, int *haar)
 ...
 * until haar level */
{
  ...
  /*--- Convolution of rows with low-pass filter ---*/
    ...
    /*--- Copy last point if number of columns is odd ---*/
        ...
        if ((K2 > dx2) && (prolong == 0))
        {
            if (haarx == 0)
                Tab->gray[lK2 + c] =
                    s * wtrans->images[J - 1][0]->gray[ldx + dx1];
            else
                Tab->gray[lK2 + c] =
                    shaar * wtrans->images[J - 1][0]->gray[ldx + dx1];
        }
        ldx += dx;
        lK2 += K2;
    }
  ...
  /*--- Convolution of column with low and high-pass filters ---*/
    ...
    COLUMN_WAVEL(Tab, wtrans->images[J][0], wtrans->images[J][1], haar, edge,
                 prolong, int_ri, edge_ri);

    if (prolong == 1)
    {
        wtrans->images[J][0]->firstcol = wtrans->images[J][1]->firstcol =
            Tabout->firstp;
        wtrans->images[J][0]->lastcol = wtrans->images[J][1]->lastcol =
            Tabout->lastp;
    }
    else
  ...
  /*--- Convolution of columns with low and high-pass filters ---*/
    ...
    COLUMN_WAVEL(Tab, wtrans->images[J][2], wtrans->images[J][3], haar, edge,
                 prolong, int_ri, edge_ri);

    if (prolong == 1)
    {
        wtrans->images[J][2]->firstcol = wtrans->images[J][3]->firstcol =
            Tabout->firstp;
        wtrans->images[J][2]->lastcol = wtrans->images[J][3]->lastcol =
            Tabout->lastp;
    }
...
void
owave2(int *NumRec, int *Haar, int *Edge, int *Precond, int *Inverse,
 ...
 * matrices */
{
  ...
  /*--- Detection of errors in command line ---*/
    ...
    COMLINE_ERR(Ri, Edge_Ri, *Edge, Inverse, *Precond, NumRec, Haar,
                Image->nrow, Image->ncol);
  ...
  /*--- Memory allocation for Haar filter ---*/
    ...
    if (Haar)
    {
        ...
        if (mw_alloc_fsignal(haar_ri, 2) == NULL)
            mwerror(FATAL, 1,
                    "Not enough memory for Haar impulse response buffer.\n");
    }
  ...
  /*--- Memory allocation for wavelet transform Output ---*/
    ...
    if (mw_alloc_ortho_wtrans2d(Output, *NumRec, Image->nrow, Image->ncol) ==
        NULL)
        mwerror(FATAL, 1,
  ...
  /*--- Unpreconditionning of resume's edges (if selected) ---*/
    ...
    if (*Precond == 2)
        precond2d(&INVERSE, Output->images[*NumRec][0],
                  Output->images[*NumRec][0], Edge_Ri);
