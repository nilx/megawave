...
static double hr(short int n)
  /*----- Reflected low-pass imulse response function -----*/
{
    return (h(-n));
}

static double g(short int n)
...
static void
INIT_RI(Fsignal signal, Fsignal Ri, int *band, int *Prolong, int edge,
        int *reflir, int uprate, int downrate)
...
/* downsampling rate of result */
{
  ...
  /*---or shrinkage near edges ---*/
    ...
    case 1:
        ...
        sizeres =
            Rddown(uprate * (signal->size - 1) + rshift,
                   downrate) + firstp + 1;
    ...
    case 2:
        sizeres =
            uprate * (signal->lastp - signal->firstp + 1) / downrate +
            (long) signal->param;
...
/* Indicates low or high-pass filtering */
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        {
            left_ri->gray[i * left_ri->ncol + j] =
                edge_ri->gray[(i + decal) * edge_ri->ncol + j];
            right_ri->gray[i * right_ri->ncol + j] =
                edge_ri->gray[(i + decal + N) * edge_ri->ncol + j];
        }
    }
}

static void CONV_0(Fsignal signal, Fsignal result, int uprate, int downrate)
...
/* downsampling rate of result */
{
    ...
    for (cs = 0; cs < sizei; cs += downrate)
    {
        ...
        for (k = ak; k <= bk; k++)
            result->values[cs / downrate] +=
                signal->values[k] * RI(cs - uprate * k);
    }
...
static void CONV_PER(Fsignal signal, Fsignal result, int uprate, int downrate)
/*--- Convolution of the input 'signal' ---*/
...
/* downsampling rate of result */
{
  ...
  /*---- Left edge processing ----*/
    ...
    for (cs = 0; cs <= rshift - uprate; cs += downrate)
    {
        ...
        for (k = ak; k <= bk; k++)
            result->values[cs / downrate] +=
                signal->values[signal->size + k] * RI(cs - uprate * k);
    }

  /*---- Right edge processing ----*/

    for (cs = ((sizei + lshift) / downrate) * downrate; cs < sizei;
         cs += downrate)
    {
        ak = signal->size;
        bk = (cs - lshift) / uprate;
        ...
        for (k = ak; k <= bk; k++)
            result->values[cs / downrate] +=
                signal->values[k - signal->size] * RI(cs - uprate * k);
...
/* Low/High-pass filtering */
{
    ...
    for (cs = 0; cs < sizei; cs += downrate)
    {
        ...
        for (k = ak; k <= bk; k++)
        {
            ...
            /*
             * assert((cs / downrate >= 0) && (cs / downrate <
             * result->size) && (k >= 0) && (k < signal->size));
             */
            result->values[cs / downrate] +=
                signal->values[k] * RI(cs - uprate * k);
        }
  ...
  /*---- Right edge processing ----*/
    ...
    for (cs = ((signal->size + lshift) / downrate) * downrate; cs < sizei;
         cs += downrate)
    {
        s = 0.0;
        ak = signal->size;
        ...
        if ((band || (downrate >= 2)) && (length % 2 == 1))
            ...
            {
                /*
                 * assert((2 * signal->size - k - 2 >= 0) && (2 *
                 * signal->size - k - 2 < signal->size));
                 */
                s += signal->values[2 * signal->size - k - 2] * RI(cs -
                                                                   uprate *
                                                                   k);
            }
        else
            ...
            {
                /*
                 * assert((2 * signal->size - k - 1 >= 0) && (2 *
                 * signal->size - k - 1 < signal->size));
                 */
                s += signal->values[2 * signal->size - k - 1] * RI(cs -
                                                                   uprate *
                                                                   k);
            }
...
static void
CONV_DEC_SPE(Fsignal signal, Fsignal result, int downrate, Fimage left_ri,
             Fimage right_ri)
 ...
 * right filters */
{
  ...
  /*---- Left edge processing ----*/
    ...
    for (c = 0; c < N; c++)
    {
        ...
        for (k = 0; k < N + c2 + 1; k++)
            result->values[c] +=
                i->values[k] * left_ri->gray[c * left_ri->ncol + k];
    }
  ...
  /*---- Right edge processing ----*/
    ...
    for (c = result->size - N; c < result->size; c++)
    {
        ...
        for (k = c2 - N + 1; k < i->size; k++)
            result->values[c] +=
                i->values[k] * right_ri->gray[(result->size - c - 1) *
                                              right_ri->ncol + i->size - k -
                                              1];
    }
...
static void
CONV_INT_SPE(Fsignal signal, Fsignal result, int uprate, Fimage left_ri,
             Fimage right_ri)
 ...
 * right filters */
{
  ...
  /*---- Left edge processing ----*/
    ...
    for (c = 0; c <= 3 * N - 2; c++)
    {
        ...
        for (k = ak; k <= bk; k++)
            result->values[c] +=
                i->values[k] * left_ri->gray[k * left_ri->ncol + c];
    }
  ...
  /*---- Right edge processing ----*/
    ...
    for (c = result->size - 3 * N + 1; c < result->size; c++)
    {
        ...
        for (k = ak; k <= bk; k++)
            result->values[c] +=
                i->values[k] * right_ri->gray[(i->size - k - 1) *
                                              left_ri->ncol + result->size -
                                              c - 1];
    }
...
static void
CONV_SPE(Fsignal signal, Fsignal result, int uprate, int downrate, int *band,
         Fimage left_ri, Fimage right_ri)
...
void
sconvolve(Fsignal Signal, Fsignal Output, int *DownRate, int *UpRate,
 ...
 * edge processing */
{
    Fimage Left_Ri = NULL, Right_Ri = NULL;
    /* Buffer containing impulse
  ...
  /*--- (if selected) ---*/
    ...
    if (*Edge == 3)
    {
        ...
        if (mw_alloc_fimage(Left_Ri, Edge_Ri->nrow / 8, Edge_Ri->ncol) ==
            NULL)
            mwerror(FATAL, 1,
                    "Allocation of buffer for edge r.i. refused!\n");
        if (mw_alloc_fimage(Right_Ri, Edge_Ri->nrow / 8, Edge_Ri->ncol) ==
            NULL)
            mwerror(FATAL, 1,
                    "Allocation of buffer for edge r.i. refused!\n");
