...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for sconvolve(), precond1d() */
...
static void
COMMENT(Wtrans1d result, Fsignal signal, int edge, int precond,
        int filternorm, Fsignal ri)
...
static void
COMLINE_ERR(Fsignal ri, Fimage edge_ri, int edge, int precond, int *numrec,
            int *haar, int *inverse, long int size)
                        ...
                        /* Size of the signal */
{
    ...
    if (size >> (*numrec) < 1)
    {
        ...
        while (size >> (*numrec) < 1)
            (*numrec)--;
        mwerror(WARNING, 1,
                "The number of level is too big!\nNumRec is set to %d\n",
                *numrec);
    }
    ...
    if (((edge == 1) || (edge == 3)) && ((size % (1 << (*numrec))) != 0))
        mwerror(FATAL, 1,
                "The size of the signal is not a multiple of 2^NumRec!\n");
...
static void
NORM_FIL(Fsignal ri, Fimage edge_ri, int filternorm, int edge,
         Fsignal haar_ri)
...
{
    double s, s2;               /* Sum of `ri`'s coefficients and
                                 * sum's square root
...
static void HAAR_WAVEL1(Wtrans1d wtrans, int haar, int filternorm)
                                /* Wavelet transform */
                                /* Continue decomposition with Haar wavelet
                                 * until ultimate level */
                                /* Type of normalisation : see adapowave2 */
{
    ...
    while ((size >= 2) && (j < haar))
    {
        ...
        {
            Tab->values[c] =
                wtrans->A[numrec][0]->values[2 * c] +
                wtrans->A[numrec][0]->values[2 * c + 1];
            Tab->values[size2 + c] =
                wtrans->A[numrec][0]->values[2 * c] -
                wtrans->A[numrec][0]->values[2 * c + 1];
        }
...
static void
WAVEL(Wtrans1d wtrans, int J, Fsignal int_ri, Fimage edge_ri, int edge,
      int *inverse, Fsignal haar_ri)
...
/* Impulse response for Haar filter */
{
    ...
    if (haarx == 0)
        sconvolve(wtrans->A[J - 1][0], wtrans->A[J][0], &DECIM, &INTERPOL,
                  &REFLIR, NULL, &edge, &prolong, int_ri, edge_ri);
    else
        sconvolve(wtrans->A[J - 1][0], wtrans->A[J][0], &DECIM, &INTERPOL,
                  &REFLIR, NULL, &EDGE_HAAR, &prolong, haar_ri, NULL);

        /*----- Computation of detail at level J -----*/

    if (haarx == 0)
        sconvolve(wtrans->A[J - 1][0], wtrans->D[J][0], &DECIM, &INTERPOL,
                  &REFLIR, &HIGH, &edge, &prolong, int_ri, edge_ri);
    else
        sconvolve(wtrans->A[J - 1][0], wtrans->D[J][0], &DECIM, &INTERPOL,
                  &REFLIR, &HIGH, &EDGE_HAAR, &prolong, haar_ri, NULL);

}

void
owave1(int *NumRec, int *Haar, int *Edge, int *Precond, int *Inverse,
 ...
 * matrices */
{
  ...
  /*--- Detection of errors in command line ---*/
    ...
    COMLINE_ERR(Ri, Edge_Ri, *Edge, *Precond, NumRec, Haar, Inverse,
                Signal->size);
  ...
  /*--- Unpreconditionning of average's edges (if selected) ---*/
    ...
    if (*Precond == 2)
        precond1d(&INVERSE, Output->A[*NumRec][0], Output->A[*NumRec][0],
                  Edge_Ri);
}
