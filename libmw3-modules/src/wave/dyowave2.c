...
function = {"Computes the orthogonal wavelet coefficients of an image
            without decimation"};
usage = {
 ...
 'n':[FiltNorm=2]->FilterNorm [0,2]  "Filter taps normalization. 0: no
                                     normalization, 1: sum equal to
                                     1.0, 2: squares sum equal to
                                     1.0",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for owave2() */
...
static void
WAVEL(Wtrans2d wtrans, int *numrec, int stopdecim, int *filternorm,
      Fsignal ri, Fimage edge_ri, int *edge, int *precond)
                                 ...
                                 * in biowave2) */
{
    ...
    {
        J = stopdecim - 1;
        owave2(&J, NULL, edge, precond, NULL, filternorm,
               wtrans->images[0][0], subwtrans, ri, edge_ri);
        for (J = 1; J < stopdecim; J++)
            for (i = 0; i <= 3; i++)
            {
                rdx = 0;
                for (r = 0; r < subwtrans->images[J][i]->nrow; r++)
                {
                    for (c = 0; c < subwtrans->images[J][i]->ncol; c++)
                        wtrans->images[J][i]->gray[rdx + c] =
                            subwtrans->images[J][i]->gray[rdx + c];
    ...
    for (sr = 0; sr < nsubim; sr++)
        for (sc = 0; sc < nsubim; sc++)
        {
            ...
            for (r = 0; r < dys; r++)
            {
                for (c = 0; c < dxs; c++)
                    Tab->gray[rdxs + c] =
                        wtrans->images[0][0]->gray[rdx + sc + c];
                rdx += dx;
                rdxs += dxs;
            }

            owave2(numrec, NULL, edge, precond, NULL, filternorm, Tab,
                   subwtrans, ri, edge_ri);
            ...
            for (J = stopdecim; J <= *numrec; J++)
            {
                ...
                if ((sr < s) && (sc < s))
                {
                    for (i = 0; i <= 3; i++)
                    {
                        ...
                        for (r = 0; r < subwtrans->images[J][i]->nrow; r++)
                        {
                            for (c = 0; c < subwtrans->images[J][i]->ncol;
                                 c++)
                                wtrans->images[J][i]->gray[rdx + c * s + sc] =
                                    subwtrans->images[J][i]->gray[rdxs + c];
...
void
dyowave2(int *NumRec, int *StopDecim, int *Ortho, int *Edge, int *Precond,
