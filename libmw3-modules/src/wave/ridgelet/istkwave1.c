...
/* mwcommand
 ...
 function ={"Reconstruction of a signal from its wavelet coefficients
            using Starck's algorithm"};
 usage = {
   np->np     "resolution np",
...
void istkwave1(int np, Fsignal in, Fsignal out)
{
    ...
    for (j = 1; j < np; j++)
    {
        ...
        if (!temp)
            mwerror(FATAL, 1, "not enough memory !\n");
        /* on place dans temp les details a la resolution 2^j, dans le
         * domaine de Fourier */
        ...
        /* les details sont additionnes a la somme des details des
         * resolutions plus fines. Comme ils sont au nombre de N/(2^j),
         * on les separe en 2 parties pour les ajouter aux  basses
         * frequences */
    ...
    /* il ne reste plus qu'a ajouter l'approximation restante */
