...
/* mwcommand
 ...
 function = {"Get the polar frequency grid used to perform ridgelet
            decomposition"};
 usage = {
...
void ridgrecpol(Fimage in_re, Fimage in_im, Fimage out_re, Fimage out_im)
{
    ...
    if (in_re == NULL)
        mwerror(USAGE, 1, "Input is empty...\n");

    n = in_re->nrow;

    if (!(n == in_re->ncol))
        mwerror(USAGE, 1, "The image is not square-shaped");
  ...
  /** j : jieme colonne de la ieme ligne **/
    ...
    for (i = 0; i < (n + 1); i++)
    {
        for (j = 0; j < (n + 1); j++)
        {
            ...
            if (indic1 == 1)
                out_re->gray[j1 + n * i] = in_re->gray[x1 + n * y1];
            if (indic2 == 1)
                out_im->gray[j1 + n * i] = in_im->gray[x1 + n * y1];
    ...
    for (i = 1; i < n; i++)
    {
        for (j = 0; j < (n + 1); j++)
        {
            ...
            if (indic1 == 1)
                out_re->gray[j1 + n * i + n * n] = in_re->gray[x1 + n * y1];
            if (indic2 == 1)
                out_im->gray[j1 + n * i + n * n] = in_im->gray[x1 + n * y1];
