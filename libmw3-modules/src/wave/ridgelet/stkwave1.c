...
/* mwcommand
 ...
 function ={"One-dimensional wavelet transform using Starck's
           algorithm (band-limited scaling function)"};
 usage = {
  np->np   "resolution np",
  in->in   "input in Fourier domain (the size of signal must be a power of 2)",
  out<-out "result in Fourier domain, from left to right : details and
           approximation"
};
*/
/*----------------------------------------------------------------------
...
#include <stdlib.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for fft1d() */
...
{
    if (fabs(x) > EPS)
        *out = 3. / 8. * pow(((sin(M_PI * x / 4.)) / (M_PI * x / 4.)), 4.);
    else
        *out = 3. / 8.;
}
...
/* p est l'inverse du pas d'echantillonnage de phi, p divise N,
 * outr est la partie reelle de la TF de phi, outi sa partie imaginaire */
{
    Fsignal phid;
    int i;
    double val, l;
    ...
    /* on translate les valeurs de phi pour x negatif a la suite des
     * valeurs de phi pour x positif */
    /* phid est le signal shifte */
    for (i = 0; i < (N / 2); i++)
    {
        l = i * 1. / p;
        phi(l, &val);
...
/*nombre de valeurs echantillonnees, N est une puissance de 2 */
/*TF du filtre h */
{
    ...
    /* on a neglige la partie imaginaire de phichapo */
...
/*si np=1,on obtient un vecteur de taille 2N, avec dans la premiere
 * partie les details, dans la seconde, l'approximation, l'un et
 * l'autre non decimes */
{
    ...
    for (j = 1; j < np; j++)
    {
        ...
        for (i = N / (pow(2.0, (double) (j + 1)));
             i < N / (pow(2.0, (double) j)); i++)
        {
        ...
        for (i = 2 * N - (N / pow(2.0, (double) (j - 1)));
             i < (2 * N - (N / pow(2.0, (double) j))); i++)
        {
