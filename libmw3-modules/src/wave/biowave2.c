...
usage = {
 ...
 'h':HaarNLevel->Haar            "Continue decomposition with Haar
                                 filter until level HaarNLevel",
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for sconvolve() */
...
static void
COMMENT(Wtrans2d result, Fimage image, int edge, int *filternorm, Fsignal ri1,
        Fsignal ri2)
...
static void
COMLINE_ERR(Fsignal ri1, Fsignal ri2, int edge, int *numrec, int *haar,
            int dy, int dx)
                        ...
                        /* Size of the image */
{
    ...
    if (!haar)
        if (edge >= 1)
        {
            if ((dx >> (*numrec) < ri1->size)
                || (dy >> (*numrec) < ri1->size))
                mwerror(FATAL, 1,
                        "The size of image must be greater "
                        "than the size of the i.r. 1!\n");

            if ((dx >> (*numrec) < ri2->size)
                || (dy >> (*numrec) < ri2->size))
                mwerror(FATAL, 1,
                        "The size of image must be greater "
                        "than the size of the i.r. 2!\n");
        }

}

static void NORM_FIL(Fsignal ri1, Fsignal ri2, int filternorm, int *haar)
...
static void HAAR_WAVEL2(Wtrans2d wtrans, int numrec, int haar)
                                /* Wavelet transform */
                                /* Level of average */
                                /* Continue decomposition with Haar wavelet
                                 * until ultimate level */
  ...
  /*--- Convolution of rows with low-pass filter ---*/
    ...
    /*--- Copy last point if number of columns is odd ---*/
        ...
        if (K2 > dx2)
        {
            if (haarx == 0)
                Tab->gray[lK2 + c] =
                    s2 * wtrans->images[J - 1][0]->gray[ldx + dx1];
            else
                Tab->gray[lK2 + c] =
...
void
biowave2(int *NumRec, int *Haar, int *Edge, int *FilterNorm, Fimage Image,
         Wtrans2d Output, Fsignal Ri1, Fsignal Ri2)
                              ...
                              /* Impulse responses of the low pass filters */
{
  ...
  /*--- Memory allocation for wavelet transform Output ---*/
    ...
    if (mw_alloc_biortho_wtrans2d(Output, *NumRec, Image->nrow, Image->ncol)
        == NULL)
