...
function = {"Compute the eigenvalues of an operator which is diagonal
           in a 2D-wavelet packet basis and approximate the
           convolution with a kernel (or its inverse)"};
usage = {
  'i':float->limit
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for wp2dfreqorder() */
...
{
    Fsignal order;
    /*for the correspondance between wavelet packets indexes and
     * frequency bands */
    int maxLevel = my_log(tree->ncol);  /*maximum possible level */
    int level;                  /*current level */
    int incr_x_tree, incr_y_tree;       /* increments in tree coordinates */
    int kx, ky, kx_tree, ky_tree;       /* for coordinates in tree and
                                         * pfilter */
    int kx1, ky1;               /*frequency band corresponding to tree
                                 * leave (kx,ky) */
    int x, y;                   /* used for the coordinates in filter */
    int x1, x2, y1, y2;         /* coordinates in filter */
    int sfx, sfy;               /*size of the frequency band (divided
                                 * by 2), in x and in y */
    float l2;                   /* computed value */
    int count;                  /* number of frequencies in a
                                 * frequency band */
    ...
    /* make correspondance between wavelet packet indexes and
     * frequency bands */
    ...
    for (level = 0, incr_x_tree = tree->ncol / 2, incr_y_tree =
         tree->nrow / 2, sfx = filter->ncol / 4, sfy = filter->nrow / 4;
         level < maxLevel;
         level++, incr_x_tree /= 2, incr_y_tree /= 2, sfx /= 2, sfy /= 2)
    {
        for (kx = 0; kx < tree->ncol; kx += incr_x_tree)
            for (ky = 0; ky < tree->nrow; ky += incr_y_tree)
                if (_(tree, kx, ky) == level + 1)
                {
                    ...
                    if (count == 0)
                        mwerror(FATAL, 1,
                                "size of filter should be larger "
                                "than size of pfilter !\n");
