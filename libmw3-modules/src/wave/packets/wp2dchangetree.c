...
usage = {
 'e':size->new_tree_size
     "The ouput Cimage describes the same tree but has the prescribed
     size (when possible).",
 'p'->prune_tree
 ...
 'M':Cimage->tree_for_max
     "The ouput tree asks for a decomposition if the Cimage OR the
     input tree ask for it",
 'm':Cimage->tree_for_min
     "The ouput tree asks for a decomposition if the Cimage AND the
     input tree ask for it",
...
#include "mw3.h"
#include "mw3-modules.h"         /* for wp2dchecktree() */

#define _(a,i,j) ((a)->gray[(j)*(a)->ncol+(i)])

/***************************************************************/
static void test_input(Cimage treeIn, char *up_tree, char *down_tree,
                       int *new_tree_size, char *prune_tree,
...
/* check input trees */
    ...
    if (nbOption != 1)
        mwerror(FATAL, 1,
                "[wp2dchangetree]One and only one option is needed !\n");
}
...
static void pruneTree(Cimage tree_in, Cimage tree_out)
{
    ...
    int extra;                  /* ratio between the size of tree_in
                                 * and the size of tree_out */
...
static void extendTree(Cimage tree_in, Cimage tree_out)
{
    int kx, ky;                 /* indexes for tree_in */
    int kx1, ky1;               /* indexes for tree_out */
    int extra;                  /* ratio between the size of tree_out
                                 * and the size of tree_in */
    ...
    if (tree_out->ncol < tree_in->ncol || tree_out->nrow < tree_in->nrow)
        mwerror(FATAL, 1,
                "The output tree Cimage is too small to contain the tree !\n");
...
static void resize_trees(Cimage tree_in1, Cimage tree_in2, Cimage tree_out1,
                         Cimage tree_out2)
{
    int finalSize;
    Cimage pruned1;
    ...
    finalSize =
        (pruned1->ncol > pruned2->ncol) ? pruned1->ncol : pruned2->ncol;
...
static void max_tree(Cimage tree_in1, Cimage tree_in2, Cimage tree_out)
{
    ...
    for (i = 0; i < size; i++)
        tree_out->gray[i] =
            (tree_in1->gray[i] >
             tree_in2->gray[i]) ? tree_in1->gray[i] : tree_in2->gray[i];
                    ...
                    Cimage tree_for_max, Cimage tree_for_min)
{
    ...
    test_input(treeIn, up_tree, down_tree, new_tree_size, prune_tree,
               tree_for_max, tree_for_min);
    ...
    if (up_tree)
    {
        ...
        pruneTree(treeIn, treeInPruned);
        /* prune treeIn before processing */
        ...
        else
        {
            ...
            treeTmp =
                mw_change_cimage(treeTmp, treeInPruned->nrow,
                                 treeInPruned->ncol);
            if (treeTmp == NULL)
                mwerror(FATAL, 1, "Not enough memory !\n");
    ...
    /* the output tree does not decompose the more decomposed leaves
     * of input tree */
    else if (down_tree)
    {
        otherTreeInPruned =
            mw_change_cimage(NULL, treeIn->nrow, treeIn->ncol);
    ...
    else if (new_tree_size)
    {
        ...
        pruneTree(treeIn, treeInPruned);
        /*prune treeIn before extending it */
    ...
    /* Computes a tree which asks for a decomposition if treeIn OR
     * tree_for_max asks for it */
    ...
    else if (tree_for_max)
    {
        ...
        treeTmp =
            mw_change_cimage(treeTmp, treeInPruned->nrow, treeInPruned->ncol);
        if (treeTmp == NULL)
            mwerror(FATAL, 1, "Not enough memory !\n");
    ...
    else if (tree_for_min)
    {
        ...
        min_tree(treeInPruned, otherTreeInPruned, treeInPruned);
        /* treeInPruned is used for the output for simplicity */
    ...
    /* checks result is a tree (this should be OK, if 'wp2dchangetree'
     * is correct) */
