...
function = {"Rapidely transform a 2D-wavelet packet decomposition into
           another one, by specifying a new tree"};
usage = {
    tree_out->tree_out
...
#include "mw3.h"
#include "mw3-modules.h"         /* for wpsconvolve(), wp2dchangetree() */
...
static void ondel(Wpack2d pack, int img11, int img12, int img21, int img22,
                  int futur_level)
                 ...
                 /* index of the high in x -high in y image */
{
    ...
    pack->images[img11] =
        mw_change_fimage(pack->images[img11], dy_new, dx_new);
    if (pack->images[img11] == NULL)
        mwerror(FATAL, 1, "Not enough memory !\n");

    pack->images[img21] =
        mw_change_fimage(pack->images[img21], dy_new, dx_new);
    if (pack->images[img21] == NULL)
        mwerror(FATAL, 1, "Not enough memory !\n");

    pack->images[img12] =
        mw_change_fimage(pack->images[img12], dy_new, dx_new);
    if (pack->images[img12] == NULL)
        mwerror(FATAL, 1, "Not enough memory !\n");

    pack->images[img22] =
        mw_change_fimage(pack->images[img22], dy_new, dx_new);
    if (pack->images[img22] == NULL)
        mwerror(FATAL, 1, "Not enough memory !\n");
    ...
    for (y = 0; y < dy_new; y++)
    {
        /*convolution of each raw for hor1 (contains low pass) */
        for (x = 0; x < dx; x++)
    ...
    for (y = 0; y < dy_new; y++)
    {
        /*convolution of each raw for hor2 (contains high pass) */
    ...
    band = NULL;
    mw_delete_fsignal(signal_in);
    signal_in = NULL;
    mw_delete_fsignal(signal_out);
    signal_out = NULL;
    mw_delete_fimage(hor1);
    hor1 = NULL;
    mw_delete_fimage(hor2);
    hor2 = NULL;
...
static void iondel(Wpack2d pack, int img11, int img12, int img21, int img22,
                   int futur_level)
                   /* the initial image corresponds to the index  img11 */
                 /* index of the low in x -low in y image */
                 /* index of the low in x -high in y image */
                 ...
                 /* index of the high in x -high in y image */
{
    ...
    mw_delete_fsignal(inx);
    inx = NULL;
    mw_delete_fsignal(outx);
    outx = NULL;
    ...
    pack->images[img11] =
        mw_change_fimage(pack->images[img11], dy_out, dx_out);
    if (pack->images[img11] == NULL)
        mwerror(FATAL, 1, "Not enough memory !\n");
    ...
    oddx = NULL;
    oddy = NULL;
    mw_delete_fsignal(iny);
    iny = NULL;
    mw_delete_fsignal(outy);
    outy = NULL;
    mw_delete_fimage(tmp_result);
    tmp_result = NULL;
...
void wp2dchangepack(Wpack2d pack_in, Wpack2d pack_out, Cimage tree_out)
{
    ...
    treesize =
        (tree_out->ncol >
         pack_in->tree->ncol) ? tree_out->ncol : pack_in->tree->ncol;
  ...
  /*--- extention of the tree sothat the two trees have the same size --*/
    ...
    wp2dchangetree(tree_out, extended_tree_out, NULL, NULL, &treesize, NULL,
                   NULL, NULL);
    ...
    for (futur_level = 1; futur_level <= level; futur_level++)
    {
        ...
        for (kx = 0; kx < treesize; kx += jump_2)
            for (ky = 0; ky < treesize; ky += jump_2)
                if (_(extended_tree_out, kx, ky) >= futur_level
                    && _(temppack->tree, kx, ky) < futur_level)
                {
                    pos = kx + ky * treesize;
    ...
    /* pruning of the tree and the Wpack2d structure sothat it
     * corresponds  to */
    ...
    temppack = NULL;
    mw_delete_cimage(extended_tree_out);
    extended_tree_out = NULL;
}
