                ...
                char *wave_flag, char *full_flag, char *mirror_flag)
{
    ...
    if (nbOptions > 1)
        mwerror(FATAL, 1,
                "One and only one tree type has to be selected !\n");
    else if (nbOptions == 0)
        mwerror(FATAL, 1,
                "One and only one tree type has to be selected !\n");
    ...
    {                           /* We do not need to reorder the
                                 * coefficient for a wavelet basis */
        ...
        for (x = 0; x < tree_size; x++)
            for (y = 0; y < tree_size; y++)
                /* We do not need to reorder the coefficient for a
