...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for w2dfreqreorder(), wp2ddecomp() */
    ...
    for (k = 0; k < pack->img_array_size; k++)
        if (pack->images[k])
        {
            ...
            for (i = 0; i < size; i++)
                max =
                    (max >
                     pack->images[k]->gray[i]) ? max : pack->images[k]->
                    gray[i];
        ...
        if (pack->images[k])
        {
            ...
            if (max != min)
            {
                ...
                for (i = 0; i < sxy; i++)
                {
                    tmp = pack->images[k]->gray[i];
                    if (tmp < mean)
                        pack->images[k]->gray[i] =
                            tmpSmaller * (tmp - min) + min0;
                    else
                        pack->images[k]->gray[i] =
                            tmpLarger * (tmp - mean) + mean0;
                            ...
                            /* color for the background */
{
    ...
    Fimage tmpImg;              /*contains the result, before it is
                                 * copied in the Wpack2d structure */
    ...
    for (k = 1; k < pack->img_array_size; k++)
        if (pack->images[k])
        {
            ...
            pack->images[k] =
                mw_change_fimage(pack->images[k], syFinal, sxFinal);
...
/***************************************************************/
static void glue(Wpack2d pack, int img, int img11, int img12, int img21,
                 int img22, float backgroundColor)
                            ...
                            /* color for the background */
{
    ...
    int sx1 = (sx11 > sx21) ? sx11 : sx21;
    /* image11 must fit into a window of size sx1 and sy1 */
    int sy1 = (sy11 > sy12) ? sy11 : sy12;
    /* image12 must fit into a window of size sx2 and sy1 */
    int sx2 = (sx12 > sx22) ? sx12 : sx22;
    /* image21 must fit into the window of size sx1 and sy2 */
    int sy2 = (sy21 > sy22) ? sy21 : sy22;
    /* image22 must fit into the window of size sx2 and sy2 */
    int tx, ty;                 /*translation vector */
    int sx = sx1 + sx2, sy = sy1 + sy2; /*size of the final image */
    int x, y;                   /*index in the image */
    Fimage result;
    /* contains the result before it is copied in the Wpack2d structure */
...
/***************************************************************/
static void wpack2dGlue(Wpack2d pack, float backgroundColor,
     ...
     /* glue all the images of pack->images together */
{
    ...
    int img;
    /* index of the element of pack->images where the result is writen */
    int img11, img12, img21, img22;
    /* index of the element of pack->images to be glued */
    ...
    for (; level >= 0; level--)
    {
        for (ky = 0, ky1 = jump; ky1 < treesize; ky += jump_2, ky1 += jump_2)
            for (kx = 0, kx1 = jump; kx1 < treesize;
                 kx += jump_2, kx1 += jump_2)
            {
                img = kx + ky * treesize;
                ...
                if (pack->images[img11] && pack->images[img21]
                    ...
                    glue(pack, img, img11, img12, img21, img22,
                         backgroundColor);
     ...
     /*displays a wavelet packet transform */
{
        ...
        /* the wavelet packet is given */
    {
        if (tree || Ri_biortho)
            mwerror(USAGE, 1,
                    "When you give the wavelet packet "
                    "with option -p, do not input also "
                    "the tree/image/impulse response !\n");
    ...
    else
    {
        if (!tree)
            mwerror(USAGE, 1,
                    "Option -p is needed when inputs "
                    "tree/image/impulse response are not given !\n");
    ...
    /* backgroundColor contains the max of the images, 'xv' displays
     * it white */
    ...
    /* glue the wavelet packet coefficients contained in the images of
     * the wpack2d */
    ...
    if (toDisplay)
    {
        mw_change_fimage(toDisplay, pack->images[0]->nrow,
                         pack->images[0]->ncol);
