...
static void COMMENT(Fsignal result, Wtrans1d wtrans, int edge,
                    int *filternorm)
...
static void
COMLINE_ERR(Wtrans1d wtrans, Fsignal ri1, Fsignal ri2, int edge, int *numrec,
            long int size)
...
/* Size of the signal */
{
  ...
  /*--- Size of image ---*/
    ...
    if ((size >> (*numrec - 1) < ri1->size)
        ...
        mwerror(FATAL, 1,
                "The size of signal must be greater "
...
static void
INV_WAVEL(int J, Wtrans1d wtrans, Fsignal S, Fsignal ri1, Fsignal ri2,
          int *edge)
 ...
 * in ibiowave1) */
{
  ...
  /*--- Reconstruction from the average ---*/
    ...
    sconvolve(wtrans->A[J][0], Tabout, &DECIM, &INTERPOL, NULL, NULL, edge,
              &PROLONG, ri1, NULL);
  ...
  /*--- Reconstruction from the detail ---*/
    ...
    sconvolve(wtrans->D[J][0], Tabout, &DECIM, &INTERPOL, NULL, &HIGH, edge,
              &PROLONG, ri2, NULL);
...
void
ibiowave1(int *NumRec, int *Edge, int *FilterNorm, Wtrans1d Wtrans,
