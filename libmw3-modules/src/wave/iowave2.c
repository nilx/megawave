...
usage = {
 ...
 'h':HaarNLevel->Haar            "Start reconstruction with Haar
                                 filter from level HaarNLevel down to
                                 level NLevel + 1",
...
static void
COMLINE_ERR(Wtrans2d wtrans, Fsignal ri, Fimage edge_ri, int edge,
            int *inverse, int precond, int *numrec, int *haar)
  ...
  /*--- Size of image ---*/
    ...
    if (!haar)
        if ((dx >> (*numrec) < ri->size) || (dy >> (*numrec) < ri->size))
            mwerror(FATAL, 1,
                    "The size of average must be "
                    "greater than the size of the i.r.!\n");
...
static void
COMMENT(Fimage result, Wtrans2d wtrans, int edge, int precond,
        int *filternorm)
...
static void INIT_SHRINK(Wtrans2d wtrans, Fsignal ri, short int numrec,
                        int edge)
 ...
 * (see `Edge` in iowave2) */
{
    ...
    if ((edge == 2) || (edge == 0))
    {
        ...
        for (j = 1; j <= numrec; j++)
        {
            ...
            ncol[j] =
                firstphigh + (ncol[j - 1] - 3 + ri->size +
                              (long) ri->shift) / 2 + 1;
            if ((wtrans->images[j][2]->ncol != ncol[j])
                || (wtrans->images[j][3]->ncol != ncol[j]))
                mwerror(FATAL, 1, "Bad number of column in detail!\n");

            ncol[j] =
                firstplow + (ncol[j - 1] - 1 - (long) ri->shift) / 2 + 1;
            ...
            nrow[j] =
                firstphigh + (nrow[j - 1] - 3 + ri->size +
                              (long) ri->shift) / 2 + 1;
            if ((wtrans->images[j][1]->nrow != nrow[j])
                || (wtrans->images[j][3]->nrow != nrow[j]))
                mwerror(FATAL, 1, "Bad number of row in detail!\n");

            nrow[j] =
                firstphigh + (nrow[j - 1] - 1 - (long) ri->shift) / 2 + 1;
 ...
 * from Haar level */
{
    double s2;                  /* Sum of `ri`'s coefficients and
                                 * sum's square root
    ...
    for (j = haar - 1; j >= numrec; j--)
    {
        ...
        if (j < jx)
        {
            ...
            for (r = 0; r < dy[j]; r++)
                for (c = 0; c < dx2; c++)
                {
                    Tab->gray[r * dxtab + 2 * c] =
                        wtrans->images[numrec][0]->gray[r * dxtab + c] +
                        wtrans->images[numrec][0]->gray[r * dxtab +
                                                        dx[j + 1] + c];
                    Tab->gray[r * dxtab + 2 * c + 1] =
                        wtrans->images[numrec][0]->gray[r * dxtab + c] -
                        wtrans->images[numrec][0]->gray[r * dxtab +
                                                        dx[j + 1] + c];
                }
            ...
            for (r = 0; r < dy[j]; r++)
                for (c = 0; c < dx2; c++)
                {
                    ...
                    wtrans->images[numrec][0]->gray[r * dxtab + dx2 + c] =
                        cnorm * Tab->gray[r * dxtab + dx2 + c];
                }
            if (dx[j] % 2 == 1)
                for (r = 0; r < dy[j]; r++)
                    wtrans->images[numrec][0]->gray[r * dxtab + dx[j] - 1] =
                        Tab->gray[r * dxtab + dx[j] - 1] / shaar;
        }
        ...
        if (j < jy)
        {
            ...
            for (r = 0; r < dy2; r++)
                for (c = 0; c < dx[j]; c++)
                {
                    Tab->gray[2 * r * dxtab + c] =
                        wtrans->images[numrec][0]->gray[r * dxtab + c] +
                        wtrans->images[numrec][0]->gray[(r + dy[j + 1]) *
                                                        dxtab + c];
                    Tab->gray[(2 * r + 1) * dxtab + c] =
                        wtrans->images[numrec][0]->gray[r * dxtab + c] -
                        wtrans->images[numrec][0]->gray[(r + dy[j + 1]) *
                                                        dxtab + c];
                }
            ...
            for (r = 0; r < dy2; r++)
                for (c = 0; c < dx[j]; c++)
                {
                    wtrans->images[numrec][0]->gray[r * dxtab + c] =
                        cnorm * Tab->gray[r * dxtab + c];
                    wtrans->images[numrec][0]->gray[(r + dy2) * dxtab + c] =
                        cnorm * Tab->gray[(r + dy2) * dxtab + c];
                }
            if (dy[j] % 2 == 1)
                for (c = 0; c < dx[j]; c++)
                    wtrans->images[numrec][0]->gray[(dy[j] - 1) * dxtab + c] =
                        Tab->gray[(dy[j] - 1) * dxtab + c] / shaar;
        }
...
static void
RECOMP_LINES(Fimage Tab, Fimage A, int J, int *haar, int *band,
             Fsignal int_ri, int prolong, int *edge, Fimage edge_ri)
 ...
 * edge processing */
{
  ...
  /*--- Initialization of Tabin and Tabout ---*/
    ...
    for (l = 0; l < dy; l++)
    {
        ...
        if (haarx == 0)
            sconvolve(Tabin, Tabout, &DECIM, &INTERPOL, NULL, band, edge,
                      &prolong, int_ri, edge_ri);
        else
            sconvolve(Tabin, Tabout, &DECIM, &INTERPOL, NULL, band,
...
static void
RECOMP_COLUMNS(Fimage Tab, Fimage AD, int J, int *haar, int *band,
               Fsignal int_ri, int prolong, int *edge, Fimage edge_ri)
...
static void
INV_WAVEL(int J, int *haar, Wtrans2d wtrans, Fimage A, Fsignal int_ri,
          int *inverse, int *edge, Fimage edge_ri)
 ...
 * edge processing */
{
    ...
    if (!inverse || (*edge == 1) || (*edge == 3))
    {
        ...
        if ((A->nrow != L2) || (A->ncol != K2))
        {
            ...
            if (A == NULL)
                mwerror(FATAL, 1,
                        "Allocation of buffer for average "
                        "at level %d refused!\n", J);
        }
    ...
    else
    {
        ...
        if (A == NULL)
            mwerror(FATAL, 1,
                    "Reallocation of buffer for "
                    "reconstructed average refused!\n");
    }
  ...
  /*--- Initialization of Tab ---*/
    ...
    if (mw_alloc_fimage(Tab, L2, dx) == NULL)
        mwerror(FATAL, 1,
                "Allocation of buffer for sum's separation refused!\n");
  ...
  /*--- Reconstruction along the columns from the average ---*/
    ...
    RECOMP_COLUMNS(Tab, wtrans->images[J][0], J, haar, NULL, int_ri, prolong,
                   edge, edge_ri);

  /*--- Reconstruction along the columns from the detail D1 ---*/

    RECOMP_COLUMNS(Tab, wtrans->images[J][1], J, haar, &HIGH, int_ri, prolong,
                   edge, edge_ri);
  ...
  /*--- Reconstruction along the columns from the detail D2 ---*/
    ...
    RECOMP_COLUMNS(Tab, wtrans->images[J][2], J, haar, NULL, int_ri, prolong,
                   edge, edge_ri);

  /*--- Reconstruction along the columns from the detail D3 ---*/

    RECOMP_COLUMNS(Tab, wtrans->images[J][3], J, haar, &HIGH, int_ri, prolong,
                   edge, edge_ri);
...
void
iowave2(int *NumRec, int *Haar, int *Edge, int *Precond, int *Inverse,
 ...
 * matrices */
{
  ...
  /*--- Preconditionning of resume's edges (if selected) ---*/
    ...
    if (*Precond >= 2)
        precond2d(NULL, Wtrans->images[*NumRec][0],
                  Wtrans->images[*NumRec][0], Edge_Ri);
  ...
  /*--- Wavelet recomposition ---*/
    ...
    for (J = *NumRec; J > 1; J--)
        INV_WAVEL(J, Haar, Wtrans, Wtrans->images[J - 1][0], Ri, Inverse,
                  Edge, Edge_Ri);
