...
function = {"Generates a fimage for visualization of orthonormal
  ...
  /*--- Copy the low-level average sub-image ---*/
    ...
    for (J = *NumRec; J >= 1; J--)
    {
      ...
      /*--- by a constant ---*/
        ...
        {
            for (c = 0; c < lx; c++)
                Output->gray[ldx + xd1 + c] =
                    fabs(Wtrans->images[J][1]->gray[llx + c]) * *Contrast;
            llx += lx;
            ldx += dx;
        }
        ...
        {
            for (c = 0; c < lx; c++)
                Output->gray[ldx + xd2 + c] =
                    fabs(Wtrans->images[J][2]->gray[llx + c]) * *Contrast;
            llx += lx;
            ldx += dx;
        }
        ...
        {
            for (c = 0; c < lx; c++)
                Output->gray[ldx + xd3 + c] =
                    fabs(Wtrans->images[J][3]->gray[llx + c]) * *Contrast;
            llx += lx;
            ldx += dx;
        }
