...
usage = {
 ...
 'D':D->D  "Use Donoho threshold to denoise signal with additive
        ...
        for (j = 1; j <= in->nlevel; j++)
        {
            for (i = 0; i < in->D[j][0]->size; i++)
                if (fabs(in->D[j][0]->values[i]) <= T)
                {
                    ...
                    if (M)
                        M->values[offset + i] = 0.0;
                }
                else if (M)
                    M->values[offset + i] = 1.0;
                   ...
                   /* Input wavelet transform */
{
    ...
    if (fabs(p - P) >= 0.1)
        mwerror(FATAL, 1,
                "Cannot get threshold bound (obtained=%g, required=%g)!\n", p,
                P);
    return (T);
}
                   ...
                   /* Output wavelet transform */
{
    ...
    if (in->nvoice > 1)
        mwerror(FATAL, 1,
                "Does not work with more than one voice per octave !\n");
    if (in->complex > 0)
        mwerror(FATAL, 1, "Does not work with complex wavelet transform !\n");
    ...
    if (P)
        t = get_threshold_bound(in, *P);
    if (D)
        t = (float) *D * sqrt(2.0 * log((double) in->size));
    if (T)
        t = *T;
