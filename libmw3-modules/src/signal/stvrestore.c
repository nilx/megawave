...
function = {"Restoration of signal using total variation and
            thresholding in orthonormal bases"};
usage = {
...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for owave1(), iowave1(), smse() */
...
static int GetWaveletDepth(Fsignal M)
{
    ...
    for (n = 0; n < N; n++)
    {
        ...
        if ((v != 1.) && (v != -1.) && (v != 0.) && (v != 2.))
            mwerror(FATAL, 1,
                    "[GetWaveletDepth] Inconsistent mask M "
                    "(consistent values are -1,0,1 and 2 only) !\n");
    }

    for (n0 = 0; (n0 < N) && (M->values[n0] != 2.0); n0++);
    if (n0 >= N)
        mwerror(FATAL, 1,
                "[GetWaveletDepth] Invalid mask M : 2 not found !\n");
    for (n = n0; (n < N) && (M->values[n0] == 2.0); n++);
    if (n != N)
        mwerror(FATAL, 1,
                "[GetWaveletDepth] Invalid mask M : "
                "does not end with 2 !\n");
    if ((N % (2 * (n - n0))) != 0)
        mwerror(FATAL, 1,
                "[GetWaveletDepth] Invalid mask M : "
                 ...
                 */
{
    ...
    if (!relax)
        /* Constraint on approximation space V_J with J = - nlevel */
    {
...
static void Proj(char *relax, Fsignal dJ, Fsignal M, Fsignal O, Fimage I,
                 int *r)
/* If != NULL, relax constraint on approximation space V_J.
 * Warning : on versions < 1.4, this option was active (bug) !
 */
{
    ...
    if (mwdbg != 0)
        mw_delete_fsignal(dJ0);
}

/* Generate comments in output signal u
*/

static void Make_Comments(Fsignal u, double tv, int Niter, double *alpha,
                          Fsignal O, Fimage I)
{
    char A[BUFSIZ];
    ...
    if (O)
    {
        strcat(u->cmt, O->name);
        if (I)
            sprintf(A, " on interval");
        else
            sprintf(A, " periodized");
        strcat(u->cmt, A);
    }
                ...
                /* Input thresholded signal */
{
    ...
    if (!O)
        mwerror(FATAL, 1, "At this time option -O is needed !\n");
    ...
    for (n = 1; n <= *Niter; n++)
    {
        ...
        /* Compute J(u) and dJ */
        if (!alpha)
            tv = J(u, dJ) / (double) u->size;
        else
            tv = J_alpha(u, dJ, *alpha) / (double) u->size;
        ...
        if ((mwdbg == 1) || (n == 1) || (n == *Niter) || ((n % 1000) == 0))
        {
            if (!alpha)
                ...
                printf
                    ("\t [%d/%d] smoothed total variation J(u)=%g  step t=%g\n",
                     n, *Niter, tv, t);
        }
    ...
    if (V)
        Make_Comments(V, mintv, minNiter, alpha, O, I);
