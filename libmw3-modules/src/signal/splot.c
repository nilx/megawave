...
#include <math.h>
#include "mw3.h"
#include "mw3-modules.h"         /* for ccputstring() */
...
static void draw_framed_ccimage(Ccimage u, int xa, int ya, int xb, int yb,
                                unsigned char r, unsigned char g,
                                unsigned char b, int xmin, int xmax, int ymin,
                                int ymax)
{
...
static void plot_signal(void)
{
    ...
    int i, k, x, y, n1, n2, line, zero, x1, x2, ox = 0, oy =
        0, fgcolor, bgcolor, max, xsub, ysub;
    char str[STRSIZE];
    ...
    /*---------- DRAW AXES ----------*/
        ...
        max = 0;
        for (k = 0; (v = yofs + (double) k * ystep * (double) ysub) <= sy2;
             k++)
        {
        ...
        for (k = 0; (v = xofs + (double) k * xstep) <= sx2; k++)
        {
            ...
            if (x >= X1 && x <= X2)
            {
                ...
                else
                {
                    ...
                    case 4:
                    case 3:
                        mw_draw_ccimage(image, x, Y1, x, Y2, 255, 200, 200);
                    case 2:
  ...
  /*---------- PLOT SIGNAL ----------*/
    ...
    for (i = n1; i <= n2; i++)
    {
        ...
        case 2:                /* boxes */
            v = (double) signal->shift + (double) i *(double) signal->scale;
            x1 = X1 + (int) ((double) (X2 - X1) * (v - sx1) / (sx2 - sx1)) +
                1;
            v = (double) signal->shift + ((double) i +
                                          1.) * (double) signal->scale;
        ...
        case 1:                /* lines */
            if (line)
            {
                draw_framed_ccimage(image, ox, oy, x, y, 0, 0, 0, X1 + 1,
                                    X2 - 1, Y1 + 1, Y2 - 1);
                break;
            }
        ...
        case 0:                /* points */
            if (x > X1 && x < X2 && y > Y1 && y < Y2)
                mw_plot_ccimage(image, x, y, 0, 0, 0);
        }
...
static void init_sxy(void)
{
    ...
    sx2 =
        (double) signal->shift + (double) (signal->size -
                                           1) * (double) signal->scale;
    if (draw_mode == 2)
...
static int win_notify(Wframe * window, void *param)
{
    ...
    case W_MS_LEFT:
        ...
        else
        {
            selrect = 1;
            copy_image =
                mw_change_ccimage(copy_image, image->nrow, image->ncol);
            if (!copy_image)
