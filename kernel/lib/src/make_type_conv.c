...
Node AddNodeAndConv(char *a, char *b)
{
     Node A,B;
     int i;
...
void GetAccessibleNodes(Node s)
{
     Node u,v;
     int i;
...
int GetMinimalPath(char *p, Node s, Node v)
{
     int ret;
     char T[TNAME+1];
...
void PrintBeginTypeIn(FILE *fp, char *typein)
{
...
void PrintEndTypeIn(FILE *fp, char *typein)
{
...
void GetTypeInPath(char *Path, int p, char *type)
{
     int i0,i1,n=1;
...
void PrintPath(FILE *fp, char *typeout, char *Path)
{
     int i,l;
...
void PrintConvInternalType(FILE *fp)
{
     Node A,B;
     int i;
     char Path[BUFSIZ];
...
int CheckIfPathExists(Node A, Node B)
{
     int i;
     char Path[BUFSIZ];
...
void PrintLoadETypeToIType(FILE *fp)
{
     Node A,B;
     int i;
     char Path[BUFSIZ];
...
int get_max_range_array(char *a, char **A)
{
     int i,j;
...
void put_range_array(char *a, char *b, int r, char **A)
{
     int i,j;
...
void PrintCreateETypeFromIType(FILE *fp)
{
     Node A,B;
     int i,r;
     char TypeA[TNAME];
...
void PrintPossibleConvType(FILE *fp)
{
     Node A,B;
     int i;
     char Path[BUFSIZ];
...
int get_conv_type(char *line, char *a, char *b)
{
...
FILE *open_out_C_file(char *command)
{
     FILE *fp;
...
FILE *open_out_I_file(char *command)
{
     FILE *fp;
