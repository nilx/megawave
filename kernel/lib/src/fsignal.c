     ...
     int N;
{
  ...
  if (signal == NULL)
    {
      mwerror(ERROR, 0, 
	      "[mw_alloc_fsignal] cannot alloc array : fsignal structure is NULL\n");
      return(NULL);
    }
  ...
  if (signal->values != NULL)
    {
      mwerror(ERROR, 0,
	      "[mw_alloc_fsignal] Attempts to alloc a fsignal which is already allocated\n");
      return(NULL);
    }
...

{
  if (signal == NULL)
    {
      mwerror(ERROR, 0,
	      "[mw_delete_fsignal] cannot delete : fsignal structure is NULL\n");
      return;
    }
