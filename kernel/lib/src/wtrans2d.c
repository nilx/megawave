...
void *_mw_alloc_wtrans2d_norient(wtrans,level,nrow,ncol,Norient,sampling)

     Wtrans2d wtrans;
     int level,Norient; 
     int nrow,ncol;    /* Size of the original image */
     int sampling;     /* 1 if sampling (0 in the dyadic or continuous case) */
{
  int dx,dy,j,d,jj,dd;
...
*/
      ...
      dx = ncol/2;
      dy = nrow/2;
      for (j=1;j<=level;j++, dx/=2, dy/=2) for (d=0;d<=Norient;d++)
	{
/*
	  if ( ((dx % 2) != 0) || ((dy % 2) != 0))
	    {
...
*/
    ...
    for (j=1;j<=level;j++) for (d=0;d<=Norient;d++)
      {
	wtrans->images[j][d] = mw_change_fimage(NULL,dy,dx);
	if (wtrans->images[j][d] == NULL) 
	  {
