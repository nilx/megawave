...
Point_plane mw_change_point_plane(point)
     Point_plane point;
{
  if (point == NULL) point = mw_new_point_plane();
  return(point);
}
...
Shape sh;
{
  ...
  do
    if((sh = sh->parent) == NULL)
      {
	mwerror(ERROR, 0,
		"[mw_get_parent_shape] the root of the shapes is removed\n"); 
	return(NULL);
      }
  while(sh->removed);
  return(sh);
...
Shape sh;
{
  ...
  if(sh == NULL)
    {
      mwerror(ERROR, 0,
	      "[mw_get_first_child_shape] input shape is NULL\n"); 
      return(NULL);
    }
  for(sh = sh->child; sh != NULL; sh = sh->next_sibling)
    if((NotSup = mw_get_not_removed_shape(sh)) != NULL)
...
Shape mw_get_smallest_shape(shs, iX, iY)
Shapes shs;
int iX, iY;
{
  Shape sh;
  ...
  if (sh == NULL)
    {
      mwerror(ERROR, 0,
	      "[mw_get_smallest_shape] smallest shape is NULL\n"); 
      return(NULL);
    }  
  if(sh->removed)
     ...
     
{
  ...
  if (shs == NULL) 
    {
      mwerror(ERROR, 0, 
	      "[mw_alloc_shapes] cannot alloc root : shapes structure is NULL\n");
      return(NULL);
    }
  ...
  if ((shs->the_shapes!=NULL)||(shs->smallest_shape!=NULL))
    {
      mwerror(ERROR, 0,
	      "[mw_alloc_shapes] Attempts to alloc root which is already allocated\n");
      return(NULL);
    }
  ...
  for (i=size-1; i>=0; i--) shs->smallest_shape[i] = root;
     ...
     
{
  ...
  if (shs->the_shapes != NULL) free(shs->the_shapes);
  if (shs->smallest_shape != NULL) free(shs->smallest_shape);
...

{
 int i;
  ...
  if (shs == NULL)
    {
      mwerror(ERROR, 0,
	      "[mw_delete_shapes] cannot delete : shapes structure is NULL\n");
      return;
    }
