/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   Basic routines to convert internal MegaWave2 formats of structures
...
void _mw_float_to_uchar(ptr_float,ptr_uchar,N,data_name)

register float *ptr_float;
register unsigned char *ptr_uchar;
int N;  /* Number of samples */
char data_name[BUFSIZ];
...
void _mw_uchar_to_float(ptr_uchar,ptr_float,N)
...
/* The array of 24 bits has to be filled like the XV pic24 structure:
   i.e. Offset:   1    2    3    4    5    6    ...
        Pixel:    r    g    b    r    g    b    ...
*/
void _mw_1x24XV_to_3x8_ucharplanes(ptr,ptr1,ptr2,ptr3,N)

register unsigned char *ptr,*ptr1,*ptr2,*ptr3;
int N; /* Number of samples in <ptr> */
...
/* The array of 24 bits is filled like the XV pic24 structure:
   i.e. Offset:   1    2    3    4    5    6    ...
        Pixel:    r    g    b    r    g    b    ...
*/
void _mw_3x8_to_1x24XV_ucharplanes(ptr1,ptr2,ptr3,ptr,N)
...
Cimage mw_fimage_to_cimage(image_fimage)
...

{
  ...
  image = mw_change_cimage(NULL,image_fimage->nrow,image_fimage->ncol);
  if (image == NULL)
    {
...
Fimage mw_cimage_to_fimage(image_cimage)

Cimage image_cimage;

{ Fimage image;
  
  image = mw_change_fimage(NULL,image_cimage->nrow,image_cimage->ncol);
  if (image == NULL)
    {
...
Ccimage mw_cfimage_to_ccimage(image_cfimage)
...

{
  ...
  image = mw_change_ccimage(NULL,image_cfimage->nrow,image_cfimage->ncol);
  if (image == NULL)
    {
...
Cfimage mw_ccimage_to_cfimage(image_ccimage)

Ccimage image_ccimage;

{ Cfimage image;
  
  image = mw_change_cfimage(NULL,image_ccimage->nrow,image_ccimage->ncol);
  if (image == NULL)
    {
...
Fimage mw_cfimage_to_fimage(image_cfimage)
...

{
  ...
  image = mw_change_fimage(NULL,image_cfimage->nrow,image_cfimage->ncol);
  if (image == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create Fimage\n");
      return(NULL);
...
Cfimage mw_fimage_to_cfimage(image_fimage)
...
{ Cfimage image;
  ...
  image = mw_change_cfimage(NULL,image_fimage->nrow,image_fimage->ncol);
  if (image == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create Cfimage !\n");
      return(NULL);
...
Cimage mw_ccimage_to_cimage(image_ccimage)
...
{ Cimage image;
  ...
  image = mw_change_cimage(NULL,image_ccimage->nrow,image_ccimage->ncol);
  if (image == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create Cimage !\n");
      return(NULL);
...
Ccimage mw_cimage_to_ccimage(image_cimage)
...

{
  ...
  image = mw_change_ccimage(NULL,image_cimage->nrow,image_cimage->ncol);
  if (image == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create Ccimage\n");
      return(NULL);
...
Polygons mw_curves_to_polygons(curves)
...
Fpolygons mw_fcurves_to_fpolygons(fcurves)
...

{
  ...
  fpolys = mw_change_fpolygons(NULL);
  if (fpolys == NULL)
    {
...
Fcurves mw_curves_to_fcurves(curves)
...

{
  ...
  fcurves = mw_change_fcurves(NULL);
  if (fcurves == NULL)
    {
...
Curves mw_fcurves_to_curves(fcurves)
...
Fpolygons mw_polygons_to_fpolygons(polys)
...
Polygons mw_fpolygons_to_polygons(fpolys)
...
Fcurves mw_fpolygons_to_fcurves(fpolys)
...
Curves mw_polygons_to_curves(polys)
...
Fpolygons mw_curves_to_fpolygons(curves)
...
{ Fcurves fcurves;
  ...
  fcurves = mw_curves_to_fcurves(curves);
  fpolys = mw_fcurves_to_fpolygons(fcurves);
  mw_delete_fcurves(fcurves);      
  return(fpolys);
}
...
Curves mw_fpolygons_to_curves(fpolys)
...
Polygons mw_fcurves_to_polygons(fcurves)
...
{ Curves curves;
  ...
  curves = mw_fcurves_to_curves(fcurves);
  polys = mw_curves_to_polygons(curves);
  mw_delete_curves(curves);      
  return(polys);
}
...
Fcurves mw_polygons_to_fcurves(polys)
...
Polygon mw_curve_to_polygon(curve)
...
Fpolygon mw_fcurve_to_fpolygon(fcurve)
...

{
  ...
  fpoly = mw_change_fpolygon(NULL,0);
  if (fpoly == NULL)
    {
...
Point_fcurve mw_point_curve_to_point_fcurve(pcurve)

Point_curve pcurve;

{
  Point_curve pp;
  Point_fcurve point, new_point, first_point;
...
Fcurve mw_curve_to_fcurve(curve)
...

{
  ...
  fcurve = mw_change_fcurve(NULL);
  if (fcurve == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create Fcurve\n");
      return(NULL);
    }
  fcurve->first = mw_point_curve_to_point_fcurve(curve->first);
  return(fcurve);
}

/*--- Point_fcurve to Point_curve ---*/

Point_curve mw_point_fcurve_to_point_curve(pfcurve)

Point_fcurve pfcurve;

{
  Point_fcurve pp;
  Point_curve point, new_point, first_point;
  char lost = 0;
...
Curve mw_fcurve_to_curve(fcurve)
...

{
  ...
  curve->first = mw_point_fcurve_to_point_curve(fcurve->first);
  return(curve);
}
...
Fpolygon mw_polygon_to_fpolygon(poly)
...
Polygon mw_fpolygon_to_polygon(fpoly)
...
Fcurve mw_fpolygon_to_fcurve(fpoly)
...
Curve mw_polygon_to_curve(poly)
...
Fpolygon mw_curve_to_fpolygon(curve)
...
{ Fcurve fcurve;
  ...
  fcurve = mw_curve_to_fcurve(curve);
  fpoly = mw_fcurve_to_fpolygon(fcurve);
  mw_delete_fcurve(fcurve);      
  return(fpoly);
}
...
Curve mw_fpolygon_to_curve(fpoly)
...
Polygon mw_fcurve_to_polygon(fcurve)
...
{ Curve curve;
  ...
  curve = mw_fcurve_to_curve(fcurve);
  poly = mw_curve_to_polygon(curve);
  mw_delete_curve(curve);      
  return(poly);
}
...
Fcurve mw_polygon_to_fcurve(poly)
...
Curve mw_curves_to_curve(curves)
...
Curves mw_curve_to_curves(curve)
...
Fcurve mw_fcurves_to_fcurve(fcurves)
...
Fcurves mw_fcurve_to_fcurves(fcurve)
...
Morpho_line  mw_curve_to_morpho_line(curve)
...

{
  ...
  if (((ll = mw_change_morpho_line(NULL)) == NULL)
      ||
...
Curve mw_morpho_line_to_curve(ll)
...

{
  ...
  if (((cv = mw_change_curve(NULL)) == NULL)
      ||
...
Fmorpho_line  mw_fcurve_to_fmorpho_line(fcurve)
...

{
  ...
  if (((fll = mw_change_fmorpho_line(NULL)) == NULL)
      ||
...
Fcurve mw_fmorpho_line_to_fcurve(fll)
...

{
  ...
  if (((cv = mw_change_fcurve(NULL)) == NULL)
      ||
...
Fmorpho_line mw_morpho_line_to_fmorpho_line(ll)
...

{
  ...
  fll->open = ll->open;
  fll->data = ll->data;
  fll->first_point = mw_point_curve_to_point_fcurve(ll->first_point);
...
Morpho_line mw_fmorpho_line_to_morpho_line(fll)
...

{
  ...
  ll->open = fll->open;
  ll->data = fll->data;
  ll->first_point = mw_point_fcurve_to_point_curve(fll->first_point);
...
Mimage mw_morpho_line_to_mimage(ll)
...

{
  ...
  if (((mimage = mw_change_mimage(NULL)) == NULL)
      ||
...
Morpho_line mw_mimage_to_morpho_line(mimage)
...
Mimage mw_fmorpho_line_to_mimage(fll)
...
Fmorpho_line mw_mimage_to_fmorpho_line(mimage)
...

{
  ...
  fll = mw_change_fmorpho_line(NULL);
  if (fll == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create a fmorpho_line\n");
      return(NULL);
...
Curves mw_mimage_to_curves(mimage)
...

{
  ...
  if ((!mimage->first_ml)&&(mimage->first_fml))
    mwerror(WARNING,0,"The mimage structure does not contain morpho lines (although it does contain fmorpho lines) !\n");
...
Mimage mw_curves_to_mimage(curves)
...
Fcurves mw_mimage_to_fcurves(mimage)
...

{
  ...
  if ((mimage->first_ml)&&(!mimage->first_fml))
    mwerror(WARNING,0,"The mimage structure does not contain fmorpho lines (although it does contain morpho lines) !\n");
...
Mimage mw_fcurves_to_mimage(fcurves)
...
Morpho_sets mw_morpho_set_to_morpho_sets(is)
...

{
  ...
  if (((morpho_sets = mw_change_morpho_sets(NULL)) == NULL)
      ||
...
Morpho_set mw_morpho_sets_to_morpho_set(morpho_sets)
...
Mimage mw_morpho_sets_to_mimage(iss)
...
Morpho_sets mw_mimage_to_morpho_sets(mimage)
...

{
  ...
  iss = mw_change_morpho_sets(NULL);
  if (iss == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create a morpho_sets\n");
      return(NULL);
...
Cmorpho_line mw_morpho_line_to_cmorpho_line(ll)
...

{
  ...
  if ( ((cll = mw_change_cmorpho_line(NULL)) == NULL) ||
...
Morpho_line mw_cmorpho_line_to_morpho_line(cll)
...
Cfmorpho_line mw_fmorpho_line_to_cfmorpho_line(ll)
...

{
  ...
  if ( ((cll = mw_change_cfmorpho_line(NULL)) == NULL) ||
...
Fmorpho_line mw_cfmorpho_line_to_fmorpho_line(cll)
...

{
  ...
  if ( ((ll = mw_change_fmorpho_line(NULL)) == NULL) ||
...
Cfmorpho_line mw_cmorpho_line_to_cfmorpho_line(ll)
...

{
  ...
  if ( ((fll = mw_change_cfmorpho_line(NULL)) == NULL) ||
      ((fll->first_type = mw_new_point_type()) == NULL) )
    {
      mwerror(ERROR, 0,"Not enough memory to create Cfmorpho_line\n");
      return(NULL);
...
Cmorpho_line mw_cfmorpho_line_to_cmorpho_line(fll)
...

{
  ...
  if ( ((ll = mw_change_cmorpho_line(NULL)) == NULL) ||
      ((ll->first_type = mw_new_point_type()) == NULL) )
    {
      mwerror(ERROR, 0,"Not enough memory to create Cmorpho_line\n");
      return(NULL);
...
/*========================= MORPHO_LINE / CFMORPHO_LINE ===============================*/

/*--- Morpho_line to Cfmorpho_line ---*/

Cfmorpho_line mw_morpho_line_to_cfmorpho_line(ll)
...

{
  ...
  cll=mw_morpho_line_to_cmorpho_line(ll);
  if (cll==NULL) return(NULL);
  fll=mw_cmorpho_line_to_cfmorpho_line(cll);
  mw_delete_cmorpho_line(cll);
  return(fll);
}

/*--- Cfmorpho_line to Morpho_line ---*/

Morpho_line mw_cfmorpho_line_to_morpho_line(fll)
...

{
  ...
  cll=mw_cfmorpho_line_to_cmorpho_line(fll);
  if (cll==NULL) return(NULL);
  ll=mw_cmorpho_line_to_morpho_line(cll);
  mw_delete_cmorpho_line(cll);
  return(ll);
}
...
Cmimage mw_cmorpho_line_to_cmimage(ll)
...

{
  ...
  if (((cmimage = mw_change_cmimage(NULL)) == NULL)
      ||
...
Cmorpho_line mw_cmimage_to_cmorpho_line(cmimage)
...
Cmimage mw_morpho_line_to_cmimage(ll)
...

{
  ...
  cll=mw_morpho_line_to_cmorpho_line(ll);
  if (cll==NULL) return(NULL);
  cmimage=mw_cmorpho_line_to_cmimage(cll);
  mw_delete_cmorpho_line(cll);
  return(cmimage);
}

Morpho_line mw_cmimage_to_morpho_line(cmimage)
...

{
  ...
  cll=mw_cmimage_to_cmorpho_line(cmimage);
  if (cll==NULL) return(NULL);
  ll=mw_cmorpho_line_to_morpho_line(cll);
  mw_delete_cmorpho_line(cll);
...
/*========================= CMORPHO_LINE / FMORPHO_LINE ===============================*/

/*--- Cmorpho_line to Fmorpho_line ---*/

Fmorpho_line mw_cmorpho_line_to_fmorpho_line(cll)
...

{
  ...
  fll=mw_morpho_line_to_fmorpho_line(ll);
  mw_delete_morpho_line(ll);
  return(fll);
}

/*--- Fmorpho_line to Cmorpho_line ---*/

Cmorpho_line mw_fmorpho_line_to_cmorpho_line(fll)
...

{
  ...
  ll=mw_fmorpho_line_to_morpho_line(fll);
  if (ll==NULL) return(NULL);
  cll=mw_morpho_line_to_cmorpho_line(ll);
  mw_delete_morpho_line(ll);
  return(cll);
...
Mimage mw_cmorpho_line_to_mimage(cll)
...
Cmorpho_line mw_mimage_to_cmorpho_line(mimage)
...

{
  ...
  ll=mw_mimage_to_morpho_line(mimage);
  if (ll==NULL) return(NULL);
  cll=mw_morpho_line_to_cmorpho_line(ll);
  mw_delete_morpho_line(ll);  
  return(cll);
...
Cmorpho_line  mw_curve_to_cmorpho_line(curve)
...
Curve mw_cmorpho_line_to_curve(ll)
...
Cfmorpho_line  mw_fcurve_to_cfmorpho_line(fcurve)
...
Fcurve mw_cfmorpho_line_to_fcurve(fll)
...
Curves mw_cmimage_to_curves(cmimage)
...

{
  ...
  if ((!cmimage->first_ml)&&(cmimage->first_fml))
    mwerror(WARNING,0,"The cmimage structure does not contain cmorpho lines (although it does contain fcmorpho lines) !\n");
...
Cmimage mw_curves_to_cmimage(curves)
...
Cmimage mw_cmorpho_sets_to_cmimage(iss)
...
Cmorpho_sets mw_cmimage_to_cmorpho_sets(cmimage)
...

{
  ...
  iss = mw_change_cmorpho_sets(NULL);
  if (iss == NULL)
    {
...
Cmorpho_sets mw_cmorpho_set_to_cmorpho_sets(is)
...

{
  ...
  if (((cmorpho_sets = mw_change_cmorpho_sets(NULL)) == NULL)
      ||
...
Cmorpho_set mw_cmorpho_sets_to_cmorpho_set(cmorpho_sets)
...
Cmimage mw_cfmorpho_line_to_cmimage(fll)
...
Cfmorpho_line mw_cmimage_to_cfmorpho_line(cmimage)
...

{
  ...
  fll = mw_change_cfmorpho_line(NULL);
  if (fll == NULL)
    {
      mwerror(ERROR, 0,"Not enough memory to create a cfmorpho_line\n");
      return(NULL);
...
Cmimage mw_mimage_to_cmimage(mimage)
...

{
  ...
  for (ml=mimage->first_ml; ml; ml=ml->next)
    {
      cml1 = mw_morpho_line_to_cmorpho_line(ml);
  ...
  for (fml=mimage->first_fml; fml; fml=fml->next)
    {
      cfml1 = mw_fmorpho_line_to_cfmorpho_line(fml);
...
Mimage mw_cmimage_to_mimage(cmimage)
...

{
  ...
  for (cml=cmimage->first_ml; cml; cml=cml->next)
    {
      ml1 = mw_cmorpho_line_to_morpho_line(cml);
  ...
  for (cfml=cmimage->first_fml; cfml; cfml=cfml->next)
    {
      fml1 = mw_cfmorpho_line_to_fmorpho_line(cfml);
