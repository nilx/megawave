...
#endif
{
  ...
  if ((mws = LOOKUP(mwname->val.text)) != NULL) {
    ...
    fprintf(fd, "  int %si;\n", MW_PFX2);
    fprintf(fd, "  char %stype[TYPE_SIZE+1];\n", MW_PFX2);
    ...
    *popt = '\0';
    ...
    l = strlen(mwversion->val.text)-2;
    strncpy(Vers, mwversion->val.text+1, l);
    Vers[l] = '\0';
    l = strlen(mwfunction->val.text)-2;
    strncpy(Fonc, mwfunction->val.text+1, l);
    Fonc[l] = '\0';
    if (mwlabo != NULL) {
      l = strlen(mwlabo->val.text)-2;
      strncpy(Labo, mwlabo->val.text+1, l);
      Labo[l] = '\0';
    }
    else
      Labo[0] = '\0';
