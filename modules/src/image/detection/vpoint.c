...
/* mwcommand
 ...
 's':csegs<-segs    "store segments indexes associated
                    to each vanishing region (output Flists)",
  ...
  output<-output    "detected vanishing regions: 9-Flist
                    (x1 y1 x2 y2 x3 y3 x4 y4 -log10(nfa))",
...
static SegList newSegList(int n)
{
...
static int deleteSegList(SegList L)
{
    if (--(*(int *) L->data) == 0)
    {
        free(L->data);
...
static SegList newSegListRef(SegList L)
{
    (*(int *) L->data)++;
    return L;
}

#define  SegListInsert(L,i) \
    mwerror(FATAL,1,"SegListInsert not implemented in non-HasSTL mode.\n"  \
...
static Flist SegListCopy(SegList L)
{
    SegListIterator p;
    Flist l;
...
static double pext(double beta1, double beta2, double theta)
{
    double f1, f2;
...
static double pinf(double beta1, double theta)
{
    return pext(beta1, M_PI_2, theta);
}

static double fzero_convex(double a0, double b0, double tolx, double tolf,
                           double beta1, double dtheta, double p)
...
static double *qtile(double p, double dtheta, int *nq, double *p_inf)
{
    double tol, b;
    double *beta;
    int i;
...
static void polar2cart(double theta, double q, double R, double Xcenter,
                       double Ycenter, float *x, float *y)
                               /* normalized polar coordinates */
                               /* parameters used for normalization */
                               /* non-normalized cartesian coordinates */
{
    *x = R * q * cos(theta) + Xcenter;
    *y = R * q * sin(theta) + Ycenter;
}

static double *tab2(int n, double p)
...
static double *binomial_tail(int n, double p)
{
    double *in, *out;
    int x;
...
typedef struct tiling {
    ...
    double *x;                  /* nx+1 array with x-coords
                                 * of the borders of the tiling */
    double *y;                  /* ny+1 array with y-coords
                                 * the borders of the tiling */
    /* For exterior tilings (x,y)=(theta,q)
     * in normalized polar coordinates */
    double *meaning;            /* nx*ny array with
                                 * the meaningfulness (-log10(nfa))
                                 * for each region */
    int *vp;                    /* nx*ny array equals 1
                                 * if the region is meaningful
                                 * with locally maximal meaningfulness */
    int *isvalid;               /* nx*ny array equals 1
                                 * if the corresponding region
                                 * is valid. (Used to eliminate
                                 * interior tiles which are
                                 ...
                                 * to be MDL-maximal-meaningful). */
    SegList *segs;              /* nx*ny array of Flists.
                                 * The Flist corresponding to
                                 * region (i,j) contains pointers
                                 * to the input segs Flist
                                 * for all segments whose
                                 * supporting line meets region (i,j)
                                 */
    double *mdlmeaning;
    int *mdlvp;
    SegList *mdlsegs;           /* same as meaning, vp and segs,
                                 * but after MDL */
} Tiling;
...
#define TOpposite(T,x,y) \
    (((x) >= T[EXTERIOR_TILING].nx/2) \
     ? ((x)-T[EXTERIOR_TILING].nx/2) \
     : (x)+T[EXTERIOR_TILING].nx/2)
 ...
 */
static double TMeaning2(Tiling * T, int ie, int ix, int iy)
{
    int nx, ny, x, y;
    nx = T[ie].nx;
    ny = T[ie].ny;
...
static Tiling *newTiling(int ntheta, double *p, double *p_inf, int *M)
                 /* Number of orientations in the exterior tiling */
                 /* (output) probability of a line meeting any of the tiles */
                 /* (output) probability of a line meeting infinite tiles
                  * (p_inf <= p) */
                 /* (input) total number of segments, and */
                 /* (output) total number of valid tiles */
{
    ...
    T[EXTERIOR_TILING].meaning =
        (double *) calloc(ntheta * nq, sizeof(double));
    T[EXTERIOR_TILING].vp = (int *) calloc(ntheta * nq, sizeof(int));
    T[EXTERIOR_TILING].isvalid = (int *) calloc(ntheta * nq, sizeof(int));
    T[EXTERIOR_TILING].segs =
        (SegList *) calloc(ntheta * nq, sizeof(SegList));
    T[EXTERIOR_TILING].mdlmeaning =
        (double *) calloc(ntheta * nq, sizeof(double));
    T[EXTERIOR_TILING].mdlvp = (int *) calloc(ntheta * nq, sizeof(int));
    T[EXTERIOR_TILING].mdlsegs =
        (SegList *) calloc(ntheta * nq, sizeof(SegList));
    ...
    T[INTERIOR_TILING].mdlmeaning =
        (double *) calloc(nx * ny, sizeof(double));
    T[INTERIOR_TILING].mdlvp = (int *) calloc(nx * ny, sizeof(int));
    T[INTERIOR_TILING].mdlsegs = (SegList *) calloc(nx * ny, sizeof(SegList));

    /* Initial size for segs Flists
     * = expected number of segments at each tile */
    max_size = floor(max(((double) *M)
                         / sqrt((double) (nx * ny + ntheta * nq)), 1.0) + .5);
            ...
            for (ix = 0; ix < T[ie].nx; ix++)
            {
                ...
                {               /* ie == EXTERIOR_TILING */
                    if ((IdentifyOppositeTiles == 1)
                        && (iy == (T[ie].ny - 1)))
                        /* Tile is invalid if it belongs
                         * to the second semi-circle
                         ...
                         && (IdentifyOppositeTiles == 1))
                {
                    ...
                    TSegs(T, ie, ix, iy) =
                        newSegListRef(TSegs(T, ie, TOpposite(T, ix, iy), iy));
                    TMDLSegs(T, ie, ix, iy) =
                        newSegListRef(TMDLSegs
                                      (T, ie, TOpposite(T, ix, iy), iy));
                }
                else
...
static void deleteTiling(Tiling * T)
{
    int ie, ix, iy;
    SegList S;
    ...
    for (ie = 0; ie <= 1; ie++)
    {
         /**/ for (iy = 0; iy < T[ie].ny; iy++)
            for (ix = 0; ix < T[ie].nx; ix++)
            {
                ...
                {
                    deleteSegList(S);
                    TSegs(T, ie, ix, iy) = NULL;
                };
                S = (TMDLSegs(T, ie, ix, iy));
                if (S != NULL)
                {
                    deleteSegList(S);
                    TMDLSegs(T, ie, ix, iy) = NULL;
                };
            }
         /**/ free(T[ie].x);
        free(T[ie].y);
...
static Flists newSegments(Fimage allsegs)
{
    int N, dim, i, j;
    Flists Segments;
    Flist s;
...
static void deleteSegments(Flists Segments)
{
    int i;
    Flist s;
...
static void cross_prod(double *a, double *b, double *c)
{
    c[2] = a[0] * b[1] - a[1] * b[0];
    c[0] = a[1] * b[2] - a[2] * b[1];
    c[1] = a[2] * b[0] - a[0] * b[2];
...
static void polar_coords(float *seg, double R, double Xcenter, double Ycenter,
                         double *rho, double *phi)
{
    ...
    x0 = seg[SegX0] + 0.5;
    y0 = seg[SegY0] + 0.5;      /* first end-point */
    x1 = seg[SegX1] + 0.5;
    y1 = seg[SegY1] + 0.5;      /* second end-point */
    dx = x1 - x0;
    dy = y1 - y0;
...
static void proj_coords(float *seg, double R, double Xcenter, double Ycenter,
                        double *L)
                /* should be called with an allocated L[3] */
{
    double x0, y0, x1, y1;
    double X0[3], X1[3];
 ...
 */
static void FlistAdd(Flist l, float *tuple)
{
    int i, sz, dim;
    sz = l->size;
    dim = l->dim;
...
static Flist FlistCat(Flist l1, Flist l2)
{
    int i, j;

    if (l1->dim != l2->dim)
        mwerror(INTERNAL, 1,
                "[FlistCat] Flists should have the same dimension!");
    ...
    for (i = 0; i < l2->size; i++)
        ...
        {
            for (j = 0; j < l1->dim; j++)
                l1->values[l1->size * l1->dim + j] =
                    l2->values[i * l2->dim + j];
            l1->size++;
        }
...
static Flist FlistFlush(Flist l1)
{
    l1->size = 0;
    return l1;
}
...
*/
static void STilesAddUnique(Flist L, Tiling ** Tilings, int i, int ie, int ix,
                            int iy)
...
/* -------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------
*/
static int TAddSegment(Tiling ** Tilings, int i, Flists S, int j, double R,
                       double Xcenter, double Ycenter)
            /* index to precision level */
            /* index to current segment */
{
  ...
  /*** A) Compute all intersections of j-th segment with exterior tiles ***/
    ...
    {
        cos_phi_theta = cos(phi - theta[itheta]);
        /* when cos==0
         * we force infinite tile */
        d1 = (fabs(cos_phi_theta) <
              EPS) ? Inf : rho / cos(phi - theta[itheta]);
        if (d1 >= d[0])
        {
            id1 = 0;
            while ((id1 < nd) && (d[id1] < d1))
                id1++;
            if (id1 > 0)
                id1--;
            ...
            if (itheta > 0)
                ...
                /* itheta==0, its circular neighbour is ntheta-1 */
  ...
  /*** B) Compute all intersections of j-th segment with interior tiles ***/
    ...
    if ((fabs(phi) <= M_PI_4) ||
        (fabs(phi - M_PI) <= M_PI_4) || (fabs(phi - 2.0 * M_PI) <= M_PI_4))
    {
...
/*
  ...
  With the current implementation we still miss:
    - intersections between interior
    and exterior tiles at the same or different levels

 */
static int FindIntersection(Tiling * T, int ie, int ix, int iy, Tiling * TT,
                            int *ie2, int *ix2, int *iy2)
{
    int k, n, m, new_tile;
    double *x, *y, *y2, *x2;
...
/*ymin = T[ie].y[0];*/ ymin2 = TT[*ie2].y[0];
    ...
    for (k = 0, n = 0; k < 4; k++)
    {
        ...
        else
        {
            ...
            while ((iy2[n] < ny2) && (y2[iy2[n]] < y[iy + DY[k]]))
                (iy2[n])++;
            if (iy2[n] > 0)
                (iy2[n])--;
            IY2 = iy2[n];
        ...
        new_tile = (1 == 1);
        for (m = 0; m < n; m++)
            new_tile = new_tile && ((ie2[n] != ie2[m]) || (iy2[n] != iy2[m])
                                    || (ix2[n] != ix2[m]));
        if (new_tile)
            n++;
           ...
           int *maskedVPs)
{
    ...
    float max_meaning;
    int IE[9], IX[9], IY[9];
    /* used to hold the output of FindIntersection */
    int n;
    ...
    {                           /* main loop */
        ...
        if (verbose)
            fprintf(stderr,
                    "\n\n*** Searching for vanishing regions "
                    "(iteration # %d)\n\n", itn);
        nvp1 = 0;
     /*** For each angular precision level ... ***/
        for (i = 0; i < n_pl; i++)
       ...
       /*** For each segment, update all tiles it meets ***/
            for (N = 0, j = 0; j < Segments->size; j++)
            {
                if (!searching_masked)
                    N++;
                /* to search non-masked VPs count all segments */
                if (SIsValid(Segments, j) > 0.0)
                {
                    TAddSegment(Tilings, i, Segments, j, R, X0, Y0);
                    if (searching_masked)
                        N++;
                    /* to search masked VPs count only remaining segments */
                }
            }
       ...
       /*** Compute NFA, meaningfulness for each tile ***/
            ...
            for (ix = 0; ix < T[ie].nx; ix++)
                if (TIsValid(T, ie, ix, iy))
                {
                    TMeaning(T, ie, ix, iy) =
                        -log10(M[i] *
                               Binf[i][SegListSize(TSegs(T, ie, ix, iy))]);
                }
                else
                {
                    TMeaning(T, ie, ix, iy) = TMDLMeaning(T, ie, ix, iy);
                    /* == -Inf by default */
                }
       ...
       /*** Vanishing points are local maxima of meaningfulness ***/
            ...
            if (verbose)
                fprintf(stderr,
                        "  Found %d maximal meaningful regions "
     ...
     /*** Vanishing points must be also
        ...
        if (verbose)
            fprintf(stderr,
                    "\nTesting inclusion relationships "
        ...
        for (i = 1; i < n_pl; i++)
        {
            ...
            for (ie = 0; ie <= 1; ie++)
                for (iy = 0; iy < T[ie].ny; iy++)
                    for (ix = 0; ix < T[ie].nx; ix++)
                        if ((TMeaning2(T, ie, ix, iy) >= threshold)
                            || TMDLVP(T, ie, ix, iy))
                        {
                            /* find all coarser intersecting tiles
                             * t_k(j,IE[k],IX[k],IY[k]) */
                            for (j = 0; j < i; j++)
                            {
                                TT = Tilings[j];
                                n = FindIntersection(T, ie, ix, iy, TT, IE,
                                                     IX, IY);
                                for (k = 0; k < n; k++)
                                    if ((TMeaning2(TT, IE[k], IX[k], IY[k]) >=
                                         threshold)
                                        || TMDLVP(TT, IE[k], IX[k], IY[k]))
                                    {
                                        if (TMeaning2(TT, IE[k], IX[k], IY[k])
                                            > TMeaning2(T, ie, ix, iy))
                                            TVP(T, ie, ix, iy) = 0;
                                        /* t(i,ie,ix,iy) is not maximal */
                                        else
                                            TVP(TT, IE[k], IX[k], IY[k]) = 0;
                                        /* t_k(j,IE[k],IX[k],IY[k])
                                         * is not maximal */
                                    }
                            }
                    ...
                    for (iy = 0; iy < T[ie].ny; iy++)
                        for (ix = 0; ix < T[ie].nx; ix++)
                            if (TVP(T, ie, ix, iy))
                            {
                                ...
                                TMDLSegs(T, ie, ix, iy) =
                                    newSegListRef(TSegs(T, ie, ix, iy));
                            }
            }
            if (verbose)
                fprintf(stderr,
                        "  %d (out of %d) regions remaining "
                        "after inclusion test\n", nvp2, nvp1);
            nvp3 = nvp2;
     ...
     /*** Minimum Description Length ***/
            ...
            if (max_meaning >= threshold)
            {
                T = Tilings[max_i];
                /* Remove t from the list of valid tiles */
                TVP(T, max_ie, max_ix, max_iy) =
                    TIsValid(T, max_ie, max_ix, max_iy) = 0;
                ...
                for (it = SegListBegin(CSegs); !SegListEnd(CSegs, it);
                     SegListNext(it))
                {
        ...
        if (nvp3 == 0)          /* no more (MDL-maximal) meaningful tiles */
        {
            if (masked && !searching_masked)
                searching_masked = 1;
            /* look for masked MDL-maximal meaningful tiles */
            else
    ...
    for (searching_masked = 0; searching_masked <= ((masked) ? 1 : 0);
         searching_masked++)
    {
        ...
        for (k = 0; k < VPlist[searching_masked]->size; k++)
        {
            ...
            if (ie == INTERIOR_TILING)
            {
                ...
                if (segs)
                    segs->list[segs->size++] =
                        SegListCopy(TMDLSegs(T, ie, ix, iy));
                ...
                if (verbose)
                    fprintf(stderr,
                            ...
                            pow(2.0, (i + min_pl)));
                if (verbose)
                    fprintf(stderr, "   meaningfulness  = %g\n", meaning);
                if (verbose)
                    fprintf(stderr,
            ...
            {                   /* ie == EXTERIOR_TILING */
                ...
                d3 = d4 =
                    (iy ==
                     (T[ie].ny - 1)) ? 2.0 * T[ie].y[iy] : T[ie].y[iy + 1];
