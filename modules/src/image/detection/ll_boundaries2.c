...
typedef struct mydata {
    ...
    char notmax;                /* meaningful boundary
                                 * known to be NOT maximal */
} *Mydata;
...
static void store_boundaries(Flists cs, Shapes tree, Fimage image, int *prec,
                             float **tabsaddles, char *all, float eps2)
{
    Shape s;
    Flist l;
    int i;
    ...
    for (i = 1; i < tree->nb_shapes; i++)
    {
        ...
        if (sdata->max || (all && sdata->meaningful))
        {
            ...
            /* FIXME: cast */
            flstb_boundary(prec, image, tree, s, NULL, l,
                           (char *) tabsaddles);
...
static void pixels_and_data(Shapes tree, Fimage NormofDu, Fimage image,
                            int *prec, float **tabsaddles, float *sumsqper)
{
    Shape s;
    int i;
    Mydata data;
    ...
    *sumsqper = 0.;
    for (i = 0; i < tree->nb_shapes; i++)
    {
        ...
        if (!s->data)
            mwerror(FATAL, 1, "Cannot allocate memory.\n");
        data = (Mydata) s->data;
        data->nfa = FLT_MAX;
        data->visit = 0;
        if (i > 0)
...
static Fsignal shape_histo(Shape shape, Fimage NormofDu, int size,
                           float scale)
{
    Point_plane p;
    int i, ncol, nrow, k;
...
static float logH(float mu, Fsignal local_repart)
{
    ...
    if (i >= local_repart->size - 1)
        i = local_repart->size - 1;
...
static void update_root_histo(Shape local_root, Fsignal local_histo,
                              Fimage NormofDu)
{
    Shape s;
    Mydata sdata;
    ...
    /*remove root boundary points from histogram
     * (unless shape is absolut root) */
    if (local_root->boundary)
    {
        ...
        {
            x = MIN(ncol - 1,
                    MAX(0, (int) (floor(tabvalues[2 * i] + .5) - 1)));
            y = MIN(nrow - 1,
                    MAX(0, (int) (floor(tabvalues[2 * i + 1] + .5) - 1)));
            if (x != ox || y != oy)
            {
                ox = x;
                oy = y;
                ...
                if (local_histo->values[k] < 0.)
                    local_histo->values[k] = 0.;
            }
        }
    }
}
...
static int update_mydata(Shape s, char type, Shape local_root,
                         Fsignal local_repart, struct myglobal Global,
                         int *new_open)
{
    Shape t;
    float mu, threshold;
    ...
    if (!sdata->notmax && !sdata->max)
    {
      ...
      /*** compute nfa (-log10) ***/
            ...
            if (sdata->nfa < threshold)
            {
                 ...
                 * in the local research, we first deal
                 * with all open shapes before
                 * detecting closed one. This is because
                 * the interior of an open shape
    ...
    if (s == local_root || !sdata->max)
        for (t = s->child; t; t = t->next_sibling)
        {
            ...
            if (tdata->nfa < threshold)
            {
                ...
                if (sdata->type == t->inferior_type && !tdata->notmax)
                    sdata->ndetect++;
            }
            else
...
static void add_boundary(Shape s, Shape local_root, float *bestnfa_inf,
                         float bestnfa_sup, char type, struct myglobal Global)
{
    Shape t;
        ...
        if (!sdata->max)
        {
            ...
            *bestnfa_inf = new_bestnfa_inf;
            ...
            if ((nfa < threshold && all) ||
                (nfa < threshold && nfa <= old_bestnfa_sup
                 && nfa < *bestnfa_inf))
            {
                if (!sdata->notmax)
                {
            ...
            if (nfa < *bestnfa_inf && (!sdata->notmax))
                *bestnfa_inf = nfa;
        }
        else
        {
...
static void total_maximal(Shape s, Shape local_root, float *bestnfa_inf,
                          float bestnfa_sup, struct myglobal Global,
                          int new_open)
{
    Shape t;
    Mydata sdata;
    ...
    if (s != local_root)
    {
        ...
        if (!sdata->max)
        {
            if (!sdata->notmax && nfa < bestnfa_sup)
                bestnfa_sup = nfa;
            ...
            for (t = s->child; t; t = t->next_sibling)
            {
                *bestnfa_inf = threshold;
                ...
                if (t->open || !new_open || !s->open
                    || bestnfa_sup >= threshold)
                    total_maximal(t, local_root, bestnfa_inf, bestnfa_sup,
                                  Global, new_open);
            ...
            *bestnfa_inf = new_bestnfa_inf;
            ...
            if (!sdata->notmax && nfa < *bestnfa_inf)
                *bestnfa_inf = nfa;
        }
    }
    else
    {
        for (t = s->child; t; t = t->next_sibling)
            total_maximal(t, local_root, bestnfa_inf, bestnfa_sup, Global,
                          new_open);
    }
}
...
static void fathom_tree(Shape local_root, Fimage NormofDu,
                        ...
                        struct myglobal Global)
{
    ...
    if (Global.local)
    {
        total_maximal(local_root, local_root, &nfa_inf, nfa_sup, Global,
                      new_open);
        s = mw_get_first_child_shape(local_root);
        while (s)
        {
    ...
    {
        mw_delete_fsignal(rdata->histo);
        /* no longer need this histogram */
        s = mw_get_first_child_shape(local_root);
                      ...
                      Shapes keep_tree)
{
    ...
    if (Global.local)
        threshold -= (float) log10((double) *visit);
    ...
    fathom_tree(ref_tree->the_shapes, NormofDu, local_histo, local_repart,
                Global);
    ...
    printf("%d %s boundaries detected over %d\n", boundaries->size,
           (all ? "" : "maximal"), ref_tree->nb_shapes);
    ...
    if (loc)
    {
        ...
        {
            printf
                ("WARNING: some boundaries "
                 "have been visited more than allowed\n");
