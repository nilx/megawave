...
function = {"Extract maximal meaningful edges
           (contrasted pieces of level lines) from a Fimage"};
usage = {
...
static float add_edge(Flist c, float bestnfa, int i, int j)
{
    ...
    if (zero_order)
        ...
        lg = (float) (length[j % n] - length[i % n]);
                ...
                int *precision, char *z)
{
    ...
    if (!zero_order)
        length = (double *) malloc(n_alloc * sizeof(double));
