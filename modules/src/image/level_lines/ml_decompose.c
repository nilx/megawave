     ...
     Fimage image_in;
{
  ...
  if (m_image == NULL) mwerror(FATAL,1,"Not enough memory.\n");
  ...
  if(m_image_in) 
	...
	mwerror(WARNING,1,"image_in and m_image_in not of same dimension!\n Dimensions of image_in are kept.\n");
  ...
  if(*ml_opt==0) {
    ...
    for(l=0;l<levels->size;l++) levels->values[l]=tmp_levels->values[l+1];
