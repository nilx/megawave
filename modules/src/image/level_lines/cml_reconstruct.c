...
static void
draw_lline(Cmorpho_line lline_ptr, struct closest_lline *c_lline,
           unsigned char **V, unsigned char *H1, int NL, int NC)
{
    Point_curve point;
    int l, c, dl, dc;
    float d;
...
static unsigned char zero_inside(struct closest_lline *c_lline)
{
    Point_curve c_p, p_p, n_p;

    c_p = c_lline->point;
    if (c_lline->lline->open == 0)
    {                           /* closed polygon */
    ...
    {                           /* open polygon   */
        ...
        {
            return (c_p->x < c_p->y);
                                /*=c_p starts in last line or first column*/
        }
        else if ((n_p = c_p->next) == NULL)
        {
            return (c_p->x > c_p->y);
                                /*=c_p starts in first line or last column*/
        }
        else
        {
            if (((p_p->x - c_p->x) * (n_p->y - c_p->y)
...
static void
fill_lline(unsigned char **V, unsigned char *H1, Color level,
           unsigned char inside, Cfimage image, int NL, int NC)
{
    int l, c;
...
Cfimage cml_reconstruct(char *v_flag, Cmimage m_image)
{
    ...
    do
    {
        if (inc_val
            &&
            (((!v_flag) && (cmpcolor(current_level, lline_ptr->minvalue) > 0))
             || ((v_flag)
                 && (cmpcolor(current_level, lline_ptr->maxvalue) < 0))))
            inc_val = FALSE;
