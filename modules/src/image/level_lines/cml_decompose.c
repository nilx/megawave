...
static int cmpcolor(const void *c1p, const void *c2p)
{
    ...
    if (c1->model != c2->model)
        mwerror(INTERNAL, 1,
                "[cmpcolor] Two different color models %d and %d !\n",
                c1->model, c2->model);
    switch (c1->model)
    {
    case MODEL_HSI:
    ...
    default:
        mwerror(INTERNAL, 1,
                "[cmpcolor] No color order defined for color model %d !\n",
                c1->model);
    }
    return 0;
}
...
static Cfsignal cfvalues(Cfimage image)
{
    ...
    for (i = 0, r = image->red, g = image->green, b = image->blue; i < smax;
         i++, r++, g++, b++)
    {
    ...
    for (i = 1; i < (unsigned long) levels->size; i++, j++)
    {
        ...
        while ((i < (unsigned long) levels->size) &&
               (cmpcolor(&levels->values[i - 1], &levels->values[i]) == 0))
            i++;
    }
    nmlevels->size = j;
    free(levels->values);
...
static void llcheck(Cmimage mimage)
{
    ...
    for (ll = mimage->first_ml; ll; ll = ll->next)
    {
        if ((ll->minvalue.model != ll->maxvalue.model) ||
            (ll->minvalue.model > 4))
            mwerror(WARNING, 0,
                    "[llcheck] Inconsistent model number "
                    "(%d for minvalue, %d for maxvalue).\n",
...
static int Inside(Cfimage im, Color minvalue, Color maxvalue, int l, int c)
{
    unsigned long p;
    float r, g, b;
    ...
    case MODEL_HSI:
        ...
        if ((b > maxvalue.blue)
            || ((b == maxvalue.blue) && (r > maxvalue.red))
            || ((b == maxvalue.blue) && (r == maxvalue.red)
                && (g > maxvalue.green)))
            return (0);
        /* ### Check if im(c,l) >= minvalue */
        if ((b < minvalue.blue)
            || ((b == minvalue.blue) && (r < minvalue.red))
            || ((b == minvalue.blue) && (r == minvalue.red)
                && (g < minvalue.green)))
            return (0);
        return (1);
    ...
    default:
        mwerror(INTERNAL, 1,
                "[Inside] No color order defined for color model %d !\n",
                (int) im->model);
    }
    return 0;
}
...
static void
produce_HV(Cfimage im, int NL, int NC, Color minvalue, Color maxvalue,
           unsigned char **H, unsigned char **V)
{
    int l, c;

    /*
     * mwdebug("[produce_HV] minvalue=(%.5f,%.5f,%.5f) "
     *         "maxvalue=(%.5f,%.5f,%.5f)\n",
     * minvalue.red,minvalue.green,minvalue.blue,
     * maxvalue.red,maxvalue.green,maxvalue.blue);
     */
        ...
        for (c = 1; c < NC; c++)
        {
            if (Inside(im, minvalue, maxvalue, l, c))
            {
                if (!Inside(im, minvalue, maxvalue, l, c - 1))
                    V[l][c - 1] = 1;
                if (!Inside(im, minvalue, maxvalue, l - 1, c))
                    H[l - 1][c] = 1;
            }
            else
            {
    ...
    for (l = 1; l < NL; l++)
    {
        ...
        {
            if (!Inside(im, minvalue, maxvalue, l - 1, 0))
                H[l - 1][0] = 1;
        }
        else
        {
    ...
    for (c = 1; c < NC; c++)
    {
        ...
        {
            if (!Inside(im, minvalue, maxvalue, 0, c - 1))
                V[0][c - 1] = 1;
        }
        else
        {
...
static Cmorpho_line produce_lline(Color minvalue, Color maxvalue,
                                  unsigned char open)
{
...
follow_open_line(int NL, int NC, unsigned char **H, unsigned char **V, int ll,
                 int cc, int sum, Cmorpho_line lline)
{
    Point_curve p0, p1;
...
static void get_open_lines(Cfimage im, int NL, int NC, Color minvalue,
                           Color maxvalue, unsigned char **H,
                           unsigned char **V, Cmorpho_line * lline,
                           unsigned int L, unsigned long *Nll,
                           unsigned long *Nllrm)
{
    ...
    for (c = 0; c < NC - 1; c++)
        ...
        {                       /* such that levelset at left */
            ...
            if (mw_length_cmorpho_line(newll) < L)
                (*Nllrm)++;
            else
            {
                *lline = newll;
                if (oldll != NULL)
                    oldll->previous = *lline;
                newll->next = oldll;
            }
        }
...
static void get_closed_lines(Cfimage im, int NL, int NC, Color minvalue,
                             Color maxvalue, unsigned char **H,
                             unsigned char **V, Cmorpho_line * lline,
                             unsigned int L, unsigned long *Nll,
                             unsigned long *Nllrm)
...
static void cml_extract(Cfsignal levels, int *opt, unsigned int L,
                        Cfimage image_org, Cmimage m_image)
{
    ...
    do
    {
        ...
        if (nb_points >= L)
        {
            get_open_lines(image_org, NL, NC, minvalue, maxvalue, H, V,
                           &current_lline, L, &Nll, &Nllrm);
            get_closed_lines(image_org, NL, NC, minvalue, maxvalue, H, V,
                             &current_lline, L, &Nll, &Nllrm);
        }
        else                    /* nb_points < L */
        {
            if (nb_points == 0)
            {
                ...
                case 1:
                    mwerror(WARNING, 1,
                            "Value (%.5f,%.5f,%.5f) is smaller "
                            "or bigger than values of image_in.\n",
                            levels->values[i].red, levels->values[i].green,
                            levels->values[i].blue);
                    break;

                case 2:
                    mwerror(WARNING, 1,
                            "Value (%.5f,%.5f,%.5f) doesn't appear image_in.\n",
                            levels->values[i].red, levels->values[i].green,
                            levels->values[i].blue);
                    break;
    ...
    mwdebug
        ("Removed %d level lines of length < %d over %d (%3.1f %%).\n"
         "      Remain %d level lines.\n",
         Nllrm, L, Nll, (100.0 * Nllrm) / Nll, Nll - Nllrm);

    if (m_image == NULL)
        m_image = mw_change_cmimage(m_image);
    m_image->nrow = NL;
    m_image->ncol = NC;
                      ...
                      Cfimage image_in)
{
    ...
    if (cmimage_in)
    {
        ...
        if ((cmimage_in->nrow != image_in->nrow) ||
            ...
            mwerror(WARNING, 1,
                    "image_in and cmimage_in not of same dimension!\n"
    ...
    if (*ml_opt != 2)
    {
        ...
        for (l = 0; l < levels->size; l++)
            levels->values[l] = levels->values[l + 1];
    }
