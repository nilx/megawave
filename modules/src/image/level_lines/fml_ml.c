...
{ 
  ...
  old_lline=NULL;
  while(flline!=NULL) {
    for(fpoint=flline->first_point,nb_points=0;fpoint!=NULL;fpoint=fpoint->next) 
      nb_points++;
    for(ftype=flline->first_type,nb_types=0;ftype!=NULL;ftype=ftype->next) 
      nb_types++;
	...
	fpoint!=NULL;
	fpoint=fpoint->next,ftype=ftype->next) {
