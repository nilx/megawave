  ...
  if (invert)
    {
      ...
      if (mimage == NULL) 
	mwerror(FATAL,1,"Cannot compute level lines (local minima) of input image\n");
      mimage=ll_remove(mimage,(int *) Lmin);

      ml_dec = 0;
      Output1 = ml_reconstruct(&ml_recons, mimage);
      mimage = ml_decompose((Mimage) NULL, &ml_dec, (char *) NULL,
			    Output1);
      if (mimage == NULL) 
	mwerror(FATAL,1,"Cannot compute level lines (local maxima) of input image\n");
