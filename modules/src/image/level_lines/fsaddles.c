...
int fsaddles(Fimage pImage, Fimage pSaddlesImage)
{
  ...
  if(mw_change_fimage(pSaddlesImage, pImage->nrow-1, pImage->ncol-1) == NULL)
  ...
  for(i = pImage->nrow -2; i >= 0; i--)
    for(j = pImage->ncol -2; j >= 0; j--) {
      if((minDiag1=tabtabPixels[i][j]) > (maxDiag1=tabtabPixels[i+1][j+1])) {
	temp = minDiag1; minDiag1 = maxDiag1; maxDiag1 = temp;
      }
      if((minDiag2=tabtabPixels[i+1][j]) > (maxDiag2=tabtabPixels[i][j+1])) {
	temp = minDiag2; minDiag2 = maxDiag2; maxDiag2 = temp;
      }
      if(minDiag1 > maxDiag2 || maxDiag1 < minDiag2) {
	temp = minDiag1+maxDiag1 - (minDiag2+maxDiag2);
	tabtabSaddleValues[i][j]= (minDiag1*maxDiag1 - minDiag2*maxDiag2)/temp;
	++ iNumber;
      } else
