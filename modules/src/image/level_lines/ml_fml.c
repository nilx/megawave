...
/* mwcommand
 ...
 function = {"Transform morpho_lines into fmorpho_lines
            with a (-0.5,-0.5) translation"};
 usage = {
...
Fmorpho_line ml_fml(Morpho_line lline)
{
    ...
    while (lline != NULL)
    {
        for (point = lline->first_point, nb_points = 0; point != NULL;
             point = point->next)
            nb_points++;
        for (type = lline->first_type, nb_types = 0; type != NULL;
             type = type->next)
            nb_types++;
        if (nb_types != nb_points)
            mwerror(FATAL, 1, "Points / types mismatch");
        fpoint =
            (Point_fcurve) malloc(nb_points * sizeof(struct point_fcurve));
        ...
        for (point = lline->first_point, type = lline->first_type;
             point != NULL; point = point->next, type = type->next)
        {
            fpoint->x = point->x - .5;
            fpoint->y = point->y - .5;
