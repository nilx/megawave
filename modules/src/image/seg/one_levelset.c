...
static void Repointer(SOMMETPTR som, BORDPTR newbord, BORDPTR oldbord),ElimBordeReg(REGIONPTR reg, BORDPTR bord);
...
static void InitPixel(LI_PIXELSPTR pixelptr, char dir)        /* Met un segment de longeur 1     et de */
...
segment(float level)
{
  ...
  while (agglutination==VRAI){
    ...
    for (regact=image.tete;regact!=NULL;regact=regact->Rsuiv) {
      for (libords=regact->Lbords;libords!=NULL;libords=libords->suiv) {
    ...
    for (regact=image.tete;regact!=NULL;regact=regact->Rsuiv) {
      for (libords=regact->Lbords;libords!=NULL;libords=libords->suiv) {
        ...
        if(  ((*regact->canal<=level)&&(*regvois->canal<=level))
           ||((*regact->canal> level)&&(*regvois->canal> level)) ){
          agglutination=VRAI;
          RegMerge(regact,regvois,bordcom);
          break;                             /* we found a candidate region */
        }
...
static void
border2polygon(Fpolygons setpoly, int nb_poly, float level)
{
  float pX,pY,dX,dY;
  ...
  if((setpoly->first=(Fpolygon)malloc(nb_poly*sizeof(struct fpolygon)))==NULL)
  ...
  for(rptr=image.tete;rptr!=NULL;rptr=rptr->Rsuiv) 
	...
	bord=lbptr->bord; 
	poly->channel[0]=level;    /* here we give the levelset to the polygon */
	...
	for ( ;lipptr!=NULL;lipptr=lipptr->suiv){
	  ...
	  switch(lipptr->direction){
	   case 'b' : {dX=  0.0;dY=  1.0 ;
		       if(old_dir=='g') {nb_angles++; break;}
		       if(old_dir=='d') {nb_angles--; break;}
		       break;}
	   case 'd' : {dX=  1.0;dY=  0.0 ;
		       if(old_dir=='b') {nb_angles++; break;}
		       if(old_dir=='h') {nb_angles--; break;}
		       break;}
	   case 'h' : {dX=  0.0;dY= -1.0 ;
		       if(old_dir=='d') {nb_angles++; break;}
		       if(old_dir=='g') {nb_angles--; break;}
		       break;}
	   case 'g' : {dX= -1.0;dY=  0.0 ;
		       if(old_dir=='h') {nb_angles++; break;}
		       if(old_dir=='b') {nb_angles--; break;}
		       break;}
	  }
	...
	if(old_dir!=first_dir)
	  switch(first_dir){
	    case 'b' : {if(old_dir=='g') {nb_angles++; break;}
			if(old_dir=='d') {nb_angles--; break;}
			break;}
	    case 'd' : {if(old_dir=='b') {nb_angles++; break;}
			if(old_dir=='h') {nb_angles--; break;}
			break;}
	    case 'h' : {if(old_dir=='d') {nb_angles++; break;}
			if(old_dir=='g') {nb_angles--; break;}
			break;}
	    case 'g' : {if(old_dir=='h') {nb_angles++; break;}
			if(old_dir=='b') {nb_angles--; break;}
			break;}
	  }
	...
	poly->channel[1]=(nb_angles==4)? (float)poly_nb : -(float)poly_nb ;
	poly++;/* this fixes the label&orientation  of the current polygon */
      }                                              /* end-for lbptr      */
      poly_nb++;
    }                                                /* end loop levelset  */
}
...
static void BlackBound(Cimage whitesheet, float level)
{
  ...
  for(rptr=image.tete;rptr!=NULL;rptr=rptr->Rsuiv) 
    if(*rptr->canal<=level) 
      for(lbptr=rptr->Lbords;lbptr!=NULL;lbptr=lbptr->suiv) 
        for (cnxeptr=lbptr->bord->cnxe;cnxeptr!=NULL;cnxeptr=cnxeptr->suiv){
	  a0=cnxeptr->sb->x;  
	  b0=cnxeptr->sb->y;  
...
static void cDess_u_LS(Cimage boundary, Cimage u, float level, char inside, char outside)  
...
{                         /* Si oui on change de region, dans les deux cas   */
  ...
  for(i=0;i<ncols;i++) {
    ...
    for(j=1;j<nrows;j++){/* Dessine les pixels restants de la i-ieme colonne*/
      ...
      u->gray[ncols*j+i]=(*regact->canal<=level)? inside:outside;
    }
  }
}
...
{                         /* Si oui on change de region, dans les deux cas   */
  ...
  for(i=0;i<ncols-1;i++) {
    ...
    u->gray[i]=*regact->canal;
                           /* Dessine le premier pixel de la i-ieme colonne   */
    for(j=1;j<nrows-1;j++){/* Dessine les pixels restants de la i-ieme colonne*/
      if(boundary->gray[ncols*j+i]==0)  regact=RegAdjB(regact,i,j) ;
...
one_levelset(float *level, Cimage cb, Fpolygons pb, Fimage fu, Cimage cu, Fimage image_org)
{
  ...
  if(cb) 
    *cb=*boundary;                               /* either spitt it out */
    ...
    if((cu)||(fu)) mw_delete_cimage(boundary);   /* else to the trash   */
