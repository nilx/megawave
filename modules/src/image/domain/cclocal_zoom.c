...
Ccimage cclocal_zoom(Ccimage Input, int *X, int *Y, int *W, int *factor)
{
  ...
  if (squareR == NULL) mwerror(FATAL,1,"Not enough memory.\n");
  squareG = (unsigned char *) malloc(sx*sy);
  if (squareG == NULL) mwerror(FATAL,1,"Not enough memory.\n");
  squareB = (unsigned char *) malloc(sx*sy);
  if (squareB == NULL) mwerror(FATAL,1,"Not enough memory.\n");
  ...
  for (x=x0;x<=x1;x++) for(y=y0;y<=y1;y++) 
      {
	i = x+sx*y;
	r = squareR[i];	g = squareG[i]; b = squareB[i];
	l = *X+((*factor)*x)-Df + dx*(*Y+((*factor)*y)-Df);
	for (i=0;i<(*factor);i++) for (j=0;j<(*factor);j++) 
	  {
	    ll = l+j*dx+i;
	    Red[ll] = r;
	    Green[ll] = g;
