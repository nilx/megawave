             ...
             char *linear)
{
    ...
    if (!norm && !N)
    {
        ...
        if (p || q || d || aff)
            mwerror(USAGE, 1,
                    "-p -q -d -a options useless for simple thresholding\n");
        if (!m0 && !m1)
            mwerror(USAGE, 1,
                    "Please specify at least min or max threshold value\n");
        for (adr = n; adr--;)
        {
            w = in->gray[adr];
            if (min && w < *min)
    ...
    {
        /* affine normalization and maybe thresholding */
        ...
        if (i == 0)
        {
            ...
            if (aff && !linear)
                mwerror(USAGE, 1,
                        "-a option is useless without -p -q -d or -l option\n");
        ...
        {                       /* discard some values */
            ...
            mwerror(USAGE, 1, "You cannot combine -d with -p or -q option\n");
        ...
        if (norm)
        {
            if (linear)
            {
                ...
                b = 0.;
                m0 = *min;
                m1 = *max;      /* for thresholding */
            }
            else
            {
                if (!min || !max)
                    mwerror(USAGE, 1,
                            "Please specify min and max values (-n option)\n");
