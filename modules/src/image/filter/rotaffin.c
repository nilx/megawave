              ...
              Cmovie cmovie)
{
    ...
    for (Na = 0; Na < (*Naffi); Na++)
    {
        ...
        Nr2 = (*Nrota);
        if ((*Type == 1) && (Na == 0))
            Nr2 = 1;            /* cas du cercle, rotations inutiles */
        ...
        for (Nr = 0; Nr < Nr2; Nr++)
        {
            ...
            /* ANGLE */
            theta =
                2.0 * 3.1415926535 * ((double) Nr) / ((double) (*Nrota)) /
                (*OptSym);
            C1 = (float) cos(theta);
            S1 = (float) sin(theta);
            ...
            for (j = -(*Size); j <= (*Size); j++)
            {
                ...
                for (i = -(*Size); i <= (*Size); i++)
                {
                    ...
                    for (kx = 0; kx < (*Definition); kx++)
                        ...
                        {
                            x = ((float) i) + (((float) kx + 0.5) / Ndef -
                                               0.50);
                            y = ((float) j) + (((float) ky + 0.5) / Ndef -
                                               0.50);
...
/*                    printf(" C1=%f S1=%f \n ",C1,S1);
                      printf(" x2=%f y2=%f \n ",x2,y2); */
                            ...
                            if (*Type == 0)
                                somme += in_rect(x2, y2);
                            if (*Type == 1)
                                somme += in_circle(x2, y2);
                            if (*Type == 2)
                                somme += in_srect(x2, y2);
                            if (*Type == 3)
                                somme++;
                            if (*Type == 5)
                            {
                                if ((x2 > -1.0) && (x2 < 1.0) && (y2 > -1.0)
                                    && (y2 < 1.0))
                                {
                                    x2 = (1.0 + x2) * f_x * 0.5;
                                    y2 = (1.0 + y2) * f_y * 0.5;
...
/*                          if (somme==0) somme=1;  */
            ...
            mwdebug(" %f %f %f %f %f\n", pict->gray[0], pict->gray[1],
                    pict->gray[2], pict->gray[3], pict->gray[4]);
            mwdebug(" %f %f %f %f %f\n", pict->gray[5], pict->gray[6],
                    pict->gray[7], pict->gray[8], pict->gray[9]);
            mwdebug(" %f %f %f %f %f\n", pict->gray[10], pict->gray[11],
                    pict->gray[12], pict->gray[13], pict->gray[14]);
            mwdebug(" %f %f %f %f %f\n", pict->gray[15], pict->gray[16],
                    pict->gray[17], pict->gray[18], pict->gray[19]);
            mwdebug(" %f %f %f %f %f\n", pict->gray[20], pict->gray[21],
                    pict->gray[22], pict->gray[23], pict->gray[24]);
