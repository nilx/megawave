...
static void ResolutionInfSup(Cimage image, Cimage A, unsigned char *G,
                             float t1, float t2, Fmovie movie, int Nmask)
                            /* Image Buffer */
              ...
              Cimage image, Fmovie fmovie, Cimage output)
{
  ...
  /*------------------------  Number of images in the fmovie...  ------*/
    ...
    if ((fmovie->first) == NULL)
        mwerror(FATAL, 1, "empty masks movie !\n");
    Nmask = 1;
    i = fmovie->first;
    while (i->next != NULL)
    {
    ...
    for (n = 0; n < (*Niter); n++)
    {
        ...
        if (average)
        {
            ...
            ResolutionInfSup(image_save, image_work, G, t2, t1, fmovie,
                             Nmask);
