...
/* mwcommand
 ...
 function = {"Convert a color float image
...
#ifdef DEGRES
#define beta(S,H) \
    (1.0 + ((S) * cos ((H) * M_PI / 180.0)) / cos(P3-((H) * M_PI / 180.0)))
#else
...
void
cfchgchannels(int *Conv, int *Inverse, int *Norm, Cfimage Image,
              Cfimage TImage)
    ...
    if (Inverse)
        ...
        case 0:
            if (Image->model != MODEL_YUV)
                mwerror(WARNING, 1,
                        "colorimetric system of input image "
                        "may not be YUV !\n");
        ...
        case 1:
            if (Image->model != MODEL_HSI)
                mwerror(WARNING, 1,
                        "colorimetric system of input image is not HSI !\n");
            TImage->model = MODEL_RGB;
        ...
        case 2:
            if (Image->model != MODEL_HSV)
                mwerror(WARNING, 1,
                        "colorimetric system of input image is not HSV !\n");
            TImage->model = MODEL_RGB;
        ...
        case 0:
            if (Image->model != MODEL_RGB)
                mwerror(WARNING, 1,
                        "colorimetric system of input image is not RGB !\n");
