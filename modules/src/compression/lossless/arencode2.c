...
static int symb_ind_direct(int symbol)
{
    return (symbol + 1);
}

static int symb_ind_list(int symbol)
{
    int z;
...
static void REALLOCATE_OUTPUT(Cimage output)
{
    int i;
    Cimage bufcomp;
...
static void
START_MODEL(Fsignal histo, long int *cap_histo, int *predic, Fimage input,
            long int size)
{
    float *ptri;
    float *ptrl;
  ...
  /*--- Compute min and max of input symbols ---*/
      ...
      /*--- adaptative encoding ---*/
        ...
        /*--- List and compute the number of different symbols in input ---*/
                ...
                if (buflist == NULL)
                    mwerror(FATAL, 1,
                            "Memory allocation refused for list "
                            "of different symbol!\n");
                sizel = 1;
                ...
                for (i = 1, ptri = input->gray + 1; i < size; i++, ptri++)
                {
                    ...
                    ptrl = buflist;
                    while ((l < sizel)
                           && (floor(*ptrl + .5) != floor(*ptri + .5)))
                    {
                        l++;
                        if (l >= sizebuf)
                            mwerror(INTERNAL, 1,
                                    "[START_MODEL] Index l of buflist "
                                    "to large\n");
                        ptrl++;
                    }
                    if (l == sizel)
        ...
        /*--- Assign a symbol for signaling end of input ---*/
                ...
                if (sizel >= sizebuf)
                    mwerror(INTERNAL, 1,
                            "[START_MODEL] Index sizel=%d of buflist "
                            "to large (>=%d)\n", sizel, sizebuf);
            ...
            if (nsymbol > MAX_NSYMB)
                mwerror(FATAL, 2,
                        "Number of symbols too large (%d > %d) "
...
static void ADD_BIT_TO_OUTPUT(int bit, Cimage output)
                               /* String of codewords */
{
    buffer >>= 1;
    if (bit)
...
static void OUTPUT_BITS(int bit, Cimage output)
                               /* String of codewords */
{
...
static void ENCODE_INT(long int symb, long int max, Cimage output)
{
...
static void MAKE_HEADER(int *predic, Cimage output, long int *nbit)
                              ...
                              /* Number of output bits */
{
    ...
    if (output)
    {
        ENCODE_INT((long) nsymbol - 1, (long) 1 << (code_value_bits - 3),
                   output);
        if (predic)
...
static void
ENCODE_SYMBOL(int symbol, int symbol_pred, long int *low, long int *high,
              long int *nbit, Cimage output)
                              ...
                              /* String of codewords */
{
    ...
    if ((symbol_pred < 0) || (symbol_pred >= nsymbol_pred) || (symbol <= 0)
        || (symbol > nsymbol))
    {
    ...
    else
    {
        ...
        *high =
            *low +
            (range * cum_freq[symbol_pred][symbol - 1]) /
            cum_freq[symbol_pred][0] - 1;
        *low =
            *low +
            (range * cum_freq[symbol_pred][symbol]) /
            cum_freq[symbol_pred][0];
        ...
        for (;;)
        {
            ...
            {
                printf
                    ("low = %ld, high = %ld, symbol = %d, symbol_pred = %d\n",
...
void
arencode2(int *Print, long int *Size, int *NSymb, long int *Cap_Histo,
          int *Predic, Fsignal Histo, int *Header, Fimage Input, double *Rate,
          Cimage Output)
                                 ...
                                 /* String of codewords */
{
  ...
  /*--- Memory allocation for Output ---*/
    ...
    if (Output)
    {
        ...
        ncolo = Input->ncol + 1;
        ncolo *=
            ((double) log((double) nsymbol) / log((double) 2.0) /
             (double) 8.0);
        if (ncolo == 0)
            ncolo = 1;
  ...
  /*--- Make header ---*/
    ...
    /*--- Encode input ---*/
        ...
        for (i = 1; i < size; i++, ptri++)
        {
            ...
            if (teststop == 1)
                mwerror(FATAL, 1,
                        "i = %d, symb = %d, symb_pred = %d, "
                        "*ptri = %.2f,\nnbit = %d, first_qrt = %d, "
                        "half = %d, top_value = %d\n",
                        i, symb, symb_pred, *ptri, nbit, (long) first_qrt,
                        (long) half, (long) top_value);
    ...
    *Rate = (double) nbit / size;
    ...
    /*--- Resize Output ---*/
        ...
        Output = mw_change_cimage(Output, nrowo, ncolo);
        if (size > ncodewords)
            for (i = ncodewords, ptro = Output->gray + ncodewords; i < size;
                 i++, ptro++)
                *ptro = 0;
