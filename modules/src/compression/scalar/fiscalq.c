...
INT2FLOAT(int n, int nbitlog, int nbitstep)
{
  ...
  if (logf == 0)
  ...
  else
    {
      ...
      f = ((int) 1 << ((int) logstep * (logf - 1))) + (n - (1 << nbitstep) * logf + 1.0) * fstep; 
    }
  return(f);
}
  ...
  *rate = ((double) NBIT_NSTEP + 1.0) / ((double) 8.0 * *nrow1 * *ncol1);
  ...
  if (*center == 1) {
	...
	*minstep = - *minstep;
      ...
      if (!print) 
	printf("Quantization steps centered\nIndex of lowest step : %d\n", *minstep);

      *rate = ((double) NBIT_MINSTEP + 1.0) / ((double) 8.0 * *nrow1 * *ncol1);
	...
	*ashift = - *ashift;
      ...
      if (!print) 
	printf("Quantization steps not centered\nShift for lowest step : %.6f\n", *ashift);

      *rate = ((double) NBIT_LOG + NBIT_STEP + 1.0) / ((double) 8.0 * *nrow1 * *ncol1);
  ...
  if (*nstep > 1) {
    ...
    *rate = ((double) NBIT_LOG + NBIT_STEP) / ((double) 8.0 * *nrow1 * *ncol1);
...

{
  ...
  if (nstep > 1) {
    ...
    ardecode2(&nstep, &nrow1, &nstep, NULL, predic, NULL, compress, &rate1, symbol);
    *rate = rate1;
  } else
    symbol = NULL;
