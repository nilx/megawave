...
/* mwcommand
 ...
 version = {"1.1"};
 function = {"Change an image by operating on its 2D-wavelet packet representation"}; 
 usage = {
     't':[translation=1]->translation
     "Repeats the process on several translations of the image and average (like cycle spinning), enter the number of horizontal and vertical translations to be performed",
     ...
     "All the trees between the input tree and the tree of full depth provided by this input  are considered for the convolution (eigenvalues provided by -c must correspond to this latter tree)",
...
static void test_input(Cimage tree, Fimage pfilter, int *translation, float *threshold_soft, float *threshold_hard, float *track_noise_hard, float *track_noise_soft, int *track_noise_level, int *convolution_level)
{int i,size=tree->ncol*tree->nrow;
 int sup,sup_translation;
 int nbOption;

 /* avoids simultaneous use of soft and hard thresholding or noise tracking*/
 nbOption=0;
 if(threshold_soft) nbOption ++;
 if(threshold_hard) nbOption ++;
 if(track_noise_hard) nbOption ++;
 if(track_noise_soft) nbOption ++;
 
 if(nbOption>1)
   mwerror(FATAL, 1, "At most one option has to be chosen among -h, -s, -H, -S !\n");
 ...
 if(nbOption==0)
   mwerror(FATAL, 1, "At least one option has to be chosen among -h, -s, -H, -S, -c !\n");

 /* checks compatibility between  track_noise_level and other noise tracking options */

 if((track_noise_hard || track_noise_soft) && track_noise_level==NULL)
   mwerror(FATAL, 1, "Option -L should be specified when -H or -S are selected !\n");
  
 if(track_noise_hard==NULL && track_noise_soft==NULL && track_noise_level)
   mwerror(FATAL, 1, "Option -L is useless when neither option -H nor -S are selected !\n");
 ...
 if(pfilter)
   {if(convolution_level)
	 ...
	 mwerror(FATAL, 1, "The size of pfilter must be 2^convolution_level!\n");
     }
   else
     if( (tree->ncol != pfilter->ncol ) || (tree->nrow != pfilter->nrow ) ) 
 ...
 if((*translation) <= 0 || *translation > sup_translation)
   mwerror(FATAL, 1, "translation (option -t) should be between 1 and %d ! \n Generaly, 2 is good enough.\n",sup_translation);
...
static void threshold_hard_wavelet_packet_transform(Wpack2d pack, float *threshold_hard)
{int k;
 int i,size;
 float tmp;
...
static void threshold_soft_wavelet_packet_transform(Wpack2d pack, float *threshold_soft)
{int k;
 int i,size;
 float tmp;
...
static void change_pfilter_and_tree(Fimage pfilter_remaining, Fimage pfilter_to_perform, Cimage tempTree, Cimage tree)
{Cimage nextTree;                /*contains the the output value of tempTree, its size could be smaller than the size of tempTree */ 
 Fimage next_pfilter_remaining;     /*contains the the output value of pfilter_remaining, its size could be smaller than the size of nextTree */ 
 int kx,ky;                                 /*indexes for nextTree and next_pfilter_remaining*/
 int kx1,ky1;                             /*indexes for tempTree and pfilter_to_perform*/
 int extra;                                 /*ratio between the size of tempTree and the size of nextTree*/
 ...
 /*computes the next tree : it less decomposed than 'tempTree' but more decomposed than 'tree'*/
 nextTree=mw_new_cimage();

 wp2dchangetree(tempTree,nextTree,NULL,&not_null,NULL,NULL,NULL,NULL); /*avoids one decomposition, when possible*/
 wp2dchangetree(nextTree,nextTree,NULL,NULL,NULL,NULL,tree,NULL);/* makes sure it is sufficiently decomposed*/
  
 /**  computes pfilter_to_perform   and  next_pfilter_remaining **/

 /* memory allocation and initialisation */
 pfilter_to_perform = mw_change_fimage(pfilter_to_perform,tempTree->nrow,tempTree->ncol);
 if(pfilter_to_perform == NULL)
   mwerror(FATAL, 1, "Not enough memory !\n");

 next_pfilter_remaining = mw_change_fimage(NULL,nextTree->nrow,nextTree->ncol);
 ...
 for(level=1;level<=levelMax;level++)
   {
     ...
     for(kx=0,kx1=0;kx<nextTree->ncol;kx+=jumpSmallTree,kx1+= jumpLargeTree)
       for(ky=0,ky1=0;ky<nextTree->nrow;ky+=jumpSmallTree,ky1+= jumpLargeTree)
	 {
	   if(_(nextTree,kx,ky) ==level &&_(nextTree,kx,ky) <_(tempTree,kx1,ky1)) 
	     /*this means _(tempTree,kx1+x,ky1+y) = _(nextTree,kx,ky)+1 , for x and y between 0 and extra-1*/
	     {
	       /*computes the value in next_pfilter_remaining */
	       /*it is the minimum, among the modulus of_(pfilter_to_perform ,kx1+x,ky1+y) */
	       /* for x and y between 0 and jumpLarge-1 */
	       /* which are larger than 1*/
	       tmp=_(pfilter_remaining,kx1,ky1) ;	   
	       min = (tmp > 0) ? tmp : -tmp;
	       
	       for(x=0;x<jumpLargeTree;x++) for(y=0;y<jumpLargeTree;y++)
		 {
	       ...
	       for(x=0;x<jumpSmallTree;x++) for(y=0;y<jumpSmallTree;y++)
		 _(next_pfilter_remaining ,kx+x,ky+y)=min;
	       ...
	       for(x=0;x<jumpLargeTree;x++) for(y=0;y<jumpLargeTree;y++)
		 _(pfilter_to_perform,kx1+x,ky1+y)=min*_(pfilter_remaining,kx1+x,ky1+y);
	       
	     }
       
	   else if(_(nextTree,kx,ky) ==level) 
	     /* it means _(nextTree,kx,ky) == _(tempTree,kx1+x,ky1+y), for x and y between 0 and extra-1*/
	     {
	       for(x=0;x<jumpSmallTree;x++) for(y=0;y<jumpSmallTree;y++)
		 _(next_pfilter_remaining ,kx+x,ky+y)=_(pfilter_remaining ,kx1,ky1);
 ...
 if(pfilter_remaining->ncol != next_pfilter_remaining->ncol)
   {pfilter_remaining = mw_change_fimage(pfilter_remaining,next_pfilter_remaining->nrow,next_pfilter_remaining->ncol);
 ...
 /** free useless memory **/
 mw_delete_fimage(next_pfilter_remaining);next_pfilter_remaining=NULL;
 mw_delete_cimage(nextTree);nextTree=NULL;
}
...
{int k;
 int i,size;
 float tmp;
 float Threshold= *track_noise_hard, minusThreshold= -(*track_noise_hard);
...
{int k;
 int i,size;
 float tmp;
 float Threshold= *track_noise_soft, minusThreshold= -(*track_noise_soft);
...
/*****************************************************************/
/* removes the large coefficients from pack */
static void noise_search(Wpack2d pack, float *track_noise_hard, float *track_noise_soft)
{
  if(track_noise_hard)
...
static void noise_tracking(Wpack2d pack, int *track_noise_level, float *track_noise_hard, float *track_noise_soft)
{Cimage inOutTree;
 Cimage tmpTree;
 Wpack2d temppack;                 
...
/* save tree, the tree of the wpack2d is the same at the beginning and the end of this function */
 ...
 for(i=0;i<nbIter;i++)
   {
     wp2dchangetree(tmpTree,tmpTree,&not_null,NULL,NULL,NULL,NULL,NULL);
 ...
 /*the last resume is set to 0, so that it is preserved after the difference with pack*/
 ...
 /* computes the difference between pack and the noise found during the noise tracking process */
 wpack2d_diff(pack,temppack);
 
 /*FREE MEMORY */
 
 mw_delete_wpack2d(temppack); temppack=NULL;
 mw_delete_cimage(inOutTree);inOutTree=NULL;
 mw_delete_cimage(tmpTree);tmpTree=NULL;
}
...
void wp2doperate(Fimage input, Fsignal Ri, Fsignal Ri_biortho, Cimage tree, Fimage output, float *threshold_hard, float *threshold_soft, int *translation, float *track_noise_hard, float *track_noise_soft, int *track_noise_level, Fimage pfilter, int *convolution_level)
{
 ...
 test_input(tree,pfilter,translation,threshold_hard,threshold_soft,track_noise_hard,track_noise_soft,track_noise_level,convolution_level);
 ...
 /*copies pfilter in pfilter_remaining, sothat on  pfilter_remaining can eventually be modified */
 if(pfilter)
   {pfilter_remaining= mw_change_fimage(NULL,pfilter->nrow,pfilter->nrow);
   if(pfilter_remaining==NULL)
     mwerror(FATAL, 1, "Not enough memory !\n");
 ...
 if(convolution_level) /*creates tempTree and pfilter_to_perform and computes the number of iteration (one iteration per level)*/ 
   { if  ((tempTree= mw_new_cimage()) == NULL)
     mwerror(FATAL, 1, "Not enough memory !\n");
   wp2dmktree(*convolution_level,tempTree,NULL,NULL,NULL,&not_null,NULL); /*full tree of depth convolution_level */
   ...
   { if  ((tempTree= mw_change_cimage(NULL,tree->nrow,tree->ncol)) == NULL)
     mwerror(FATAL, 1, "Not enough memory !\n");
 ...
 for(tx=0;tx< (*translation);tx++) for(ty=0;ty< (*translation);ty++)
   {/* translation of the input image */ 
	 ...
	 if(threshold_hard)
	   threshold_hard_wavelet_packet_transform(pack,threshold_hard);
	 
	 if(threshold_soft)
	   threshold_soft_wavelet_packet_transform(pack,threshold_soft);
	 
	 if(track_noise_hard || track_noise_soft)
	   noise_tracking(pack,track_noise_level,track_noise_hard,track_noise_soft);
	 
	 /* apply operator diagonal in the wavelet packet domain*/
	 if(pfilter_remaining)
	   {if(iter == nbIter-1)
	     convol_wavelet_packet_transform(pack,pfilter_remaining);
	   else 
	     {change_pfilter_and_tree(pfilter_remaining,pfilter_to_perform,tempTree,tree);
	     
	     convol_wavelet_packet_transform(pack,pfilter_to_perform);
