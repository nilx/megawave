...
/* mwcommand
   ...
   function = {"Get the rectangular frequency grid used to perform
...
void ridgpolrec(Fimage out_re, Fimage out_im, Fimage in_re, Fimage in_im)
{
    ...
    if ((indic == 0) && (indic3 == 0))
        mwerror(USAGE, 1, "No output specified");
        ...
        for (j = 0; j < (n + 1); j++)
        {
            ...
            if (!(j1 == (n / 2)))
            {
                ...
                if (indic3 == 1)
                    ...
                    out_re->gray[x1 + n * y1] =
                        (((A->gray[x1 + n * y1]) -
                          1) * (out_re->gray[x1 + n * y1])
                         + in_re->gray[j1 + n * i]) / (A->gray[x1 + n * y1]);

                if (indic == 1)
                    out_im->gray[x1 + n * y1] =
                        ((A->gray[x1 + n * y1] -
                          1) * (out_im->gray[x1 + n * y1])
                         + in_im->gray[j1 + n * i]) / (A->gray[x1 + n * y1]);
            }
        }
    }
            ...
            if (!(j1 == (n / 2)))
            {
                ...
                if (indic3 == 1)
                    out_re->gray[x1 + n * y1] =
                        ((A->gray[x1 + n * y1] -
                          1) * (out_re->gray[x1 + n * y1])
                         + in_re->gray[j1 + n * i + n * n]) /
                        (A->gray[x1 + n * y1]);
                if (indic == 1)
                    out_im->gray[x1 + n * y1] =
                        ((A->gray[x1 + n * y1] -
                          1) * (out_im->gray[x1 + n * y1])
                         + in_im->gray[j1 + n * i + n * n]) /
                        (A->gray[x1 + n * y1]);
            }
        }
    }
