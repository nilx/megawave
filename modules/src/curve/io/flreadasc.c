...
Flists flreadasc(int dim)
{
  ...
  printf("Enter the values of each Flist, separated by <space> or <enter>\n");
  printf("End the current Flist by 'e' and the whole Flists with 'q'\n");
  
  do {
    l = mw_change_flist(NULL,10,0,dim);
    if (ls->size == ls->max_size) mw_enlarge_flists(ls);
    ls->list[ls->size++] = l;
    i=0;
    do {
      end = !scanf("%f",&x);
      if (!end) {
	if (i == l->max_size) mw_enlarge_flist(l);
	l->values[i++] = x;
      }      
    } while (!end);
    l->size = (i+dim-1)/dim;
  ...
  for (i=0;i<ls->size;i++) printf("%d ",ls->list[i]->size);
  printf("\n");
