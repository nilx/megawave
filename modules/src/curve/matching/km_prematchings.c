...
usage = {
   maxError->maxError     "maximum allowed Hausdorff distance
                          between two codes to be considered as pre-matching",
   dict1->dict1           "dictionary codes of image 1 (Flists)",
   dict2->dict2           "dictionary codes of image 2 (Flists)",
   matchings<-matchings   "output Flists: for each code of dict1,
                          the indices of the dict2 codes
                          which are closer than maxError in Hausdorff distance"
        };
*/
...
static unsigned char search_value(float x, float y, Flists dict, float maxErr,
                                  Flist list_matchings, int j)
{
    int i;
...
static void matchings_code(Flist liste, Flists dict, float maxErr, int Nnorm,
                           Flist matchaux)
{
    int i, n, m;
    unsigned char ok;
    ...
    if ((matchs_counter =
         mw_change_flist(NULL, dict->size, dict->size, 1)) == NULL)
        mwerror(FATAL, 1, "error, not enough memory\n");
    mw_clear_flist(matchs_counter, 0);
    i = 0;
    do
    {
        ok = search_value(_(liste, i, 0), _(liste, i, 1), dict, maxErr,
                          list_matchings, i);
        if (ok)
        {
        ...
        if ((list_matchings =
             mw_change_flist(NULL, dict->size, 0, 1)) == NULL)
            mwerror(FATAL, 1, "error, not enough memory\n");
    }
    while ((i < Nnorm) && ok);
                       ...
                       Flists matchings)
{
    ...
    if (maxError < 0.0)
        mwerror(FATAL, 1,
                "invalid argument type : maxError must be non-negative\n");
    if (dict1->size == 0)
    ...
    if (dict1->list[0]->size != dict2->list[0]->size)
        mwerror(FATAL, 1,
                "Codes in dict1 and dict2 must have "
                "the same number of points\n");

    if ((matchings =
         mw_change_flists(matchings, dict1->size, dict1->size)) == NULL)
        mwerror(FATAL, 1, "error, not enough memory\n");
