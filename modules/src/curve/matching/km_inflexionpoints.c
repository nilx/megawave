...
Flist km_inflexionpoints(Flist curve, Flist curve_IP)
{
    ...
    if (Closed)
    {
        x1 = _(curve, N_Points - 3, 0);
        y1 = _(curve, N_Points - 3, 1);
        x2 = _(curve, N_Points - 2, 0);
        y2 = _(curve, N_Points - 2, 1);
        x3 = _(curve, 0, 0);
        y3 = _(curve, 0, 1);
    ...
    for (i = 3; i < N_Points; i++)
    {
        ...
        sign2 = signdet(x3 - x2, y3 - y2, x1 - x2, y1 - y2);
        if (sign1 * sign2 < 0)
            _(curve_IP, curve_IP->size++, 0) = (float) (i - 1);
        if ((sign2 == 0) && (sign1 != 0))
        {
            signaux = sign1;
            iaux = i;
    ...
    if ((Closed)
        && ((sign2 * signauxinit < 0) || (signaux * signauxinit < 0)))
    {
        iaux = ((i - 1 + iauxinit + N_Points) / 2) % N_Points;
        if (iaux < N_Points / 2)
            _(curve_IP, curve_IP->size++, 0) = (float) N_Points - 1;
        else
            _(curve_IP, curve_IP->size++, 0) = (float) iaux;
    }
