...
static int get_next_point_angle(Flist fcrv, int i1, int i_max,
...
static int get_central_index(int i1, int i2)
{
    ...
    if (i1 + i > N_Points - 1)
        return (i1 + i - (N_Points - 1));
    return (i1 + i);
}

/* flat points of a closed curve without inflexion point */

static void get_flat_points_closed_curve(Flist curve_FP, Flist fcrv,
...
static void get_flat_points_convex(Flist curve_FP, Flist fcrv, int iFirst,
                                   int iLast, float dist, float angle,
                                   unsigned char noIPs)
{
                    ...
                    float dist)
{
    ...
    if (curve_IP->size != 0)
    {
        ...
        if (!Closed)
            get_flat_points_convex(curve_FP, curve, 0,
                                   (int) _(curve_IP, 0, 0), dist, angle, 0);

        for (i = 0; i < curve_IP->size - 1; i++)
            get_flat_points_convex(curve_FP, curve, (int) _(curve_IP, i, 0),
                                   (int) _(curve_IP, i + 1, 0), dist, angle,
                                   0);

        if (!Closed)
            get_flat_points_convex(curve_FP, curve,
                                   (int) _(curve_IP, curve_IP->size - 1, 0),
                                   N_Points - 1, dist, angle, 0);
        else
            get_flat_points_convex(curve_FP, curve,
                                   (int) _(curve_IP, curve_IP->size - 1, 0),
                                   (int) _(curve_IP, 0, 0), dist, angle, 0);
    ...
    {
        get_flat_points_convex(curve_FP, curve, 0, N_Points - 1, dist, angle,
                               1);
    }
