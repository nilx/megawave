...
/* mwcommand
   ...
   version = {"1.1"};
   function = {"Split a curve (Dlist) into convex components"};
   usage = {
  'e':eps->eps   "numerical precision (default 1e-15) CHANGE WITH CAUTION",
...
Dlists dsplit_convex(Dlist in, Dlists out, int *ncc, double *eps)
{
  ...
  /*** initialization ***/
    ...
    out = mw_change_dlists(out, 1, 1);
    if (!out)
        mwerror(FATAL, 1, "Not enough memory\n");
    out->list[0] = mw_change_dlist(out->list[0], 2, 0, 2);
    l = out->list[0];
  ...
  /*** dsplit_convex() is evaluated only if in has at least 4 points ***/
    ...
    px3 = *(p++);
    py3 = *(p++);
    d2 = dir(px1, py1, px2, py2, px3, py3, epsilon);
  ...
  /*** MAIN LOOP : d1=angle(P1-P2-P3) and d2=angle(P2-P3-P4) ***/
    ...
    while (ok)
    {
        ...
        px4 = *(p++);
        py4 = *(p++);
        d2 = dir(px2, py2, px3, py3, px4, py4, epsilon);
            ...
            if (ok)
            {
                if (!is_closed || ni)
                {
                    if (out->size == out->max_size)
                        mw_enlarge_dlists(out);
                    l = mw_change_dlist(out->list[out->size], 5, 0, 2);
                    out->list[out->size] = l;
                    out->size++;
                    if (!l)
                        mwerror(FATAL, 1, "Not enough memory\n");
  ...
  /*** END OF MAIN LOOP ***/
    ...
    if (!ni)
        mwdebug("This is a convex%s curve\n", is_closed ? " closed" : "");
    else
        mwdebug("This is a non-convex%s curve with %d inflexion point%s.\n",
