...
Dlists dsplit_convex(Dlist in, Dlists out, int *ncc, double *eps)
{
  ...
  while (ok) {
    ...
    if (d1*d2>0) {
      ...
      if (j/2+1>l->max_size) {mw_realloc_dlist(l,j+1); q=l->values;}
      q[j++] = px1 = px2;
      q[j++] = py1 = py2;
      px2 = px3; py2 = py3;
      px3 = px4; py3 = py4;
      ...
      /*** split curve ***/
      if (j/2+2>l->max_size) {mw_realloc_dlist(l,j/2+2); q=l->values;}
      q[j++] = px2;
      q[j++] = py2;
      q[j++] = mx = .5*(px2+px3);
      q[j++] = my = .5*(py2+py3);
      ...
      if (ok) {
	if (!is_closed || ni) {
	  if (out->size==out->max_size) mw_enlarge_dlists(out);
