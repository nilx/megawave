     ...
     float t;
{
  ...
  for (i=1;i<in->size;i++) {
	...
	out->values[j*2  ] = l*in->values[i*2  ]+(1.-l)*in->values[i*2-2];
	out->values[j*2+1] = l*in->values[i*2+1]+(1.-l)*in->values[i*2-1];
	j++;
      }
      cur -= (double)n*step;
    } 
     ...
     char *P;
{
  ...
  if (!in || !in->size) return(in);
  precision = 1.;
  g = sgauss(*std,NULL,NULL,&precision);
  tmp = mw_change_flist(NULL,in->size,0,2);
  for (i=*n;i--;) {
    if (!P) fksample(in,tmp,*t); 
    else mw_copy_flist(in,tmp);
    convol(tmp,in,g);
  }
  mw_delete_flist(tmp);
