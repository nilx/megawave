...
static double norm(double *u, double *v)
{
    return (sqrt
            ((*u - *v) * (*u - *v) +
             (*(u + 1) - *(v + 1)) * (*(u + 1) - *(v + 1))));
...
static double norm2(double *u, double *v)
{
    return ((*u - *v) * (*u - *v) +
            (*(u + 1) - *(v + 1)) * (*(u + 1) - *(v + 1)));
}
...
static double dot3(double *u, double *v, double *w)
{
    return ((*v - *u) * (*w - *u) +
            (*(v + 1) - *(u + 1)) * (*(w + 1) - *(u + 1)));
}
...
static int dir(double ax, double ay, double bx, double by, double cx,
               double cy)
{
    double det, prec;
...
static int sgdot(double ux, double uy, double vx, double vy)
{
    double dot, prec;
...
static int my_split_convex(Dlist in, double **out, int *ncc)
{
    ...
    double *p, *q, *pmax, mx, my, px1, py1, px2, py2, px3, py3, px4, py4,
        *first;

    if (!in)
        return (0);
    if (in->dim != 2)
        mwerror(FATAL, 1, "Dlist dimension must be equal to 2\n");
...
static int sample_chord(double *in, double *ch_in, int size, double *out,
                        double *ch_out, double eps2)
{
...
static double searchfar(double *in, double *pmax, int is_closed,
                        double *first, double *last, double *fixed,
                        double *pt, double **far, double **prevfar,
                        int *firstfar)
{
...
static void gamma_point(double *ext1, double *ext2, double *far,
                        double height, double *gp, double gam, double a,
                        double alpha)
{
...
static int gaceros(double *in, double *ch, int size, double *out, double gam,
                   double area_sz, double a, double alpha, double eps2,
                   double *sign)
{
  ...
  /*** compute points associated to significative chords ***/
    ...
    do
    {
        ...
        /*** search the furthest point on the arc ***/
                ...
                /* add the new point if it is far enough
                 * from the previous one */
                if (j == 0
                    || (j >= 2 && (norm2(out + (j - 2), gp) > eps2 / 25.0)))
                {
                    out[j] = *gp;
                    ch[j++] = *v - *p0;
                    out[j] = *(gp + 1);
        ...
        /*** compute middle point ***/
                ...
                *(w + 1) = ((1 - lambda) ** (p0 + 1) + lambda ** (p1 + 1));
                height =
                    -searchfar(p, pmax, is_closed, p1, q1, q0, w, &far,
                               &prevfar, &firstfar);
                prevfar = far;
                 ...
                 * from the previous one */
                if (j == 0
                    || (j >= 2 && (norm2(out + (j - 2), gp) > eps2 / 2500.0)))
                {
                    out[j] = *gp;
                    ch[j++] = *q0 - *w;
                    out[j] = *(gp + 1);
    ...
    /*** more precise computation of cur_area if needed ***/
        ...
        if (is_closed)
            ...
            stop = ((q0 + 2 == pmax) && (cur_area - inc_area <= area_sz));
    }
    while (!stop);
...
static double height(double *in, double *pmax, int is_closed, double *first,
                     double *last, double *fixed, double *pt, double **far,
                     int *firstfar)
{
    double *p, *f;
    double a, h, l;
...
static double saturation(double *in, int size, double area_sz)
{
 ...
 /*** compute points associated to significative chords ***/
    ...
    do
    {
        ...
        /*** search the furthest point on the arc ***/
                a = height(in, pmax, is_closed, p0, q1, p0, pt, &far,
                           &firstfar);
                if (a > h)
                    h = a;
            }
        ...
        else
        {
            ...
            if (cur_area - inc_area <= area_sz)
            {
                ...
                *(pt + 1) = ((1 - lambda) ** (p0 + 1) + lambda ** (p1 + 1));
                a = height(in, pmax, is_closed, p0, q0, q0, pt, &far,
                           &firstfar);
                if (a > h)
                    h = a;
            }
            ...
            {
                cur_area -= inc_area;
                /*force furthest point to stay between
...
static int convexify(double *in, double *ch, double *cvx_remove, int size,
                     double sign)
{
    ...
    for (j = 2; j < 2 * size; j += 2)
        removed += cvx_remove[j] =
            (sgdot(ch[j], ch[j + 1], in[j] - in[j - 2], in[j + 1] - in[j - 1])
             < 0);
    pmax = in + 2 * size;
    ...
    {
        pmax -= 2;
        removed += cvx_remove[0] =
            (sgdot(ch[0], ch[1], in[0] - *(pmax - 2), in[1] - *(pmax - 1)) <
             0);
    }
    else
        cvx_remove[0] = cvx_remove[2 * size - 2] = 0;
    /* cannot remove extremal points */
  ...
  /*** return curve if size < 3 ***/
    ...
    while (cvx_remove[p1 - in] && p1 + 2 != pmax)
        p1 += 2;
    p0 = p1 - 2;
    if (p1 + 2 == pmax)
        q1 = in;
        ...
        q1 = p1 + 2;
    while (cvx_remove[q1 - in] && q1 + 2 != pmax)
        q1 += 2;
    q0 = q1 - 2;
    p = p1;
    forward = TRUE;
                ...
                if (!is_closed)
                {
                    ...
                    else
                    {
                        ...
                        while (cvx_remove[p1 - in] && p1 != in)
                            p1 -= 2;
                        if (p1 == in)
                        {       /*p1 is first point : force to go forward */
                            if (q1 + 2 == pmax)
                                stop = TRUE;
                            else
                            {
                                ...
                                p0 = p1 - 2;
                                q1 += 2;
                                while (q1 + 2 != pmax && cvx_remove[q1 - in])
                                    q1 += 2;
                                q0 = q1 - 2;
                            }
                        }
...
static void g_eros(Dlist li, double gam, double *scale, double *area_sz,
                   double eps2, int *ncc, int conv)
                      /* input/output Dlist */
                      /* relative precision squared */
                      /* power of the curvature */
                      /* final scale (return real one) */
                      /* erosion area */
                      /* number of convex components after erosion */
                      /*do NOT convexify after erosion */
{
  ...
  /*** compute minimal area ***/
    ...
    /*** resample curve ***/
        first = pts2[i];
        pts2[i + 1] =
            sample(pts1[i], (pts1[i + 1] - pts1[i]) / 2, first, eps2);
  ...
  /*** apply gaceros to convex components and link result ***/
    ...
    for (i = 0; i < nc; i++)
    {
        ...
        if (npc)
            npc =
                sample_chord(pts1[0], chord2[i], npc,
                             li->values + li->size * 2, chord1[0], eps2);
        if (!conv)
        {
            if (npc)
                li->size += convexify(li->values + li->size * 2, chord1[0],
                                      chord2[0], npc, sign);
        }
        else
            li->size += npc;
            ...
            int *iter, char *conv)
{
    ...
    if (*n < 0)
        mwerror(FATAL, 1, "n must be positive\n");
    ...
    for (j = 0; j < in->size; j++)
    {
        if (in->list[j]->size > maxsize)
            maxsize = in->list[j]->size;
        per[j] = 0.;
  ...
  /*** absolute precision (squared) ***/
    ...
    if (!pts1[0] || !pts2[0] || !chord1[0] || !chord2[0])
        mwerror(FATAL, 1, "Not enough memory\n");
  ...
  /*----- FIRST LOOP (all curves) -----*/
    ...
    /*** convert scale *final from meters to semigoup unit (modulo cte) ***/
        ...
        newlast =
            pow(*last, *gam + 1.) / (*gam + 1.0) / pow(omega, 3.0 * (*gam));
    ...
    /*----- SECOND LOOP (iterations) -----*/
      ...
      /*** one step ***/
            g_eros(out->list[j], *gam, &a, &are, eps2, &ncc, conv && iter
                   && (i == *iter));
            if (iter && i == *iter)
                remaining = 0;
            else
            ...
            if (v)
            {
                t = pow((*gam + 1.) * pow(omega,
                                          3. ** gam) * (newlast - remaining),
                        1. / (*gam + 1.));;
                printf
                    ("Iter %4d : %4d cc, area = %.4f, "
                     "current scale = %.4f\033[K\n\033[1A", i, ncc, are, t);
                if (!out->list[j]->size)
        ...
        if (out->list[j]->size)
        {
            for (i = 0; i < out->list[j]->size * 2; i++)
                pts1[0][i] = out->list[j]->values[i];
            out->list[j]->size =
                (sample
                 (pts1[0], out->list[j]->size, out->list[j]->values,
                  eps2) - out->list[j]->values) / 2;
        }
        else
            collapsed++;
    }
    ...
    free(per);
    if (collapsed)
        printf("%d curves over %d collapsed.\n", collapsed, in->size);
