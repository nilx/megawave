                      ...
                      unsigned long lg)
{
    ...
    while (tab < end)
    {
        if (*tab == fold)
            *tab = fnew;
        else if (*tab > fold && *tab != MAXSHAPES)
            (*tab)--;
        tab++;
    }
}

static void DecaleStart(long int *start, int ou, int nbforme)
{
    start += ou;
...
static unsigned long *AllocTab(long unsigned int size)
{
        ...
        for (j = 0; j < w; j++)
        {
            ...
            if (in->gray[i * w + j] <= grey)
            {
                ...
                else
                {
                    ...
                    if (fsd != MAXSHAPES && fsd != f && fsd != fsg
                        && fsd != fs)
                    {
                    ...
                    if (fg != MAXSHAPES && fg != f && fg != fsg && fg != fs
                        && fg != fsd)
                    {
