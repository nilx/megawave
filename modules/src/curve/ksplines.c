...
static void compute_spline(Curve S, Curve P, int *X, Fimage N, int C, int NPC,
                           float Step)
                 /* Spline to compute */
                 /* Control points */
                 /* Node vector */
                 ...
                 /* Step in the discretization of the spline */
{
    ...
    for (a = C - 1; a <= NPC; a++)
    {
        ...
        for (t = X[a]; t <= X[a + 1] - Step + 0.1 * Step; t += Step)
        {
            for (j = 2; j <= C; j++)
            {
                ...
                for (i = 0, pc = P->first; i <= NPC; i++, pc = pc->next)
                {
                    ...
                    if (v == 0.0)
                        ...
                        d = ((t - X[i]) * v) / (X[i + j - 1] - X[i]);
                    v = mw_getdot_fimage(N, i + 1, j - 1);
                    if (v == 0.0)
                        e = 0.0;
                    else
                        e = ((X[i + j] - t) * v) / (X[i + j] - X[i + 1]);
                    s = d + e;
            ...
            yy = floor((double) y + .5);
            mwdebug
                ("\tSpline point #%d computed: (%f,%f) quantized to (%d,%d)\n",
                 k, x, y, xx, yy);
            ...
            if ((k == 1) || ((xx != xx0) || (yy != yy0)))
            {
                ...
                {
                    mwerror(ERROR, 1,
                            "Not enough memory to create spline point #%d: "
                            "Abort.\n", k);
                    return;
                }
                if (S->first == NULL)
    ...
    {
        mwdebug
            ("\tPut the last control point (%d,%d) as the spline point #%d\n",
             pc->x, pc->y, k);
        ...
        {
            mwerror(ERROR, 1,
                    "Not enough memory to create spline point #%d: Abort.\n",
                    k);
            return;
        }
        if (S->first == NULL)
                  ...
                  /* Step in the discretization of the spline */
{
    ...
    if (*C < 2)
        mwerror(USAGE, 1, "Illegal spline order j=%d\n", *C);
    ...
    for (P = ctrl_pts->first, cp_number = 1; P; P = P->next, cp_number++)
    {
        ...
        if (M < *C)
            mwerror(WARNING, 1,
                    "Cannot generate spline associated to the control\n"
