...
void sop(Fsignal B, Fsignal out, Fsignal A, float *a, char *plus, char *minus,
         ...
         char *sup, char *less, char *greater, char *equal)
{
    ...
    for (i = B->size; i--;)
    {
        ...
        if (plus)
            ...
            res = ABS(left - B->values[i]);
        else if (norm)
            res = sqrt((double) left * (double) left
                       + (double) (B->values[i]) * (double) (B->values[i]));
        else if (inf)
            res = (left < B->values[i] ? left : B->values[i]);
        else if (sup)
            res = (left > B->values[i] ? left : B->values[i]);
        else if (less)
            res = (left < B->values[i] ? 1.0 : 0.0);
        else if (greater)
            res = (left > B->values[i] ? 1.0 : 0.0);
        else if (equal)
            res = (left == B->values[i] ? 1.0 : 0.0);
