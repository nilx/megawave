...
short _mw_create_wtrans2d_header(char *fname, Wtrans2d wtrans)
{
    ...
    if (wtrans == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Wtrans2d structure is NULL\n");
...
Wtrans2d _mw_wtrans2d_load_wtrans(char *fname, char *type)
{
    ...
    for (j = 1; j <= wtrans->nlevel; j++)
        for (d = 0; d <= wtrans->norient; d++)
        {
            ...
            if (image == NULL)
                mwerror(INTERNAL, 0,
                        "[_mw_wtrans2d_load_wtrans] NULL image returned "
                        "in loading file \"%s\"\n", wfname);
...
short _mw_wtrans2d_create_wtrans(char *fname, Wtrans2d wtrans, char *type)
{
    ...
    {
        mwerror(INTERNAL, 0,
                "[_mw_wtrans2d_create_wtrans] Cannot create wtrans2d : "
                "NULL wtrans.\n");
        return (-1);
    }

    if (_mw_create_wtrans2d_header(fname, wtrans) != 0)
        return (-1);
    ...
    for (j = 1; j <= wtrans->nlevel; j++)
        for (d = 0; d <= wtrans->norient; d++)
        {
            ...
            if (image == NULL)
                mwerror(INTERNAL, 0,
                        "[_mw_wtrans2d_create_wtrans] NULL "
                        "wtrans->image[%d][%d]\n", j, d);
