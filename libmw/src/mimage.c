...
void mw_delete_point_type(Point_type point)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_point_type] cannot delete : "
                "point_type structure is NULL\n");
        return;
    }
...
Point_type mw_copy_point_type(Point_type in, Point_type out)
{
    ...
    if (!out)
    {
        ...
        if (!out)
            mwerror(ERROR, 0,
                    "[mw_copy_point_type] Not enough memory "
                    "to create a point_type\n");
        return (NULL);
    }
...
void mw_delete_morpho_line(Morpho_line morpho_line)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_morpho_line] cannot delete : "
                "morpho_line structure is NULL\n");
        return;
    }

    if (morpho_line->first_point)
        mw_delete_point_curve(morpho_line->first_point);
    morpho_line->first_point = NULL;
    if (morpho_line->first_type)
        mw_delete_point_type(morpho_line->first_type);
    morpho_line->first_type = NULL;
...
Morpho_line mw_copy_morpho_line(Morpho_line in, Morpho_line out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_morpho_line] Not enough memory "
                    "to create a morpho_line !\n");
            return (NULL);
        }
    }
...
unsigned int mw_length_morpho_line(Morpho_line morpho_line)
{
    ...
    if ((!morpho_line) || (!morpho_line->first_point))
        return (0);
    ...
    if (!morpho_line->first_type)
    {
        ...
        if ((n * m != 0) && (n != m))
            mwerror(INTERNAL, 1,
                    "[mw_length_morpho_line] Inconsistent Morpho_line "
                    "structure \n");
    }
    return (n);
}
...
unsigned int mw_num_morpho_line(Morpho_line ml_first)
{
    ...
    if (!ml_first)
    {
        mwerror(ERROR, 0, "[mw_num_morpho_line] NULL input morpho_line\n");
        return (0);
    }
...
Fmorpho_line mw_new_fmorpho_line(void)
{
    ...
    if (!
        (fmorpho_line = (Fmorpho_line) (malloc(sizeof(struct fmorpho_line)))))
    {
...
void mw_delete_fmorpho_line(Fmorpho_line fmorpho_line)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_fmorpho_line] cannot delete : "
                "fmorpho_line structure is NULL\n");
        return;
    }

    if (fmorpho_line->first_point)
        mw_delete_point_fcurve(fmorpho_line->first_point);
    fmorpho_line->first_point = NULL;
    if (fmorpho_line->first_type)
        mw_delete_point_type(fmorpho_line->first_type);
    fmorpho_line->first_type = NULL;
...
Fmorpho_line mw_copy_fmorpho_line(Fmorpho_line in, Fmorpho_line out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_fmorpho_line] Not enough memory "
                    "to create a fmorpho_line !\n");
            return (NULL);
        }
    }
    out->minvalue = in->minvalue;
...
unsigned int mw_length_fmorpho_line(Fmorpho_line fmorpho_line)
{
    ...
    if ((!fmorpho_line) || (!fmorpho_line->first_point))
        return (0);
    ...
    if (!fmorpho_line->first_type)
    {
        ...
        if ((n * m != 0) && (n != m))
            mwerror(INTERNAL, 1,
                    "[mw_length_fmorpho_line] Inconsistent Fmorpho_line "
                    "structure \n");
    }
    return (n);
}
...
void mw_delete_hsegment(Hsegment segment)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_hsegment] cannot delete : "
                "segment structure is NULL\n");
        return;
    }
...
void mw_delete_morpho_set(Morpho_set morpho_set)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_morpho_set] cannot delete : "
                "morpho_set structure is NULL\n");
        return;
    }

    if (morpho_set->first_segment)
        mw_delete_hsegment(morpho_set->first_segment);
    morpho_set->first_segment = morpho_set->last_segment = NULL;
    morpho_set->neighbor = NULL;
    free(morpho_set);
    morpho_set = NULL;
...
Morpho_set mw_copy_morpho_set(Morpho_set in, Morpho_set out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_morpho_set] Not enough memory "
                    "to create a morpho_set !\n");
            return (NULL);
        }
    }
...
unsigned int mw_length_morpho_set(Morpho_set morpho_set)
{
    ...
    if ((!morpho_set) || (!morpho_set->first_segment))
        return (0);
...
void mw_delete_morpho_sets(Morpho_sets morpho_sets)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_morpho_sets] cannot delete : "
...
Morpho_sets mw_copy_morpho_sets(Morpho_sets in, Morpho_sets out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_morpho_sets] Not enough memory "
                    "to create a morpho_sets !\n");
            return (NULL);
        }
    }
    ...
    for (p = in; p && p->morphoset; p = p->next)
    {
        ...
        if (!newiss->morphoset)
            newiss->morphoset = mw_new_morpho_set();
        if (!newiss->morphoset)
        {
            mwerror(ERROR, 0, "Not enough memory to create a morpho_sets\n");
            return (NULL);
    ...
    for (p = in, q = out, n = 1; p && q;
         p->morphoset->num = n, q->morphoset->num = n, p = p->next, q =
         q->next, n++);
    ...
    for (p = in, q = out; p && q; p = p->next, q = q->next)
        for (nin = p->morphoset->neighbor; nin; nin = nin->next)
        {
            oldiss = newiss = NULL;
            n = nin->morphoset->num;
            for (qq = out; qq && qq->morphoset && (qq->morphoset->num != n);
                 qq = qq->next);
            if (!qq || !(qq->morphoset))
            {
                mwerror(ERROR, 0,
                        "Cannot copy morpho sets : "
            ...
            {
                mwerror(ERROR, 0,
                        "Not enough memory to create a morpho_sets\n");
                return (NULL);
            }
            newiss->morphoset = qq->morphoset;
            if (q->morphoset->neighbor == NULL)
                q->morphoset->neighbor = newiss;
            if (oldiss)
                oldiss->next = newiss;
            newiss->previous = oldiss;
            oldiss = newiss;
...
unsigned int mw_length_morpho_sets(Morpho_sets morpho_sets)
{
    ...
    if ((!morpho_sets) || (!morpho_sets->morphoset))
        return (0);
...
unsigned int mw_num_morpho_sets(Morpho_sets mss_first)
{
    ...
    if (!mss_first)
    {
        mwerror(ERROR, 0, "[mw_num_morpho_sets] NULL input morpho_sets\n");
        return -1;
    }
    ...
    if (mss && !mss->morphoset)
        mwerror(ERROR, 0,
                "Morphosets chain cut "
                "due to the morphosets #%d which has no morphoset field\n",
                n);
    return (n - 1);
}
...
void mw_morpho_sets_clear_stated(Morpho_sets mss_first)
{
    ...
    for (mss = mss_first; mss && mss->morphoset;
         mss->morphoset->stated = 0, mss = mss->next);
...
void mw_delete_mimage(Mimage mimage)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_mimage] cannot delete : "
    ...
    if (mimage->first_ms)
        mw_delete_morpho_sets(mimage->first_ms);
...
Mimage mw_copy_mimage(Mimage in, Mimage out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_mimage] Not enough memory "
                    "to create a mimage !\n");
            return (NULL);
        }
    }
