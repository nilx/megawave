...
int BitOffset = 0,              /* Bit Offset of next code */
    ...
    CurCode = 0, OldCode = 0, InCode = 0,       /* Decompressor variables */
...
Cimage _mw_cimage_load_gif(char *fname)
{
    ...
    if ((fread(ptr, filesize, 1, fp) != 1) || (strncmp((char *) ptr, id, 6)))
    {
        mwerror(ERROR, 0,
                ...
                "Not a GIF format or file corrupted !\n", fname);
        free(ptr);
        free(Raster);
        fclose(fp);
        return (NULL);
    ...
    {
        mwerror(ERROR, 0,
                "GIF file \"%s\" is corrupted (no image separator) !\n",
                fname);
        free(ptr);
        free(Raster);
        fclose(fp);
    ...
    if (!HasColormap && !(Misc & 0x80))
    {
        ...
        mwerror(WARNING, 0,
                "No colormap in the GIF file \"%s\": Assuming Gray scale.\n",
                fname);
    }
    ...
    while (ch1);
    free(RawGIF);
    RawGIF = NULL;              /* We're done with the raw data now */
    ...
    while (Code != EOFCode)
    {
        ...
        else
        {
             ...
             * MaxCode value, increment the code size unless it's
             * already 12.  If it is, do nothing: the next code
    ...
    if (not_a_gray_image != 0)
        mwerror(WARNING, 0,
                "Convert a %d colors image to a gray levels image\n",
                ColorMapSize);

    picptr = image->gray;
    for (i = 1; i <= npixels; i++, picptr++)
        *picptr = MONO(r[*picptr], g[*picptr], b[*picptr]);

    _mw_flip_image((unsigned char *) image->gray, sizeof(char), Width, Height,
                   FALSE);
...
short _mw_cimage_create_gif(char *fname, Cimage image)
{
    ...
    {
        mwerror(FATAL, 1,
                "Image too big to be saved using GIF external type !\n");
        return (-1);
    }
    ...
    if (_BitsPerPixel <= 1)
        ...
        _InitCodeSize = _BitsPerPixel;

    curx = cury = 0;

    if (6 > fwrite("GIF87a", 1, 6, fp))
...
/********************************/
/* FIXME : variable shadows, temporary dirty fix */
static void cl_hash(count_int _hsize)
{                               /* reset code table */
