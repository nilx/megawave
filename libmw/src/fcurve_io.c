...
Fcurve _mw_load_fcurve_mw2_fcurve(char *fname)
{
    ...
    if (strcmp(ftype, "MW2_FCURVE") != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_fcurve_mw2_fcurve] File \"%s\" is not "
                "in the MW2_FCURVE format\n", fname);
    ...
    remainsize = filesize / sizeof(float);      /* Number of coordinates */
    if ((remainsize % 2) != 0)
    {
        mwerror(ERROR, 0,
    ...
    while (remainsize > 0)
    {
        ...
        if (readsize > buf_size)
            readsize = buf_size;
        /* FIXME: wrong types, dirty temporary fix */
        if (fread(buffer, sizeof(float), readsize, fp) !=
...
Fcurve _mw_load_fcurve(char *fname, char *type)
{
    ...
    if (type[0] == '?')
        ...
        mwerror(FATAL, 1,
                ...
                "I Don't know how to load such external type into a Fcurve !\n",
                fname, type);
    return NULL;
}
...
short _mw_create_fcurve_mw2_fcurve(char *fname, Fcurve cv)
{
    ...
    if (cv == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_fcurve_mw2_fcurve] Cannot create file: "
                "Fcurve structure is NULL\n");
     ...
     * if (cv->first == NULL)
     * mwerror(INTERNAL, 1, "[_mw_create_fcurve_mw2_fcurve] Fcurve has "
     *         "no point fcurve !\n");
     */
...
Fcurves _mw_load_fcurves_mw2_fcurves_1_00(char *fname)
{
    ...
    if (strcmp(ftype, "MW2_FCURVES") != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_fcurves_mw2_fcurves] File \"%s\" is not "
                "in the MW2_FCURVES format\n", fname);
    ...
    while (eof != 1)
    {
        ...
        if (xsave >= END_OF_CURVE_THRESHOLD)
...
Fcurves _mw_load_fcurves_mw2_fcurves(char *fname)
{
    ...
    if (version == 1.0)
        return (_mw_load_fcurves_mw2_fcurves_1_00(fname));
...
short _mw_create_fcurves_mw2_fcurves(char *fname, Fcurves cvs)
{
    ...
    if (cvs == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_fcurves_mw2_fcurves] Cannot create file: "
                "Fcurves structure is NULL\n");
    ...
    for (pl = cvs->first, n = 1; pl; pl = pl->next, n++)
    {
        for (pc = pl->first; pc; pc = pc->next)
        {
            if ((pc->x >= END_OF_CURVES_THRESHOLD) ||
                ...
                mwerror(INTERNAL, 1,
                        "[_mw_create_fcurves_mw2_fcurves] Fcurve #%d "
                        "has a point which coordinates (%f,%f) "
                        "exceed float capacity\n", n, pc->x, pc->y);
            vx = pc->x;
            vy = pc->y;
            if (1 > fwrite(&vx, sizeof(float), 1, fp)
                || 1 > fwrite(&vy, sizeof(float), 1, fp))
