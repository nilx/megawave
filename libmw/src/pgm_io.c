...
Cimage _mw_cimage_load_pgma(char *file)
{
    ...
    if ((getc(fp) != 'P') || (getc(fp) != '2'))
        mwerror(INTERNAL, 1,
                "[_mw_cimage_load_pgma] Error while reading file \"%s\": "
                "not a PGMA format !\n", file);
    ...
    {
        mwerror(ERROR, 0,
                "Error while reading file \"%s\": bad PGMA format !\n", file);
        fclose(fp);
        return (NULL);
    }
    if (maxgl > 255)
    {
        mwerror(ERROR, 0,
                "Cannot load PGMA image with gray levels "
                "exceeding value 255\n");
        fclose(fp);
        return (NULL);
    }
    ...
    for (l = 0; l < ncol * nrow; l++)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "Error while reading PGMA image \"%s\": "
    ...
    _mw_flip_image((unsigned char *) image->gray, sizeof(char), ncol, nrow,
                   FALSE);
    return (image);
}
...
short _mw_cimage_create_pgma(char *file, Cimage image)
{
    ...
    {
        mwerror(INTERNAL, 0,
                "[_mw_cimage_create_pgma] NULL cimage or cimage plane\n");
        return (-2);
    }
...
Cimage _mw_cimage_load_pgmr(char *file)
{
    ...
    if ((getc(fp) != 'P') || (getc(fp) != '5'))
        mwerror(INTERNAL, 1,
                "[_mw_cimage_load_pgmr] Error while reading file \"%s\": "
                "not a PGMR format !\n", file);
    ...
    {
        mwerror(ERROR, 0,
                "Error while reading file \"%s\": bad PGMR format !\n", file);
        fclose(fp);
        return (NULL);
    }
    ...
    {
        mwerror(ERROR, 0,
                "Error while reading PGMR image \"%s\": "
                "unexpected end of file !\n", file);
        fclose(fp);
        mw_delete_cimage(image);
        return (NULL);
...
short _mw_cimage_create_pgmr(char *file, Cimage image)
{
    ...
    {
        mwerror(INTERNAL, 0,
                "[_mw_cimage_create_pgmr] NULL cimage or cimage plane\n");
        return (-2);
    }
