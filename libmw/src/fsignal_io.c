...
Fsignal _mw_load_fsignal_ascii(char *fname, Fsignal signal)
{
    ...
    if (_mw_fascii_get_optional_field
        (fp, fname, "bpsample:", "%d\n", &BitsPerSample) == 1)
...
short _mw_create_fsignal_ascii(char *fname, Fsignal signal)
{
    ...
    if (signal == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Fsignal structure is NULL\n");
...
Fsignal _mw_load_fsignal(char *fname, char *type, Fsignal signal)
{
    ...
    need_flipping =
        _mw_get_file_type(fname, type, mtype, &hsize, &version) - 1;
    if (need_flipping >= 0)
    {
        ...
        if (strcmp(type, "WAVE_PCM") == 0)
            return ((Fsignal)
                    _mw_fsignal_load_wave_pcm(fname, signal, need_flipping));
    }
