...
void mw_delete_vpoint_wmax(Vpoint_wmax vpoint)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_vpoint_wmax] cannot delete : "
                "vpoint_wmax structure is NULL\n");
        return;
    }
    vpoint->previous = NULL;
    vpoint->next = NULL;
...
void mw_delete_vchain_wmax(Vchain_wmax vchain)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_vchain_wmax] cannot delete : "
                "vchain_wmax structure is NULL\n");
        return;
    }
    ...
    if (vpoint != NULL)
    {
        ...
        for (; vpoint && vpoint->previous; vpoint = vpoint->previous)
            if (vpoint->next)
                mw_delete_vpoint_wmax(vpoint->next);
...
void mw_delete_vchains_wmax(Vchains_wmax vchains)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_vchains_wmax] cannot delete : "
                "vchains_wmax structure is NULL\n");
        return;
    }
    ...
    if (vchain != NULL)
    {
        ...
        for (; vchain && vchain->previous; vchain = vchain->previous)
            if (vchain->next)
                mw_delete_vchain_wmax(vchain->next);
...
Vchain_wmax mw_copy_vchain_wmax(Vchain_wmax vchain1, Vchain_wmax vchain0)
{
    ...
    for (vpoint0 = vchain0->first; vpoint0; vpoint0 = vpoint0->next)
    {
        ...
        if (vpoint1 == NULL)
            return (NULL);
        if (vchain1->first == NULL)
            vchain1->first = vpoint1;
        if (oldvpoint1 != NULL)
            oldvpoint1->next = vpoint1;
...
int mw_give_nlevel_vchain(Vchain_wmax vchain)
{
    ...
    nlevel = 0;
    for (vpoint = vchain->first; vpoint && (nlevel == 0);
         vpoint = vpoint->next)
        for (nlevel = 0;
             (nlevel < mw_max_nlevel)
