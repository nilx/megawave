...
void mw_delete_point_curve(Point_curve point)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_point_curve] cannot delete : "
                "point_curve structure is NULL\n");
        return;
    }
...
/* Copy all the points of a curve starting from a Point_curve into
 * another chain */
...
Point_curve mw_copy_point_curve(Point_curve in, Point_curve out)
{
    ...
    if (!out)
    {
        ...
        if (!out)
            mwerror(ERROR, 0,
                    "[mw_copy_point_curve] Not enough memory "
...
Curve mw_copy_curve(Curve in, Curve out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_curve] Not enough memory to create a curve !\n");
            return (NULL);
        }
    }
    ...
    if (out->first == NULL)
    {
        ...
        mwerror(ERROR, 0,
                "[mw_copy_curve] Not enough memory to copy the curve\n");
        return (NULL);
    }
...
void mw_delete_curves(Curves curves)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_curves] cannot delete : "
