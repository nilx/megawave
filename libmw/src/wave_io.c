...
#define BS 65536
    ...
    {
        mwerror(ERROR, 1,
                "Cannot load WAVE file '%s' : format chunk too short !\n",
                fname);
        fclose(fp);
        return (NULL);
    }
    ...
    if (wFormatTag != 0x0001)
        ...
        mwerror(INTERNAL, 1,
                "[_mw_fsignal_load_wave_pcm] WAVE file '%s' : "
                "no PCM encoding !\n", fname);
    /* Number of channels */
    ...
    if (need_flipping == 1)
        _mw_in_flip_b4(dwSamplesperSecond);
    ...
    if (need_flipping == 1)
        _mw_in_flip_b4(dwAvgBytesPerSec);
    ...
    if (need_flipping == 1)
        _mw_in_flip_b2(wBitsPerSample);
    /*printf("Bits per Sample = %d\n",wBitsPerSample); */
    if (!((wBitsPerSample == 8) || (wBitsPerSample == 16)
          || (wBitsPerSample == 32)))
    {
        mwerror(WARNING, 1,
                "Cannot load WAVE file '%s' : no code to handle "
                "data of %d bits/sample, sorry !\n", fname, wBitsPerSample);
        fclose(fp);
        return (NULL);
    }
    ...
    {
        mwerror(ERROR, 1,
                "Cannot load WAVE file '%s' : not enough memory "
                "for the %d samples ! \n", fname, N);
    ...
    if (wChannels > 2)
        mwerror(WARNING, 1,
                "Converting %d-channels audio data '%s' to mono.\n",
                wChannels, fname);
...
#define BS 65536
    ...
    if (signal == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_fsignal_create_wave_pcm] Cannot create file: "
                "Fsignal structure is NULL\n");

    if (signal->size <= 0)
        mwerror(INTERNAL, 1,
                "[_mw_fsignal_create_wave_pcm] Cannot create file: "
                "No values in the Fsignal structure\n");
    ...
    {
        mwerror(ERROR, 1,
                "Cannot load WAVE file '%s' : "
                "not enough memory for I/O buffering ! \n", fname);
        fclose(fp);
        return (-1);
    }
    wbuf = (short *) buf;
    ...
    while (remain > 0)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "Cannot write WAVE PCM data into file \"%s\" !\n", fname);
            free(buf);
            fclose(fp);
            return (-1);
        }
    ...
    if (lost == 1)
        mwerror(WARNING, 0,
                "Information has been lost due to WAVE PCM quantization.\n");
