...
Vchains_wmax _mw_load_vchains_wmax(char *fname)
{
    ...
    if ((_mw_fascii_get_field(fp, fname,
        ...
        || (_mw_fascii_get_field(fp, fname,
                                 "size:", "%d", &vchains->size) != 1)
        || (_mw_fascii_get_field(fp, fname,
                                 "ref_level:", "%d",
                                 &vchains->ref_level) != 1)
        || (_mw_fascii_get_field(fp, fname, "nlevel:", "%d", &vchains->nlevel)
            != 1)
        || (_mw_fascii_get_field(fp, fname, "nrow:", "%d", &vchains->nrow) !=
            1)
        || (_mw_fascii_get_field(fp, fname, "ncol:", "%d", &vchains->ncol) !=
            1))
    {
    ...
    {
        mwerror(WARNING, 1,
                "%d vchains have be found in the file \"%s\" "
                "but the header records %d vchains.\n",
...
short _mw_create_vchains_wmax(char *fname, Vchains_wmax vchains)
{
    ...
    if (vchains == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Vchains_wmax structure is NULL\n");

    if (vchains->first == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: No vchain_wmax "
                "in the Vchains_wmax structure\n");
...
short _mw_create_vchain_wmax(char *fname, Vchain_wmax vchain)
{
    ...
    if (vchain == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Vchain_wmax structure is NULL\n");

    if (vchain->first == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Vchain_wmax has no vpoint !\n");
