                        ...
                        int N, char *data_name)
{
    ...
    {
        mwerror(WARNING, 0,
                "%s information has been lost due to quantization.\n",
                data_name);
        _mw_convert_struct_warning++;
    }
}
                                   ...
                                   register unsigned char *ptr3, int N)
{
    ...
    if ((N % 3) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_1x24XV_to_3x8_ucharplanes] Not a 24 bits plane\n");
    for (l = 1; l <= N; l += 3)
    {
        *ptr1++ = *ptr++;
        *ptr2++ = *ptr++;
                                   ...
                                   register unsigned char *ptr, int N)
{
    ...
    if ((N % 3) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_3x8_to_1x24XV_ucharplanes] Not a 24 bits plane\n");
    for (l = 1; l <= N; l += 3)
    {
        *ptr++ = *ptr1++;
        *ptr++ = *ptr2++;
...
Cimage mw_fimage_to_cimage(Fimage image_fimage, Cimage image)
{
    ...
    _mw_float_to_uchar(image_fimage->gray, image->gray,
                       image->nrow * image->ncol, "Gray");
...
Ccimage mw_cfimage_to_ccimage(Cfimage image_cfimage, Ccimage image)
{
    ...
    {
        mwerror(ERROR, 0,
                "Colors of input Cfimage don't follow the RGB model\n");
        return (NULL);
    }
    image =
        mw_change_ccimage(image, image_cfimage->nrow, image_cfimage->ncol);
    if (image == NULL)
    {
    ...
    _mw_float_to_uchar(image_cfimage->red, image->red,
                       image->nrow * image->ncol, "Red");
    _mw_float_to_uchar(image_cfimage->green, image->green,
                       image->nrow * image->ncol, "Green");
    _mw_float_to_uchar(image_cfimage->blue, image->blue,
                       image->nrow * image->ncol, "Blue");
...
{
    image =
        mw_change_cfimage(image, image_ccimage->nrow, image_ccimage->ncol);
    if (image == NULL)
    {
...
Fimage mw_cfimage_to_fimage(Cfimage image_cfimage, Fimage image)
{
    ...
    clost = 0;
    for (i = 1, gr = image->gray, r = image_cfimage->red, g =
         image_cfimage->green, b = image_cfimage->blue;
         i <= image->nrow * image->ncol; i++, gr++, r++, g++, b++)
    {
        *gr = FMONO(*r, *g, *b);
        if ((*r != *g) || (*g != *b))
            clost++;
...
Cfimage mw_fimage_to_cfimage(Fimage image_fimage, Cfimage image)
{
    ...
    for (i = 1, gr = image_fimage->gray, r = image->red, g = image->green, b =
         image->blue; i <= image->nrow * image->ncol;
         i++, gr++, r++, g++, b++)
        *r = *g = *b = *gr;
...
Fimage mw_ccimage_to_fimage(Ccimage image_ccimage, Fimage image)
{
    ...
    clost = 0;
    for (i = 1, gr = image->gray, r = image_ccimage->red, g =
         image_ccimage->green, b = image_ccimage->blue;
         i <= image->nrow * image->ncol; i++, gr++, r++, g++, b++)
    {
        *gr = FMONO(*r, *g, *b);
        if ((*r != *g) || (*g != *b))
...
Ccimage mw_cimage_to_ccimage(Cimage image_cimage, Ccimage image)
{
    ...
    for (i = 1, gr = image_cimage->gray, r = image->red, g = image->green, b =
         image->blue; i <= image->nrow * image->ncol;
         i++, gr++, r++, g++, b++)
        *r = *g = *b = *gr;
...
Fpolygons mw_fcurves_to_fpolygons(Fcurves fcurves, Fpolygons fpolys)
{
    ...
    for (p = fcurves->first; p; p = p->next)
    {
        ...
        if (fpolys->first == NULL)
            fpolys->first = newfpoly;
        if (oldfpoly != NULL)
            oldfpoly->next = newfpoly;
...
Fcurves mw_curves_to_fcurves(Curves curves, Fcurves fcurves)
{
    ...
    for (p = curves->first; p; p = p->next)
    {
        ...
        if (fcurves->first == NULL)
            fcurves->first = newfcurve;
        if (oldfcurve != NULL)
            oldfcurve->next = newfcurve;
...
Curves mw_fcurves_to_curves(Fcurves fcurves, Curves curves)
{
    ...
    for (p = fcurves->first; p; p = p->next)
    {
        ...
        if (curves->first == NULL)
            curves->first = newcurve;
        if (oldcurve != NULL)
            oldcurve->next = newcurve;
        ...
        for (pp = p->first; pp; pp = pp->next)
        {
            ...
            new_point->x = floor(pp->x + .5);
            new_point->y = floor(pp->y + .5);
            if ((((float) new_point->x) != pp->x) ||
                (((float) new_point->y) != pp->y))
                lost = 1;
            new_point->previous = point;
            if (point == NULL)
                newcurve->first = new_point;
            else
                point->next = new_point;
            point = new_point;
        }
    ...
    {
        mwerror(WARNING, 0,
                "coordinates information has been lost "
                "due to quantization of the plane\n");
        _mw_convert_struct_warning++;
    }
    return (curves);
}
...
Fcurves mw_dcurves_to_fcurves(Dcurves dcurves, Fcurves fcurves)
{
    ...
    for (p = dcurves->first; p; p = p->next)
    {
        ...
        for (pp = p->first; pp; pp = pp->next)
        {
            ...
            new_point->x = pp->x;
            new_point->y = pp->y;
            if ((new_point->x != pp->x) || (new_point->y != pp->y))
                lost = 1;
            new_point->previous = point;
            if (point == NULL)
                newfcurve->first = new_point;
    ...
    {
        mwerror(WARNING, 0,
                "coordinates accuracy has been reduced "
                "due to conversion from double to float.\n");
        _mw_convert_struct_warning++;
    }
...
Dcurves mw_fcurves_to_dcurves(Fcurves fcurves, Dcurves dcurves)
{
    ...
    for (p = fcurves->first; p; p = p->next)
    {
        ...
        if (dcurves->first == NULL)
            dcurves->first = newcurve;
        if (oldcurve != NULL)
            oldcurve->next = newcurve;
        newcurve->previous = oldcurve;
        newcurve->next = NULL;
...
Fcurves mw_fpolygons_to_fcurves(Fpolygons fpolys, Fcurves fcurves)
{
    ...
    for (p = fpolys->first; p; p = p->next)
    {
        ...
        for (pp = p->first, old_pp = NULL; pp || old_pp;)
        {
            ...
            {
                /* Close the curve since a polygon is a closed curve */
                                            ...
                                            Point_fcurve first_point)
{
    ...
    for (pp = pcurve; pp; pp = pp->next)
    {
        ...
        {
            if (first_point)
                mw_delete_point_fcurve(first_point);
                                           ...
                                           Point_curve first_point)
{
    ...
    for (pp = pfcurve; pp; pp = pp->next)
    {
        ...
        {
            if (first_point)
                mw_delete_point_curve(first_point);
                                             ...
                                             Point_dcurve first_point)
{
    ...
    for (pp = pfcurve; pp; pp = pp->next)
    {
        ...
        {
            if (first_point)
                mw_delete_point_dcurve(first_point);
...
Curve mw_morpho_line_to_curve(Morpho_line ll, Curve cv)
{
    ...
    if ((ll->minvalue != 0.0) || (ll->maxvalue != 0.0))
        mwerror(WARNING, 0,
                "Gray level values of the morpho line have been lost !\n");
    if (ll->first_type)
        mwerror(WARNING, 0,
                "Point types of the morpho line have been lost !\n");
...
Fcurve mw_fmorpho_line_to_fcurve(Fmorpho_line fll, Fcurve cv)
{
    ...
    if ((fll->minvalue != 0.0) || (fll->maxvalue != 0.0))
        mwerror(WARNING, 0,
                "Gray level values of the fmorpho line have been lost !\n");
    if (fll->first_type)
        mwerror(WARNING, 0,
                "Point types of the fmorpho line have been lost !\n");
...
Morpho_line mw_mimage_to_morpho_line(Mimage mimage, Morpho_line ll)
{
    ...
    {
        mwerror(WARNING, 0,
                "Conversion failed : input mimage does not contain "
                "any morpho_line.\n");
        return (NULL);
    }
...
Curves mw_mimage_to_curves(Mimage mimage, Curves curves)
{
    ...
    if (!mimage->first_ml && !mimage->first_fml)
    {
        mwerror(WARNING, 0,
                "Conversion failed : input mimage does not contain "
                "any morpho_line or fmorpho_line.\n");
        return (NULL);
    }
    ...
    if (mimage->first_ml && mimage->first_fml)
        mwerror(WARNING, 0,
                "The mimage structure contains both morpho lines "
                "and fmorpho lines. Using morpho lines only.\n");
    ...
    for (fll = mimage->first_fml; fll; fll = fll->next)
    {
        ...
        newcurve->next = NULL;
        newcurve->first =
            mw_point_fcurve_to_point_curve(fll->first_point, NULL);
    }
    return (curves);
}
...
Mimage mw_curves_to_mimage(Curves curves, Mimage mimage)
{
    ...
    for (cv = curves->first; cv; cv = cv->next)
    {
        ...
        if (mimage->first_ml == NULL)
            mimage->first_ml = newll;
        if (oldll != NULL)
            oldll->next = newll;
...
Fcurves mw_mimage_to_fcurves(Mimage mimage, Fcurves fcurves)
{
    ...
    if (mimage->first_ml && mimage->first_fml)
        mwerror(WARNING, 0,
                "The mimage structure contains both morpho lines "
                "and fmorpho lines. Using fmorpho lines only.\n");
    ...
    for (ll = mimage->first_ml; ll; ll = ll->next)
    {
        ...
        newfcurve->next = NULL;
        newfcurve->first =
            mw_point_curve_to_point_fcurve(ll->first_point, NULL);
    }
...
Mimage mw_fcurves_to_mimage(Fcurves fcurves, Mimage mimage)
{
    ...
    for (cv = fcurves->first; cv; cv = cv->next)
    {
        ...
        if (mimage->first_fml == NULL)
            mimage->first_fml = newfll;
        if (oldfll != NULL)
            oldfll->next = newfll;
...
Cmorpho_line mw_morpho_line_to_cmorpho_line(Morpho_line ll, Cmorpho_line cll)
{
    ...
    cll->minvalue.red = cll->minvalue.green = cll->minvalue.blue =
        ll->minvalue;
    cll->maxvalue.model = MODEL_RGB;
    cll->maxvalue.red = cll->maxvalue.green = cll->maxvalue.blue =
        ll->maxvalue;
    cll->open = ll->open;
    cll->data = ll->data;
...
Morpho_line mw_cmorpho_line_to_morpho_line(Cmorpho_line cll, Morpho_line ll)
{
    ...
    case MODEL_RGB:
        if ((cll->minvalue.red != cll->minvalue.green)
            || (cll->minvalue.green != cll->minvalue.blue))
            clost++;
        ll->minvalue =
            FMONO(cll->minvalue.red, cll->minvalue.green, cll->minvalue.blue);
        break;

    case MODEL_YUV:
        if ((cll->minvalue.red == 0.0) || (cll->minvalue.blue == 0.0))
            clost++;
        ll->minvalue = cll->minvalue.green;     /* Y in the Green channel */
        break;

    case MODEL_HSI:
        if (cll->minvalue.green == 0.0)
            clost++;
        ll->minvalue = cll->minvalue.blue;
        /* Intensity in the Blue channel */
        break;
    ...
    case MODEL_RGB:
        if ((cll->maxvalue.red != cll->maxvalue.green)
            || (cll->maxvalue.green != cll->maxvalue.blue))
            clost++;
        ll->maxvalue =
            FMONO(cll->maxvalue.red, cll->maxvalue.green, cll->maxvalue.blue);
        break;

    case MODEL_YUV:
        if ((cll->maxvalue.red == 0.0) || (cll->maxvalue.blue == 0.0))
            clost++;
        ll->maxvalue = cll->maxvalue.red;       /* Y in the Green channel */
        break;

    case MODEL_HSI:
        if (cll->maxvalue.green == 0.0)
            clost++;
        ll->maxvalue = cll->maxvalue.blue;
        /* Intensity in the Blue channel */
        break;
...
Cmorpho_line mw_cmimage_to_cmorpho_line(Cmimage cmimage, Cmorpho_line ll)
{
    ...
    {
        mwerror(WARNING, 0,
                "Conversion failed : input dmimage does not contain "
                "any cmorpho_line.\n");
        return (NULL);
    }
...
Curve mw_cmorpho_line_to_curve(Cmorpho_line ll, Curve cv)
{
    ...
    if (ll->first_type)
        mwerror(WARNING, 0,
                "Point types of the cmorpho line have been lost !\n");
...
Fcurve mw_cfmorpho_line_to_fcurve(Cfmorpho_line fll, Fcurve cv)
{
    ...
    mw_copy_point_fcurve(fll->first_point, cv->first);
    mwerror(WARNING, 0,
            "Gray level values of the cfmorpho line have been lost !\n");
    if (fll->first_type)
        mwerror(WARNING, 0,
                "Point types of the cfmorpho line have been lost !\n");
...
Curves mw_cmimage_to_curves(Cmimage cmimage, Curves curves)
{
    ...
    {
        mwerror(WARNING, 0,
                "Conversion failed : input cmimage does not contain "
                "any cmorpho_line or cfmorpho_line.\n");
        return (NULL);
    }
    ...
    if (cmimage->first_ml && cmimage->first_fml)
        mwerror(WARNING, 0,
                "The cmimage structure contains both cmorpho lines "
                "and cfmorpho lines. Using cmorpho lines only.\n");
...
Fcurves mw_cmimage_to_fcurves(Cmimage cmimage, Fcurves fcurves)
{
    ...
    if (cmimage->first_ml && cmimage->first_fml)
        mwerror(WARNING, 0,
                "The cmimage structure contains both morpho lines "
                "and fmorpho lines. Using fmorpho lines only.\n");
...
Cmimage mw_fcurves_to_cmimage(Fcurves fcurves, Cmimage cmimage)
{
    ...
    for (cv = fcurves->first; cv; cv = cv->next)
    {
        ...
        if (cmimage->first_fml == NULL)
            cmimage->first_fml = newfll;
        if (oldfll != NULL)
            oldfll->next = newfll;
        newfll->previous = oldfll;
        newfll->next = NULL;
...
/*     Cmorpho_sets cms0,cms1; */
    ...
    cmimage->minvalue.red = cmimage->minvalue.green = cmimage->minvalue.blue =
        mimage->minvalue;
    cmimage->maxvalue.model = MODEL_RGB;
    cmimage->maxvalue.red = cmimage->maxvalue.green = cmimage->maxvalue.blue =
        mimage->maxvalue;
...
/*     Cmorpho_sets cms;    */
    ...
    case MODEL_RGB:
        if ((cmimage->minvalue.red != cmimage->minvalue.green)
            || (cmimage->minvalue.green != cmimage->minvalue.blue))
            clost++;
        mimage->minvalue =
            FMONO(cmimage->minvalue.red, cmimage->minvalue.green,
                  cmimage->minvalue.blue);
        break;

    case MODEL_YUV:
        if ((cmimage->minvalue.red == 0.0) || (cmimage->minvalue.blue == 0.0))
            clost++;
        mimage->minvalue = cmimage->minvalue.green;
        /* Y in the Green channel */
        break;

    case MODEL_HSI:
        if (cmimage->minvalue.green == 0.0)
            clost++;
        mimage->minvalue = cmimage->minvalue.blue;
        /* Intensity in the Blue channel */
        break;
    ...
    case MODEL_RGB:
        if ((cmimage->maxvalue.red != cmimage->maxvalue.green)
            || (cmimage->maxvalue.green != cmimage->maxvalue.blue))
            clost++;
        mimage->maxvalue =
            FMONO(cmimage->maxvalue.red, cmimage->maxvalue.green,
                  cmimage->maxvalue.blue);
        break;

    case MODEL_YUV:
        if ((cmimage->maxvalue.red == 0.0) || (cmimage->maxvalue.blue == 0.0))
            clost++;
        mimage->maxvalue = cmimage->maxvalue.red;
        /* Y in the Green channel */
        break;

    case MODEL_HSI:
        if (cmimage->maxvalue.green == 0.0)
            clost++;
        mimage->maxvalue = cmimage->maxvalue.blue;
        /* Intensity in the Blue channel */
        break;
...
Flist mw_fcurve_to_flist(Fcurve c, Flist l)
{
    ...
    if (l && l->dim >= 2)
        ...
        l = mw_change_flist(l, size, size, 2);
...
Flists mw_fcurves_to_flists(Fcurves cs, Flists ls)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_fcurves_to_flists] Fcurves structure is NULL\n");
        return (NULL);
    }
...
Fcurves mw_flists_to_fcurves(Flists ls, Fcurves cs)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_flists_to_fcurves] flists structure is NULL\n");
        return (NULL);
    }
...
Dlist mw_dcurve_to_dlist(Dcurve c, Dlist l)
{
    ...
    if (l && l->dim >= 2)
        ...
        l = mw_change_dlist(l, size, size, 2);
    for (i = 0, p = c->first; p; p = p->next, i++)
    {
        l->values[i * l->dim] = p->x;
        l->values[i * l->dim + 1] = p->y;
...
Dlists mw_dcurves_to_dlists(Dcurves cs, Dlists ls)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_dcurves_to_dlists] Dcurves structure is NULL\n");
        return (NULL);
    }
...
Dcurves mw_dlists_to_dcurves(Dlists ls, Dcurves cs)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_dlists_to_dcurves] dlists structure is NULL\n");
        return (NULL);
    }
...
Dlists mw_flists_to_dlists(Flists in, Dlists out)
{
    ...
    if (in->data_size && in->data)
    {
        if (out->data_size < in->data_size)
        {
            ...
            {
                mwerror(ERROR, 0,
                        "[mw_flists_to_dlists] Not enough memory\n");
                return (out);
            }
        }
        memcpy(out->data, in->data, in->data_size);
...
Flists mw_dlists_to_flists(Dlists in, Flists out)
{
    ...
    if (in->data_size && in->data)
    {
        if (out->data_size < in->data_size)
        {
            ...
            {
                mwerror(ERROR, 0,
                        "[mw_dlists_to_flists] Not enough memory\n");
                return (out);
            }
        }
        memcpy(out->data, in->data, in->data_size);
