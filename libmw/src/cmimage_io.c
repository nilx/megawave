...
Cmorpho_line _mw_load_cmorpho_line(char *fname, char *type)
{
    ...
    ml = (Cmorpho_line) _mw_load_etype_to_itype(fname, mtype, "cmorpho_line",
                                                type);
    if (ml != NULL)
        return (ml);
...
*/
    ...
    if ((npc * npt != 0) && (npc != npt))
        mwerror(INTERNAL, 1,
                "[_mw_write_cml_mw2_cml] Cannot create file: "
                "inconsistent Cmorpho_line structure "
                "(# pt curve %d != # pt type %d)\n", npc, npt);
...
short _mw_create_cml_mw2_cml(char *fname, Cmorpho_line ll)
{
    ...
    if (ll == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_cml_mw2_cml] Cannot create file: "
...
Cfmorpho_line _mw_load_cfmorpho_line(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    ml = (Cfmorpho_line) _mw_load_etype_to_itype(fname, mtype,
                                                 "cfmorpho_line", type);
    if (ml != NULL)
        return (ml);
...
*/
    ...
    if ((npc * npt != 0) && (npc != npt))
        mwerror(INTERNAL, 1,
                "[_mw_write_cfml_mw2_cfml] Cannot create file: "
                "inconsistent Cfmorpho_line structure \n");
...
short _mw_create_cfml_mw2_cfml(char *fname, Cfmorpho_line fll)
{
    ...
    if (fll == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_cfml_mw2_cfml] Cannot create file: "
...
Cmorpho_set _mw_load_cmorpho_set(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    ms = (Cmorpho_set) _mw_load_etype_to_itype(fname, mtype, "cmorpho_set",
                                               type);
    if (ms != NULL)
        return (ms);
...
short _mw_create_cms_mw2_cms(char *fname, Cmorpho_set is)
{
    ...
    if (is == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_cms_mw2_cms] Cannot create file: "
...
Cmorpho_sets _mw_read_cmss_mw2_cmss(char *fname, FILE * fp, int need_flipping)
{
    ...
    for (p = iss; p; p = p->next)
    {
        ...
        for (i = 1; i <= n; i++)
        {
            ...
            if (need_flipping == 1)
                _mw_in_flip_b4(num);
            /* Search the cmorpho sets
             * containing the given Cmorpho set number */
            for (q = iss; q && q->cmorphoset && (q->cmorphoset->num != num);
                 q = q->next);
            if (!q || !(q->cmorphoset))
            {
                mw_delete_cmorpho_sets(iss);
                mwerror(INTERNAL, 0,
                        "[_mw_load_cmss_mw2_cmss] Cannot find "
                        "Cmorpho set number %d in file \"%s\"\n", num, fname);
            }
            ...
            if (p->cmorphoset->neighbor == NULL)
                p->cmorphoset->neighbor = newiss;
            if (oldiss)
                oldiss->next = newiss;
            newiss->previous = oldiss;
            oldiss = newiss;
...
Cmorpho_sets _mw_load_cmorpho_sets(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    ms = (Cmorpho_sets) _mw_load_etype_to_itype(fname, mtype, "cmorpho_sets",
                                                type);
    if (ms != NULL)
        return (ms);
...
void _mw_write_cmss_mw2_cmss(FILE * fp, Cmorpho_sets iss)
{
    ...
    if (!iss || !iss->cmorphoset)
        return;

    /* Record each cmorpho set */
    for (p = iss; p; p = p->next)
        _mw_write_cms_mw2_cms(fp, p->cmorphoset);
...
short _mw_create_cmss_mw2_cmss(char *fname, Cmorpho_sets iss)
{
    ...
    if (iss == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_cmss_mw2_cmss] Cannot create file: "
...
*/
    ...
    if (cmimage->first_ml)
    {
        ...
        for (i = 1, ll = cmimage->first_ml; i <= size; i++)
        {
            ...
            /* Search the cmorpho sets containing the given cmorpho
             * set number msnum */
            for (mss = cmimage->first_ms;
                 mss && mss->cmorphoset && (mss->cmorphoset->num != msnum);
                 mss = mss->next);
            if (!mss || !(mss->cmorphoset))
            {
                mw_delete_cmimage(cmimage);
                mwerror(INTERNAL, 0,
                        "[_mw_load_cmimage_mw2_cmimage] Cannot find "
                        "cmorpho set number %d in file \"%s\"\n", msnum,
                        fname);
                fclose(fp);
                return (NULL);
...
short _mw_create_cmimage_mw2_cmimage(char *fname, Cmimage cmimage)
{
    ...
    if (cmimage == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_cmimage_mw2_cmimage] Cannot create file: "
    ...
    if (1 > fwrite(&(cmimage->nrow), sizeof(int), 1, fp)
        ...
        || 1 > fwrite(&(cmimage->minvalue.model), sizeof(unsigned char), 1,
                      fp)
        ...
        || 1 > fwrite(&(cmimage->maxvalue.model), sizeof(unsigned char), 1,
                      fp)
