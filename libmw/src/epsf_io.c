...
Cimage _mw_cimage_load_epsf(char *fname)
{
    ...
    mwerror(FATAL, 0,
            "Sorry, EPSF file format for input not available ! "
...
short _mw_cimage_create_epsf(char *fname, Cimage image)
{
    ...
    float sx, sy;               /* Size of image, in inches
                                 * (72 dpi - dot per inch) */
    float tx, ty;               /* Top-left offset of image, in inch */
    int bx, by;                 /* Bottom-left offset of image,
                                 * in points (pixels) */
    unsigned char white;
    ...
    {
        mwerror(INTERNAL, 0,
                "[_mw_cimage_create_epsf] NULL cimage or cimage plane\n");
        return (-2);
    }
    ...
    {
        fprintf(fp,
                "%d %d 8\t\t\t%% dimensions of data (8 bits gray levels)\n",
                dx, dy);
