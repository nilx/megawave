...
static unsigned int getshort(FILE * fp)
{
    ...
    if ((c = getc(fp)) == EOF || (c1 = getc(fp)) == EOF)
        return EOF;
...
static int putshort(int i, FILE * fp)
{
    ...
    if (putc(c, fp) == EOF || putc(c1, fp) == EOF)
        return EOF;
...
static int putint(int i, FILE * fp)
{
    ...
    if (putc(c, fp) == EOF ||
        putc(c1, fp) == EOF || putc(c2, fp) == EOF || putc(c3, fp) == EOF)
        return EOF;
...
FILE *_mw_read_bmp_header(char *fname,
                          ...
                          unsigned int *planes,
                          unsigned int *bitcount, unsigned int *compression)
{
    ...
    *compression = 0;
    ...
    if (getint(fp) == (unsigned int) EOF ||     /* bfSize      */
    ...
    {
        mwerror(ERROR, 0,
                "Error while reading header of file \"%s\"... "
                "Not a BMP format or file corrupted !\n", fname);
        fclose(fp);
        return (NULL);
    }
    ...
    {
        /* FIXME: wrong types, dirty temporary fix */
        if ((*nx = getint(fp)) == (unsigned int) EOF || /* biWidth  */
            (*ny = getint(fp)) == (unsigned int) EOF || /* biHeight */
            (*planes = getshort(fp)) == (unsigned int) EOF ||
            (*bitcount = getshort(fp)) == (unsigned int) EOF ||
            (*compression = getint(fp)) == (unsigned int) EOF
            /* New format may include compression */
            )
        {
            mwerror(ERROR, 0,
                    "Error while reading header of file \"%s\"... "
                    "Not a new BMP format or file corrupted !\n", fname);
            fclose(fp);
            return (NULL);
        }
...
Cimage _mw_cimage_load_bmp(char *file)
{
    ...
    if (planes != 1)
        mwerror(INTERNAL, 1,
                "BMP image file \"%s\" : unexpected number of planes = %d\n",
                file, planes);
    if (bitcount != 8)
        mwerror(INTERNAL, 1,
                "BMP image file \"%s\" : unexpected number of bits = %d\n",
                file, bitcount);
    if (compression != 0)
    {
        mwerror(ERROR, 1,
                "BMP image file \"%s\" : cannot handle compression !\n",
                file);
        fclose(fp);
        return (NULL);
    }
...
short _mw_cimage_create_bmp(char *file, Cimage image)
{
    ...
    if (
           ...
           putint(14 + 40 + 1024 + ny * allocsize, fp) == EOF ||
           /* bfSize      */
...
short _mw_ccimage_create_bmp(char *file, Ccimage image)
{
    ...
    if ((image == NULL)
        ...
        mwerror(INTERNAL, 0,
                "[_mw_ccimage_create_bmp] NULL cimage or cimage plane\n");
    nx = image->ncol;
    ny = image->nrow;
    /*allocsize = nx + ((nx&3) ? 4-(nx&3) : 0); */
    allocsize = nx + ((4 - ((nx * 3) % 4)) & 0x03);
    ...
    if (
           ...
           putc('B', fp) == EOF ||      /* bfType      */
           putc('M', fp) == EOF ||      /* bfType      */
           putint(14 + 40 + ny * allocsize, fp) == EOF ||
           /* bfSize      */
           putshort(0, fp) == EOF ||    /* bfReserved1 */
           putshort(0, fp) == EOF ||    /* bfReserved2 */
