...
Fimage mw_alloc_fimage(Fimage image, int nrow, int ncol)
{
     ...
     {
	  mwerror(ERROR, 0, 
		  "[mw_alloc_fimage] cannot alloc plane : fimage structure is NULL\n");
	  return(NULL);
     }
     ...
     {
	  mwerror(ERROR, 0,
		  "[mw_alloc_fimage] Attempts to alloc a fimage which is already allocated\n");
	  return(NULL);
     }
...
void mw_delete_fimage(Fimage image)
{
     ...
     {
	  mwerror(ERROR, 0,
		  "[mw_delete_fimage] cannot delete : fimage structure is NULL\n");
	  return;
     }
...
Fimage mw_change_fimage(Fimage image, int nrow, int ncol)
{
     ...
     if (image == NULL) image = mw_new_fimage();
...
float mw_getdot_fimage(Fimage image, int x, int y)
{
     ...
     {
	  mwerror(ERROR, 0,
		  "[mw_getdot_fimage] NULL image struct or NULL gray plane... Return 0\n");
	  return(0);
     ...
     {
	  mwerror(ERROR, 0,
		  "[mw_getdot_fimage] Point (%d,%d) out of image... Return 0.0\n",x,y);
	  return(0.0);
     }
...
void mw_copy_fimage(Fimage in, Fimage out)
{
     ...
     {
	  mwerror(ERROR, 0,
		  "[mw_copy_fimage] NULL input or output image or images of different sizes\n");
	  return;
     }
...
float ** mw_newtab_gray_fimage(Fimage image)
{
     ...
     {
	  mwerror(ERROR, 0,
		  "[mw_newtab_gray_fimage] NULL image struct or NULL gray plane\n");
	  return(NULL);
     }
