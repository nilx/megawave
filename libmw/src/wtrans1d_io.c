...
Wtrans1d _mw_load_wtrans1d_header(char *fname)
{
    ...
    {
    case mw_orthogonal:
        if (mw_alloc_ortho_wtrans1d(wtrans, wtrans->nlevel, wtrans->size) ==
            NULL)
        {
            mw_delete_wtrans1d(wtrans);
            return (NULL);
    ...
    case mw_biorthogonal:
        if (mw_alloc_biortho_wtrans1d(wtrans, wtrans->nlevel, wtrans->size) ==
            NULL)
        {
            mw_delete_wtrans1d(wtrans);
            return (NULL);
    ...
    case mw_dyadic:
        if (mw_alloc_dyadic_wtrans1d(wtrans, wtrans->nlevel, wtrans->size) ==
            NULL)
        {
            mw_delete_wtrans1d(wtrans);
            return (NULL);
...
short _mw_create_wtrans1d_header(char *fname, Wtrans1d wtrans)
{
    ...
    if (wtrans == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Wtrans1d structure is NULL\n");
        ...
        for (v = 0; v < wtrans->nvoice; v++)
            if ((j > 0) || (v > 0))
            {
                if (v == 0)
                    ...
                    sprintf(wfname, "%s_%2.2d.%2.2d_%s.wtrans1d", fname, j, v,
                            Sname);
                ...
                if (signal == NULL)
                    mwerror(INTERNAL, 0,
                            "[_mw_wtrans1d_load_wtrans] NULL signal "
                            "returned in loading file \"%s\"\n", wfname);
                if (size != signal->size)
                {
                    if (sizedif == 0)
                        mwerror(WARNING, 0,
                                "Unexpected size %d of the signal "
                                "in \"%s\" (disable size control)\n",
                                signal->size, wfname);
                    sizedif = 1;
 ...
 *  if (signal != S[j][v])
 *  mwerror(INTERNAL, 0,
 *          "[_mw_wtrans1d_load_wtrans] New memory allocation done "
 *          "by _mw_load_fsignal in loading \"%s\"\n",wfname);
 */
...
Wtrans1d _mw_wtrans1d_load_wtrans(char *fname, char *type)
{
    ...
    if ((use_average == 1) &&
        (_mw_wtrans1d_load_signal_wtrans(fname, type,
                                         wtrans, wtrans->A, "A") == NULL))
        return (NULL);

    if (_mw_wtrans1d_load_signal_wtrans(fname, type,
                                        wtrans, wtrans->D, "D") == NULL)
        return (NULL);

    if (wtrans->complex == 1)
    {
        if ((use_average == 1) &&
            (_mw_wtrans1d_load_signal_wtrans
             (fname, type, wtrans, wtrans->AP, "AP") == NULL))
            return (NULL);
        if (_mw_wtrans1d_load_signal_wtrans
            (fname, type, wtrans, wtrans->DP, "DP") == NULL)
            return (NULL);
    }
    ...
    for (j = 0; j <= wtrans->nlevel; j++)
        for (v = 0; v < wtrans->nvoice; v++)
            if ((j > 0) || (v > 0))
            {
                ...
                if (signal == NULL)
                    mwerror(INTERNAL, 0,
                            "[_mw_wtrans1d_create_signal_wtrans] NULL "
                            "wtrans->%s[%d][%d]\n", Sname, j, v);
...
short _mw_wtrans1d_create_wtrans(char *fname, Wtrans1d wtrans, char *type)
{
    ...
    {
        mwerror(INTERNAL, 0,
                "[_mw_wtrans1d_create_wtrans] Cannot create wtrans1d : "
                "NULL wtrans.\n");
        return (-1);
    }

    if (_mw_create_wtrans1d_header(fname, wtrans) != 0)
        return (-1);
    ...
    if (use_average == 1)
        _mw_wtrans1d_create_signal_wtrans(fname, type, wtrans, wtrans->A,
                                          "A");
    ...
    {
        if (use_average == 1)
            _mw_wtrans1d_create_signal_wtrans(fname, type, wtrans, wtrans->AP,
                                              "AP");
        _mw_wtrans1d_create_signal_wtrans(fname, type, wtrans, wtrans->DP,
                                          "DP");
    }
