...
Rawdata mw_alloc_rawdata(Rawdata rd, int newsize)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_rawdata] cannot alloc data : "
                "rawdata structure is NULL\n");
        return (NULL);
    }
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_rawdata] Attempts to alloc a rawdata "
...
void mw_delete_rawdata(Rawdata rd)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_rawdata] cannot delete : "
                "rawdata structure is NULL\n");
        return;
    }
    if (rd->data != NULL)
...
void mw_copy_rawdata(Rawdata in, Rawdata out)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_copy_rawdata] NULL input or output rd "
                " or rawdatas of different sizes\n");
        return;
    }
