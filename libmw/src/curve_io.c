...
Curve _mw_load_curve_mw2_curve(char *fname)
{
    ...
    if (strcmp(ftype, "MW2_CURVE") != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_curve_mw2_curve] File \"%s\" is not "
                "in the MW2_CURVE format\n", fname);
    ...
    while (remainsize > 0)
    {
        ...
        if (readsize > buf_size)
            readsize = buf_size;
        /* FIXME: wrong types, dirty temporary fix */
        if (fread(buffer, sizeof(int), readsize, fp) !=
...
short _mw_create_curve_mw2_curve(char *fname, Curve cv)
{
    ...
    if (cv == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_curve_mw2_curve] Cannot create file: "
                "Curve structure is NULL\n");
     ...
     * if (cv->first == NULL)
     * mwerror(INTERNAL,1,"[_mw_create_curve_mw2_curve] Curve has no "
     *         "point curve !\n");
     */
...
static Curves _mw_load_curves_mw2_curves_1_00(char *fname)
{
    ...
    if (strcmp(ftype, "MW2_CURVES") != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_curves_mw2_curves] File \"%s\" is not "
                "in the MW2_CURVES format\n", fname);
...
Curves _mw_load_curves_mw2_curves(char *fname)
{
    ...
    if (version == 1.0)
        return (_mw_load_curves_mw2_curves_1_00(fname));
    ...
    while (1)
    {
        ...
        if (need_flipping == 1)
            X[n] = _mw_get_flip_b4(X[n]);
        if (X[n] == END_OF_CURVES)
        {
            fclose(fp);
            return (cvs);
        }
        else if (X[n] == END_OF_CURVE)
        {
            n = -1;
            oldcv = newcv;
            newcv = mw_new_curve();
...
Curves _mw_load_curves(char *fname, char *type)
{
    ...
    if (type[0] == '?')
        ...
        mwerror(FATAL, 1,
                "External type of file \"%s\" is %s. "
                "I Don't know how to load such external type into a Curves !\n",
                fname, type);
    return NULL;
}
...
short _mw_create_curves_mw2_curves(char *fname, Curves cvs)
{
    ...
    if (cvs == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_curves_mw2_curves] Cannot create file: "
                "Curves structure is NULL\n");
    ...
    for (pl = cvs->first, n = 1; pl; pl = pl->next, n++)
    {
        ...
        {
            if ((pc->x == END_OF_CURVE) || (pc->y == END_OF_CURVE))
                mwerror(INTERNAL, 1,
                        "[_mw_create_curves_mw2_curves] Curve #%d "
                        "has a point which coordinates (%d,%d) has "
                        "reserved value. Sorry !\n", n, pc->x, pc->y);
            if (1 > fwrite(&(pc->x), sizeof(int), 1, fp)
                || 1 > fwrite(&(pc->y), sizeof(int), 1, fp))
            {
                mwerror(ERROR, 0, "Error while writing "
