 ...
 * @author Simon Masnou <masnou@ann.jussieu.fr>(1991 - 2001)
...
int WX_Init(char *theDisplayName)
{
    ...
    WLIB_ERROR;
    fprintf(stderr,
            "WX_Init: Debug ON causes synchronized X errors display\n");
...
void WX_AllocColors(void)
{
    ...
    if (_W_nfcols > 0)
        WX_FreeColors();        /* Free previously allocation */
    ...
    for (i = 0; (i < _W_NumCols) && (unique < _W_NumCols); i++)
    {
        ...
        if (ret)
        {
            ...
            for (j = 0, fcptr = _W_freecols; j < _W_nfcols && *fcptr != pixel;
                 j++, fcptr++);
            if (j == _W_nfcols)
                unique++;
...
void WX_CreateXImage(Wframe * window, int dx, int dy)
{
    ...
    case 8:                    /* The screen is a 8-bits plane */
        ...
        window->ximage =
            XCreateImage(_W_Display, _W_Visual, _W_Depth, ZPixmap, 0, NULL,
                         dx, dy, 8, 0);
        sizepix = dx * dy;
        break;
    ...
    case 1:                    /* The screen is monochrome */
        ...
        window->ximage =
            XCreateImage(_W_Display, _W_Visual, _W_Depth, XYPixmap, 0, NULL,
                         dx, dy, 8, 0);
    ...
    case 4:
    case 6:                    /* The screen has 16 or 64 levels */
    ...
    default:                   /* No code supported */
        WLIB_ERROR;
        fprintf(stderr,
                "[WX_CreateXImage] No code to handle this display "
                "of %d bits deep\n", _W_Depth);
...
void WX_AllocXPixmap(Wframe * window, int dx, int dy)
{
    ...
    if (window->pixmap == (Pixmap) NULL)
        window->px = window->py = 0;    /* For Security */
...
/*  if ( (dx != window->px) || (dy != window->py) ) */
        /* Has to be allocated */
    {
        ...
        window->pixmap =
            XCreatePixmap(_W_Display, window->win, dx, dy, _W_Depth);
        if (window->pixmap == (Pixmap) NULL)
        {
            fprintf(stderr,
                    "WX_AllocXPixmap : Not enough memory to pixmap "
                    "the window !\n");
            window->px = 0;
            window->py = 0;
            return;
        ...
        window->px = dx;
        window->py = dy;
        XCopyArea(_W_Display, window->win, window->pixmap, _W_GC, 0, 0, dx,
                  dy, 0, 0);
    }
}
...
void WX_Ditherize(Wframe * window, int dx, int dy)
{
    ...
    if ((window == NULL) || (window->ximage == NULL))
        return;
    ...
    for (i = 0; i < dy; i++)
    {
        ...
        if (order == LSBFirst)
        {
            ...
            if (bit)
                *pp++ = pix8 >> (7 - bit);      /* write partial byte
                                                 * at end of line */
        }
...
int WX_KeyPress(XKeyEvent * event)
{
    ...
    /*
     * This test to avoid a memory fault into the function
     * XLookupString() (on HP-UX A.09.03 with X11R5), when sometimes a
     * XKeyEvent is received by WUserEvent and it does not correspond
     * to a key pressed. In that case, the key ID given by keycode as
     * an enormous (and probably inconsistant) value. JF 8/8/94
     */

    if (((*event).keycode <= 0) || ((*event).keycode >= 0xFF00))
        return (-1);

    length = XLookupString(event, &key, 1, &theKeySym, &theComposeStatus);
