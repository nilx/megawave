 ...
 * @author Jacques Froment <jacques.froment@univ-ubs.fr> (1991 - 2001)
...
unsigned char _W_Red[256] = { 0 }, _W_Green[256] =
...
static unsigned char theIcon_bits[] = {
    ...
    0x00, 0x00, 0x00, 0x30, 0xe4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x73,
    0xe4, 0x01, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x73, 0xe4, 0x03, 0x00, 0x00,
    0x00, 0x00, 0xe0, 0x73, 0xc4, 0x07, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x71,
    0x84, 0x0f, 0x00, 0x00, 0x00, 0x00, 0xf8, 0x70, 0x04, 0x1f, 0x00, 0x00,
    0x00, 0x00, 0x7c, 0x70, 0x04, 0x3e, 0x00, 0x00, 0x00, 0x00, 0x3e, 0x70,
    0x04, 0x7c, 0x00, 0x00, 0x00, 0x00, 0x1f, 0x70, 0x04, 0xf8, 0x00, 0x00,
    0x00, 0x80, 0x0f, 0x70, 0x04, 0xf0, 0x01, 0x00, 0x00, 0xc0, 0x07, 0x70,
    ...
    0x00, 0x3e, 0x00, 0x70, 0x04, 0x00, 0x7c, 0x00, 0x00, 0x1f, 0x00, 0x70,
    ...
    0x00, 0x3e, 0x00, 0x70, 0x04, 0x00, 0x1f, 0x00, 0x00, 0x7c, 0x00, 0x70,
    ...
    0xfc, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f, 0xf0, 0xff, 0xff, 0xff,
...
int WIsAnActiveWindow(Wframe * window)
{
    ...
    /*
     * Call a X function which generates BadWindow error
     * in case of non-window
     */
...
void WFlushWindow(Wframe * window)
{
    ...
    XCopyArea(_W_Display, window->pixmap, window->win, _W_GC, 0, 0,
              window->px, window->py, 0, 0);
    XFlush(_W_Display);
}
                  ...
                  /* Rectangle to be flushed */
{
    ...
    XCopyArea(_W_Display, window->pixmap, window->win, _W_GC, x0, y0, dx, dy,
              x0, y0);
    XFlush(_W_Display);
}
...
void WSetColorPencil(Wframe * window, int color)
{
    ...
    if ((color < 0) || (color >= _W_NumCols))
    {
        WLIB_ERROR;
        fprintf(stderr,
                "[WSetColorPencil] color #%d is not "
                "in the allowed range #0...#%d\n", color, _W_NumCols - 1);
        return;
    }
...
void WDrawString(Wframe * window, int x, int y, char *text)
{
    ...
    if (_W_NumCols <= 0)
        WSetColorMap();
    XDrawImageString(_W_Display, window->pixmap, _W_GC, x, y, text,
                     strlen(text));
}
...
void WDrawRectangle(Wframe * window, int x0, int y0, int x1, int y1)
{
    ...
    if (_W_NumCols <= 0)
        WSetColorMap();
    XDrawRectangle(_W_Display, window->pixmap, _W_GC, x0, y0, x1 - x0,
                   y1 - y0);
}
...
void WFillRectangle(Wframe * window, int x0, int y0, int x1, int y1)
{
    ...
    if (_W_NumCols <= 0)
        WSetColorMap();
    XFillRectangle(_W_Display, window->pixmap, _W_GC, x0, y0, x1 - x0 + 1,
                   y1 - y0 + 1);
}
...
void WClearWindow(Wframe * window)
{
    ...
    XCopyArea(_W_Display, window->win, window->pixmap, _W_GC, 0, 0,
              window->px, window->py, 0, 0);
}
...
void WDestroyWindow(Wframe * window)
{
    ...
    if (window->pixmap != (Pixmap) NULL)
        XFreePixmap(_W_Display, window->pixmap);
    if (window->win != 0)
        XDestroyWindow(_W_Display, window->win);
    window->pixmap = 0;
    window->win = 0;
    /*
     * free(window);
...
void WSaveImageWindow(Wframe * window, int x, int y, int width, int height)
{
    ...
    if ((x + width) > window->dx)
        width = window->dx - x;
    if ((y + height) > window->dy)
        height = window->dy - y;
    if ((width <= 0) || (height <= 0))
    {
        WLIB_ERROR;
        fprintf(stderr,
                "[WSaveImageWindow] Illegal Dimensions (out of frame)\n");
        return;
    }
...
void WRestoreImageWindow(Wframe * window, int x, int y, int width, int height)
{
    ...
    if ((x + width) > window->px)
        width = window->px - x;
    if ((y + height) > window->py)
        height = window->py - y;
    ...
    if ((width <= 0) || (height <= 0))
    {
        WLIB_ERROR;
        fprintf(stderr,
                "[WRestoreImageWindow] Illegal Dimensions (out of frame)\n");
        return;
    }

    if (window->ximage != NULL)
        /* modification Lionel Moisan (fev 2001) */
        /* XPutImage(_W_Display,window->pixmap,_W_GC,window->ximage,
         *           0,0,x,y,width,height); */
        XPutImage(_W_Display, window->pixmap, _W_GC, window->ximage, x, y, x,
                  y, width, height);
    else
        WClearWindow(window);
}
                     ...
                     int width, int height)
{
    ...
    if (window->pic == NULL)
    {
        WLIB_ERROR;
        fprintf(stderr,
                "[WLoadBitMapImage] cannot load bitmap (pic is NULL)\n");
        return (-1);
    }

    switch (_W_Depth)
    {
    case 8:                    /* Good case : bitmap has the same structure
                                 * than pix */
        /* Copy bitmap into window->pic with Quantization of the picture */
        /* >> 2 because 64 gray levels set in the colormap */
        for (i = width * height, p = window->pic, q = bitmap; i;
             i--, p++, q++)
                          ...
                          unsigned char *Blue, int width, int height)
{
    ...
    if (window->pic == NULL)
    {
        WLIB_ERROR;
        fprintf(stderr,
                "[WLoadBitMapColorImage] cannot load bitmap (pic is NULL)\n");
        return (-1);
    }
    ...
    default:
        WLIB_ERROR;
        fprintf(stderr,
                "[WLoadBitMapColorImage] No code implemented "
                "for this screen\n");
        return (-1);
        break;
    }
...
void WSystemEvent(Wframe * window)
{
    ...
    if (XPending(_W_Display) == 0)
        return;                 /* No event in queue */
    ...
    case NoExpose:
        ...
        {                       /* This is the last Expose event */
            WFlushWindow(window);       /* Then restore the content
                                         * of the window */
#ifdef W_DEBUG_ON
            fprintf(stderr,
    ...
    case ConfigureNotify:
        ...
        if (window->win != conf_event->window)
            break;
        /* Set new location */
...
int WUserEvent(Wframe * window)
{
    ...
    if (XPending(_W_Display) == 0)
        return (0);             /* No event in queue */
    if (XCheckWindowEvent(_W_Display, window->win, window->event_mask, &event)
        == 0)
        return (0);
    ...
    case ButtonPress:          /* A Mouse button has been pressed */
        ...
        default:
            WLIB_ERROR;
            fprintf(stderr,
                    "[WUserEvent] This mouse button (#%d) "
    ...
    case KeyPress:
        if (WX_KeyPress((XKeyEvent *) & event) < 0)
            return (-1);
        else
            return (W_KEYPRESS);
        break;
    ...
    case ConfigureNotify:
            ...
            || (window->dy != conf_event->height))
        {
            ...
            return (-1);        /* Window's size remains the same */
        break;
...
{
    Window root, child;         /* Root (and child) windows ID
                                 * where the mouse is */
    int root_x, root_y;         /* coordinates relative to the root's origin */
    int win_x, win_y;           /* coordinates relative to the origin
                                 * of window */
    ...
    *button_mask = 0;
    ...
    if ((keys_buttons & Button1Mask) == Button1Mask)
        *button_mask |= W_MS_LEFT;
    if ((keys_buttons & Button2Mask) == Button2Mask)
        *button_mask |= W_MS_MIDDLE;
    if ((keys_buttons & Button3Mask) == Button3Mask)
        *button_mask |= W_MS_RIGHT;
...
Wframe *WOpenImageWindow(int width, int height, int ltx, int lty, char *label)
{
    ...
    window->win =
        XCreateWindow(_W_Display, RootWindow(_W_Display, _W_Screen), ltx, lty,
                      width, height, BORDER_WIDTH, _W_Depth, InputOutput,
                        ...
                        int ltx, int lty, char *label)
{
    ...
    if (_W_Display == NULL)
    {
        WLIB_ERROR;
        fprintf(stderr,
                "[WReOpenImageWindow] No connection to an X server ! "
                "(_W_Display is NULL)\n");
        return;
    }
    ...
    if ((theWindowAttributes.x != ltx) || (theWindowAttributes.y != lty))
