...
void Wp_SetButton(int type, Wpanel wp, void *b)
{
    ...
    case WP_INT:
        ...
        if (wi->scale)
        {
            ...
            if (i > wi->scale)
                i = wi->scale;
            Wp_DrawScale(wp->window, x, wi->y, i, wi->divscale, wi->scale,
                         wi->color);
    ...
    if (event == W_MS_LEFT && y >= 0 && y < wp->ny && x >= 0 && x < wp->nx)
    {
            ...
            if (wi)
            {
                ...
                if (b < 0)
                    wi->value = wi->firstscale +
                        ((wi->lastscale - wi->firstscale) * (-b - 1) +
                         wi->scale / 2) / wi->scale;
                else
                    wi->value += wi->button_inc[b];
                if (wi->proc)
                    ret = wi->proc(wi, wp->button[adr]);
                if (ret == 0)
                    ret = event;
        ...
        case WP_FLOAT:
            ...
            if (wf)
            {
                b = wp->button[adr];
                if (b >= 0)
                    wf->value += wf->button_inc[b];
                if (wf->proc)
                    ret = wf->proc(wf, wp->button[adr]);
                if (ret == 0)
                    ret = event;
