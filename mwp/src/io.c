...
void error(char *fmt, ...)
{
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "%s",
            str_fileposition(position_info, source_file_global));
    abort();
}
...
int getsentence(FILE * sfile, char *s)
{
    ...
    *p = '\0';
    n = 0;
    while (((l = getc(sfile)) != EOF) && ((intext == 1) || (l != ';'))
    ...
    while ((l = getc(sfile)) != EOF)
    {
        ...
        case '{':
            if ((in_quote == 0) && (in_dquote == 0))
            {
                 ...
                 */
                if (n + 1 > STRSIZE)
                    error(MSG_ERROR_GETINSTRUCTION_LONG_SENTENCE, STRSIZE);
                q = p - 1;
                *(p++) = '{';
                n++;
                l00 = l0;
