...
#define MSG_ERROR_UNEXPECTED_FUNCTION \
    "Parse error in \"%s\" : unexpected declaration of function " \
    "while scanning previous block of instructions.\nTip : check " \
    "if there is no missing declaration of parameter in the previous function."
#define MSG_ERROR_UNEXPECTED_PAREN \
    "Parse error : unexpected ')' outside function"
#define MSG_ERROR_TYPES_NUMBER \
    "Parse error in \"%s\" : number of types (%d) does not match " \
    "number of parameters (%d) in ANSI function prototype"
...
#define MSG_ERROR_FUNCTION_PAREN \
    "Parse error : cannot interpret instruction \"%s\" " \
    "as a function (missing right parenthesis)"
#define MSG_ERROR_FUNCTION_ENTRY \
    "Parse error : cannot interpret instruction \"%s\" " \
    "as a function (nparam<0 : function's entry not found)"
#define MSG_ERROR_PARAM_NB \
    "Parse error : cannot interpret instruction \"%s\" " \
    "as a function (only %d parameter(s) for %d variable(s)).\nTip : " \
    "check declaration of last function"
...
#define MSG_ERROR_NON_STATIC_FUNCTION \
    "Non-static function definition is not allowed " \
    "(but for the main one) : \"%s\""
...
#define MSG_ERROR_FUNCTION_END \
    "Parse error : unexpected end of file while decoding " \
    "a block of instructions.\nTip : check if there is no missing '}' " \
    "nor missing declaration of parameter in a function."
...
/* added to c99 language specification  */
static char *Cfuncspecifier[] = { "inline", "" };
...
static int Is_previousCword_datatype(t_token * w)
{
    ...
    for (wp = w->previous; wp && (wp->Wtype != W_SEPARATOR);
         wp = wp->previous)
        if (wp->Wtype == W_DATATYPE)
            return (1);
...
static int AnalyseInstruction(t_statement * c)
{
    ...
    for (cw = c->wfirst, cw0 = NULL; cw; cw0 = cw, cw = cw->next)
    {
        ...
        if (Is_Cdatatype(w))
        {
            ...
            {
                /* If inside function declaration, count number of data types */
                if (Is_Cvoiddatatype(w))
                    /* data type void */
                    voidencountered = 1;
                else
                    /* do not count void data type as it is not
                     * associated to a parameter
                     * (case of "void *toto" recognized below)
                     */
                if (Is_previousCword_datatype(cw) != 1)
                {
                    /* do not count 2 times composed type
                     * such as long unsigned int */
                    c->ndatatype++;
                    if (debug_flag)
                        debug("Number of data types incremented (1); now %d",
                              c->ndatatype);
                }
            }
            continue;
        ...
        if (Is_Cpointer(w))
        {
            ...
            if (debug_flag)
                debug("W_CPOINTER");
            if ((c->Itype == I_FUNC_IN) && (voidencountered == 1))
            {
                /* If inside function declaration,
                 * increment data type in case of "void *toto" */
                c->ndatatype++;
                if (debug_flag)
                    debug("Number of data types incremented (2); now %d",
                          c->ndatatype);
            }
            continue;
        }
        if (Is_Cmwdatatype(w))
        {
            ...
            if (c->Itype == I_FUNC_IN)
            {
                c->ndatatype++;
                if (debug_flag)
                    debug("Number of data types incremented (3); now %d",
                          c->ndatatype);
            }
            continue;
        }
        if (Is_Cuserdatatype(w))
        {
            ...
            if (c->Itype == I_FUNC_IN)
            {
                c->ndatatype++;
                if (debug_flag)
                    debug("Number of data types incremented (4); now %d",
                          c->ndatatype);
            }
            continue;
        }
        ...
        if (!IsStringCid(w))
        {
            ...
            if (strcmp(w, ")") == 0)
            {
                ...
                /* Inside function, check if it's the end
                 * of a prototype or a declaration */
                if ((cw->next) &&
                ...
                if (debug_flag)
                    debug
                        ("Don't know what it is (1)... "
                         "skip until next ',' or ')'");
                for (; cw && cw->next && (strcmp(cw->next->Name, ",") != 0)
                     && (strcmp(cw->next->Name, ")") != 0);
                     cw0 = cw, cw = cw->next);
                if (cw)
                    continue;
             ...
             (strcmp(cw->next->Name, ")") == 0)))
        {
            ...
            c->nparam++;
            if (debug_flag)
                debug("Number of parameters incremented; now %d", c->nparam);
            continue;
        }
 ...
 *                  cw0 = cw, cw = cw->next);                     */
  ...
  EndofPass1:
    ...
    if (c->nparam >= 0)
    {
        ...
        if (c->nparam < c->nvar)
            error(MSG_ERROR_PARAM_NB, c->phrase, c->nparam, c->nvar);
        if (c->nparam > c->nvar)
            return 1;
        else
            return 0;
    ...
    case W_CMODIFIER:
        ...
        else
        {
            ...
            else if (strcmp(cw->Name, "long") == 0)
            {
                ...
                {
                    if (v->Ctype != NONE)
                        error(MSG_ERROR_COMP_LONG, c->phrase, v->Stype);
                    v->Ctype = LONG_T;
                }
            }
            else if (strcmp(cw->Name, "short") == 0)
            {
                ...
                {
                    if (v->Ctype != NONE)
                        error(MSG_ERROR_COMP_SHORT, c->phrase, v->Stype);
                    v->Ctype = SHORT_T;
                }
            }
            else if (strcmp(cw->Name, "char") == 0)
            {
                ...
                {
                    if (v->Ctype != NONE)
                        error(MSG_ERROR_COMP_CHAR, c->phrase, v->Stype);
                    v->Ctype = CHAR_T;
                }
            }
            else if (strcmp(cw->Name, "int") == 0)
            {
                if (v->Ctype == UNSIGNED_T)
                    v->Ctype = UINT_T;
                else if ((v->Ctype != USHORT_T) &&
                         (v->Ctype != ULONG_T) &&
                         (v->Ctype != SHORT_T) && (v->Ctype != LONG_T))
                    /*
                     * to deal with
            ...
            {
                if (v->Ctype != NONE)
                    error(MSG_ERROR_COMP_FLOAT, c->phrase, v->Stype);
                v->Ctype = FLOAT_T;
            }
            else if (strcmp(cw->Name, "double") == 0)
            {
                if (v->Ctype != NONE)
                    error(MSG_ERROR_COMP_DOUBLE, c->phrase, v->Stype);
                v->Ctype = DOUBLE_T;
            }
        }
        break;
                                    ...
                                    t_token * cwb)
{
    ...
    while (1)
    {
        ...
        while (ISCWORD_FULLTYPE(cw) || (strcmp(cw->Name, "(") == 0))
        {
            if (strcmp(cw->Name, "(") != 0)
            {
/* avoid error such in "float (*bmse)[50]", but Ftype will be wrong */
...
static void FillParam_Funcdecl_KR(t_varfunc * f, t_statement * c,
                                  t_token * cwb)
{
    ...
    do
    {
        ...
        cw0 = cwn->next;
        if (!cw0)
            error(MSG_ERROR_KR_PARAM_DECLARATION, c->phrase, cwn->Name);
        cw1 = cw0->next;
        /* printf("\n**** Search for '%s'\n", cwn->Name); */
        ...
        for (cw = cw1, nbcomma = 0;
             ...
             (strcmp(cw->Name, ";") != 0); cw = cw->previous)
            if (strcmp(cw->Name, ",") == 0)
                nbcomma++;
        cw = cw->next;
        commapassed = 0;
        ...
        while (cw != cw1->next)
        {
            if (strcmp(cw->Name, ",") == 0)
                commapassed++;
            else if (ISCWORD_SCALARTYPE(cw) ||
                     ((cw->Wtype == W_CPOINTER) && (commapassed == nbcomma)))
            {
...
static void FillNewFunction(t_varfunc * f, t_statement * c)
{
    ...
    if ((f->Itype == I_FUNCDECL_ANSI) || (f->Itype == I_FUNCDECL_KR))
    {
        ...
        if (strcmp(f->v->Name, module_name) == 0)
        {
             ...
             * Therefore, we are here in case of a non-static function
             * definition that is not the main function.
             */
        {
