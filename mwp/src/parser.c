...
#define MSG_ERROR_MAIN_FUNCTION                 \
    ...
    "Remember that the main function's name "           \
    "must be the same that the module's filename"
...
#define MSG_ERROR_UNSUPPORTED_RETURN                                    \
    ...
    "Also, remember that the light preprocessor does not process "      \
...
#define MSG_ERROR_NO_RETURN                              \
    "Invalid usage for C_id=\"%s\" : the main function " \
...
#define MSG_ERROR_UNSUPPORTED_PARAMETER                           \
    "Unsupported type \"%s\" for the parameter \"%s\" "                \
    "of the main function (Ctype=%d).\nI/O variables can be only "      \
    "of scalar types or of megawave internal types (or pointers to).\n" \
    "Also, remember that the light preprocessor does not process "      \
    "#define and #include directives."
...
static void CompleteHC(void)
{
    ...
    {
        if (f->v->Ctype != NONE)
            error(MSG_ERROR_UNSUPPORTED_RETURN, f->v->Ftype, f->v->Ctype);
    ...
    for (v = f->param; v; v = v->next)
    {
        if ((v->Ctype != MW2_T) && ((v->Ctype < SCALARMIN_T) ||
                                    (v->Ctype > SCALARMAX_T)))
            error(MSG_ERROR_UNSUPPORTED_PARAMETER, v->Ftype, v->Name,
                  v->Ctype);
        if (!v->arg)
            error(MSG_ERROR_MISSING_PARAMETER, v->Name);
    }
