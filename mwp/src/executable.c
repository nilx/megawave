...
#define MSG_ERROR_CONVERSION \
    "[SetScalarConvFunction] No scalar conversion method " \
    "implemented for C_id=\"%s\" of Ctype=%d (Stype=\"%s\")"
#define MSG_ERROR_PRINT \
    "[print_value_scalar_arg] No scalar print value method " \
    "implemented for C_id=\"%s\" of Ctype=%d (Stype=\"%s\")"
#define MSG_ERROR_INVALID_DT_FTYPE_AUXVAR \
    "[writedefvar] Invalid declaration type DT_Ftype_auxvar " \
    "for C_id=\"%s\" of Ctype=%d (Stype=\"%s\", Ftype=\"%s\") : not a pointer"
#define MSG_ERROR_INVALID_DT_FTYPE_AUXVARNULL \
    "[writedefvar] Invalid declaration type DT_Ftype_auxvarnull " \
    "for C_id=\"%s\" of Ctype=%d (Stype=\"%s\", Ftype=\"%s\") : not a pointer"
#define MSG_ERROR_UNIMPLEMENTED_DECLARATION \
    "[writedefvar] Unimplemented declaration type dt=%d " \
    "for C_id=\"%s\" of Ctype=%d (Stype=\"%s\", Ftype=\"%s\")"
#define MSG_ERROR_USAGE_FLAG \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of a flag option must be a pointer to a scalar (such as char *)"
#define MSG_ERROR_USAGE_DEFAULT \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an option with default value must be a pointer " \
    "to a scalar (such as float *)"
#define MSG_ERROR_USAGE_IC \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an option with interval checking must be a pointer " \
"to a scalar (such as float *)"
#define MSG_ERROR_USAGE_OPTION \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an option must be a pointer to a scalar, " \
    "or a megawave internal type"
#define MSG_ERROR_USAGE_INPUT \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an input needed argument cannot be an explicit pointer"
#define MSG_ERROR_USAGE_OUTPUT \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an output needed argument must be a pointer " \
    "to a scalar, a megawave internal type, or the value of the return function"
#define MSG_ERROR_USAGE_RETURN \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "type of the return main function cannot be " \
    "an explicit pointer (such as float *)"
#define MSG_ERROR_VARIABLE_IC \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of variable arguments with interval checking " \
    "must be a pointer to a scalar (such as float *)"
#define MSG_ERROR_VARIABLE_INPUT \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of input variable arguments must be a pointer " \
    "to a scalar, or a megawave internal type"
#define MSG_ERROR_VARIABLE_OUTPUT \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of output variable arguments must be of megawave internal type"
#define MSG_ERROR_VARIABLE_OPTION_DEFAULT \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an optional argument with default value " \
    "must be a pointer to a scalar (such as float *)"
#define MSG_ERROR_VARIABLE_OPTION_IC \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an optional argument with interval checking " \
    "must be a pointer to a scalar (such as float *)"
#define MSG_ERROR_VARIABLE_OPTION \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of an optional argument must be a pointer " \
    "to a scalar, or a megawave internal type"
#define MSG_ERROR_VARIABLE_UNUSED \
    "Invalid usage for C_id=\"%s\" of type \"%s\" : " \
    "variable of a not used argument must be a pointer " \
    "to a scalar, or a megawave internal type"
...
#define MSG_ERROR_UNEXPECTED \
    "Unexpected error for C_id=\"%s\" : option is not a FILE, " \
    "SCALAR or FLAGOPT"
#define MSG_ERROR_UNEXPECTED2 \
    "Unexpected error for C_id=\"%s\" : needed argument " \
    "is not a FILE nor a SCALAR"
#define MSG_ERROR_USAGE_COMPOSITE \
    "Invalid usage for C_id=\"%s\" : unsupported composite " \
    "megawave type \"%s\""
...
#define CODE_WRITE_OUTPUT_VAR_ARG \
    "      if (%s)\n" \
    "      {\n" \
    "        if (_mwsave_%s(argv[%si], %stype, " \
    "            type_force, %scomment, %s) < 0)\n" \
...
#define CODE_WRITE_OUTPUT_NEEDED2 \
    "if (_mwsave_%s(argv[_mwoptind+%d], %stype, " \
    "               type_force, %scomment, %s) < 0)\n"
...
static void writeargdecl(FILE * afile)
{
            ...
            if (ISARG_NEEDED(a))
            {
                if (ISARG_INPUT(a))
                {
                    if (ISARG_EXPLICITPOINTER(a))
                        error(MSG_ERROR_USAGE_INPUT, a->C_id, a->var->Ftype);
                    writedefvar(afile, a, DT_Stype);
                }
                else
                {
                    ...
                    if (!ISARG_IMPLICITPOINTER(a) && !ISARG_RETURNFUNC(a))
                        error(MSG_ERROR_USAGE_OUTPUT, a->C_id, a->var->Ftype);
    ...
    if (H->NbVarArg > 0)
    {
        ...
        for (a = H->usage; a; a = a->next)
        {
            if (ISARG_VARIABLE(a))
            {
                ...
                {
                    if (!ISARG_IMPLICITPOINTER(a))
                        error(MSG_ERROR_VARIABLE_INPUT, a->C_id,
                              a->var->Ftype);
                }
                else if (!ISARG_FILE(a))
                    error(MSG_ERROR_VARIABLE_OUTPUT, a->C_id, a->var->Ftype);
                if (ISARG_INPUT(a))
                    writedefvar(afile, a, DT_Ftype_alone);
                else if (ISARG_EXPLICITPOINTER(a))
                    writedefvar(afile, a, DT_Stype);
            ...
            if (ISARG_OPTARG(a))
            {
                if (ISARG_DEFAULT(a) && (!ISARG_POINTERSCALAR(a)))
                    error(MSG_ERROR_VARIABLE_OPTION_DEFAULT,
                          a->C_id, a->var->Ftype);
                if (ISARG_INTERVAL(a) && (!ISARG_POINTERSCALAR(a)))
                    error(MSG_ERROR_VARIABLE_OPTION_IC, a->C_id,
                          a->var->Ftype);
                if (!ISARG_IMPLICITPOINTER(a))
                    error(MSG_ERROR_VARIABLE_OPTION, a->C_id, a->var->Ftype);
                if (ISARG_DEFAULT(a))
    ...
    if (H->NbNotUsedArg > 0)
    {
        ...
        for (a = H->usage; a; a = a->next)
        {
            ...
            {
                if (!ISARG_IMPLICITPOINTER(a))
                    error(MSG_ERROR_VARIABLE_UNUSED, a->C_id, a->var->Ftype);
                writedefvar(afile, a, DT_Ftype_alone);
            }
        }
    }
...
static void print_check_interval_arg(FILE * afile, t_argument * a, char *s)
{
    ...
    fprintf(afile,
            ...
            s, t_m, a->var->Stype, a->Min, a->var->Stype, a->Max, t_M, fc, s);
    fprintf(afile, "{\n");
    if (a->IOtype == READ)
    {
...
static char *str_chkint( /*@ out @ */ char *str, t_argument * a, char *s)
{
    ...
    {
        sprintf(tmp, "          printf(\"output data %s ", s);
        strcat(str, tmp);
        sprintf(tmp,
                    ...
                    if (ISARG_INPUT(a))
                    {
                        ...
                        if (ISARG_INTERVAL(a))
                        {
                            ...
                            fprintf(afile, CODE_INPUT_SCALAR_ARGUMENT,
                                    a->H_id, n,
                                    str_chkint(tmp, a, buffer), a->H_id);
                        }
                        else
                        {
                ...
                if (ISARG_FILE(a))
                {
                    ...
                    if (ISARG_OUTPUT(a))
                        ...
                        fprintf(afile, CODE_OUTPUT_MW_OPTION3,
                                a->C_id, buffer, n, MWPF, MWPF, a->C_id, n);
                    n++;
                }
                else if (ISARG_SCALAR(a))
                {
