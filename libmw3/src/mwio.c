...
{                               /* Return the size of the file */
    ...
    if (0 != fseek(fp, 0, SEEK_END))
        mwerror(INTERNAL, 0,
                "[fsize] Cannot change the position in the file\n");
                    ...
                    short size, short dx, short dy, char flip)
{
    ...
    if (flip == TRUE)
    {                           /* Flip the image data (reverse byte order) */
        switch (size)
        {
        case 2:                /* Bytes 01 -> 10 */
            if (sizeof(*ptr2) != size)
                mwerror(FATAL, 0,
                        "Cannot flip %d-bytes data "
                        "with this machine architecture\n", size);
            for (i = 0, ptr2 = (unsigned short *) ptr; i < dx * dy;
                 i++, ptr2++)
                _mw_in_flip_b2(*ptr2);
            break;
        ...
        case 4:                /* Bytes 0123 -> 3210 */
            if (sizeof(*ptr4) != size)
                mwerror(FATAL, 0,
                        "Cannot flip %d-bytes data "
                        "with this machine architecture\n", size);
            for (i = 0, ptr4 = (unsigned long *) ptr; i < dx * dy;
                 i++, ptr4++)
                _mw_in_flip_b4(*ptr4);
            break;
...
FILE *_mw_write_header_file(char *fname, char *type, float IDvers)
{
    ...
    if ((strlen(type) <= 4) || (strncmp(type, "MW2_", 4) != 0))
        mwerror(INTERNAL, 1,
                "[_mw_write_header_file] Cannot create header for type %s\n",
                type);

    if (IDvers >= 10.0)
        mwerror(INTERNAL, 1,
                "[_mw_write_header_file] IDvers must be "
                "less than 10 (or change SIZE_OF_MW2_BIN_TYPE_ID)\n");
...
long _mw_find_pattern_in_file(FILE * fp, char *label)
{
    ...
    while (1)
    {
        if (fread(buf, l, 1, fp) != 1)
            return (-1);
        if (strncmp(label, buf, l) == 0)
            break;
        if (fseek(fp, 1 - l, SEEK_CUR) != 0)
            return (-1);
        p++;
    }
    return (p + l);
                     ...
                     char comment[], Cimage im)
{
    ...
    if (im->cmt[0] == '?')
        sprintf(im->cmt, "%s(%s)", mwname, comment);
                     ...
                     char comment[], Cmovie movie)
{
    _mw_choose_type(type, type_force, "cimage");
    if (movie->cmt[0] == '?')
        sprintf(movie->cmt, "%s(%s)", mwname, comment);
                     ...
                     char comment[], Curves cv)
{
    ...
    if (cv->cmt[0] == '?')
        sprintf(cv->cmt, "%s(%s)", mwname, comment);
                       ...
                       char comment[], Polygons poly)
{
    ...
    if (poly->cmt[0] == '?')
        sprintf(poly->cmt, "%s(%s)", mwname, comment);
                      ...
                      char comment[], Fsignal signal)
{
    ...
    if (signal->cmt[0] == '?')
        sprintf(signal->cmt, "%s(%s)", mwname, comment);
                       ...
                       char comment[], Wtrans1d wtrans)
{
    _mw_choose_type(type, type_force, "fsignal");
    if (wtrans->cmt[0] == '?')
        sprintf(wtrans->cmt, "%s(%s)", mwname, comment);
                           ...
                           char comment[], Vchains_wmax vchains)
{
    ...
    if (type_force[0] != '?')
        strcpy(type, type_force);
    if (vchains->cmt[0] == '?')
        sprintf(vchains->cmt, "%s(%s)", mwname, comment);
                     ...
                     char comment[], Mimage mimage)
{
    ...
    if (mimage->cmt[0] == '?')
        sprintf(mimage->cmt, "%s(%s)", mwname, comment);
                      ...
                      char comment[], Cmimage cmimage)
{
    ...
    if (cmimage->cmt[0] == '?')
        sprintf(cmimage->cmt, "%s(%s)", mwname, comment);
                     ...
                     char comment[], Shapes shapes)
{
    ...
    if (shapes->cmt[0] == '?')
        sprintf(shapes->cmt, "%s(%s)", mwname, comment);
                     ...
                     char comment[], Flists lsts)
{
    ...
    if (lsts->cmt[0] == '?')
        sprintf(lsts->cmt, "%s(%s)", mwname, comment);
                      ...
                      char comment[], Wpack2d pack)
{
    ...
    if (pack->cmt[0] == '?')
        sprintf(pack->cmt, "%s(%s)", mwname, comment);
