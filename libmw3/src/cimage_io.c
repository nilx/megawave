...
Cimage _mw_cimage_load_megawave1(char *NomFic, char *Type)
{
    ...
    else if ((BufferEntete[3] == 1) && (BufferEntete[2] == 256))
    {
        /* INR */
        strcpy(Type, "INR");
        dx = BufferEntete[0];
    ...
    else if ((BufferEntete[2] == 8) && (BufferEntete[3] == 0))
    {
        /* MTI */
        strcpy(Type, "MTI");
        dx = BufferEntete[0];
...
short _mw_cimage_create_megawave1(char *NomFic, Cimage image, char *Type)
{
    ...
    {
        mwerror(FATAL, 1,
                "Image too big to be saved using %s external type !\n", Type);
        return (-1);
    }
    dx = image->ncol;
    dy = image->nrow;
    ...
    if (Type[0] == '?')
        _mw_put_range_type(Type, "cimage", 1);
...
Cimage _mw_cimage_load_native(char *NomFic, char *Type)
{
    if ((strcmp(Type, "IMG") == 0) || (Type[0] == '?'))
        /* Includes INR,MTI and BIN types
         * (not supported by _mw_get_file_type()) */
...
short _mw_cimage_create_native(char *NomFic, Cimage image, char *Type)
{
        ...
        (_mw_is_of_ftype(Type, "MTI")) || (_mw_is_of_ftype(Type, "BIN")))
    {
        ...
        {
            mwerror(WARNING, 1,
                    "Image too big to be saved "
                    "using %s external type. Using PM_C...\n", Type);
            strcpy(Type, "PM_C");
        }
        else
            return (_mw_cimage_create_megawave1(NomFic, image, Type));
    }
        ...
        /* Write GIF87 file with a grey scales colormap */
    {
        if ((image->ncol >= 65536) || (image->nrow >= 65536))
        {
            ...
            return (_mw_cimage_create_gif(NomFic, image));
    }
...
Cimage _mw_cimage_load_image(char *NomFic, char *Type)
{
    ...
    if (Type[0] == '?')
        ...
        mwerror(FATAL, 1,
                ...
                "I Don't know how to load such external type into a Cimage !\n",
                NomFic, Type);
    return NULL;
}
