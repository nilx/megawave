...
int _mw_fascii_search_string(FILE * fp, char *str)
{
    ...
    {
        mwerror(INTERNAL, 1,
                "[_mw_fascii_search_string]: Null input string\n");
        return (0);
    }
                         ...
                         char *str_control, void *ptr)
{
    ...
    {
        mwerror(ERROR, 0,
                "Cannot find the field \"%s\" "
                "in the MegaWave2 Data Ascii file \"%s\"\n", field_name,
                fname);
        return (0);
    }
    ...
    {
        mwerror(ERROR, 0,
                "Error at or before the field \"%s\" "
                "in the MegaWave2 Data Ascii file \"%s\"\n", field_name,
                fname);
        return (0);
    }
 ...
 * field is not found (this allows to read fields added in an extended
 * format).
 */
/* FIXME: unsafe */
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
                                  ...
                                  void *ptr)
{
    ...
    if (fscanf(fp, str_control, ptr) != 1)
    {
        mwerror(WARNING, 0,
                "Error at or before the field \"%s\" "
                "in the MegaWave2 Data Ascii file \"%s\" (field ignored).\n",
                field_name, fname);
        return (0);
    }
    return (1);
