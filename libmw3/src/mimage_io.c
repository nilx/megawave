...
static Morpho_line _mw_read_ml_mw2_ml(char *fname, FILE * fp,
                                      int need_flipping)
{
    Morpho_line ll;
    ...
    fflush(fp);
    /* Should not be here, but this may avoid error on the next
    ...
    if ((npc * npt != 0) && (npc != npt))
    {
        mwerror(ERROR, 0,
                ...
                "inconsistent Cmorpho_line structure "
                "(# pt curve %d != # pt type %d)\n", fname, npc, npt);
        return (NULL);
    }
    ...
    for (i = 1; i <= npc; i++)
    {
        ...
        if (ll->first_point == NULL)
            ll->first_point = newpc;
        if (oldpc != NULL)
            oldpc->next = newpc;
...
Morpho_line _mw_load_ml_mw2_ml(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_MORPHO_LINE", 15) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_ml_mw2_ml] File \"%s\" is not "
                "in the MW2_MORPHO_LINE format\n", fname);
...
Morpho_line _mw_load_morpho_line(char *fname, char *type)
{
    ...
    ml = (Morpho_line) _mw_load_etype_to_itype(fname, mtype, "morpho_line",
                                               type);
    if (ml != NULL)
        return (ml);
...
void _mw_write_ml_mw2_ml(FILE * fp, Morpho_line ll, unsigned int nml)
{
    ...
    npc = 0;
    if (ll->first_point)
        for (pc = ll->first_point; pc; pc = pc->next, npc++);
...
*/
    ...
    npt = 0;
    if (ll->first_type)
        for (pt = ll->first_type; pt; pt = pt->next, npt++);
...
*/
    ...
    if ((npc * npt != 0) && (npc != npt))
        mwerror(INTERNAL, 1,
                "[_mw_write_ml_mw2_ml] Cannot create file: "
                "inconsistent Morpho_line structure \n");
...
short _mw_create_ml_mw2_ml(char *fname, Morpho_line ll)
{
    ...
    if (ll == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_ml_mw2_ml] Cannot create file: "
                "Morpho_line structure is NULL\n");
...
*/
    ...
    for (i = 1; i <= npc; i++)
    {
        ...
        if (fll->first_point == NULL)
            fll->first_point = newpc;
        if (oldpc != NULL)
            oldpc->next = newpc;
        newpc->previous = oldpc;
        newpc->next = NULL;
    ...
    for (i = 1; i <= npt; i++)
    {
        ...
        if (fll->first_type == NULL)
            fll->first_type = newpt;
        if (oldpt != NULL)
            oldpt->next = newpt;
        newpt->previous = oldpt;
        newpt->next = NULL;
...
Fmorpho_line _mw_load_fml_mw2_fml(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_FMORPHO_LINE", 16) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_fml_mw2_fml] File \"%s\" is not "
                "in the MW2_FMORPHO_LINE format\n", fname);
...
Fmorpho_line _mw_load_fmorpho_line(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    ml = (Fmorpho_line) _mw_load_etype_to_itype(fname, mtype, "fmorpho_line",
                                                type);
    if (ml != NULL)
        return (ml);
...
*/
    ...
    if ((npc * npt != 0) && (npc != npt))
        mwerror(INTERNAL, 1,
                "[_mw_write_fml_mw2_fml] Cannot create file: "
                "inconsistent Fmorpho_line structure \n");
...
short _mw_create_fml_mw2_fml(char *fname, Fmorpho_line fll)
{
    ...
    if (fll == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_fml_mw2_fml] Cannot create file: "
                "Fmorpho_line structure is NULL\n");
...
short _mw_fmorpho_line_create_native(char *fname, Fmorpho_line ll, char *Type)
{
    if (strcmp(Type, "MW2_FMORPHO_LINE") == 0)
...
Morpho_set _mw_read_ms_mw2_ms(char *fname, FILE * fp, int need_flipping)
{
    ...
    for (i = 1; i <= ns; i++)
    {
        ...
        if (is->first_segment == NULL)
            is->first_segment = news;
        if (olds != NULL)
            olds->next = news;
        news->previous = olds;
        news->next = NULL;
...
Morpho_set _mw_load_ms_mw2_ms(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_MORPHO_SET", 14) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_ms_mw2_ms] File \"%s\" is not "
                "in the MW2_MORPHO_SET format\n", fname);
...
Morpho_set _mw_load_morpho_set(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    ms = (Morpho_set) _mw_load_etype_to_itype(fname, mtype, "morpho_set",
                                              type);
    if (ms != NULL)
        return (ms);
...
short _mw_create_ms_mw2_ms(char *fname, Morpho_set is)
{
    ...
    if (is == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_ms_mw2_ms] Cannot create file: "
                "Morpho_set structure is NULL\n");
...
Morpho_sets _mw_read_mss_mw2_mss(char *fname, FILE * fp, int need_flipping)
{
    ...
    for (p = iss; p; p = p->next)
    {
        ...
        for (i = 1; i <= n; i++)
        {
            ...
            for (q = iss; q && q->morphoset && (q->morphoset->num != num);
                 q = q->next);
            if (!q || !(q->morphoset))
            {
                mw_delete_morpho_sets(iss);
                mwerror(INTERNAL, 0,
                        "[_mw_load_mss_mw2_mss] Cannot find Morpho set "
                        "number %d in file \"%s\"\n", num, fname);
            }
            ...
            if (p->morphoset->neighbor == NULL)
                p->morphoset->neighbor = newiss;
            if (oldiss)
                oldiss->next = newiss;
            newiss->previous = oldiss;
            oldiss = newiss;
...
Morpho_sets _mw_load_mss_mw2_mss(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_MORPHO_SETS", 15) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_mss_mw2_mss] File \"%s\" is not "
                "in the MW2_MORPHO_SETS format\n", fname);
...
Morpho_sets _mw_load_morpho_sets(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    ms = (Morpho_sets) _mw_load_etype_to_itype(fname, mtype, "morpho_sets",
                                               type);
    if (ms != NULL)
        return (ms);
...
void _mw_write_mss_mw2_mss(FILE * fp, Morpho_sets iss)
{
    ...
    if (!iss || !iss->morphoset)
        return;

    /* Record each morpho set */
    for (p = iss; p; p = p->next)
        _mw_write_ms_mw2_ms(fp, p->morphoset);
    ...
    for (p = iss; p; p = p->next)
    {
        ...
        for (q = neig; q; q = q->next)
            if (1 > fwrite(&(q->morphoset->num), sizeof(unsigned int), 1, fp))
            {
                mwerror(ERROR, 0, "Error while writing file...\n");
                exit(EXIT_FAILURE);
            }
...
short _mw_create_mss_mw2_mss(char *fname, Morpho_sets iss)
{
    ...
    if (iss == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_mss_mw2_mss] Cannot create file: "
                "Morpho_sets structure is NULL\n");
...
Mimage _mw_load_mimage_mw2_mimage(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_MIMAGE", 10) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_mimage_mw2_mimage] File \"%s\" is not "
                "in the MW2_MIMAGE format\n", fname);
    ...
    {
        mwerror(ERROR, 0,
                "Error while reading file \"%s\" (cmt; cmt size=%d)...\n",
                fname, size);
        fclose(fp);
        return (NULL);
    }
...
*/
    ...
    if (mimage->first_ml)
    {
        ...
        for (i = 1, ll = mimage->first_ml; i <= size; i++)
        {
            ...
            /* Search the morpho sets containing
             * the given morpho set number msnum */
            for (mss = mimage->first_ms;
                 mss && mss->morphoset && (mss->morphoset->num != msnum);
                 mss = mss->next);
            if (!mss || !(mss->morphoset))
            {
                mw_delete_mimage(mimage);
                mwerror(INTERNAL, 0,
                        "[_mw_load_mimage_mw2_mimage] Cannot find "
                        "morpho set number %d in file \"%s\"\n", msnum,
                        fname);
                fclose(fp);
                return (NULL);
            }
...
short _mw_create_mimage_mw2_mimage(char *fname, Mimage mimage)
{
    ...
    if (mimage == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_mimage_mw2_mimage] Cannot create file: "
                "Mimage structure is NULL\n");
