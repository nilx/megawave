...
Cfimage mw_alloc_cfimage(Cfimage image, int nrow, int ncol)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_cfimage] cannot alloc plane : "
                "cfimage structure is NULL\n");
        return (NULL);
    }
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_cfimage] Attempts to alloc a cfimage "
                "with null size\n");
        return NULL;
    }
...
void mw_delete_cfimage(Cfimage image)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_cfimage] cannot delete : "
                "cfimage structure is NULL\n");
        return;
    }
                       ...
                       float *r, float *g, float *b)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_getdot_cfimage] Point (%d,%d) out of image... "
                "Return (0,0,0)\n", x, y);
        *r = *g = *b = 0.0;
        return;
    }
    ...
    if (!image->red)
    {
        mwerror(WARNING, 0, "[mw_getdot_cfimage] NULL red plane... "
                "Return 0\n");
        *r = 0.0;
        return;
    }
...
void mw_clear_cfimage(Cfimage image, float r, float g, float b)
{
    ...
    for (l = 1, ptr1 = image->red, ptr2 = image->green, ptr3 = image->blue;
         l <= image->ncol * image->nrow; l++, ptr1++, ptr2++, ptr3++)
    {
        *ptr1 = r;
        *ptr2 = g;
        *ptr3 = b;
...
void mw_copy_cfimage(Cfimage in, Cfimage out)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_copy_cfimage] NULL input or output image "
...
float **mw_newtab_red_cfimage(Cfimage image)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_newtab_red_cfimage] NULL image struct "
                "or NULL red plane\n");
        return (NULL);
    }
...
float **mw_newtab_green_cfimage(Cfimage image)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_newtab_green_cfimage] NULL image struct "
                "or NULL green plane\n");
        return (NULL);
    }
...
float **mw_newtab_blue_cfimage(Cfimage image)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_newtab_blue_cfimage] NULL image struct "
                "or NULL blue plane\n");
        return (NULL);
    }
