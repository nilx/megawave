                             ...
                             char *comment, float a, float b)
{
    ...
    short lenfloat = 14;        /* Longueur en bytes d'un float
                                 * sous forme Ascii */
...
Fimage _mw_fimage_load_megawave1(char *NomFic, char *Type)
{
    ...
    {
        if (L > 65000L)
            n = (65000 / sizeof(float)) * sizeof(float);
        else
            n = L;
        L -= n;
    ...
    _mw_flip_image((unsigned char *) ptr, sizeof(float), dx, dy,
                   need_flipping);
    return (image);
}
...
short _mw_fimage_create_megawave1(char *NomFic, Fimage image, char *Type)
{
    ...
    if (Type[0] == '?')
        _mw_put_range_type(Type, "fimage", 1);
    ...
    if (strcmp(Type, "RIM") == 0)
    {
        if ((image->ncol >= 65536) || (image->nrow >= 65536))
        {
            mwerror(WARNING, 1,
                    ...
                    "using %s external type. Using PM_F...\n", Type);
            strcpy(Type, "PM_F");
        }
        else
...
Fimage _mw_fimage_load_image(char *NomFic, char *Type)
{
    ...
    if (Type[0] == '?')
        ...
        mwerror(FATAL, 1,
                ...
                "I Don't know how to load such external type into a Fimage !\n",
                NomFic, Type);
