...
static int _mw_get_range_array(char *a, char *b, char *A[])
{
    ...
    for (i = 0;
         (((A[i] != NULL) || (A[i + 1] != NULL)) &&
          ((A[i] == NULL) || (strcmp(a, A[i]) != 0)
           || ((i != 0) && (A[i - 1] != NULL)))); i++);
    if (A[i] == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_get_range_array] Unknown line "
                "beginning by the string \"%s\" on array \"%s\",\"%s\",...\n",
                a, A[0], A[1]);
...
static void _mw_put_range_array(char *a, char *b, int r, char *A[])
{
    ...
    if (A[i] == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_put_range_array] Unknown line "
                "beginning by the string \"%s\" on array \"%s\",\"%s\",...\n",
                a, A[0], A[1]);
    /* i = index of a in the array A */
    ...
    if ((r <= 0) || (r >= j - i))
        mwerror(INTERNAL, 1,
                "[_mw_put_range_array] Invalid range %d "
                "(out of [1,%d]) for line beginning by the string \"%s\" "
                "on array \"%s\",\"%s\",...\n", r, j - i - 1, a, A[0], A[1]);
    strcpy(b, A[i + r]);
}

/*  Say if the string <b> in the line beginnning by the string <a>
 *  exists or not in the array A[]. Return 1 if it exists, 0 if
 *  not. Added in July 2002 (V 2.19) : since b may be a type with an
 *  argument (as JFIFC:50), check before the keyword ':' only.
*/
...
void _mw_lower_type(char type[])
{
    ...
    for (i = 0; type[i] != '\0'; i++)
        type[i] = (char) simple_tolower((int) type[i]);
}
...
static int _mw_ftype_exists_for_output(char *ftype, char *mtype)
{
    ...
    if (r <= 0)
        return (0);             /* No conversion functions for mtype */
...
void _mw_choose_type(char type[], char *type_force, char *mtype)
{
        ...
        /* User forced the type */
    {
        if ((_mw_native_ftype_exists(type_force, mtype) == 0) &&
            ...
            /* Forced type is neither native
             * neither available with conversion */
        {
            mwerror(WARNING, 0,
                    "Invalid external type \"%s\" "
                    "for the internal type \"%s\"; Using \"%s\" instead.\n",
                    type_force, mtype, type);
            _mw_print_available_ftype_for_output(mtype);
        }
        else
            strcpy(type, type_force);
    }
}
...
int _mw_is_of_ftype(char *in, char *type)
{
    ...
    for (i = 0, a = in, b = type; i < l; i++, a++, b++)
    {
        ...
        {
            if (*b == '\0')
                return (1);     /* match in case of option */
            else
...
/* Find the sub-type of a RIFF file.
   Return 0 if not a loadable RIFF file, 1 if doesn't need flipping, 2
   if it does.
...
static int what_kind_of_RIFF(char *file, char *subtype, char *mtype)
{
    int need_flipping;
    FILE *fp;
    unsigned int dwFmtSize;     /* length of format chunk
                                 * minus 8 byte header (dw = 4 bytes) */
    ...
    if (!(fp = fopen(file, "r")))
        return (0);
    /* bytes 0-1 and 2-3 */
    if (fread(&bytes1_2, 2, 1, fp) != 1)
    {
        fclose(fp);
        return (0);
    }
    if (fread(&bytes3_4, 2, 1, fp) != 1)
    {
        fclose(fp);
        return (0);
    }

    /* Note on byte ordering with RIFF files : the default byte
     * ordering is little-endian.
     * File written using big-endian byte ordering scheme have the
     * identifier RIFX instead of RIFF. (At least for WAVE files, has
     * to be checked with other RIFF files).
     */
    if (_mw_byte_ordering_is_little_endian() == 1)
        /* Byte ordering is little endian */
    {
        if (((bytes1_2 == 0x5249)
             || (bytes1_2 == 0x4952)) && (bytes3_4 == 0x4646))
            /* RIFF identifier */
            need_flipping = 0;
        else if (((bytes1_2 == 0x5249)
                  || (bytes1_2 == 0x4952))
                 && ((bytes3_4 == 0x4658) || (bytes3_4 == 0x5846)))
            /* RIFX identifier */
            need_flipping = 1;
        else
        {
        ...
        /* RIFF WAVE format */
    {
        ...
        {
            mwerror(WARNING, 1,
                    "Corrupted WAVE file '%s' : no format chunk !\n", file);
            fclose(fp);
            return (0);
        }
        ...
        {
            mwerror(WARNING, 1,
                    "Corrupted WAVE file '%s' : unexpected EOF !\n", file);
            fclose(fp);
            return (0);
        }
        if (need_flipping == 1)
            _mw_in_flip_b4(dwFmtSize);
        if (dwFmtSize < 16)
        {
            mwerror(ERROR, 1,
                    "Corrupted WAVE file '%s' : format chunk too short !\n",
                    file);
            fclose(fp);
            return (0);
        }
        /* Format Tag : indentifies WAVE encoding such as PCM, ADPCM,
         * ULAW, ... */
        ...
        default:
            mwerror(WARNING, 1,
                    "WAVE file '%s' : unsupported encoding %x, sorry !\n",
                    file, wFormatTag);
            return (0);
        }
    }
...
/* Return 0 if not a loadable PM file, 1 if doesn't need flipping, 2
 * if it does */
...
static int what_kind_of_PM(char *file, char *subtype, char *mtype)
{
    ...
    if (pm->pm_np > 1)
    {
        ...
        {                       /* 24-bit color image */
            ...
            strcat(subtype, "?");       /* other cases not implemented */
    }
...
static int what_kind_of_BMP(char *file, char *subtype, char *mtype)
{
    ...
    fp = _mw_read_bmp_header(file, &nx, &ny, &offset, &size, &planes,
 ...
 *              JFIFC means 24-bits Color JPEG-JFIF format
...
static void _Get_MW2_X_V_header(char header[],
                                int Xsize, int *X_Vsize, float *version)
{
    ...
    *version = 0.0;
    ...
    if (strlen(header) < (unsigned int) Xsize + SIZE_OF_MW2_BIN_TYPE_ID)
        ...
        return;
    if ((header[Xsize] != '/')
        || (header[Xsize + SIZE_OF_MW2_BIN_TYPE_ID - 1] != '/')
        || (header[Xsize + 2] != '.'))
                                    ...
                                    int *hsize, float *version)
{
    ...
    *version = 0.0;
    ...
    case 0x5249:
        /* RIM or RIFF */
        if ((bytes3_4 == 0x4646) || (bytes3_4 == 0x4658))
            /* RIFF or RIFX */
    ...
    case 0x4952:
        /* RIM or RIFF */
        if ((bytes3_4 == 0x4646) || (bytes3_4 == 0x5846))
            /* RIFF or RIFX */
            return (what_kind_of_RIFF(fname, ftype, mtype));
        else
            /* Assume RIM */
    ...
    case 0x4D57:
    case 0x574D:
        /* MW : One of the home MegaWave2 binary format */
                      ...
                      int *hsize, float *version)
{
    ...
    if ((ret =
         _mw_get_binary_file_type(fname, ftype, mtype, hsize, version)) == 0)
