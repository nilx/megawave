/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Basic memory routines for the cmorpho_line, cmorpho_set
  & cmimage internal types
...
Cmorpho_line mw_new_cmorpho_line(void)
{
    ...
    if (!
        (cmorpho_line = (Cmorpho_line) (malloc(sizeof(struct cmorpho_line)))))
    {
    ...
    cmorpho_line->minvalue.red = cmorpho_line->minvalue.green =
        cmorpho_line->minvalue.blue = 0.0;
    cmorpho_line->open = 0;
...
void mw_delete_cmorpho_line(Cmorpho_line cmorpho_line)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_cmorpho_line] cannot delete : "
                "cmorpho_line structure is NULL\n");
        return;
    }

    if (cmorpho_line->first_point)
        mw_delete_point_curve(cmorpho_line->first_point);
    cmorpho_line->first_point = NULL;
    if (cmorpho_line->first_type)
        mw_delete_point_type(cmorpho_line->first_type);
    cmorpho_line->first_type = NULL;
...
Cmorpho_line mw_copy_cmorpho_line(Cmorpho_line in, Cmorpho_line out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_cmorpho_line] Not enough memory "
                    "to create a cmorpho_line !\n");
            return (NULL);
        }
    }
...
unsigned int mw_length_cmorpho_line(Cmorpho_line cmorpho_line)
{
    ...
    if ((!cmorpho_line) || (!cmorpho_line->first_point))
        return (0);
    ...
    if (!cmorpho_line->first_type)
    {
        ...
        if ((n * m != 0) && (n != m))
            mwerror(INTERNAL, 1,
                    "[mw_length_cmorpho_line] Inconsistent "
                    "Cmorpho_line structure \n");
    }
    return (n);
}
...
unsigned int mw_num_cmorpho_line(Cmorpho_line ml_first)
{
    ...
    if (!ml_first)
    {
        mwerror(ERROR, 0, "[mw_num_cmorpho_line] NULL input cmorpho_line\n");
        return (0);
    }
...
Cfmorpho_line mw_new_cfmorpho_line(void)
{
    ...
    if (!
        (cfmorpho_line =
         (Cfmorpho_line) (malloc(sizeof(struct cfmorpho_line)))))
    {
    ...
    cfmorpho_line->minvalue.red = cfmorpho_line->minvalue.green =
        cfmorpho_line->minvalue.blue = 0.0;
...
void mw_delete_cfmorpho_line(Cfmorpho_line cfmorpho_line)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_cfmorpho_line] cannot delete : "
                "cfmorpho_line structure is NULL\n");
        return;
    }

    if (cfmorpho_line->first_point)
        mw_delete_point_fcurve(cfmorpho_line->first_point);
    cfmorpho_line->first_point = NULL;
    if (cfmorpho_line->first_type)
        mw_delete_point_type(cfmorpho_line->first_type);
    cfmorpho_line->first_type = NULL;
...
Cfmorpho_line mw_copy_cfmorpho_line(Cfmorpho_line in, Cfmorpho_line out)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_copy_cfmorpho_line] NULL input cfmorpho_line\n");
        return (NULL);
    }
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_cfmorpho_line] Not enough memory "
                    "to create a cfmorpho_line !\n");
            return (NULL);
        }
    }
    out->minvalue = in->minvalue;
    ...
    for (pc = in->first_point; pc; pc = pc->next)
    {
        ...
        if (qc1 == NULL)
        {
            ...
            mwerror(ERROR, 0,
                    "Not enough memory to create a cfmorpho_line\n");
            return (NULL);
        }
        qc1->x = pc->x;
        qc1->y = pc->y;
...
unsigned int mw_length_cfmorpho_line(Cfmorpho_line cfmorpho_line)
{
    ...
    if ((!cfmorpho_line) || (!cfmorpho_line->first_point))
        return (0);
    ...
    if (!cfmorpho_line->first_type)
    {
        ...
        if ((n * m != 0) && (n != m))
            mwerror(INTERNAL, 1,
                    "[mw_length_cfmorpho_line] Inconsistent "
                    "Cfmorpho_line structure \n");
    }
    return (n);
}
...
Cmorpho_set mw_new_cmorpho_set(void)
{
    ...
    cmorpho_set->minvalue.red = cmorpho_set->minvalue.green =
        cmorpho_set->minvalue.blue = 0.0;
...
void mw_delete_cmorpho_set(Cmorpho_set cmorpho_set)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_cmorpho_set] cannot delete : "
                "cmorpho_set structure is NULL\n");
        return;
    }

    if (cmorpho_set->first_segment)
        mw_delete_hsegment(cmorpho_set->first_segment);
    cmorpho_set->first_segment = cmorpho_set->last_segment = NULL;
    cmorpho_set->neighbor = NULL;
    free(cmorpho_set);
    cmorpho_set = NULL;
...
Cmorpho_set mw_copy_cmorpho_set(Cmorpho_set in, Cmorpho_set out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_cmorpho_set] Not enough memory "
                    "to create a cmorpho_set !\n");
            return (NULL);
        }
    }
    out->minvalue = in->minvalue;
...
unsigned int mw_length_cmorpho_set(Cmorpho_set cmorpho_set)
{
    ...
    if ((!cmorpho_set) || (!cmorpho_set->first_segment))
        return (0);
...
Cmorpho_sets mw_new_cmorpho_sets(void)
{
    ...
    if (!
        (cmorpho_sets = (Cmorpho_sets) (malloc(sizeof(struct cmorpho_sets)))))
    {
...
void mw_delete_cmorpho_sets(Cmorpho_sets cmorpho_sets)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_cmorpho_sets] cannot delete : "
                "cmorpho_sets structure is NULL\n");
        return;
    }
...
/* Copy a cmorpho_sets into another cmorpho_sets
...
Cmorpho_sets mw_copy_cmorpho_sets(Cmorpho_sets in, Cmorpho_sets out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_cmorpho_sets] Not enough memory "
                    "to create a cmorpho_sets !\n");
            return (NULL);
        }
    }
    ...
    for (p = in; p && p->cmorphoset; p = p->next)
    {
        ...
        if (!newiss->cmorphoset)
            newiss->cmorphoset = mw_new_cmorpho_set();
        if (!newiss->cmorphoset)
        {
            mwerror(ERROR, 0, "Not enough memory to create a cmorpho_sets\n");
            return (NULL);
    ...
    for (p = in, q = out, n = 1; p && q;
         p->cmorphoset->num = n, q->cmorphoset->num = n, p = p->next, q =
         q->next, n++);
    ...
    for (p = in, q = out; p && q; p = p->next, q = q->next)
        for (nin = p->cmorphoset->neighbor; nin; nin = nin->next)
        {
            ...
            for (qq = out; qq && qq->cmorphoset && (qq->cmorphoset->num != n);
                 qq = qq->next);
            if (!qq || !(qq->cmorphoset))
            {
                mwerror(ERROR, 0,
                        "Cannot copy cmorpho sets : "
                        "unconsistent neighbor list in input\n");
                return (NULL);
            }
            if ((newiss = mw_new_cmorpho_sets()) == NULL)
            {
                mwerror(ERROR, 0,
                        "Not enough memory to create a cmorpho_sets\n");
                return (NULL);
            }
            newiss->cmorphoset = qq->cmorphoset;
            if (q->cmorphoset->neighbor == NULL)
                q->cmorphoset->neighbor = newiss;
            if (oldiss)
                oldiss->next = newiss;
            newiss->previous = oldiss;
            oldiss = newiss;
...
unsigned int mw_length_cmorpho_sets(Cmorpho_sets cmorpho_sets)
{
    ...
    if ((!cmorpho_sets) || (!cmorpho_sets->cmorphoset))
        return (0);
...
unsigned int mw_num_cmorpho_sets(Cmorpho_sets mss_first)
{
    ...
    if (!mss_first)
    {
        mwerror(ERROR, 0, "[mw_num_cmorpho_sets] NULL input cmorpho_sets\n");
        return -1;
    }
    ...
    if (mss && !mss->cmorphoset)
        mwerror(ERROR, 0,
                "Cmorphosets chain cut "
                "due to the cmorphosets #%d which has no cmorphoset field\n",
                n);
    return (n - 1);
}
...
void mw_cmorpho_sets_clear_stated(Cmorpho_sets mss_first)
{
    ...
    for (mss = mss_first; mss && mss->cmorphoset;
         mss->cmorphoset->stated = 0, mss = mss->next);
...
void mw_delete_cmimage(Cmimage cmimage)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_cmimage] cannot delete : "
                "cmimage structure is NULL\n");
        return;
    }
    ...
    if (cmimage->first_ms)
        mw_delete_cmorpho_sets(cmimage->first_ms);
...
Cmimage mw_copy_cmimage(Cmimage in, Cmimage out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_cmimage] Not enough memory "
                    "to create a cmimage !\n");
            return (NULL);
        }
    }
