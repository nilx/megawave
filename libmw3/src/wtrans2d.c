...
Wtrans2d mw_new_wtrans2d(void)
{
    ...
    for (j = 0; j < mw_max_nlevel; j++)
        for (d = 0; d < mw_max_norient; d++)
            wtrans->images[j][d] = NULL;

    for (j = 0; j < mw_max_nfilter; j++)
                                 ...
                                 int sampling)
{
    ...
    {
        mwerror(ERROR, 0,
                "cannot alloc wtrans2d: too many levels (%d) "
    ...
    {
        mwerror(ERROR, 0,
                "cannot alloc wtrans2d: too many orientations (%d) "
                "in the decomposition\n", Norient);
        return (NULL);
    }
 ...
 * {
 * mwerror(ERROR, 0,
 *         "cannot alloc wtrans2d: Size of original image "
 *         "is (%d,%d) - not an even size -\n",ncol,nrow);
 * return(NULL);
 * }
 */
 ...
 */
                ...
                {
                    mwerror(ERROR, 0,
                            "cannot alloc wtrans2d: Not enough memory\n");
                    for (jj = 1; jj <= j; jj++)
...
void *mw_alloc_ortho_wtrans2d(Wtrans2d wtrans, int level, int nrow, int ncol)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_ortho_wtrans2d] Cannot alloc "
                "orthogonal wtrans2d.\n");
        return (NULL);
    }
                                ...
                                int ncol)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_biortho_wtrans2d] Cannot alloc "
                "biorthogonal wtrans2d.\n");
        return (NULL);
    }
...
void *mw_alloc_dyadic_wtrans2d(Wtrans2d wtrans, int level, int nrow, int ncol)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_dyadic_wtrans2d] Cannot alloc dyadic wtrans2d.\n");
        return (NULL);
    }
...
void mw_delete_wtrans2d(Wtrans2d wtrans)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_wtrans2d] cannot delete : "
                "wtrans2d structure is NULL\n");
        return;
    }

    wtrans->images[0][0] = NULL;

    for (j = 1; j <= wtrans->nlevel; j++)
        for (d = 0; d <= wtrans->norient; d++)
            if (wtrans->images[j][d] != NULL)
            {
