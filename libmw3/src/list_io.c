...
Flist _mw_read_mw2_flist(char *fname, FILE * fp, int need_flipping)
{
    ...
    if (need_flipping == 1)
    {
        /* Strange that we need {} here to avoid run-time errors ! */
        for (i = 0; i < n; i++)
            _mw_in_flip_float((lst->values[i]));
    }
...
Flist _mw_load_mw2_flist(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_FLIST", 9) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_mw2_flist] File \"%s\" is not "
                "in the MW2_FLIST format\n", fname);
...
Flist _mw_load_flist(char *fname, char *type)
{
    ...
    if (type[0] == '?')
        ...
        mwerror(FATAL, 1,
                ...
                "I Don't know how to load such external type into a Curve !\n",
                fname, type);
    return NULL;
}
...
short _mw_create_mw2_flist(char *fname, Flist lst)
{
    ...
    if (lst == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_mw2_flist] Cannot create file: "
                "Flist structure is NULL\n");
...
Flists _mw_load_mw2_flists(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_FLISTS", 10) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_mw2_flists] File \"%s\" is not "
                "in the MW2_FLISTS format\n", fname);
...
short _mw_create_mw2_flists(char *fname, Flists lsts)
{
    ...
    if (lsts == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_mw2_flists] Cannot create file: "
                "flists structure is NULL\n");
...
Dlist _mw_load_mw2_dlist(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_DLIST", 9) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_mw2_dlist] File \"%s\" is not "
                "in the MW2_DLIST format\n", fname);
...
short _mw_create_mw2_dlist(char *fname, Dlist lst)
{
    ...
    if (lst == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_mw2_dlist] Cannot create file: "
                "Dlist structure is NULL\n");
...
Dlists _mw_load_mw2_dlists(char *fname)
{
    ...
    if (strncmp(ftype, "MW2_DLISTS", 10) != 0)
        mwerror(INTERNAL, 0,
                "[_mw_load_mw2_dlists] File \"%s\" is not "
                "in the MW2_DLISTS format\n", fname);
...
short _mw_create_mw2_dlists(char *fname, Dlists lsts)
{
    ...
    if (lsts == NULL)
        mwerror(INTERNAL, 1,
                "[_mw_create_mw2_dlists] Cannot create file: "
                "dlists structure is NULL\n");
