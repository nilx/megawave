...
Fpolygon _mw_load_fpolygon_a_fpoly(char *fname)
{
    ...
    for (i = 0; i < nc; i++)
    {
        ...
        if (_mw_fascii_get_field(fp, fname, channel, "%f", &fpoly->channel[i])
            != 1)
        {
            mw_delete_fpolygon(fpoly);
            fclose(fp);
...
short _mw_create_fpolygon_a_fpoly(char *fname, Fpolygon fpoly)
{
    ...
    if (fpoly == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Fpolygon structure is NULL\n");
    ...
    do
    {
        ...
        for (i = 0; i < nc; i++)
        {
            ...
            if (_mw_fascii_get_field
                (fp, fname, channel, "%f", &newp->channel[i]) != 1)
            {
                mw_delete_fpolygons(poly);
                fclose(fp);
                return (NULL);
...
Fpolygons _mw_load_fpolygons(char *fname, char *type)
{
    ...
    /* If failed, try other formats with memory conversion */
    poly =
        (Fpolygons) _mw_load_etype_to_itype(fname, mtype, "fpolygons", type);
    if (poly != NULL)
        return (poly);
...
short _mw_create_fpolygons_a_fpoly(char *fname, Fpolygons poly)
{
    ...
    if (poly == NULL)
        mwerror(INTERNAL, 1,
                "Cannot create file: Fpolygons structure is NULL\n");
