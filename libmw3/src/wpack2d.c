/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Vers. 0.0
  Authors : Adrien Costagliola, David Serre, Francois Malgouyres,
  Jacques Froment
...
*/
static Fimage alloc_image(int kx, Wpack2d pack, int start_nrow,
                          int start_ncol)
{
...
*/
static int indice_alloc(Wpack2d pack, int posx, int posy,
                        int current_level, int start_nrow, int start_ncol)
{
    int i1, i2, i3, i4;
    int posx1, posy1;
    int shift, r;
    ...
    {
        if ((pack->images[i1] =
             alloc_image(i1, pack, start_nrow, start_ncol)) == NULL)
            return (-1);
    }
    else
    {
        r = indice_alloc(pack, posx, posy, current_level + 1, start_nrow,
                         start_ncol);
        if (r < 0)
            return (r);
    }
    if (current_level)
    {
        /* those allocation can only be performed if current_level != 0 */
        if (current_level == pack->tree->gray[i2])
        {
            if ((pack->images[i2] =
                 alloc_image(i2, pack, start_nrow, start_ncol)) == NULL)
                return (-2);
        }
        else
        {
            r = indice_alloc(pack, posx1, posy, current_level + 1, start_nrow,
                             start_ncol);
            if (r < 0)
                return (r);
        }
        ...
        {
            if ((pack->images[i3] =
                 alloc_image(i3, pack, start_nrow, start_ncol)) == NULL)
                return (-3);
        }
        else
        {
            r = indice_alloc(pack, posx, posy1, current_level + 1, start_nrow,
                             start_ncol);
            if (r < 0)
                return (r);
        }
        ...
        {
            if ((pack->images[i4] =
                 alloc_image(i4, pack, start_nrow, start_ncol)) == NULL)
                return (-4);
        }
        else
        {
            r = indice_alloc(pack, posx1, posy1, current_level + 1,
                             start_nrow, start_ncol);
            if (r < 0)
                return (r);
        }
    }
...
/*
 * Check if an image can be considered a tree, for a wavelet packet
 * decomposition.
 * If the image is a tree, the function returns its maximum level of
 * decomposition.
 */
...
int mw_checktree_wpack2d(Cimage tree)
{
    ...
    if (tree->ncol != tree->nrow)
        mwerror(FATAL, 1,
                "[mw_checktree_wpack2d] Not a tree : the image "
                "does not have the same number of column and raws\n");
    if (tree->ncol == 0)
        mwerror(FATAL, 1,
                "[mw_checktree_wpack2d] Not a tree : empty image\n");
    ...
    if (tree_size != 1)
        mwerror(FATAL, 1,
                "[mw_checktree_wpack2d] Not a tree : the number "
                "of columns and raws is not a power of 2\n");

    tree_size = tree->ncol;

    /* Test if the maximum of the image corresponds to a tree  structure */
    for (x = 0; x < tree_size; x++)
        for (y = 0; y < tree_size; y++)
            if (_(tree, x, y) > level)
                mwerror(FATAL, 1,
                        "[mw_checktree_wpack2d] Not a tree : an image "
                        "value exceeds the maximum level (%d)\n", level);

    /* Test if the image corresponds to a tree structure.
     */
    for (k = 0, sub_tree_size = tree_size, ratio = 1; k < level; k++)
    {
        for (kx = 0; kx < ratio; kx++)
            for (ky = 0; ky < ratio; ky++)
            {
                ...
                if (v > k)
                     ...
                     */
                {
                    for (x = 0; x < sub_tree_size; x++)
                        for (y = 0; y < sub_tree_size; y++)
                            if (_
                                (tree, kx * sub_tree_size + x,
                                 ky * sub_tree_size + y) <= k)
                                mwerror(FATAL, 1,
                                        "[mw_checktree_wpack2d] Not a tree : "
                                        "a child is less decomposed "
                                        "than its father\n");
                }
                else if (v == k)
                    /* Check if all the image values corresponding to children
                     * equals the decomposition level of the father,
                     * when the father is a terminal node
                     */
                {
                    for (x = 0; x < sub_tree_size; x++)
                        for (y = 0; y < sub_tree_size; y++)
...
/*
 * Allocates memory for fully fonctionnal Wpack2d and fills fields
 ...
 * start_ncol : image to decompose number of columns, used to
 * pre-alloc decomposition images
 * start_nrow : image to decompose number of rows, used to pre-alloc
 * decomposition images
 * may be 0 if you don't want pre-allocation
 * returns address of the pack
*/

Wpack2d mw_alloc_wpack2d(Wpack2d pack, Cimage tree,
                         Fsignal signal1, Fsignal signal2,
                         int start_nrow, int start_ncol)
{
    int i;
    int i1, i2;
    ...
    {
        mwerror(WARNING, -1,
                "[mw_alloc_wpack2d] NULL tree or signal structure\n");
        return (NULL);
    }
    ...
    if (signal2)
    {
        i1 = ((signal1->size + (int) signal1->shift)
              > (2 - (int) signal1->shift)
              ? (signal1->size + (int) signal1->shift)
              : (2 - (int) signal1->shift));
        i2 = ((signal2->size + (int) signal2->shift)
              > (2 - (int) signal2->shift)
              ? (signal2->size + (int) signal2->shift)
              : (2 - (int) signal2->shift));
        i = i1 > i2 ? i1 : i2;
    }
    else
        i = ((signal1->size + (int) signal1->shift)
             > (2 - (int) signal1->shift)
             ? (signal1->size + (int) signal1->shift)
             : (2 - (int) signal1->shift));
    if (mw_bandsize_wpack2d(start_ncol, pack->level) < i
        || mw_bandsize_wpack2d(start_nrow, pack->level) < i)
        mwerror(FATAL, -1,
                "[mw_alloc_wpack2d] Decomposition level is too large\n");
    pack->img_ncol = start_ncol;
    pack->img_nrow = start_nrow;
    /* initializes pack->tree */
    if ((pack->tree =
         mw_change_cimage(pack->tree, tree->nrow, tree->ncol)) == NULL)
        return (NULL);
    ...
    /* allocates memory for images, first we need to free those of
     * pack->images which are not NULL */
    if (pack->images == NULL)
    {
        pack->img_array_size = pack->tree->ncol * pack->tree->nrow;
        if ((pack->images =
             (Fimage *) malloc(pack->img_array_size * sizeof(Fimage))) ==
            NULL)
            mwerror(FATAL, -1,
                    "[mw_alloc_wpack2d] Not enough memory "
                    "for image array\n");
    ...
    else
    {
        ...
        pack->img_array_size = pack->tree->ncol * pack->tree->nrow;
        if ((pack->images =
             (Fimage *) realloc(pack->images,
                                pack->img_array_size * sizeof(Fimage))) ==
            NULL)
            mwerror(FATAL, -1,
                    "[mw_alloc_wpack2d] Not enough "
                    "memory for image array\n");
        for (i = 0; i < pack->img_array_size; i++)
            pack->images[i] = NULL;
    }
    ...
    if (indice_alloc(pack, 0, 0, 0, start_nrow, start_ncol) < 0)
...
Wpack2d mw_change_wpack2d(Wpack2d pack, Cimage tree,
                          Fsignal signal1, Fsignal signal2,
                          int start_nrow, int start_ncol)
{
    if (pack == NULL)
    {
        ...
        if (((pack = mw_new_wpack2d()) == NULL) ||
            (mw_alloc_wpack2d
             (pack, tree, signal1, signal2, start_nrow, start_ncol) == NULL))
            return (NULL);
    }
    else
    {                           /* alloc only */
...
void mw_copy_wpack2d(Wpack2d in, Wpack2d out, int new_tree_size)
{
    ...
    if (in->tree->ncol < new_tree_size)
    {
        ...
        for (kx = 0, kx1 = 0; kx < in->tree->ncol; kx++, kx1 += extra)
            for (ky = 0, ky1 = 0; ky < in->tree->nrow; ky++, ky1 += extra)
            {
                ...
                for (x = 0; x < extra; x++)
                    for (y = 0; y < extra; y++)
                        tree->gray[(kx1 + x) * new_tree_size + ky1 + y] =
                            value;
            }
        strcpy(tree->name, in->tree->name);
        out =
            mw_change_wpack2d(out, tree, in->signal1, in->signal2,
                              in->img_nrow, in->img_ncol);
        if (!out)
            mwerror(FATAL, -1, "[mw_copy_wpack2d] Not enough memory\n");
        ...
        for (kx = 0, kx1 = 0; kx < in->tree->ncol; kx++, kx1 += extra)
            for (ky = 0, ky1 = 0; ky < in->tree->nrow; ky++, ky1 += extra)
            {
                ...
                if (in->images[i] != NULL)
                {
                    ...
                    if (!
                        (out->images[i1] =
                         mw_change_fimage(NULL, in->images[i]->nrow,
                                          in->images[i]->ncol)))
                        mwerror(FATAL, -1,
                                "[mw_copy_wpack2d] Not enough memory\n");
    ...
    else
    {
        ...
        out =
            mw_change_wpack2d(out, in->tree, in->signal1, in->signal2,
                              in->img_nrow, in->img_ncol);
        if (!out)
            mwerror(FATAL, -1, "[mw_copy_wpack2d] Not enough memory\n");
        /*copy images */
        for (i = 0; i < in->img_array_size; i++)
            if (in->images[i] != NULL)
            {
                if (!
                    (out->images[i] =
                     mw_change_fimage(out->images[i], in->images[i]->nrow,
                                      in->images[i]->ncol)))
                    mwerror(FATAL, -1,
                            "[mw_copy_wpack2d] Not enough memory\n");
...
void mw_prune_wpack2d(Wpack2d in, Wpack2d out, Cimage tree)
{
        ...
        /* modify tree if needed */
    {
        ...
        /*AT THIS STAGE :
         * the tree, level, the signals, img_ncol,img_nrow and
         * img_array_size are copied in "out"
                    ...
                    if (!
                        (out->images[i1] =
                         mw_change_fimage(NULL, in->images[i]->nrow,
                                          in->images[i]->ncol)))
                        mwerror(FATAL, -1,
                                "[mw_prune_wpack2d] Not enough memory\n");
                    mw_copy_fimage(in->images[i], out->images[i1]);
                }
            }
