...
Fpolygon mw_alloc_fpolygon(Fpolygon fpolygon, int nc)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_alloc_fpolygon] cannot alloc channels : "
                "fpolygon structure is NULL\n");
        return (NULL);
    }
    if (nc <= 0)
    {
...
void mw_delete_fpolygon(Fpolygon fpolygon)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_fpolygon] cannot delete : "
                "fpolygon structure is NULL\n");
        return;
    }

    mw_delete_point_fcurve(fpolygon->first);
    if (fpolygon->nb_channels > 0)
        free(fpolygon->channel);
    free(fpolygon);
    fpolygon = NULL;
}
...
void mw_delete_fpolygons(Fpolygons fpolygons)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_fpolygons] cannot delete : "
                "fpolygons structure is NULL\n");
        return;
    }
