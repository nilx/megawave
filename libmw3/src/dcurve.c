...
void mw_delete_point_dcurve(Point_dcurve point)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_point_dcurve] cannot delete : "
                "point_dcurve structure is NULL\n");
        return;
    }
    point_first = point;
    do
...
/* Copy all the points of a dcurve starting from a Point_dcurve
 * into another chain */
...
Point_dcurve mw_copy_point_dcurve(Point_dcurve in, Point_dcurve out)
{
    ...
    if (!out)
    {
        ...
        if (!out)
            mwerror(ERROR, 0,
                    "[mw_copy_point_dcurve] Not enough memory "
                    "to create a point_curve\n");
        return (NULL);
    }
...
void mw_delete_dcurve(Dcurve dcurve)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_dcurve] cannot delete : "
...
Dcurve mw_copy_dcurve(Dcurve in, Dcurve out)
{
    ...
    if (!out)
    {
        ...
        {
            mwerror(ERROR, 0,
                    "[mw_copy_dcurve] Not enough memory "
                    "to create a dcurve !\n");
            return (NULL);
        }
    }
    ...
    if (out->first == NULL)
    {
        ...
        mwerror(ERROR, 0,
                "[mw_copy_dcurve] Not enough memory to copy the dcurve\n");
        return (NULL);
    }
...
void mw_delete_dcurves(Dcurves dcurves)
{
    ...
    {
        mwerror(ERROR, 0,
                "[mw_delete_dcurves] cannot delete : "
